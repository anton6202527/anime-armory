...
function packageDirectory(packageName) {
  ...
  return path.dirname(require.resolve(`${packageName}/package.json`, { paths: [appDirectory] }));
}

function fixPtyPrebuilds() {
  const prebuilds = path.join(packageDirectory('node-pty'), 'prebuilds');
  let fixed = 0;
...
if (require.main === module) fixPtyPrebuilds();

module.exports = { fixPtyPrebuilds, packageDirectory };
