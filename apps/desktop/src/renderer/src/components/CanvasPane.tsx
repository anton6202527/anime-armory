...
function miniMapViewportRectEqual(
...
function MiniMapViewportShade() {
  const viewportRect = useStore((state) => {
  ...
  }, miniMapViewportRectEqual);

  return <rect className="canvas-minimap-viewport-shade" {...viewportRect} pointerEvents="none" />;
}

function CanvasMiniMapNode(props: MiniMapNodeProps) {
  const viewportHostNodeId = useContext(MiniMapViewportHostContext);
  return (
    <>
      <MiniMapNode {...props} />
      {props.id === viewportHostNodeId && <MiniMapViewportShade />}
    </>
  );
}
...
const EDGE_STYLE = { stroke: "rgba(154, 163, 173, .86)", strokeWidth: 1.25 } as const;
      ...
      >
        ...
        {miniMapOpen && (
          <MiniMapViewportHostContext.Provider value={nodes.at(-1)?.id ?? null}>
            ...
            />
          </MiniMapViewportHostContext.Provider>
        )}
        <CanvasControlDock
