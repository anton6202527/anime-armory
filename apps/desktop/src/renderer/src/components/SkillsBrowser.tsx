...
/** Hide entries under any collapsed directory, working on the flat depth-ordered tree. */
...
export function SkillsBrowser(props: {
  ...
  const prunedTree = tree?.filter((node) => !isPreviewArtifact(node.path));
  const visibleTree = prunedTree ? visibleEntries(prunedTree, collapsedDirs) : undefined;
              ...
              <div className="tree-pane">
                <div className="skill-tree">
                  ...
                  {visibleTree?.map((node) => {
                    const collapsed = node.is_dir && collapsedDirs.has(node.path);
                    return (
                      <div
                        className={
                          "tree-line" +
                        ...
                        key={node.path}
                        style={{ paddingLeft: node.depth * 16, cursor: "pointer" }}
                        onClick={() => (node.is_dir ? toggleDir(node.path) : loadFile(current, node.path))}
                      >
                        <span className="tree-caret">{node.is_dir ? (collapsed ? "▸" : "▾") : ""}</span>
                        <span className="tree-icon">{node.is_dir ? (collapsed ? "📁" : "📂") : "📄"}</span>
                        {node.name}
                        {node.is_dir ? "/" : ""}
                      </div>
                    );
                  ...
                  <div className="skill-media-preview">
                    {isOptimizedImage && !staticPreviewMissing && (
                      <div className="skill-media-toolbar">
                        <span>{useStaticPreview ? t("skills.lightPreview") : t("skills.original")}</span>
                        <button type="button" onClick={() => setUseOriginalMedia((value) => !value)}>
                          {useStaticPreview ? t("skills.viewOriginal") : t("skills.lightweight")}
                        </button>
                      </div>
                    )}
                    <DecodedImage
