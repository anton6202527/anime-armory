...
export function ChangesPane({
  ...
  function openChangedFile(change: WorkChangeEntry) {
    ...
    window.setTimeout(() => scheduleWindowResizeEvent(), 0);
  }
            ...
            <div className="changes-file-body">
              {openedError ? (
                ...
                openedMediaSrc ? (
                  ...
                  <LoadingHint className="changes-empty" label={t("common.loading")} />
                )
              ) : openedKind === "video" ? (
                openedMediaSrc ? (
                ...
                <Suspense fallback={<LoadingHint className="changes-empty" label={t("common.loading")} />}>
                  <MonacoFileEditor
                    // Key by path only: reloads and saves flow through
                    // loadVersion's model-reuse path instead of recreating the
                    // whole Monaco editor instance on every content change.
                    key={`changes:${openedEntry.path}`}
                    rootPath={root.path}
