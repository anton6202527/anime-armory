 ...
 */
// Each cached doc holds full text snapshots of a work's files (up to 2MB per
// file), so an unbounded cache grows with every work visited in a session.
const DOC_CACHE_LIMIT = 6

export class BaselineService {
  private docCache = new Map<string, BaselineDoc>()

  /** Insert with LRU behavior: refresh recency, evict the oldest over the cap. */
  private cacheDoc(key: string, doc: BaselineDoc): void {
    this.docCache.delete(key)
    this.docCache.set(key, doc)
    while (this.docCache.size > DOC_CACHE_LIMIT) {
  ...
  private async load(root: string): Promise<BaselineDoc | null> {
    ...
    if (cached) {
      this.cacheDoc(key, cached) // refresh LRU recency
      return cached
    }
    try {
  ...
  private async save(root: string, doc: BaselineDoc): Promise<void> {
    this.cacheDoc(path.resolve(root), doc)
  ...
  async restoreAll(root: string): Promise<WorkChangeSummary> {
    ...
    for (const c of changes) {
      dirty = (await this.restoreEntry(root, doc, c)) || dirty
    }
    if (dirty) await this.save(root, doc)
    return this.summary(root)
  }
  ...
  async restoreOne(root: string, rel: string): Promise<WorkChangeSummary> {
    ...
    if (c) {
      const doc = await this.ensureSeeded(root)
      if (await this.restoreEntry(root, doc, c)) await this.save(root, doc)
    }
    return this.summary(root)
  }

  /** Restores one entry into the work tree, mutating `doc` in memory.
   *  Returns true when `doc` changed and needs a save. */
  private async restoreEntry(root: string, doc: BaselineDoc, c: WorkChangeEntry): Promise<boolean> {
