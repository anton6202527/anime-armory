...
function pushSharedAssetFiles(root: string, dir: string, depth: number, out: CanvasFrame[]): void {
  ...
  for (const p of paths) {
    ...
    if (isDir) {
      pushSharedAssetFiles(root, p, depth + 1, out)
      continue
    }
...
async function buildCanvas(root: string, ep: string): Promise<CanvasData> {
  const out: CanvasData = {
    ...
    shared_assets: sharedAssetFrames(root),
    clips: [],
    seams: [],
  }
