...
async function collectGenericProduction(
  ...
  for (const entry of entries) {
    ...
    // Rust filters on path.is_file() (follows symlinks); mirror with stat.
    if (entry.isFile() || (await isFile(path.join(prod, entry.name)))) {
  ...
  for (const name of names.slice(0, 24)) {
    const rel = `生产数据/${name}`
    const data = await readJson(path.join(prod, name))
    if (data !== undefined) {
...
async function buildQualityInsights(
  ...
  if (line === 'novel') {
    ...
    await collectGenericProduction(root, out, cleanEp)
  }
