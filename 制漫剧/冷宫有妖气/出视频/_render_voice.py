...
if LANG=='zh':
    for ln in open('脚本/第1集/voiceover.txt',encoding='utf-8'):
        ...
        if m: items.append((m.group(1).strip(),m.group(2).strip()))
else:
...
# 每句窗口 = 到下一句起点；末句到片尾
wins=[(starts[i+1]-starts[i]) if i<len(starts)-1 else (DUR-starts[i]) for i in range(len(starts))]
...
subprocess.run([FF,'-y','-loglevel','error']+inputs+['-filter_complex',';'.join(filt),'-map','[out]','-ar','44100','-ac','2',os.path.join(W,f'voice_{LANG}.wav')],check=True)
print(f"配音 {LANG}: {n} 句（后端={'MiniMax' if USE_MM else '火山' if USE_VOLC else 'say'}，已按字幕窗口贴速+统一电平）→ voice_{LANG}.wav")
