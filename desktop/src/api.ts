...
/** Extract readable text from a docx file inside a work root. Runs in a backend blocking task. */
export async function readWorkDocx(root: string, rel: string): Promise<string> {
  return invoke<string>("read_work_docx", { root, rel });
}
...
export async function saveCanvasCapture(
  ...
  return invoke<CanvasCaptureSaveResult>("save_canvas_capture", { root, imageData, label });
}
