...
本机已就绪：✅ ffmpeg ✅ dreamina(即梦，仅视频) ✅ gemini。缺：Suno、librosa、whisperx、Pillow（按需 pip 装）。
> 2026-06-07 起统一规则：图片生成只用 Codex / 官方 OpenAI 路由，禁止 Dreamina/即梦出图。本文保留的 dreamina 只用于 `image2video` 视频阶段。
...
## 3) 出图（mv-image）—— Codex only
- 按 `出图/共享/00_出图.md` + `出图/段落/图片/01_分镜出图.md` 用 Codex / 官方 OpenAI 生图入口逐张生成；禁止 `dreamina text2image` / `dreamina image2image`。
- 先出定妆锁脸 → 用定妆图做多图参考派生 → 出 20 张分镜（每张已含视频后端锚点句）。落 `出图/`。
