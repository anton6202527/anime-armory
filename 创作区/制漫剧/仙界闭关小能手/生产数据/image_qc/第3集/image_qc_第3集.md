...
- episode: 第3集
- 总判定: **review** · 硬阻断 0（必须修） · 非阻断初筛 2 · 视觉降级 0
- 机检能力: **none** · 当前解释器: `/opt/homebrew/Caskroom/miniconda/base/bin/python3`
- 阶段跳转: **image_qc_setup** · 像素质检不可用，不能把图片视为机检通过
- 缺失/降级: pixel checks disabled by --no-pixel
...
## 本集图片命名空间（硬闸）
- 🟢 当前 prompt 声明目标 46 张；未声明 live Clip PNG 0 张
- note: 本集图片目录不存在。

## 人工逐图拒收（硬闸）
- 🟢 active rejects 0 · review `/Users/lalala/learn/anime-armory/创作区/制漫剧/仙界闭关小能手/生产数据/image_qc/第3集/human_image_review.json`
...
## 角色脸定妆比对覆盖（硬闸）
- 🟢 已落档角色图 required 0 · covered 0 · missing 0 · pending 46 · precision none
