...
| 角色 | ⛔ block | 11 | 0 | 112 | detect, gate:image_preflight, gate:image_prompt_preflight, gate:image |
...
| 生产操作 | ⛔ block | 9 | 0 | 50 | detect, gate:image_preflight, gate:image_prompt_preflight, gate:image, score, expression_state_consistency |
...
- block [detect] 脸(G1): CHAR_WUDA__日常卖饼态 脸(G1)    
- block [detect] 脸(G1): CHAR_PANJINLIAN__25岁武大家常态 脸(G1)    
...
- warn [detect] 景深一致(DOF1):  景深一致(DOF1)   图片/Clip05_first.png：景深档与同场景其它镜不一致——本镜偏深焦(背景偏清)（景深比 1.344 vs 场景中位 0.832）；同场景深焦↔浅景深横跳像换相机，人核对是否有意，否则统一景深档重出。 
...
- warn [detect] 色温调色(GRADE1):  色温调色(GRADE1)   图片/Clip04_end.png：色温/调色与同场景其它镜不一致——本镜偏冷(蓝)（暖冷 -0.063 vs 场景中位 0.133）；同场景调色横跳像换相机/换调色，人核对是否有意，否则统一白平衡/调色重出。 
- warn [detect] 色温调色(GRADE1):  色温调色(GRADE1)   图片/EP01_CLIP05_a2.png：色温/调色与同场景其它镜不一致——本镜偏暖(琥珀)（暖冷 0.324 vs 场景中位 0.133）；同场景调色横跳像换相机/换调色，人核对是否有意，否则统一白平衡/调色重出。 
...
- block · character:character · 脸(G1) / 服装配色(N1) / 发型(H1) / 表情连续(EXP1) / 真值源(TRUTH) / 强配方Schema(RCP2) / image_prompt_lint
  - warn [detect] 脸(G1): CHAR_WUSONG__28岁打虎态 脸(G1)    
  - block [detect] 脸(G1): CHAR_WUDA__日常卖饼态 脸(G1)    
  - block [detect] 脸(G1): CHAR_PANJINLIAN__25岁武大家常态 脸(G1)    
- block · ops:production_events.jsonl · 生成配方证据
  - block [gate:image] 生成配方证据 @ 创作区/制漫剧/金瓶梅/生产数据/production_events.jsonl: 生成配方证据 出图/第1集/图片/Clip05_end.png 是本集最终媒体，但 production_events.jsonl 缺对应 image/video generation/redraw pass 记录；无法追溯 provider/model/channel/route_hash、capability_evidence_id、recipe_has
  - block [gate:image] 生成配方证据 @ 创作区/制漫剧/金瓶梅/生产数据/production_events.jsonl: 生成配方证据 出图/第1集/图片/Clip05_first_a3.png 是本集最终媒体，但 production_events.jsonl 缺对应 image/video generation/redraw pass 记录；无法追溯 provider/model/channel/route_hash、capability_evidence_id、recip
  - block [gate:image] 生成配方证据 @ 创作区/制漫剧/金瓶梅/生产数据/production_events.jsonl: 生成配方证据 出图/第1集/图片/Clip06_first.png 是本集最终媒体，但 production_events.jsonl 缺对应 image/video generation/redraw pass 记录；无法追溯 provider/model/channel/route_hash、capability_evidence_id、recipe_h
- block · ops:production_events.jsonl:line 743 · 固定 Seed / 生成配方证据
  - warn [gate:image] 固定 Seed @ 创作区/制漫剧/金瓶梅/生产数据/production_events.jsonl:line 743: 固定 Seed image 生成事件记录了 requested_seed，但缺少 seed_strategy；支持 seed 时要记 effective_seed/seed_effective=true，不支持时要记 seed_effective=false + seed_support=unsupported_or_unknown。
  - block [gate:image] 生成配方证据 @ 创作区/制漫剧/金瓶梅/生产数据/production_events.jsonl:line 743: 生成配方证据 出图/第1集/图片/Clip04_end.png 生成事件缺必填配方证据：route_hash, recipe_hash。每个最终媒体必须记录 provider/model/channel/route_hash/capability_evidence_id/recipe_hash/prompt_sha256/reference_bundle_s
...
- nodes=74 · edges=202 · clips=15 · images=23 · videos=0
...
| 实体 | 类型 | 综合 | 事前 | 落档 | 契约 |
|---|---|---|---|---|---|
| 武松（CHAR_WUSONG） | character | ⛔ block | 🟡 | ⛔ | 🟢 |
| 武大（CHAR_WUDA） | character | ⛔ block | 🟡 | ⛔ | 🟢 |
| 潘金莲（CHAR_PANJINLIAN） | character | ⛔ block | 🟡 | ⛔ | 🟢 |
...
- [block] CHAR_WUDA__日常卖饼态 脸(G1)    

## ⛔ 潘金莲（CHAR_PANJINLIAN）
- [warn] CHAR_PANJINLIAN__25岁武大家常态 锚点门(N3)    
- [block] CHAR_PANJINLIAN__25岁武大家常态 脸(G1)    
- [block] CHAR_PANJINLIAN__25岁武大家常态 脸(G1)    
