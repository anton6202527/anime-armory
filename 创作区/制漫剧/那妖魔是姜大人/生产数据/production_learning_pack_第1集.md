...
| prompt | 77 | 中文图片 prompt 缺字段：身份保持；中文图片 prompt 缺字段：镜头构图；中文图片 prompt 缺字段：动作瞬间 |
...
| 场景/构图连续性 | 37 | 场景(O2): block=0 warn=3 ok=0 skipped=False；接缝接力: block=0 warn=0 ok=0 skipped=False；轴线视线(X1): block=0 warn=0 ok=0 skipped=False |
...
| 生成配方证据 | 31 | 出视频/第1集/视频/Clip_02_看见虎妖尸身_part1.mp4 是本集最终媒体，但 production_events.jsonl 缺对应 image/；出视频/第1集/视频/Clip_02_看见虎妖尸身_part2.mp4 是本集最终媒体，但 production_events.jsonl 缺对应 image/；出视频/第1集/视频/Clip_03_镇魔司压迫交易_part1.mp4 是本集最终媒体，但 production_events.jsonl 缺对应 image |
...
| 角色 DNA/形体一致性（脸/发型/身形/手） | 27 | 锚点门(N3): block=0 warn=0 ok=0 skipped=True；脸(G1): block=0 warn=0 ok=31 skipped=False；无脸崩坏(G1b): block=0 warn=0 ok=0 skipped=True |
| image_prompt_lint | 27 | 镜头 1（`EP01_CLIP01` · 死人堆惊醒 · ）：近景/特写镜头缺乏物理镜头参数（如 85mm, f/1.4）。建议补充以增强物理光学透视和电影感。；镜头 1（`EP01_CLIP01` · 死人堆惊醒 · ）：缺『身份锁定句』（多参考/编辑类后端最敏感的锁脸句）；镜头 2（`EP01_CLIP02` · 看见虎妖尸身 · realm_portal）：近景/特写镜头缺乏物理镜头参数（如 85mm, f/1.4）。建议补充以 |
| 角色一致性 | 20 | 含角色镜头只看到主参考；侧脸/半身/全身锚或角色ID缺失时容易漂；含角色镜头只看到主参考；侧脸/半身/全身锚或角色ID缺失时容易漂；含角色镜头只看到主参考；侧脸/半身/全身锚或角色ID缺失时容易漂 |
| 物料漂移预案 | 19 | 本集物料漂移风险 high（分54）：本集高频场景：登记 scene_atlas.base_views（G-I2 场景多机位锁：front + 反打/侧机位 r；本集物料漂移风险 medium（分46）：结构/件数强锁：参考图标清拓扑（单镜面/三件套/唯一圆口），逐镜 prompt 锁件数不增减。；本集物料漂移风险 medium（分42）：结构/件数强锁：参考图标清拓扑（单镜面/三件套/唯一圆口），逐镜 prompt 锁件数不增减。 |
...
| 角色资产包 | 17 | 角色资产包分区不存在：prompts；角色资产包分区不存在：lora；角色资产包分区不存在：voice |
...
| 节奏密度(Rhythm) | 15 | [production一致性升级:重复同维度] 节奏/留存 advisory 总分偏低：57.4。如确认为可接受，写入 生产数据/consistency_adv；[production一致性升级:重复同维度] 连续 10 个长镜聚集（EP01_CLIP01→EP01_CLIP02→EP01_CLIP03→EP01_CLI；[production一致性升级:重复同维度] 开场镜未见冷开场/钩子标注（rhythm/label=『铺垫·长镜 死人堆惊醒』），疑慢热；开场镜时长 9.2s |
| 成片/包装一致性 | 14 | 成片统一(C1): block=0 warn=3 ok=0 skipped=False；成片统一(C1) detail: 成片响度不贴目标：LUFS=-17.99 target=-16.0 true_peak=-2.13 定位产物：合成/第1集、出；成片统一(C1) detail: storyboard 存在多档节奏，但缺 tension_mix/BGM 增益证据；BGM 全集一刀切会削弱钩子与对白清晰度。 |
| 音画同步 | 13 | 音画同步(AV1): block=0 warn=0 ok=0 skipped=True；多人对话音画(DAV): block=0 warn=1 ok=0 skipped=False；多人对话音画(DAV) detail: 检测到原生音/多人对话视频产物，但缺 dialogue_av_alignment；无法核验说话人、口型、镜头对人和台词顺 |
| 物理尺寸对账 | 13 | 多人同框镜头（姜月初、虎妖、虎山神、裴长青）中 姜月初「比裴长青矮约一个头；与虎山神同框时体量差极大，突出凡人压迫感。」；虎妖「远大于姜月初和裴长青，同框必须保；多人同框镜头（姜月初、虎妖、虎山神、裴长青）中 姜月初「比裴长青矮约一个头；与虎山神同框时体量差极大，突出凡人压迫感。」；虎妖「远大于姜月初和裴长青，同框必须保；多人同框镜头（姜月初、虎妖、虎山神、裴长青）中 姜月初「比裴长青矮约一个头；与虎山神同框时体量差极大，突出凡人压迫感。」；虎妖「远大于姜月初和裴长青，同框必须保 |
...
| 打斗撞点(SPEC-APEX) | 8 | Clip_06（fight_exchange）：剪辑峰值钉在 [5.0]s，但本镜 continuity.anchors 无 keyframe 锚——跑 `an；Clip_10（fight_exchange）：剪辑峰值钉在 [5.0]s，但本镜 continuity.anchors 无 keyframe 锚——跑 `an；Clip_06（fight_exchange）：剪辑峰值钉在 [5.0]s，但本镜 continuity.anchors 无 keyframe 锚——跑 `an |
| 场景(O2) | 8 | 一致性审计发现问题；一致性审计发现问题；一致性审计发现问题 |
...
| 强配方Schema(RCP2) | 8 | [production一致性升级:重复同维度] 脚本/第1集/storyboard.json 强配方 schema 缺字段：prompt_sha256, ref；[production一致性升级:重复同维度] 合成/第1集/配音/voice_zh.wav 强配方 schema 缺字段：prompt_sha256, ref；[production一致性升级:重复同维度] 脚本/第1集/storyboard.json 强配方 schema 缺字段：prompt_sha256, ref |
| 生成配方(RCP) | 8 | [production一致性升级:重复同维度] 脚本/第1集/storyboard.json 生成事件缺配方字段：mode, seed/seed_degrade；[production一致性升级:重复同维度] 合成/第1集/配音/voice_zh.wav 生成事件缺配方字段：seed/seed_degrade, back；[production一致性升级:重复同维度] 脚本/第1集/storyboard.json 生成事件缺配方字段：mode, seed/seed_degrade |
| 语义继承 | 6 | 语义谱系(P0): block=0 warn=0 ok=0 skipped=False；称谓口头禅(A1): block=0 warn=0 ok=0 skipped=True；台词语域(D1): block=0 warn=0 ok=0 skipped=True |
...
| 语义谱系(P0) | 6 | `钩子` 留存标记未进入 storyboard 节奏/导演意图。；缺：钩子；`钩子` 留存标记未进入 storyboard 节奏/导演意图。；缺：钩子；`钩子` 留存标记未进入 storyboard 节奏/导演意图。 |
...
| 运动质量(MOT1) | 6 | [production一致性升级:重复同维度] 高动作后验报告缺字段：speed_curve, spatial_path/distance_curve；动作镜不；[production一致性升级:重复同维度] 高动作后验报告缺字段：speed_curve, spatial_path/distance_curve；动作镜不；[production一致性升级:重复同维度] 高动作后验报告缺字段：speed_curve, spatial_path/distance_curve；动作镜不 |
...
| 跨集脸漂(G5) | 5 | CHAR_01__囚犯初醒态 跨集脸漂：第1集(均值0.4057)→第2集(均值0.4461)，相对基线掉幅 -0.0404，且本集均值低于绝对下限——已系统性；CHAR_01__囚犯初醒态 跨集脸漂：第1集(均值0.4057)→第2集(均值0.4429)，相对基线掉幅 -0.0372，且本集均值低于绝对下限——已系统性；CHAR_01__囚犯初醒态 跨集脸漂：第1集(均值0.4057)→第2集(均值0.4461)，相对基线掉幅 -0.0404，且本集均值低于绝对下限——已系统性 |
...
| 进度凭据对账 | 5 | 进度「出图」标 ✅ 却无新鲜通过的闸门凭据（gate_failed）：闸门未过：image 仍有 4 个 block 级问题（见 gate_findings_i；进度「视频」标 ✅ 却无新鲜通过的闸门凭据（gate_failed）：闸门未过：video 仍有 9 个 block 级问题（见 gate_findings_v；进度「出图」标 ✅ 却无新鲜通过的闸门凭据（gate_failed）：闸门未过：image 仍有 4 个 block 级问题（见 gate_findings_i |
...
| 逐镜仲裁 | 4 | 1 个镜被多检测器同时报，已按证据族归并（severity 以最坏维度为准，勿按条数重复计=双计数）：Clip_07(3维/2族/6条·最坏block)；1 个镜被多检测器同时报，已按证据族归并（severity 以最坏维度为准，勿按条数重复计=双计数）：Clip_08(2维/2族/4条·最坏block)；1 个镜被多检测器同时报，已按证据族归并（severity 以最坏维度为准，勿按条数重复计=双计数）：Clip_07(3维/2族/3条·最坏block) |
| fidelity-gate | 4 | fidelity-gate 未激活；vlm_verify --write 可在出图后跑 canonical 通过表。image 阶段不硬拦（出图后还没建 can；出视频后建议跑 vlm_verify --write 落 canonical 通过表，否则 compose/review gate 会 BLOCK。；前集已有 canonical 表，本集 stale（未重跑 vlm_verify --write）——终验须 fidelity-gate 激活——跑 vlm_v |
| 一致性总审 | 4 | 另有 35 条一致性 WARN 未在此展开（已超过单次 12 条上限）——完整清单见 生产数据/consistency_findings_第1集.json，勿当；另有 97 条一致性 WARN 未在此展开（已超过单次 12 条上限）——完整清单见 生产数据/consistency_findings_第1集.json，勿当；另有 84 条一致性 WARN 未在此展开（已超过单次 12 条上限）——完整清单见 生产数据/consistency_findings_第1集.json，勿当 |
...
| 主角装备库 | 3 | 该 VFX/法术资产看起来承担武器/法宝识别功能；若它是实体武器或主角本命法宝，请拆成 WEAPON_xx 实体资产 + VFX_xx 光效表现，并在角色 si；该 VFX/法术资产看起来承担武器/法宝识别功能；若它是实体武器或主角本命法宝，请拆成 WEAPON_xx 实体资产 + VFX_xx 光效表现，并在角色 si；该 VFX/法术资产看起来承担武器/法宝识别功能；若它是实体武器或主角本命法宝，请拆成 WEAPON_xx 实体资产 + VFX_xx 光效表现，并在角色 si |
...
| 锚点门(N3) | 3 | 一致性审计发现问题；一致性审计发现问题；一致性审计发现问题 |
...
| 结构化交互图谱(I2) | 2 | 接触/持有镜缺结构化 interaction_graph；自由文本提示无法稳定约束接触点、身体部位归属与遮挡顺序。；接触/持有镜缺结构化 interaction_graph；自由文本提示无法稳定约束接触点、身体部位归属与遮挡顺序。 |
...
| 视频 | 2 | clip 数 16 与 storyboard clips 11 不一致；clip 数 16 与 storyboard clips 11 不一致 |
| 原生音轨 | 2 | clip 含原生音轨；当前策略=丢弃，compose 会剥离以避免原生台词与配音双人声；clip 含原生音轨；当前策略=丢弃，compose 会剥离以避免原生台词与配音双人声 |
| 时长 | 2 | clip 总长 127.86s 与镜头时长累计 120.52s 差 7.34s；clip 总长 127.86s 与镜头时长累计 120.52s 差 7.34s |
...
| 专项镜头模板 | 1 | 复杂镜头疑似「realm_portal」，但缺 template/template_contract；回 n2d-script 按 references/专项镜 |
| 预防式合同 | 1 | pilot_release_gate: 第1集缺 pilot_acceptance；先用 2-3 个代表镜头验证脸/场景/动作/口型/接缝/路由。 |
| 验收总账 | 1 | 一致性验收总账未清零：block=6 high=0 medium=20。review 不再按单镜看着像放行；请按 consistency_ledger 的交付域 |
...
| 视频证据完整性(EVID) | 1 | video_eval_manifest 已建立，但这些风险 sidecar 尚未写回：dialogue_av:生产数据/dialogue_av_alignmen |
