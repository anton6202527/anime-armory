...
- 集：`第2集`
- 图片 targets：Clip10_first, Clip10_mid, Clip10_end
- 视频 targets：（无）
...
## 已有报告证据（逐 target 预判）
- image_qc：ok（`/Users/lalala/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人/生产数据/image_qc/第2集/image_qc_第2集.json`）
- contract_inheritance：ok（`/Users/lalala/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人/生产数据/contract_inheritance_第2集.json`）
  - `Clip10_first` → **warn** · image_qc.outfit=warn(outfit)
  - `Clip10_mid` → **warn** · image_qc.outfit=warn(outfit)
  - `Clip10_end` → **warn** · image_qc.outfit=warn(outfit)
...
1. shell
```bash
python3 skills/n2d-update/scripts/update_plan.py check "/Users/lalala/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人" 第2集 --write-plan --regen-mode 严审刷新
```
2. shell
```bash
python3 skills/n2d-image/scripts/image_qc.py "/Users/lalala/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人" 第2集 --regen-list
```
3. AI agent：只在 image_qc/dashboard gate/n2d-review findings 或显式人工输入确认这些图片 target 不符合最新 prompt/QC/review 标准时，才执行：`python3 skills/n2d-batch/scripts/queue.py plan "/Users/lalala/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人" --episodes 2 --rerun-from image --affected-shot "Clip10_first" --affected-shot "Clip10_mid" --affected-shot "Clip10_end" --scope "媒体刷新·证据确认后重出图片" --max-concurrency 1 --max-retries 1`；没有证据时保留为待复核，不排队。
4. AI agent：如果上一步已经排队并由 n2d-batch/对应 stage skill 实际完成图片重出，再执行验收：`python3 skills/n2d-dashboard/scripts/dashboard.py gate "/Users/lalala/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人" 第2集 --stage image`；若只是生成队列计划，不能把这一步当成已验收。

## 备注
- 已有报告证据·命中 warn（交人判，不默认重出）：Clip10_first、Clip10_mid、Clip10_end
