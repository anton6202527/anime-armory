...
- 阈值：85
- 状态：回流
- 生成时间：2026-07-03T14:59:20+00:00

## 角色一致性 KPI（报告型·非扣分·对标 EntityBench/MSVBench）

- 本集 intra：0.4057 · 跨集 cross：0.4057 · 可投放线：0.85 → **—**
- encoder：arcface_fallback · fidelity-gate：absent · 计入角色数：1
- 基准：EntityBench/MSVBench 风格的主体/实体一致性评测轴；风格化脸另参 Face Consistency Benchmark for GenAI Video；叙事级长视频参 NarrLV/DirectorBench；具体 release line 必须用本项目 encoder 与人审校准集重标定

## 长篇叙事一致性 KPI（报告型·非扣分·NarrLV/DirectorBench 轴）

- 叙事连续性：0.9357 · profile standard · 参考线 0.7 → **达标**（集数 10）
- 子分：伏笔已回收 0.7857 · 伏笔已规划 1.0 · 冷开场链 1.0 · 反套路 1.0 · 情绪起伏 0.8571 · 叙事原子 1.0 · 实体排程 1.0
...
| 交互/接触因果一致性 | 8 | 0 | 回流 | 1 | 10 | script_stage2 |
| 成片/包装一致性 | 8 | 16 | 需复核 | 0 | 7 | compose |
...
- 锚点门(N3): block=1 warn=2 ok=0 skipped=False
- 脸(G1): block=0 warn=3 ok=28 skipped=False
...
- 手部/解剖(N5): block=0 warn=0 ok=0 skipped=False
- ...另有 17 条
### 角色 DNA 一致性（服装/配饰）
...
- mechanical[完整性] 第1集: 产物快照：配音句 28 · clip 16 · 成片 1
...
- visual[av_duration]: block=0 warn=0 skipped=True metrics={"final_sec": null, "srt_sec": 120.515, "storyboard_sec": 120.515, "voice_sec": 120.515034}
...
- visual[final_rhythm_density]: block=0 warn=0 skipped=True
- visual[final_rhythm_density] 缺成片/故事板时长或 clips[]，成片节奏密度跳过
### 风格一致性
...
- mechanical[视频] 第1集: clip MP4 数 16 与 storyboard clips 11 不一致
### 状态百科
...
- 结构化交互图谱(I2): block=1 warn=8 ok=0 skipped=False
- 结构化交互图谱(I2) detail: 接触/持有镜缺结构化 interaction_graph；自由文本提示无法稳定约束接触点、身体部位归属与遮挡顺序。 定位镜头：Clip_01 定位产物：脚本/第1集/storyboard.json
- 结构化交互图谱(I2) detail: 接触/持有镜缺结构化 interaction_graph；自由文本提示无法稳定约束接触点、身体部位归属与遮挡顺序。 定位镜头：Clip_02 定位产物：脚本/第1集/storyboard.json
- 结构化交互图谱(I2) detail: 接触/持有镜缺结构化 interaction_graph；自由文本提示无法稳定约束接触点、身体部位归属与遮挡顺序。 定位镜头：Clip_03 定位产物：脚本/第1集/storyboard.json
- 结构化交互图谱(I2) detail: 接触/持有镜缺结构化 interaction_graph；自由文本提示无法稳定约束接触点、身体部位归属与遮挡顺序。 定位镜头：Clip_04 定位产物：脚本/第1集/storyboard.json
- 物理因果链(CG1): block=0 warn=1 ok=0 skipped=False
- ...另有 3 条
### 成片/包装一致性
- 成片统一(C1): block=0 warn=2 ok=0 skipped=False
...
- 系列调色(GRD): block=0 warn=1 ok=0 skipped=False
- ...另有 5 条
### 生产操作一致性
- 生成配方(RCP): block=0 warn=2 ok=0 skipped=False
- 生成配方(RCP) detail: 脚本/第1集/storyboard.json 生成事件缺配方字段：mode, seed/seed_degrade, backend_version/model_version, declared_recipe_hash；已可推导 hash=ee84f2db209c284e，但复跑审计证据不完整。 定位产物：生产数据/production_events.jsonl、生产数据/generation_recipe_第1集.json、脚本/第1集/storyboard.json
- 生成配方(RCP) detail: 合成/第1集/配音/voice_zh.wav 生成事件缺配方字段：seed/seed_degrade, backend_version/model_version, declared_recipe_hash；已可推导 hash=584f6548fdede418，但复跑审计证据不完整。 定位产物：生产数据/production_events.jsonl、生产数据/generation_recipe_第1集.json、合成/第1集/配音/voice_zh.wav
- 强配方Schema(RCP2): block=0 warn=2 ok=0 skipped=False
- 强配方Schema(RCP2) detail: 脚本/第1集/storyboard.json 强配方 schema 缺字段：prompt_sha256, reference_bundle_sha256/reference_manifest, input_fingerprint, settings_sha256, artifact_sha256, adapter_version, qc_version, backend_version/model_version, seed_effective_or_unsupported；recipe_hash 已有但还不能完整复现/归因。 定位产物：生产数据/production_events.jsonl、生产数据/generation_recipe_第1集.json、脚本/第1集/storyboard.json
- 强配方Schema(RCP2) detail: 合成/第1集/配音/voice_zh.wav 强配方 schema 缺字段：prompt_sha256, reference_bundle_sha256/reference_manifest, input_fingerprint, settings_sha256, artifact_sha256, adapter_version, qc_version, backend_version/model_version, seed_effective_or_unsupported；recipe_hash 已有但还不能完整复现/归因。 定位产物：生产数据/production_events.jsonl、生产数据/generation_recipe_第1集.json、合成/第1集/配音/voice_zh.wav
