...
| 角色 | ⛔ block | 33 | 0 | 63 | detect, gate:image_preflight, gate:image_prompt_preflight, gate:image |
| 资产 | ⛔ block | 4 | 0 | 6 | detect, gate:image_preflight, gate:image_prompt_preflight, gate:image |
| 镜头 | ⛔ block | 4 | 0 | 54 | detect, gate:image_preflight, gate:image_prompt_preflight, gate:image |
...
| 生产操作 | ⛔ block | 5 | 0 | 36 | detect, gate:image_preflight, gate:image_prompt_preflight, gate:image, score, expression_state_consistency |
...
### 角色问题
- block [detect] 跨集脸漂(G5): CHAR_01__囚犯初醒态 跨集脸漂(G5)    CHAR_01__囚犯初醒态 跨集脸漂：第1集(均值0.427)→第2集(均值0.3627)，相对基线掉幅 0.0643，且本集均值低于绝对下限——已系统性偏离定妆锚
- warn [detect] 服装配色(N1): CHAR_01__囚犯初醒态 服装配色(N1)
- warn [detect] 服装配色(N1): CHAR_01__囚犯初醒态 服装配色(N1)
- warn [detect] 服装配色(N1): CHAR_01__囚犯初醒态 服装配色(N1)
- warn [detect] 发型(H1): CHAR_01__囚犯初醒态 发型(H1)
- warn [detect] 发型(H1): CHAR_01__囚犯初醒态 发型(H1)
- warn [detect] 发型(H1): CHAR_01__囚犯初醒态 发型(H1)
...
- warn [detect] multimodal_continuity: multimodal_continuity  None outfit 语义漂移疑似（调色板未报）：参考「CHAR_01__囚犯初醒态」↔ 本镜 图片/Clip02_first.png DINO/CLIP cosine=0.25 < 0.55——同色但剪裁/结构/布局可能变了（palette/dHash 抓不到），人判。
- warn [detect] multimodal_continuity: multimodal_continuity  None outfit 语义漂移疑似（调色板未报）：参考「CHAR_01__囚犯初醒态」↔ 本镜 图片/Clip02_mid.png DINO/CLIP cosine=0.24 < 0.55——同色但剪裁/结构/布局可能变了（palette/dHash 抓不到），人判。
- warn [detect] multimodal_continuity: multimodal_continuity  None outfit 语义漂移疑似（调色板未报）：参考「CHAR_01__囚犯初醒态」↔ 本镜 图片/Clip03_end.png DINO/CLIP cosine=0.20 < 0.55——同色但剪裁/结构/布局可能变了（palette/dHash 抓不到），人判。
...
- warn [detect] 成本路由(K1):  成本路由(K1)   创作区/制漫剧/那妖魔是姜大人/合成/第2集/配音/voice_zh.wav 有多次 pass 记录但缺 redraw_reason/验收选择原因；后续无法判断哪版是正片依据。
...
- block · asset:asset · 物件状态(OST) / 交互接触(I1) / 结构化交互图谱(I2) / 系统面板(UI1)
  - warn [detect] 物件状态(OST):  物件状态(OST)   道具『横刀』状态前后矛盾：EP02_CLIP07 写「满」（满），EP02_CLIP08 写「空」（空），中间无已声明的状态转换——若确有变化请在 visual_state_ledger 给该道具登记 timeline 转换，否则修穿帮。
  - warn [detect] 交互接触(I1):  交互接触(I1)   物理接触/持有镜缺 interaction_graph/contact_graph 或左右手/接触点描述；人物接触、递物、拉扯容易跨镜乱跳。
  - block [detect] 结构化交互图谱(I2):  结构化交互图谱(I2)   接触/持有镜缺结构化 interaction_graph；自由文本提示无法稳定约束接触点、身体部位归属与遮挡顺序。
...
- block · character:CHAR_01__囚犯初醒态 · character_consistency
  - block [detect] character_consistency @ CHAR_01__囚犯初醒态: character_consistency  CHAR_01__囚犯初醒态 跨集脸漂移趋势 high：CHAR_01__囚犯初醒态 第1集→第2集 mean 0.427→0.3627 drop=0.0643。high 级系统性退化必须先回 n2d-image 补主体库/参考包/重抽并重跑 identity/image_qc。
...
- block · character:character · 跨集脸漂(G5) / 服装配色(N1) / 发型(H1) / 真值源(TRUTH) / 台词语域(D1) / 叙事状态(NS1)
  - block [detect] 跨集脸漂(G5): CHAR_01__囚犯初醒态 跨集脸漂(G5)    CHAR_01__囚犯初醒态 跨集脸漂：第1集(均值0.427)→第2集(均值0.3627)，相对基线掉幅 0.0643，且本集均值低于绝对下限——已系统性偏离定妆锚
  - warn [detect] 服装配色(N1): CHAR_01__囚犯初醒态 服装配色(N1)
  - warn [detect] 服装配色(N1): CHAR_01__囚犯初醒态 服装配色(N1)
- block · character:consistency_findings_第2集.json · 一致性总审
  - block [gate:image] 一致性总审 @ 创作区/制漫剧/那妖魔是姜大人/生产数据/consistency_findings_第2集.json: 一致性总审 一致性审计精度为 degraded（insightface 等不可用，脸/像素一致性未真正验证）；出图后闸门不放行——请在 full 环境复跑，或显式 N2D_ALLOW_DEGRADED_QC=1 放行并自负其责。
- block · character:定妆_CHAR_01__囚犯初醒态_正面.png · 资产身份注册层
  - block [gate:image] 资产身份注册层 @ 创作区/制漫剧/那妖魔是姜大人/出图/共享/图片/定妆_CHAR_01__囚犯初醒态_正面.png: 资产身份注册层 three_quarter 同源派生 source_sha256 与当前母本不一致；母本变更后必须重新裁切派生。
  - block [gate:image] 资产身份注册层 @ 创作区/制漫剧/那妖魔是姜大人/出图/共享/图片/定妆_CHAR_01__囚犯初醒态_正面.png: 资产身份注册层 side 同源派生 source_sha256 与当前母本不一致；母本变更后必须重新裁切派生。
  - block [gate:image] 资产身份注册层 @ 创作区/制漫剧/那妖魔是姜大人/出图/共享/图片/定妆_CHAR_01__囚犯初醒态_正面.png: 资产身份注册层 back 同源派生 source_sha256 与当前母本不一致；母本变更后必须重新裁切派生。
- block · character:定妆_CHAR_02__濒死战损态_正面.png · 资产身份注册层
  - block [gate:image] 资产身份注册层 @ 创作区/制漫剧/那妖魔是姜大人/出图/共享/图片/定妆_CHAR_02__濒死战损态_正面.png: 资产身份注册层 three_quarter 同源派生 source_sha256 与当前母本不一致；母本变更后必须重新裁切派生。
  - block [gate:image] 资产身份注册层 @ 创作区/制漫剧/那妖魔是姜大人/出图/共享/图片/定妆_CHAR_02__濒死战损态_正面.png: 资产身份注册层 side 同源派生 source_sha256 与当前母本不一致；母本变更后必须重新裁切派生。
  - block [gate:image] 资产身份注册层 @ 创作区/制漫剧/那妖魔是姜大人/出图/共享/图片/定妆_CHAR_02__濒死战损态_正面.png: 资产身份注册层 back 同源派生 source_sha256 与当前母本不一致；母本变更后必须重新裁切派生。
- block · character:定妆_CHAR_03__诈死复苏态_正面.png · 资产身份注册层
  - block [gate:image] 资产身份注册层 @ 创作区/制漫剧/那妖魔是姜大人/出图/共享/图片/定妆_CHAR_03__诈死复苏态_正面.png: 资产身份注册层 three_quarter 同源派生 source_sha256 与当前母本不一致；母本变更后必须重新裁切派生。
...
- [block] CHAR_01__囚犯初醒态 跨集脸漂(G5)    CHAR_01__囚犯初醒态 跨集脸漂：第1集(均值0.427)→第2集(均值0.36
- [warn] CHAR_01__囚犯初醒态 服装配色(N1)

## ⛔ 虎山神 / 虎妖（CHAR_03）
- [block] CHAR_03__诈死复苏态 锚点门(N3)
- [warn] style_consistency  None 风格归属佐证：本集 11 帧整体风格指纹偏离风格锚（明暗对比偏离最大，归一距离 1.03 ≥
