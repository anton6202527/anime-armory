...
- episode: 第5集 · 默认后端: codex
- ⛔ 阻断 0 · 🔴 预测高危 1 · 🟡 中危 2

| 角色 | 风险 | 分 | 锁脸档 | 主驱动 |
|---|---|---|---|---|
| 姜月初（CHAR_01/囚犯初醒态） | 🔴 high | 55.4 | multi_reference | 锁脸档位=multi_reference(+22)；极端角度 3 镜(+18)；大表情 1 镜(+8) |
| 陈青源（CHAR_04/常态） | 🟡 medium | 51.0 | multi_reference | 锁脸档位=multi_reference(+22)；多人同框 2/2(+20.0)；近景占比 1/2(+15.0) |
| 虎山神 / 虎妖（CHAR_03/诈死复苏态） | 🟡 medium | 36.0 | multi_reference | 锁脸档位=multi_reference(+22)；多人同框 1/1(+20.0)；同源场景 in-context 记功(strong·如 GPT Image 2)(+-6) |

## 🔴 姜月初（CHAR_01/囚犯初醒态）· 分 55.4
...
## 🟡 陈青源（CHAR_04/常态）· 分 51.0
- GPT Image 2（渠道 Codex CLI） 无持久主体 ID：每镜必须喂定妆组/场景图并拼身份锁定句；不要只靠文字外貌描述。
- 近景占比高：补脸部特写主参考，近景镜锁脸型/五官比例/发型发饰。
- 多人同框多：换用支持持久主体的官方后端（Seedream/可灵/Sora）或把同框拆成正反打分别出；无论哪种都必须逐主体写画面槽位+各自参考（空间绑定硬约束，否则模型把多张脸平均成一张）。

## 🟡 虎山神 / 虎妖（CHAR_03/诈死复苏态）· 分 36.0
- GPT Image 2（渠道 Codex CLI） 无持久主体 ID：每镜必须喂定妆组/场景图并拼身份锁定句；不要只靠文字外貌描述。
...
| `VFX_道行计数overlay`（vfx） | faceless | 🟢 low | faceless·须背身/裁脸·落档像素验 0 清晰脸 |
...
| `VFX_妖气`（vfx） | faceless | 🟢 low | faceless·须背身/裁脸·落档像素验 0 清晰脸 |
| `VFX_虎山神摹影`（vfx） | faceless | 🟢 low | faceless·须背身/裁脸·落档像素验 0 清晰脸 |
| `VFX_百妖谱金光`（vfx） | faceless | 🟢 low | faceless·须背身/裁脸·落档像素验 0 清晰脸 |
