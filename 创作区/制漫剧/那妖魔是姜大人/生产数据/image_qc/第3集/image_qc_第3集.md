...
- episode: 第3集
- 总判定: **review** · 硬阻断 0（必须修） · 非阻断初筛 16 · 视觉降级 0
...
## 本集图片命名空间（硬闸）
- 🟢 当前 prompt 声明目标 14 张；未声明 live Clip PNG 0 张
...
## 角色脸定妆比对覆盖（硬闸）
- 🟢 已落档角色图 required 10 · covered 10 · missing 0 · pending 0 · precision full
...
- 🟡 CHAR_01__囚途残损态：第1集→第3集 均值 0.3531→0.3121（掉幅 0.041）（跌破绝对下限）
...
## 执行层 lint（逐镜 prompt）
- 🟡 8 镜已 lint · block 0 · warn 6
  - 🟡 脸部锚弱信噪比 CHAR_03/常态「基础」（出图/共享/图片/定妆_CHAR_03__常态_脸部特写_脸锚裁切.png）：脸占画面仅 5%（建议 ≥30%，最低线 ≥12%）——弱脸锚会把脸漂带进下游每一镜；核心/长线角色必须重出更紧的脸部特写（脸占 30–50%、≥1024px）后再放行。
...
## 高风险道具禁形/尺寸逐图复核（硬闸）
- total 34 · pending 0 · confirmed 34
