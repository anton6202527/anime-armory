...
- episode: 第2集
- 总判定: **block** · 硬阻断 13（必须修） · 非阻断初筛 32 · 视觉降级 0
...
## 执行层 lint（逐镜 prompt）
- 🔴 8 镜已 lint · block 12 · warn 10
  - 🔴 镜头 1（`EP02_CLIP01` · 杀人余震与二十年到账 · ）：资产引用 `VFX_百妖谱` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 1（`EP02_CLIP01` · 杀人余震与二十年到账 · ）：资产引用 `VFX_系统面板` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 4（`EP02_CLIP04` · 百年道行与收录选择 · system_panel）：资产引用 `VFX_百妖谱` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 4（`EP02_CLIP04` · 百年道行与收录选择 · system_panel）：资产引用 `VFX_系统面板` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 5（`EP02_CLIP05` · 摹影虎山神获圆满刀法 · system_panel）：资产引用 `VFX_墨虎谱影` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 5（`EP02_CLIP05` · 摹影虎山神获圆满刀法 · system_panel）：资产引用 `VFX_百妖谱` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 5（`EP02_CLIP05` · 摹影虎山神获圆满刀法 · system_panel）：资产引用 `VFX_系统面板` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 6（`EP02_CLIP06` · 结算闻弦初境与二十五年余额 · ）：资产引用 `VFX_百妖谱` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 6（`EP02_CLIP06` · 结算闻弦初境与二十五年余额 · ）：资产引用 `VFX_系统面板` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 8（`EP02_CLIP08` · 摹影进阶会变成什么 · system_panel）：资产引用 `VFX_墨虎谱影` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 8（`EP02_CLIP08` · 摹影进阶会变成什么 · system_panel）：资产引用 `VFX_百妖谱` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
  - 🔴 镜头 8（`EP02_CLIP08` · 摹影进阶会变成什么 · system_panel）：资产引用 `VFX_系统面板` 在 asset_registry 不存在（场景/道具/武器/服装/特效 id 写错或未登记）
...
## 高风险道具禁形/尺寸逐图复核（硬闸）
- total 41 · pending 1 · confirmed 40
- 确认文件: `/Users/wesley/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人/生产数据/image_qc/第2集/prop_shape_confirmations.json`
  - 🔴 shared_primary 出图/共享/图片/定妆_道具_镇魔司制服.png（PROP_镇魔司制服 镇魔司制服） 禁形=现代物件、文字水印、结构漂移、数量漂移；尺寸=None；/Users/wesley/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人/生产数据/image_qc/第2集/prop_shape_review/PROP_镇魔司制服_shared_primary_定妆_道具_镇魔司制服_compare.png
