...
- episode: 第2集
- 总判定: **block** · 硬阻断 47（必须修） · 非阻断初筛 47 · 视觉降级 0
...
- 崩脸 G1: 🔴 block 5 · warn 5
- 发型 H1: 🔴 block 8 · warn 0
- 服装 N1: 🔴 block 11 · warn 0
...
## 角色脸定妆比对覆盖（硬闸）
- 🔴 已落档角色图 required 21 · covered 17 · missing 4 · pending 0 · precision full
  - 🔴 镜头 1（`EP02_CLIP01` · 杀人余震与二十年到账 · ） 图片/EP02_CLIP01_start.png：face_verdict_warn
  - 🔴 镜头 5（`EP02_CLIP05` · 摹影虎山神获圆满刀法 · system_panel） 图片/EP02_CLIP04_end_a1.png：face_verdict_warn
  - 🔴 镜头 5（`EP02_CLIP05` · 摹影虎山神获圆满刀法 · system_panel） 图片/EP02_CLIP04_end_a2.png：face_verdict_warn
  - 🔴 镜头 6（`EP02_CLIP06` · 结算闻弦初境与二十五年余额 · ） 图片/EP02_CLIP06_start_a1.png：face_verdict_warn
- 人工脸部确认: applied 3 · 确认文件 `/Users/wesley/learn/anime-armory/创作区/制漫剧/那妖魔是姜大人/生产数据/image_qc/第2集/face_confirmations.json`
...
- 🟡 CHAR_01__囚途残损态：第1集→第2集 均值 0.3531→0.4421（掉幅 -0.089）（跌破绝对下限）
...
- 🟡 8 镜已 lint · block 0 · warn 7
  ...
  - 🟡 镜头构图重复 EP02_CLIP04 ↔ EP02_CLIP05：首帧 dHash=5/64——观众在成片里会看到两个几乎一样的镜头。换景别/机位/构图重出其一，或合并两镜。
  - 🟡 VLM 设定核验未运行（未配置 N2D_VLM_CMD）——服装剪裁/配饰/识别特征是否违反 canonical 设定未机检，缺左腕疤、月白窄袖画成交领这类设定漂移可能漏过；正式定稿前在 full+VLM 环境复跑。

## 高风险道具禁形/尺寸逐图复核（硬闸）
- total 79 · pending 38 · confirmed 41
