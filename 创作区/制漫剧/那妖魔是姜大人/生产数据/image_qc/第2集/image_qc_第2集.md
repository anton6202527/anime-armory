...
## 本集图片命名空间（硬闸）
- 🟢 当前 prompt 声明目标 30 张；未声明 live Clip PNG 0 张
...
## 角色脸定妆比对覆盖（硬闸）
- 🟢 已落档角色图 required 21 · covered 21 · missing 0 · pending 9 · precision full

## 跨集脸漂移趋势（B·治每集过floor但逐集偏离·advisory）
- 🔴 CHAR_01__囚犯初醒态：第1集→第2集 均值 0.427→0.4442（掉幅 -0.0172）（跌破绝对下限）
