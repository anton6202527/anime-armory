...
**身份锁定执行绑定**：reference_group=identity_registry.CHAR_01.囚途残损态 + identity_registry.CHAR_02.濒死重伤态 + identity_registry.BEAST_01.穿心复生态；首/中/尾均使用同一组定妆母版；CHAR_01 expressions=`出图/共享/图片/定妆_CHAR_01__囚途残损态_表情_六联表.png`。
...
**角色身份注册层**：CHAR_01/囚途残损态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；CHAR_02/濒死态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；BEAST_01/复生态焦外：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；本镜绑定=CHAR_01/囚途残损态、CHAR_02/濒死态、BEAST_01/复生态焦外；资产引用注册层=LOC_01, PROP_横刀。
...
**身份锁定执行绑定**：reference_group=identity_registry.CHAR_01.囚途残损态 + identity_registry.CHAR_02.濒死重伤态 + identity_registry.BEAST_01.穿心复生态；首/中/尾均使用同一组定妆母版。

**首帧**：`出图/第1集/图片/Clip02_first.png`
**尾帧**：`出图/第1集/图片/Clip02_end.png`
**锚帧1**：`出图/第1集/图片/EP01_CLIP02_a1.png`（at_sec=5.76）
**锚帧2**：`出图/第1集/图片/Clip02_end.png`（at_sec=11.2）
**场景**：LOC_01/一炷香前
...
**角色身份注册层**：CHAR_01/囚途残损态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；CHAR_02/半跪重伤态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；BEAST_01/伪死态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；本镜绑定=CHAR_01/囚途残损态、CHAR_02/半跪重伤态、BEAST_01/伪死态；资产引用注册层=LOC_01, PROP_断刀, PROP_横刀, PROP_翻覆囚车。
...
**身份锁定执行绑定**：reference_group=identity_registry.CHAR_01.囚途残损态 + identity_registry.CHAR_02.濒死重伤态；首/中/尾均使用同一组定妆母版。
...
**Motion Control / 物理交互控制**：level=none；manifest_path=无；required_inputs=；failure_modes=；degrade_plan=后端不支持音频参考口型 / 口型对不齐 → 回退 image2video 静音出片 + 后期 MuseTalk 对口型 pass（见 n2d-video 对口型节）；或分镜规避用侧脸/背身/OTS 配旁白。
**角色身份注册层**：CHAR_01/囚途残损态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；CHAR_02/半跪重伤态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；本镜绑定=CHAR_01/囚途残损态、CHAR_02/半跪重伤态；资产引用注册层=LOC_01, PROP_断刀, PROP_横刀。
**近景/反打身份锁定**：主焦点=CHAR_01/囚途残损态；脸部特写/表情参考/expressions 优先；表情锚=起幅情绪→落幅情绪；表情幅度=中；锁脸不锁情：只动眉眼嘴角，脸型/五官比例/眼距/鼻梁/下颌/发际线/痣疤保持；配角不稳则 MCU/OTS/侧脸/手部/物件反应保真实现。
**近景升格守卫**：近景升格守卫：不得把首/中/尾锚帧里脸部很小、侧背、遮挡或非主焦点的 CHAR_01/囚途残损态 直接推成清晰近脸；只有已落档且 full image_qc 通过的同源近景锚帧/脸部特写/表情参考可支撑 CU/MCU。缺该锚帧时，落幅停在原锚帧景别，或改 MCU/OTS/侧脸/手部/物件反应镜，禁止让视频模型补一张新脸。
...
**Motion Control / 物理交互控制**：level=none；manifest_path=无；required_inputs=；failure_modes=；degrade_plan=后端不支持音频参考口型 / 口型对不齐 → 回退 image2video 静音出片 + 后期 MuseTalk 对口型 pass（见 n2d-video 对口型节）；或分镜规避用侧脸/背身/OTS 配旁白。
**角色身份注册层**：CHAR_01/囚途残损态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；CHAR_02/重伤态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；BEAST_01/伪死态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；本镜绑定=CHAR_01/囚途残损态、CHAR_02/重伤态、BEAST_01/伪死态；资产引用注册层=LOC_01, PROP_横刀。
**近景/反打身份锁定**：主焦点=CHAR_01/囚途残损态；脸部特写/表情参考/expressions 优先；表情锚=起幅情绪→落幅情绪；表情幅度=中；锁脸不锁情：只动眉眼嘴角，脸型/五官比例/眼距/鼻梁/下颌/发际线/痣疤保持；配角不稳则 MCU/OTS/侧脸/手部/物件反应保真实现。
**近景升格守卫**：近景升格守卫：不得把首/中/尾锚帧里脸部很小、侧背、遮挡或非主焦点的 CHAR_01/囚途残损态 直接推成清晰近脸；只有已落档且 full image_qc 通过的同源近景锚帧/脸部特写/表情参考可支撑 CU/MCU。缺该锚帧时，落幅停在原锚帧景别，或改 MCU/OTS/侧脸/手部/物件反应镜，禁止让视频模型补一张新脸。
...
**角色身份注册层**：CHAR_01/囚途残损态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；CHAR_02/重伤搀扶态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；BEAST_01/复生态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；本镜绑定=CHAR_01/囚途残损态、CHAR_02/重伤搀扶态、BEAST_01/复生态；资产引用注册层=LOC_01, PROP_横刀。
**近景/反打身份锁定**：主焦点=CHAR_01/囚途残损态；脸部特写/表情参考/expressions 优先；表情锚=起幅情绪→落幅情绪；表情幅度=大；锁脸不锁情：只动眉眼嘴角，脸型/五官比例/眼距/鼻梁/下颌/发际线/痣疤保持；配角不稳则 MCU/OTS/侧脸/手部/物件反应保真实现。
**近景升格守卫**：近景升格守卫：不得把首/中/尾锚帧里脸部很小、侧背、遮挡或非主焦点的 CHAR_01/囚途残损态 直接推成清晰近脸；只有已落档且 full image_qc 通过的同源近景锚帧/脸部特写/表情参考可支撑 CU/MCU。缺该锚帧时，落幅停在原锚帧景别，或改 MCU/OTS/侧脸/手部/物件反应镜，禁止让视频模型补一张新脸。
**尾端落幅保持**：未触发；按 continuity.end_state 自然停住，不提前预演下一 Clip。
**原生音画策略**：audio_intent=none; risk=low; mouth_visible=no; speech_policy=no_native_speech; compose_policy=丢弃; review=生成后确认无原生人声。
...
**角色身份注册层**：CHAR_01/囚途残损态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；CHAR_02/搏命冲锋至倒地濒死态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；BEAST_01/复生态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；本镜绑定=CHAR_01/囚途残损态、CHAR_02/搏命冲锋至倒地濒死态、BEAST_01/复生态；资产引用注册层=LOC_01, PROP_横刀。
**近景/反打身份锁定**：主焦点=CHAR_01/囚途残损态；脸部特写/表情参考/expressions 优先；表情锚=起幅情绪→落幅情绪；表情幅度=大；锁脸不锁情：只动眉眼嘴角，脸型/五官比例/眼距/鼻梁/下颌/发际线/痣疤保持；配角不稳则 MCU/OTS/侧脸/手部/物件反应保真实现。
**近景升格守卫**：近景升格守卫：不得把首/中/尾锚帧里脸部很小、侧背、遮挡或非主焦点的 CHAR_01/囚途残损态 直接推成清晰近脸；只有已落档且 full image_qc 通过的同源近景锚帧/脸部特写/表情参考可支撑 CU/MCU。缺该锚帧时，落幅停在原锚帧景别，或改 MCU/OTS/侧脸/手部/物件反应镜，禁止让视频模型补一张新脸。
...
**Motion Control / 物理交互控制**：level=none；manifest_path=无；required_inputs=；failure_modes=；degrade_plan=后端不支持音频参考口型 / 口型对不齐 → 回退 image2video 静音出片 + 后期 MuseTalk 对口型 pass（见 n2d-video 对口型节）；或分镜规避用侧脸/背身/OTS 配旁白。
**角色身份注册层**：CHAR_01/囚途残损态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；CHAR_02/濒死态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；BEAST_01/复生态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；本镜绑定=CHAR_01/囚途残损态、CHAR_02/濒死态、BEAST_01/复生态；资产引用注册层=LOC_01, PROP_横刀, VFX_百妖谱, VFX_系统面板。
**近景/反打身份锁定**：主焦点=CHAR_01/囚途残损态；脸部特写/表情参考/expressions 优先；表情锚=起幅情绪→落幅情绪；表情幅度=大；锁脸不锁情：只动眉眼嘴角，脸型/五官比例/眼距/鼻梁/下颌/发际线/痣疤保持；配角不稳则 MCU/OTS/侧脸/手部/物件反应保真实现。
**近景升格守卫**：近景升格守卫：不得把首/中/尾锚帧里脸部很小、侧背、遮挡或非主焦点的 CHAR_01/囚途残损态 直接推成清晰近脸；只有已落档且 full image_qc 通过的同源近景锚帧/脸部特写/表情参考可支撑 CU/MCU。缺该锚帧时，落幅停在原锚帧景别，或改 MCU/OTS/侧脸/手部/物件反应镜，禁止让视频模型补一张新脸。
**尾端落幅保持**：未触发；按 continuity.end_state 自然停住，不提前预演下一 Clip。
...
**角色身份注册层**：CHAR_01/囚途残损态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；CHAR_02/濒死受刀态：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；BEAST_01/焦外：registry form 未在 adapter matrix 摘要中命中，使用首帧+reference_group 兜底。；本镜绑定=CHAR_01/囚途残损态、CHAR_02/濒死受刀态、BEAST_01/焦外；资产引用注册层=LOC_01, PROP_横刀。
**近景/反打身份锁定**：主焦点=CHAR_01/囚途残损态；脸部特写/表情参考/expressions 优先；表情锚=起幅情绪→落幅情绪；表情幅度=大；锁脸不锁情：只动眉眼嘴角，脸型/五官比例/眼距/鼻梁/下颌/发际线/痣疤保持；配角不稳则 MCU/OTS/侧脸/手部/物件反应保真实现。
**近景升格守卫**：近景升格守卫：不得把首/中/尾锚帧里脸部很小、侧背、遮挡或非主焦点的 CHAR_01/囚途残损态 直接推成清晰近脸；只有已落档且 full image_qc 通过的同源近景锚帧/脸部特写/表情参考可支撑 CU/MCU。缺该锚帧时，落幅停在原锚帧景别，或改 MCU/OTS/侧脸/手部/物件反应镜，禁止让视频模型补一张新脸。
