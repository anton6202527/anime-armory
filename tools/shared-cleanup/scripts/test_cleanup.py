...
def test_default_root_remains_repo_skills_after_tool_move():
    assert cleanup.DEFAULT_SKILLS_ROOT == cleanup.DEFAULT_REPO_ROOT / "skills"
...
def test_tracked_allowlist_hits_are_review_only(tmp_path):
    root = tmp_path / "skills"
    cache = root / "n2d" / "__pycache__"
    cache.mkdir(parents=True)
    (cache / "tracked.pyc").write_bytes(b"tracked cache")
    log = root / "n2d-video" / "references" / "cli_snapshots" / "_probe.log"
    log.parent.mkdir(parents=True)
    log.write_text("tracked probe log", encoding="utf-8")
    junk = root / "n2d-video" / "scratch.log"
    junk.write_text("untracked local log", encoding="utf-8")

    candidates = cleanup.scan(
        root,
        tracked_paths={
            "n2d/__pycache__/tracked.pyc",
            "n2d-video/references/cli_snapshots/_probe.log",
        },
    )
    by_path = {c.path: c for c in candidates}

    assert by_path["n2d/__pycache__"].kind == "tracked-generated-dir"
    assert by_path["n2d/__pycache__"].auto_clean is False
    assert by_path["n2d-video/references/cli_snapshots/_probe.log"].kind == "tracked-generated-file"
    assert by_path["n2d-video/references/cli_snapshots/_probe.log"].auto_clean is False
    assert by_path["n2d-video/scratch.log"].auto_clean is True

    for candidate in candidates:
        if candidate.auto_clean:
            cleanup.remove_candidate(root, candidate)
    assert (cache / "tracked.pyc").exists()
    assert log.exists()
