...
Repo maintenance tool for checking that each skill series and standalone skill remains independently distributable.
...
- executable files inside `skills/app/<app-skill>/`
...
- A standalone skill, including one under `skills/app/`, may describe ideas learned from a series, but its `.py` and `.sh` files must not import, path-load, or invoke any series implementation.
