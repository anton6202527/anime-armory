...
`anime-armory` is **not** just an application — it's a library of Claude Code **skills** that form multiple creative production lines, including n2d, comic, novel, song, mv, and ad workflows. The "source code" is the skills under `skills/`; the top-level Chinese folders under `创作区/` hold work outputs and demos.
...
**Self-contained, separately-packageable lines, dispatcher-routed.** Each production line has its own dispatcher and stage skills. **As of the 2026-06 full-independence refactor there is NO shared layer**: `skills/common/` is deleted, and line-owned modules live in their own `_lib` or craft areas. (Per-line faceswap/watermark skills were retired 2026-06.) The independence gate `tools/independence-audit/scripts/check_independence.py` enforces "no `skills/common`, no cross-line import, no `shared-*` reference". Repo-level meta-tools that aren't owned by a creative line stay single-copy under `tools/`; line-owned progress and update skills remain normal workflow skills, such as `novel-progress`, `n2d-progress`, `comic-progress`, `song-progress`, `mv-progress`, `ad-progress`, `novel-update`, `n2d-update`, `song-update`, `mv-update`, and `ad-update`.

**`skills/` is flat, grouped by name prefix** (`n2d-*`, `novel-*`, etc.). A SKILL.md's frontmatter `description` + the `Triggers`/`Use when` lines **are the routing logic** — match user intent against them. `.claude/skills → ../skills` is a symlink so Claude Code auto-discovers them.

**n2d-supervisor is the agentic layer, not a replacement state machine.** It consumes `skills/n2d/run.py next --json`, uses context packs / creative loops / action contracts, and dispatches a few specialists; it must not bypass `_进度.md`, gates, batch, dashboard, or stage skills. Novel text-project update planning lives in `novel-update`; it writes only plan/baseline artifacts under `生产数据/`.
