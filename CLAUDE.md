...
`anime-armory` is **not** an application — it's a library of Claude Code **skills** that form the **n2d** creation + production factory: 写小说→制漫剧 (novel → AI comic-drama). The "source code" is the skills under `skills/`; the top-level Chinese folder `制漫剧/` holds **demo outputs**, not application data.
