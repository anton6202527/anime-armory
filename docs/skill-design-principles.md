...
- **B7 分档人物定妆基础包不可缺失铁律 / Clip 图前分档资产基础包铁律** ✅ — n2d 中所有入镜具名人物都必须有作品根 `角色库/<CHAR_ID>__<slug>/manifest.json`，但资产生产深度不得一刀切。`library_tier` 固定为四档：`core_full`（主角/核心长线/计划出场≥10集；“≥10集”是 n2d 项目默认启发式，不是普适行业定律）要求五个独立可喂图视角 `front/three_quarter/side/rear_three_quarter/back` + `half_body|full_body|outfit` + `face_anchor_refs|expressions` + `turnaround`；`recurring_standard` 要 `front/three_quarter` + body/outfit + face anchor，侧背按镜头补；`named_minimal` 要 `front` + body/outfit + face anchor，近景/转头/过肩/侧背/多集复用时升档；`restricted_partial` 只建允许的手部/肩背/服装/剪影局部。档位必须由剧情权重与计划出场数推导的最低值和显式升档取高值，角色记录、asset bundle、manifest 与 `reference_atlas.build_tier` 四处必须精确对账；单集功能型 `named_minimal` 可按结构化 restricted/no-face policy 走局部资产，复现 ≥3 集或核心/长线具名人物只有包含 `status=approved`、充分 reason、reviewer、allowed_parts、与角色记录完全一致的 no-face policy，且 allowed_parts 不含 face/head/front/portrait/正脸/头部的 `restricted_partial_contract` 才能走局部例外；`GROUP_`/`CROWD_` 群像可按局部资产处理。该档位只控制资产深度，不替代主体 ID/face embedding/LoRA 身份锁档。已有拆分 PNG 必须同源派生：45°/侧/后45°/背从同一有人工声明收据的母本派生，半身/脸锚从已通过正面裁切，`ready` 登记 `derivation.method/source_path/source_sha256/crop_box`；`planned` 不能冒充本档或本镜必需项。核心人物每种形态的五个视角必须是不同真实 PNG 路径和不同当前像素；“独立”指可单独喂图，不要求独立生成，同一已签母本的合规派生可以使用，但复制同字节文件、同像素重编码换 SHA、软链别名或换标签不能冒充独立角度。证据只能使用解析后仍在作品根内的规范相对路径，绝对路径、`..` 越界和非规范别名不得放行。每个视角、turnaround 与 expression/face anchor 的人工声明收据必须绑定当前可完整解码 PNG SHA、角色/形态/档位/视角/路径、对应 review contract、完整 criteria、人工声明 reviewer（非空且不得含明显自动化标识）、带时区时间与 `confirmation={"kind":"explicit_current_pixels_acceptance","accepted_current_pixels":true}`；普通文件/PNG 空壳、旧收据、明显自动化 reviewer 标识、同图换标签或跨形态复用均不得放行。本地 reviewer 字符串与 confirmation 只能证明收据声明已查看所绑定的当前像素，不能认证真实身份或独立性；需要强保证时必须接外部认证 ID、签名或审批系统收据，并绑定同一对象合同与当前 PNG SHA。任何 Clip PNG 生成前，本集共享资产必须先满足**所属档位 + 当前镜头实际需求**及 registry 约束；场景/道具/武器/VFX 仍须先有参考与结构锁，核心武器须 `weapon_profile`。缺本档/本镜必需项时只能补共享层，**不得生成 Clip 分镜图**；共享库先行顺序不可被 `--skip-preflight`、P0 垂直切片、抽样或局部 `--shots` 绕过。机器覆盖：tier-aware `check_identity_registry` / `reference_planner` 与 `core_full/recurring_standard/named_minimal/restricted_partial` 回归测试、自报降档反例、`rear_three_quarter` 回归、逐视图 current SHA 收据、同图多角度/复制同像素/同像素重编码换 SHA/软链与越界路径/跨形态/伪 PNG/PNG 空壳/伪人工元数据反例、同源不同裁切像素正例、`enforce_shared_first_interlock`、prompt/checklist 和 `tools/validate_skills.py --only B7`。
...
- **本线特有工艺原则**留在各线 `*-craft/SKILL.md` 的 `## 设计原则`：n2d 默认“声音选角 + 时间基准先行、逐镜混合音画路由、最终配音后置” + 两层出图；ad 不拆集 + cutdown 轴、song/mv 多版是默认工程事实。
