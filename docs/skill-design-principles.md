...
- **B7 人物定妆基础包不可缺失铁律 / Clip 图前完整资产基础包铁律** ✅ — n2d 中任何会入镜的人物、角色形态、服装/觉醒/妖形变体，只要不是明示 `restricted_partial` 且永不露完整脸的局部参考，都必须先具备**七类基础定妆包**：① 正面主参考 `front`；② 45°/三分之二侧脸 `three_quarter`；③ 侧面 `side`；④ 背面 `back`；⑤ 半身或全身服装体态 `half_body`/`full_body`；⑥ 同源脸部特写或基础表情脸锚 `face_anchor_refs`/`expressions`；⑦ 三视图/设定表人审拼版 `turnaround`。**三视图不能替代拆分 PNG**，且拆分 PNG 也不能逐张重生：45°/侧/背必须从同一张人审通过 turnaround 同源母本派生，半身/脸部特写必须从已通过正面裁切，`ready` 必须登记 `derivation.method/source_path/source_sha256/crop_box`。`planned` 只能表示待补缺口，不能放行；主角、配角、功能角色、路人和一次性人物同一基础门槛，角色体量只影响表情库、动作参考、主体库/LoRA 是否升档。同一条也管 Clip 前共享资产基础包：任何 `Clip_*` / `镜头*` 分镜 PNG 生成前，本集引用到的人物、形态、场景、配饰、武器、道具、VFX 等共享资产必须先完成可传给模型的基础包与 registry 约束；缺正面/45°/侧面/背面/半身或全身/脸锚或同源表情/三视图人审拼版、配饰/标志物、道具/武器/VFX 参考图、核心武器 `weapon_profile` 等任一项时，只能先补共享资产，**不得生成 Clip 分镜图**；**共享库先行顺序不可被 `--skip-preflight`、P0 垂直切片、抽样验证或 `--shots Clip_XX` 局部跑数豁免**，runner 必须在生成循环前执行不可跳过的 shared-first interlock。机器覆盖：`n2d-image/scripts/derive_makeup_pack.py` 同源派生、`n2d-review/scripts/gate.py` 的 `check_identity_registry` 与回归测试，`n2d-image/scripts/codex_image_runner.py` 的 `enforce_shared_first_interlock` 与回归测试，`n2d-image/references/prompt_format.md`、`角色一致性checklist.md`、`QUICKSTART.md` 的 Clip 前置锚点，`tools/validate_skills.py --only B7` 防止宪法、n2d-image 铁律、runner 锁、gate 常量、prompt 模板或测试被删弱。
