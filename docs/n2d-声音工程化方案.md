...
- 仍未把某一音乐生成厂商写成唯一路径；generated BGM 必须经项目适配层落真实文件与 provenance。多爽点音乐编排与真实 SFX 库仍可继续增强，但不再以正弦波占位假装已完成。

## 0. 一句话诊断

n2d 的历史问题曾是**声音分裂**；2026-07-11 后，以下内容作为问题来源而非当前行为：
...
3. **复用时间基准先行的经验**。不是全项目 final voice 先行；按逐镜 route 决定表演轨、粗时间、画面先行和原生音画。BGM/最终配音在合成边界对锁定时间线做新鲜度对账。
