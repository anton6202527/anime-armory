...
| 防降级宪章 | `scripts/consistency_charter.py` + `test_consistency_charter.py` | n2d 同哲学：每条承重硬闸（block code）在宪章占一行（带日期理由），守卫测试内省 gate.py 源码 + 功能验证 advisory 降档/新鲜度/占位 VO 纪律；静默降档立即红灯，降档必须先改宪章行=显式审计决策 |
