#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""广告**生成次数预算**账本（事前·advisory·n2d clip_economy 的广告薄版）。

为什么：stop_loss 管的是**事后**止损（重抽率/credit 已花超线），管不了**结构性浪费**——
分镜定稿那一刻，这条片要花多少次付费生成（首帧+尾帧+图生视频逐镜）就已经定了。
n2d 线的教训（take 爆炸：8 clip 被拆成 24 次付费 take）在广告线的对应病：连续同场景
...
安全边界（与 n2d shot_split_decision 同口径的保守面）：endcard/产品 beauty 等有意独立镜
