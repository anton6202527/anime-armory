...
1. **核对输入**：缺哪条问哪条。
2. **生成 5–8 个候选**：按 `references/title-patterns.md` 的配比（主推平台 2-3 + 跨平台稳健 1-2 + 高钩子尝试 1-2 + 保守雅名 0-1 + 用户暂定 0-1）。若 `<repo>/生产战绩/差异化候选.json` 存在，本书题材命中其中白空间组合时，让"高钩子尝试"名向该差异化卖点靠（如倒叙/马甲/反套路角度），把选题闭环的差异化优势带进书名。
3. **5 维评分**：钩子 / 平台契合 / 角色识别 / 抗撞名 / 可记忆性，各 1-5 分，满分 25。
4. **撞名检查（硬要求）**：前 4 名候选必须联网/榜单查重；把 web 搜索结果或榜单页面交给 `scripts/check_collision.py`，落 `设定/书名撞名检查_<YYYY-MM-DD>.json|md`。硬撞名直接淘汰，软撞名降分并说明。若浏览器/搜索引擎已查但无明确命中，用 `--checked "候选|查询说明|URL|平台"` 留下无命中搜索记录；若搜到疑似撞名，用 `--hit "候选|命中标题|URL|平台"`。**`status=unchecked`（没给任何 `--source`/`--hit`/`--checked`、或来源全抓取失败）= 没查过，≠ 不撞名**——绝不能当 `clear` 推荐，必须补来源/命中重查。
5. **排表 + 推荐**：按总分降序；给一句话定位；标"我推" + "备选 A / B" + 弃用说明。
6. **用户选定**：把通过撞名检查的前 4 名作为选项给用户确认。
7. **回写**：
...
- `设定/书名撞名检查_<YYYY-MM-DD>.json|md`（如果有项目目录）—— 候选撞名证据
- 主对话报告：表格 + 推荐 + 前 4 名选项让用户选
...
| 给的候选撞名严重 | 前 4 名必须跑 `scripts/check_collision.py`；硬撞名直接淘汰，不进推荐 |
