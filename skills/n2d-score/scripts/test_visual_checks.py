...
def test_image_similarity_large_cut_is_warning_not_block(tmp_path: Path, monkeypatch) -> None:
    img_dir = tmp_path / "出图" / "第1集" / "图片"
    img_dir.mkdir(parents=True)
    end = img_dir / "Clip_01_end.png"
    first = img_dir / "Clip_02_first.png"
    ...
    sb.write_text(json.dumps({
        "clips": [
            {"id": "Clip_01", "label": "一", "continuity": {"endframe_png": "出图/第1集/图片/Clip_01_end.png"}},
            {"id": "Clip_02", "label": "二", "firstframe_png": "出图/第1集/图片/Clip_02_first.png"},
        ]
    }, ensure_ascii=False), encoding="utf-8")
    monkeypatch.setattr(visual, "dhash", lambda path: 0 if path.endswith("end.png") else (1 << 30) - 1)

    sec = visual.check_image_similarity(str(tmp_path), "第1集")
