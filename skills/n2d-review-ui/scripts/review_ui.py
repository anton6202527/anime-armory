...
def infer_first_frame(root: Path, ep: str, clip: Dict[str, Any], number: int) -> Optional[str]:
    ...
    if direct:
        direct_path = resolve_path(root, direct)
    ...
    if found:
        return rel_to_root(root, Path(found))
...
def load_video_batch_items(root: Path, ep: str) -> Dict[int, Dict[str, Any]]:
    """Latest video runner manifest item per Clip number.

    review_ui is read-only, but it should expose whether storyboard anchors were
    actually consumed by the backend rather than only declared in prompt text.
    """
    out: Dict[int, Dict[str, Any]] = {}
    for path in sorted(production_dir(root).glob(f"video_batch_{ep}_*.json"), key=lambda p: p.stat().st_mtime):
        data = load_json(path)
        ...
        for item in data.get("items") or []:
            if not isinstance(item, dict):
                continue
            num = clip_number(item.get("clip") or item.get("heading") or item.get("target"), -1)
            if num <= 0:
                continue
            copied = dict(item)
            copied["_manifest_path"] = rel_to_root(root, path)
            out[num] = copied
    return out
...
def _frame_item(
    ...
    *,
    ...
    media = asset(root, html_dir, value, kind="image")
    if not media:
        return None
    media.update({
        "label": label,
...
def _dedupe_frames(frames: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for frame in frames:
        path = str(frame.get("path") or frame.get("url") or frame.get("name") or "").lower()
        key = path or f"{frame.get('label')}:{frame.get('at_sec')}:{frame.get('role')}"
        if key in seen:
            continue
        seen.add(key)
        out.append(frame)
...
def collect_anchor_frames(
    ...
    first = _frame_item(root, html_dir, first_frame, label="首帧", role="first", at_sec=0, use="timeline")
    if first:
        frames.append(first)

    mid = continuity.get("midframe") or clip.get("midframe")
    mid_item = _frame_item(root, html_dir, mid, label="中段锚帧", role="anchor", use="timeline")
    if mid_item:
        frames.append(mid_item)

    for idx, raw in enumerate(continuity.get("anchors") or [], 1):
        if not isinstance(raw, dict):
            continue
        value = raw.get("anchor_png") or raw.get("png") or raw.get("path") or raw.get("image") or raw.get("image_path")
        label = raw.get("label") or raw.get("name") or f"锚帧{idx}"
        item = _frame_item(
            root,
            html_dir,
            value,
            label=str(label),
            role=str(raw.get("role") or "anchor"),
            at_sec=raw.get("at_sec") or raw.get("time_sec") or raw.get("t"),
            use=raw.get("use"),
...
def collect_consumed_frames(root: Path, html_dir: Path, manifest_item: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
    if not isinstance(manifest_item, dict):
        return []
    values = manifest_item.get("multiframe_images_rel") or manifest_item.get("multiframe_images") or []
    ...
    for idx, value in enumerate(values, 1):
        item = _frame_item(
            ...
            label=f"入参{idx}",
            role="backend_input",
            source=str(manifest_item.get("_manifest_path") or "video_batch"),
        )
        if item:
            frames.append(item)
    return _dedupe_frames(frames)
...
def anchor_consumption_summary(root: Path, manifest_item: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    if not isinstance(manifest_item, dict):
        return None
    raw = manifest_item.get("anchor_consumption") if isinstance(manifest_item.get("anchor_consumption"), dict) else {}
    return {
        "manifest_path": manifest_item.get("_manifest_path"),
        "mode": manifest_item.get("anchor_consumption_mode") or raw.get("consumption_mode"),
        "frame_control_mode": manifest_item.get("frame_control_mode") or raw.get("frame_control_mode"),
        "backend": raw.get("execution_backend") or raw.get("backend") or manifest_item.get("cost_provider"),
        "consumes_endframe": raw.get("consumes_endframe"),
        "consumes_mid_anchors_natively": raw.get("consumes_mid_anchors_natively"),
        "requires_split_relay": raw.get("requires_split_relay"),
        "anchor_count": raw.get("anchor_count"),
    }
...
def collect_clips(
    ...
    flags: List[Dict[str, Any]],
    video_items: Optional[Dict[int, Dict[str, Any]]] = None,
) -> List[Dict[str, Any]]:
    ...
    for idx, clip_raw in enumerate(raw, 1):
        ...
        video_item = video_items.get(num)
        clip = {
            "index": idx,
            "number": num,
            ...
            "anchor_frames": collect_anchor_frames(root, ep, html_dir, clip_raw, first, end, num),
            "consumed_frames": collect_consumed_frames(root, html_dir, video_item),
            "anchor_consumption": anchor_consumption_summary(root, video_item),
        }
...
def build_manifest(root: Path, ep: str) -> Dict[str, Any]:
    ...
    video_items = load_video_batch_items(root, ep)
    clips = collect_clips(root, ep, html_dir, storyboard, flags, video_items)
    return {
        "kind": KIND,
        "version": VERSION,
        "root": str(root),
...
.anchor-strip {{ border-top:1px solid var(--line); padding:8px; background:#fbfcff; }}
.anchor-title {{ display:flex; justify-content:space-between; gap:8px; color:var(--muted); font-size:12px; margin:0 0 6px; }}
.anchor-row {{ display:flex; gap:6px; overflow-x:auto; padding-bottom:2px; }}
.anchor-frame {{ width:92px; flex:0 0 auto; border:1px solid var(--line); border-radius:6px; overflow:hidden; background:#eef2f7; }}
.anchor-thumb {{ height:92px; display:flex; align-items:center; justify-content:center; position:relative; }}
.anchor-thumb img {{ width:100%; height:100%; object-fit:cover; display:block; }}
.anchor-meta {{ min-height:42px; padding:4px 5px; border-top:1px solid var(--line); font-size:11px; line-height:1.25; color:var(--muted); overflow-wrap:anywhere; }}
.anchor-missing {{ padding:8px; text-align:center; font-size:11px; color:var(--muted); }}
...
function hasMissing(clip) {{
  return ['first_frame','end_frame','video'].some(k => clip[k] && !clip[k].exists) ||
    (clip.anchor_frames || []).some(f => f && !f.exists) ||
    (clip.consumed_frames || []).some(f => f && !f.exists);
}}
...
function anchorBox(item) {{
  const at = item.at_sec !== null && item.at_sec !== undefined && item.at_sec !== '' ? `${{esc(item.at_sec)}}s` : '';
  const use = item.use ? ` · ${{esc(item.use)}}` : '';
  const reason = item.reason ? `<br>${{esc(item.reason)}}` : '';
  const body = item.exists ? `<img src="${{item.url}}" loading="lazy">` : `<span class="anchor-missing">缺<br>${{esc(item.path || '')}}</span>`;
  return `<div class="anchor-frame"><div class="anchor-thumb"><span class="media-label">${{esc(item.label || item.role || '锚帧')}}</span>${{body}}</div><div class="anchor-meta">${{at}}${{use}}${{reason}}</div></div>`;
}}
function renderAnchorStrip(clip) {{
  const frames = clip.anchor_frames || [];
  const consumed = clip.consumed_frames || [];
  const ac = clip.anchor_consumption || {{}};
  if (!frames.length && !consumed.length) return '';
  const status = [ac.mode, ac.frame_control_mode, ac.backend].filter(Boolean).join(' · ') || '未登记后端消费';
  const planned = frames.length ? `<div class="anchor-title"><span>锚帧链（计划/审片）</span><span>${{esc(status)}}</span></div><div class="anchor-row">${{frames.map(anchorBox).join('')}}</div>` : '';
  const used = consumed.length ? `<div class="anchor-title" style="margin-top:8px"><span>后端实际入参</span><span>${{esc(ac.manifest_path || '')}}</span></div><div class="anchor-row">${{consumed.map(anchorBox).join('')}}</div>` : '';
  return `<div class="anchor-strip">${{planned}}${{used}}</div>`;
}}
