...
DEFAULTS = {
    ...
    "原稿规格": "数字条漫",
    "版式模板策略": "自动缩略分镜",
    # 默认由制作代理完成常规、可逆、内部审阅并留下证据与哈希；只有合规、
    # 预算扩大、不可逆发布、核心方向冲突和最终验收才回到用户。
    "审阅策略": "用户授权制作代理",
    # 新项目明确写入视觉代理授权；代理必须实际查看当前像素，只能推进可逆
    # 内部生产并写 human_signoff=false，最终具名验收仍是独立硬边界。
    "视觉审阅策略": "用户授权制作代理实际查看当前像素",
    "网点策略": "风格驱动",
    "效果线策略": "剧情驱动",
    "基础视觉风格": "彩色国漫条漫",
    "风格锚": "未指定",
...
SETTING_SPECS: Tuple[SettingSpec, ...] = (
    ...
    SettingSpec("传统原稿流程", ("comic",), ("启用", "关闭")),
    SettingSpec(
        "出图稿层",
        ("comic",),
        ("完成稿", "草稿/name", "铅笔稿", "清线稿", "墨线+黑场", "网点完成稿", "彩色完成稿", "自定义"),
        parameterized=True,
    ),
    SettingSpec(
        "原稿规格",
        ("comic",),
        ("数字条漫", "数字页漫", "B5商漫", "A4投稿", "四格原稿", "自定义"),
        parameterized=True,
    ),
    SettingSpec(
        "版式模板策略",
        ("comic",),
        ("自动缩略分镜", "手工缩略分镜", "模板库", "关闭", "自定义"),
        aliases={"自动\u30cd\u30fc\u30e0": "自动缩略分镜", "手工\u30cd\u30fc\u30e0": "手工缩略分镜"},
        parameterized=True,
    ),
    SettingSpec(
        "审阅策略",
    ...
    SettingSpec(
        "网点策略",
        ("comic",),
        ("风格驱动", "显式tone_plan", "灰阶替代", "关闭", "自定义"),
        parameterized=True,
    ),
    SettingSpec(
        "效果线策略",
        ("comic",),
        ("剧情驱动", "显式effects_plan", "轻量", "关闭", "自定义"),
        parameterized=True,
    ),
    SettingSpec(
        "基础视觉风格",
    ...
    SettingSpec(
        "生图渠道",
        ("comic",),
        ("Codex CLI", "内置 imagegen", "OpenAI API", "Dreamina/即梦官方 CLI", "Seedream", "可灵主体库", "Nano Banana", "manual", "自定义"),
    ...
    SettingSpec(
        "生图AI",
        ("comic",),
        ("Codex", "Codex内置imagegen", "OpenAI", "Dreamina", "即梦", "Seedream", "可灵主体库", "Nano Banana", "manual", "自定义"),
        key_aliases=("生图后端",),
        parameterized=True,
    ),
    SettingSpec(
...
def write_settings(
    ...
    *,
    ...
    lines += [
        ...
        "改了在此更新。付费生成按阶段预算包一次授权；权利合规、超预算、"
        "不可逆发布和最终验收仍向用户确认。",
    ]
