...
STAGE_NEXT_COMMAND = {
    "source": 'python3 skills/comic/comic-script/scripts/development_pack.py "{root}" check --strict --json',
    "script": 'python3 skills/comic/comic-script/scripts/source_semantics_gate.py "{root}" --chapter {ch}',
    "name": 'python3 skills/comic/comic-name/scripts/build_name_board.py "{root}" --chapter {ch}',
    "layout": 'python3 skills/comic/comic-layout/scripts/build_layout.py "{root}" --chapter {ch}',
    "finishing": 'python3 skills/comic/comic-finishing/scripts/build_finishing_plan.py "{root}" --chapter {ch}',
    "image_jobs": 'python3 skills/comic/comic-image/scripts/build_panel_jobs.py "{root}" --chapter {ch}',
    "image": 'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage image_preflight',
    "compose": 'python3 skills/comic/comic-compose/scripts/export_longstrip.py "{root}" --chapter {ch} --render --qc-slots',
    "review": 'python3 skills/comic/comic-review/scripts/review.py "{root}" --chapter {ch}',
}
# Fallback for blocker/special fronts whose stage label is not in STAGE_KEYS.
SKILL_NEXT_COMMAND = {
    "comic-script": 'python3 skills/comic/comic-script/scripts/development_pack.py "{root}" check --strict --json',
    "comic-identity": 'python3 skills/comic/comic-identity/scripts/model_pack.py "{root}" check --write --json',
    "comic-name": 'python3 skills/comic/comic-name/scripts/build_name_board.py "{root}" --chapter {ch} --check',
    "comic-layout": 'python3 skills/comic/comic-layout/scripts/build_layout.py "{root}" --chapter {ch} --check',
    "comic-finishing": 'python3 skills/comic/comic-finishing/scripts/build_finishing_plan.py "{root}" --chapter {ch}',
