...
- 自动出图后端适配层。
- 更强像素级角色/场景一致性机检：已落地 CCIP 动漫身份 embedding 快筛（装 `dghs-imgutils` 时启用，阈值 0.178；缺依赖降级色彩指纹并明示）、CANVAS 三轴 VLM 并排判定任务包（脸/服装体型、背景连续、道具身份位置，agent 看图裁决、sha 绑定防过期）、场景锚组 contact sheet + 布局指纹、跨话风格基准回归（`出图/共享/style_baseline.json`）。后续可再接人脸检测框/深度布局检测，但不得降低现有 gate。
