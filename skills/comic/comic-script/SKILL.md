...
```bash
python3 skills/comic/comic-script/scripts/source_semantics_gate.py "创作区/画漫画/作品名" --chapter 第1话 --source-language 文言 --target-text-language 中文 --force-normalization
```

连续多话消费同一源章时，先为新话运行上面的 gate 生成当前合同骨架，再继承前话已经审定的释义；该迁移只复用语言字段，当前话合同、源 SHA、消费决策与签收不会沿用：

```bash
python3 skills/comic/comic-script/scripts/inherit_source_semantics.py "创作区/画漫画/作品名" \
...
- **追更再入前情机检（SP2·report-only·gate image_preflight 自动跑）**：条漫是长线周更连载，读者隔周回来看第 N 话。`continuity_audit` 只查**声明态**链、`_breadth_issues` 只查**源覆盖广度**、`chapter_beat_audit` 只查首格有没有 opening_hook 这个功能位——**没有谁查「读者进入第 N 话接不接得上」**。`reentry_context_audit.py <作品根> 第N话 --write` 对 N≥2 的连载话查两条：①**前情锚不足**（开场前 K 格里旁白回顾/常驻角色再登场/回顾建立类功能格/承接上一话场景锚**全都没有**）；②**未交代实体**（本话中段冒出的结构化 `character_id` 既不在 identity 登记表、留存前话、也不在合同 entry_state，且首现不在开场窗口/登场功能格）。自足话（standalone/oneshot/四格）与首话自动跳过。参照同仓成熟视频线 midstart_context/antecedent_audit/narrative_state_audit 合三为一，用结构化在场数据省掉自由文本 NER、低误报。全部 warn 交人判，`--strict` 仅供人手排查。
- **实体在场契约机检（SP3·report-only·gate image_preflight 自动跑）**：2026-07-17 实证（第1话虎妖被画成四足普通虎/断横刀连续六格被换成别的刀剑）暴露：脚本层非角色实体是自由文本，「画面描述提到虎妖/断刀，但该格 references 忘绑对应 ID」时出图不带参考、事后机检也不核对。`entity_presence_audit.py <作品根> 第N话 --write` 参照同仓视频线 storyboard `entity_schedule`（required/offscreen/forbidden_presence）重实现：①**提到未绑定**——画面面文本（description/art_notes/location）命中 registry 实体名/别名（`assets[*].aliases` 人工维护）但该格 characters/references/scene_anchor 未绑 → warn；仅台词/旁白提及 → info。②**entity_schedule 契约**（panel 可选字段·渐进采用）——required⊄绑定集/required∩forbidden/forbidden∩绑定集：显式写了排程的格属**确定性契约**，gate 按 `deterministic_state_contract` 阻断（与 continuity 状态链同级）；未写排程的格在报告 `derived_schedule` 给出派生底稿，`--adopt-derived` 可一键物化为显式契约（panel_script 变更会触发下游重闸）。③**ack-or-fix**——画面提及确认不入画时，在该格写 `unbound_mention_ack: {ENTITY_ID: "一句话理由"}` 显式签收，warn 降 info；空理由不算签收。本检治「忘绑参考」，与 vlm_judge（绑了仍画错）两层正交。
