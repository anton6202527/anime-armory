...
from provenance import append_event, binding, load_events, sign_c2pa, validate_chain, verify_c2pa_receipt, write_c2pa_sidecar  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("command", choices=("append", "verify", "sidecar", "sign", "verify-signature")); parser.add_argument("project_root"); parser.add_argument("asset", nargs="?", default="")
    ...
    parser.add_argument("--adapter-id", default=""); parser.add_argument("--output", default=""); parser.add_argument("--receipt", default="")
    ...
    try:
        ...
        elif args.command == "sign": result = sign_c2pa(root, asset, output=(root / args.output).resolve() if args.output else None, adapter_id=args.adapter_id)
        elif args.command == "verify-signature":
            receipt_path = (root / args.receipt).resolve() if args.receipt else asset
            result = verify_c2pa_receipt(root, receipt_path)
        else:
    ...
    print(json.dumps(result, ensure_ascii=False, indent=2)); return 1 if result.get("errors") or result.get("status") == "fail" else 0
