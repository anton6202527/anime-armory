...
- `出图/共享/图片/<CHAR_ID>__front.png` / `__three_quarter.png` / `__side.png` / `__back.png` / `__face.png`：人物标准多视图包。项目 `_设置.md` 的 `定妆级别` 默认是 `长线专门定妆`，常驻角色进入批量生产前必须补齐。`MON_` 妖物、`BEAST_` 走兽异兽、`ANIMAL_` 真实动物与角色同标准（生物级）：views/report/model-pack 一并覆盖（2026-07-17 起生物默认纳管、2026-07-24 起任意档位默认纳管，`model_pack_required: false` 可显式退出）。`WEAPON_` 与 `PROP_` 同为道具级参考。
...
- `生产数据/comic_identity_report_第N话.json/md`：缺失引用、每格真实参考输入数、重抽目标；视图完整性与 model-pack 就绪度审计覆盖 `CHAR_`+`MON_`+`BEAST_`+`ANIMAL_`（一切生物/动物纳管，不再豁免）。
...
```bash
python3 skills/comic/comic-identity/scripts/wardrobe_props.py "创作区/画漫画/作品名" scaffold --write --json
# 填入研究来源、wardrobe / props；历史可考项目须有实物/考古与图像/文献/馆校研究交叉证据
python3 skills/comic/comic-identity/scripts/wardrobe_props.py "创作区/画漫画/作品名" check --strict --json
python3 skills/comic/comic-identity/scripts/wardrobe_props.py "创作区/画漫画/作品名" apply --write --json
```
...
```bash
python3 skills/comic/comic-identity/scripts/registry_v2.py "创作区/画漫画/作品名" init --write --json
```
...
```bash
python3 skills/comic/comic-identity/scripts/registry_v2.py "创作区/画漫画/作品名" upsert \
...
```bash
python3 skills/comic/comic-identity/scripts/identity.py "创作区/画漫画/作品名" --chapter 第1话 outfits \
...
```bash
python3 skills/comic/comic-identity/scripts/identity.py "创作区/画漫画/作品名" --chapter 第1话 expressions \
...
```bash
python3 skills/comic/comic-identity/scripts/identity.py "创作区/画漫画/作品名" --chapter 第1话 probe-backend \
...
```bash
python3 skills/comic/comic-identity/scripts/identity.py "创作区/画漫画/作品名" --chapter 第1话 adopt-anchor \
...
```bash
python3 skills/comic/comic-identity/scripts/library.py "创作区/画漫画/作品名" --write
```

旧项目若存在纯机器派生的 `角色库/`、`资产库/`，可一次性安全迁移：

```bash
python3 skills/comic/comic-identity/scripts/library.py "创作区/画漫画/作品名" --write --remove-legacy-views
```
