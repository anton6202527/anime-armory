...
def skill_dir(skill: str) -> str:
    ...
    candidates = [os.path.join(REPO_SKILLS, skill), os.path.join(REPO_SKILLS, line, skill)]
    if os.path.basename(os.path.normpath(REPO_SKILLS)) == line and skill == line:
        candidates.insert(0, REPO_SKILLS)
    return next((path for path in candidates if os.path.isfile(os.path.join(path, "SKILL.md"))), candidates[0])
...
def iter_skill_files(skill: str) -> Iterable[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames[:] = [
            ...
            and not (skill == skill.split("-", 1)[0] and os.path.isfile(os.path.join(dirpath, d, "SKILL.md")))
        ]
...
def existing_skill_names(names: Iterable[str]) -> list[str]:
    return sorted(name for name in set(names) if os.path.isdir(skill_dir(name)))
...
def build_execution_steps(root: str, plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not plan.get("rebuild_needed"):
        steps.append({"type": "command", "purpose": "查看当前前沿", "command": f'python3 skills/comic/comic-progress/scripts/scan.py "{root}"'})
        if plan.get("changed_skills") or plan.get("baseline_bootstrapped"):
            steps.append({
                "type": "command",
                "purpose": "接受当前产物后记录新基线",
                "command": f'python3 skills/comic/comic-update/scripts/update_plan.py record "{root}"',
                "run_when": "用户确认无需重制",
            })
        return steps
    ...
    for chapter in plan.get("affected_chapters") or []:
        ...
        if all_receipt_only:
            for gate_stage in sorted(receipt_only, key=lambda value: stage_index("image_jobs" if value == "image_preflight" else value)):
                steps.append({
                    ...
                    "command": f'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage {gate_stage}',
                })
            continue
        if stage_index(rerun_from) <= stage_index("script"):
            ...
            steps.append({
                ...
                "command": f'python3 skills/comic/comic-script/scripts/development_pack.py "{root}" check --strict --json',
            })
            steps.append({"type": "command", "purpose": f"{ch} 脚本 gate", "command": f'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage script'})
        if stage_index(rerun_from) <= stage_index("name"):
            steps.append({"type": "command", "purpose": f"{ch} 生成缩略分镜 draft", "command": f'python3 skills/comic/comic-name/scripts/build_name_board.py "{root}" --chapter {ch}'})
            steps.append({"type": "command", "purpose": f"{ch} 提交缩略分镜复核", "command": f'python3 skills/comic/comic-name/scripts/build_name_board.py "{root}" --chapter {ch} --submit-review', "run_when": "draft 人工检查完成"})
            steps.append({"type": "command", "purpose": f"{ch} 签收缩略分镜", "command": f'python3 skills/comic/comic-name/scripts/build_name_board.py "{root}" --chapter {ch} --approve --reviewed-by "REVIEWER_NAME"', "run_when": "review 通过；执行前把 REVIEWER_NAME 替换为真实签收人"})
            steps.append({"type": "command", "purpose": f"{ch} 缩略分镜 gate", "command": f'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage name'})
        if stage_index(rerun_from) <= stage_index("layout"):
            steps.append({"type": "command", "purpose": f"{ch} 重建页面排版 draft", "command": f'python3 skills/comic/comic-layout/scripts/build_layout.py "{root}" --chapter {ch}'})
            steps.append({"type": "command", "purpose": f"{ch} 提交排版复核", "command": f'python3 skills/comic/comic-layout/scripts/build_layout.py "{root}" --chapter {ch} --submit-review', "run_when": "draft 人工检查完成"})
            steps.append({"type": "command", "purpose": f"{ch} 签收排版", "command": f'python3 skills/comic/comic-layout/scripts/build_layout.py "{root}" --chapter {ch} --approve --reviewed-by "REVIEWER_NAME"', "run_when": "review 通过；执行前把 REVIEWER_NAME 替换为真实签收人"})
            steps.append({"type": "command", "purpose": f"{ch} 排版 gate", "command": f'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage layout'})
        if not traditional_off and stage_index(rerun_from) <= stage_index("finishing"):
            steps.append({"type": "command", "purpose": f"{ch} 生成原稿收尾计划", "command": f'python3 skills/comic/comic-finishing/scripts/build_finishing_plan.py "{root}" --chapter {ch}'})
            steps.append({"type": "command", "purpose": f"{ch} 原稿收尾 gate", "command": f'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage finishing'})
        if stage_index(rerun_from) <= stage_index("image_jobs"):
            steps.append({"type": "command", "purpose": f"{ch} 校验定妆包与 SHA 签收", "command": f'python3 skills/comic/comic-identity/scripts/model_pack.py "{root}" check --write --json'})
            steps.append({"type": "command", "purpose": f"{ch} 重建出图包", "command": f'python3 skills/comic/comic-image/scripts/build_panel_jobs.py "{root}" --chapter {ch}'})
            steps.append({"type": "command", "purpose": f"{ch} 出图前 gate", "command": f'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage image_preflight'})
        ...
        if stage_index(chapter.get("current_stage")) >= stage_index("compose"):
            ...
            steps.append({"type": "command", "purpose": f"{ch} 合成 gate", "command": f'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage compose'})
        steps.append({"type": "command", "purpose": f"{ch} 重制后审查 gate", "command": f'python3 skills/comic/comic-review/scripts/gate.py "{root}" --chapter {ch} --stage review'})
    steps.append({
        "type": "command",
        "purpose": "验收后记录新基线",
        "command": f'python3 skills/comic/comic-update/scripts/update_plan.py record "{root}"',
