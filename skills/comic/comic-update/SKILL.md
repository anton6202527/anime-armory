...
- 实际返修用 `revision_transaction.py start → verify → promote`：`verify` 进入 `prepared`，提升先持有话级锁并把 `promoting` 写入 fsync WAL，再逐文件原子替换；gate 通过才进入 `committed`，失败按逐目标 CAS 恢复 backup。进程在 promoting 中被杀时执行 `revision_transaction.py recover <root> <tx-id>`；显式 rollback 仅允许 `promoting/committed`，且当前 SHA 必须仍是本 tx 的 base/candidate 前沿。若已有更新版本，绝不覆盖，写 `three_way_recovery_plan.json` 后交新 revision 处理。
