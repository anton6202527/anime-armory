...
## 与别的线
- `novel`(写小说) / **`song`(写歌)** 是两条**创作线**；`n2d`(制漫剧) / `mv`(制MV) 是两条**生产线**。写小说→制漫剧、**写歌→制MV** 各自对应。
- 各线**互不依赖**（自包含）；song 线不做换脸，歌声转换各写各的。
