...
本 skill 的可选项**不写死在源码里**。按 `../skills/song/song-craft/references/选择点与偏好.md` 读用户私有选择：先读 `<作品根>/_设置.md`，其次读全局默认；仍缺失的普通、可逆项采用本线推荐值写回并继续，同项目沉默沿用。版权/真人音色合规、当前音频与最终成品验收、不可逆发布/覆盖，以及阶段预算包创建、扩大、过期或合同变化才停。
