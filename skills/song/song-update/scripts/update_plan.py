#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Plan song project rework after song skill updates.

Pure standard-library helper. It records and compares content fingerprints for
the song skill family, then writes a project-local rework plan.
"""
...
CRAFT_DIR = os.path.join(REPO_SKILLS, "song-craft", "scripts")
LIB_DIR = os.path.join(REPO_SKILLS, "_lib")
...
LINE = "song"
LINE_LABEL = "写歌"
KIND_SNAPSHOT = "song_skill_update_snapshot"
KIND_PLAN = "song_skill_update_plan"
SCHEMA_VERSION = 1
SNAPSHOT_FILE = "song_skill_update_snapshot.json"
PLAN_JSON = "song_skill_update_plan.json"
PLAN_MD = "song_skill_update_plan.md"
TEXT_SUFFIXES = {".md", ".py", ".sh", ".json", ".html"}
EXCLUDED_DIRS = {"__pycache__", ".pytest_cache", ".mypy_cache", ".ruff_cache"}
OPTIONAL_RE = re.compile(r"可选|optional", re.I)
...
OBSERVE_ONLY_SKILLS = {"song-progress", "song-update"}
ALWAYS_RELEVANT_SKILLS = {"song", "song-craft"} | OBSERVE_ONLY_SKILLS
SKILL_DEFAULT_STAGE = {
    "song": "setup",
    "song-craft": "setup",
    "song-lyrics": "lyrics",
    "song-score": "lyrics",
    "song-compose": "compose_plan",
    "song-cover": "cover",
    "song-review": "review",
    "song-progress": "setup",
    "song-update": "setup",
...
FILE_STAGE_HINTS: dict[str, tuple[tuple[str, tuple[str, ...]], ...]] = {
    "song-craft": (
        ("handoff", ("ai_usage.py", "AI使用", "合规")),
        ("setup", ("contract.py", "init_project.py", "选择点与偏好")),
    ),
    "song-compose": (
        ("selection", ("select", "score")),
        ("takes", ("register", "takes_manifest", "take")),
        ("compose_plan", ("compose_song.py", "compose_task")),
    ),
}
...
def state_of(status: str) -> str:
    ...
    if match:
        ...
        if current > 0:
            return "partial"
    if "[~]" in raw or "⏳" in raw or "rough" in low or "block" in low or "🔴" in raw:
        return "partial"
...
def is_optional(label: str) -> bool:
    return bool(OPTIONAL_RE.search(label or ""))
...
def progress_context(root: str) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        stage = label_to_stage.get(re.sub(r"[（(].*?[）)]", "", label).strip()) or SKILL_DEFAULT_STAGE.get(skill or "")
        ...
        if first_open is None and state != "done":
            if state == "todo" and is_optional(label):
                continue
...
def build_execution_steps(root: str, plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not plan.get("rebuild_needed"):
        steps.append({"type": "command", "purpose": "查看当前前沿", "command": f'python3 skills/song/song-progress/scan.py "{root}"'})
        if plan.get("changed_skills") or plan.get("baseline_bootstrapped"):
            steps.append({
                "type": "command",
                "purpose": "接受当前产物后记录新基线",
                "command": f'python3 skills/song/song-update/scripts/update_plan.py record "{root}"',
                "run_when": "用户确认无需返工",
            })
        return steps
    steps.append({
        "type": "agent_step",
        "purpose": "确认返工范围",
        "instruction": f"向用户确认从 {stage_label(plan.get('rerun_from'))} 回放到 {stage_label(plan.get('rerun_until'))}，再进入对应 song skill。",
    })
    if stage_index(plan.get("rerun_from")) <= stage_index("takes"):
        steps.append({
            "type": "agent_step",
            "purpose": "保留当前歌词/音频/take",
            "instruction": "返工会触及歌词、作曲任务或音频产物；执行前先保留当前 lyrics、takes_manifest 和 song.wav。",
        })
    steps.append({"type": "command", "purpose": "查看生产前沿", "command": f'python3 skills/song/song-progress/scan.py "{root}"', "run_when": "返工前后"})
    steps.append({
        "type": "command",
        "purpose": "验收后记录新基线",
        "command": f'python3 skills/song/song-update/scripts/update_plan.py record "{root}"',
...
def build_plan(root: str, *, bootstrap: bool = True) -> dict[str, Any]:
    ...
    if observe_only_changed and not production_changed_files:
        plan["notes"].append("本次变化只影响控制面，不要求歌曲返工。")
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="写歌 skill 更新影响扫描")
