---
name: comic-layout
description: 画漫画排版阶段。Use when turning panel_script.json into page or long-scroll layouts, reading order, panel rectangles, gutters, bubble placeholders, and layout.json for comic projects under 创作区/画漫画. Triggers 漫画排版, 页面排版, 条漫排版, 分格排版, 气泡占位, 阅读顺序, layout.json, comic-layout.
---
...
- `_设置.md`：漫画形态、阅读方向、页面尺寸、单话分段高度。
- `脚本/第N话/panel_script.json`。
- `设定库/story_bible.md`：角色重要性和视觉重心。

## 输出

- `排版/第N话/layout.json`：schema 见 `references/layout_schema.md`。
...
1. 读 `panel_script.json`，按 `story_function` 标出大格、反应格、过渡格。
...
6. 输出 `layout.json`。如果文本过多，回 `comic-script` 压缩台词，不靠缩小字号硬塞。
7. 回写 `_进度.md` 的 `页面排版`。
