# FaceFusion 接入参考（ad-image-faceswap 本地图片换脸）

> 仅在第 0 步合规闸门通过后使用。FaceFusion 是开源、全本地、免费的换脸平台，支持 Apple Silicon。**与 ad-video-faceswap 是同一份安装**，只是 target/output 换成图片。

## 安装（Mac M1+ / Linux CUDA）
```bash
# 需 Python 3.12 + conda；若已为 ad-video-faceswap 装过，直接复用同一个 facefusion env
...
换完务必调公共 `ad-watermark` skill（`watermark.py --mode ai`，图/视频同一工具，按扩展名自动判定）烧可见提示 + 写元数据（PNG Comment / JPEG EXIF ImageDescription）。中国《标识办法》要求**可见 + 隐式**双标识；平台投放按平台隐式水印要求再补。**绝不**做去水印。
