...
| 1 剧本改编 | `n2d-script` | 编剧室 + 导演预演 + 制片交接合同的上游入口：拆集 + 中段开工前情资产包 + 精修前 5-10 集窗口复核边界 + **改编取舍 triage（成戏/旁白/后文带出/并入/删除 + 有账短剧化改写）** + 配音台词/BGM/封面/角色场景卡/global_style |
...
| 3 分镜设计 | `n2d-script` | `原生音画`：按 storyboard 规划时长生成分镜剧本/故事板/素材清单/字幕/镜头时长；`配音先行`：配音后回跑，按实测时长定稿；storyboard 同步锁 `entity_schedule` 在场/画外/禁入/出入场链；定稿必须写 `script_quality_contract_第N集.json/md`，把核心看点、首屏钩、留存账本、逐镜戏剧功能、观众问题账本拆成可签收字段 |
