...
本项目的自定义 skills 按 创作线×生产线 + 公共能力 组织（写小说→制漫剧、写歌→制MV，外加公共 shared-video-faceswap / shared-image-faceswap 换脸、shared-watermark 水印、shared-cleanup 瘦身清理）。**目录保持扁平**（每个 skill 仍是 `skills/<name>/SKILL.md`）——
...
| 进度·下一步（横切·只读）| `n2d-progress` | 扫 `制漫剧/<剧名>/_进度.md` 逐集矩阵 → 压缩出每部剧完成度 + 生产前沿（下一步该跑哪个 n2d skill）+ 次要缺口，按 `制作模式` 解释 `配音=⏳rough` 与原生音画跳过配音硬依赖；出图/视频/成片/配音等花钱·不可逆·合规步骤先提醒确认。**只读·不改文件·不碰其它三条线**。脚本 `scan.py` 纯标准库。触发词：进度 / 当前进度 / 下一步 / 还差什么 / progress / check |
...
> 四条线两两对应、**互不依赖**：**写小说→制漫剧**、**写歌→制MV**。创作线产成品（小说/歌）→ 生产线做视频（漫剧/MV）。衔接只在成品文件层面，不是 skill 依赖。换脸是公共 `shared-video-faceswap`（视频）/ `shared-image-faceswap`（图片）。
