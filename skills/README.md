...
> 统计时间：2026-06-23。`SKILL.md 总行数` 仅统计 `skills/*/SKILL.md` 的物理行数（`wc -l`）；`目录文本总行数` 统计每个 skill 目录下的 `.md/.py/.sh/.json/.html` 文本文件，包含 `scripts/`、`references/`、测试与示例，排除 `__pycache__/*.pyc`、根级 README/偏好文档与项目产物。原 `skills/common/` 公共层已删除，不再单独计入。
...
> **选择点 `制作模式`（出片顺序）**：默认 `原生音画`（native AV：Seedance 2.0/Veo 3/Sora 类后端对说话镜一次出同步音画[台词+口型+环境声]，最快看到出图/出视频，且原生音轨即成片音轨、无重同步返工；代价是少了逐句音色控制，仿真人音色仍需授权）。强配音控制、指定配音演员音色或后端 image2video-only 时选 `配音先行`（真实配音时长驱动镜头，音画准·表演可控）；另支持 `先出视频后配音`（**快速 demo·不推荐**：镜头时长靠估算锁死，后期补真音对不上 → 音画不同步/可能重切重出视频）。三种流程图 + 完整理由见 `n2d/SKILL.md`「制作模式」节；选择点定义见 n2d 线 `references/选择点与偏好.md`。
