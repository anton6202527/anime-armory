...
|---|---|---:|---:|---:|
| n2d | `n2d` + `n2d-*` | 21 | 4273 | 190800 |
| novel | `novel` + `novel-*` | 28 | 3008 | 60884 |
| song | `song` + `song-*` | 7 | 414 | 3857 |
| mv | `mv` + `mv-*` | 11 | 874 | 9539 |
| ad | `ad` + `ad-*` | 10 | 701 | 10754 |
| **合计** | `skills/*/SKILL.md` | **77** | 9270 | 275834 |
...
| 1 剧本改编 | `n2d-script` | 拆集 + 中段开工前情资产包 + 精修前 5-10 集窗口复核边界 + **改编取舍 triage（成戏/旁白/后文带出/并入/删除 + 有账短剧化改写）** + 配音台词/BGM/封面/角色场景卡/global_style |
...
| 3 分镜设计 | `n2d-script` | `原生音画`：按 storyboard 规划时长生成分镜剧本/故事板/素材清单/字幕/镜头时长；`配音先行`：配音后回跑，按实测时长定稿；storyboard 同步锁 `entity_schedule` 在场/画外/禁入/出入场链 |
...
> **三轨音字执行铁律**：`n2d-script` 把每个物理 Clip 拆成 `dialogue / narration / screen_text` 三轨并写入 `dialogue_fact_contract_第N集.json`。`n2d-video` 只生成画面和必要的画内角色对白口型；旁白不交视频模型生成，屏幕文案/花字/字幕不让视频模型烤字。`n2d-compose` 是唯一执行旁白音频、字幕和屏幕文案 overlay 的阶段：旁白用 TTS/配音轨混入，字幕/花字/系统面板数值用 Pillow/overlay 叠加，发布前再用 whisperx 或等效词级对齐补齐原生音画字幕证据。
