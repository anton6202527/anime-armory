...
本项目的自定义 skills 按 创作线×生产线 + 公共能力 组织（写小说→制漫剧、写歌→制MV、拍广告(ad)，外加公共 progress/update 分发、shared-video-faceswap / shared-image-faceswap 换脸、shared-watermark 水印、shared-cleanup 瘦身清理）。**目录保持扁平**（每个 skill 仍是 `skills/<name>/SKILL.md`）——
...
| Skill | 职责 |
|---|---|
| `progress` | 通用**只读进度分发**：识别 `写小说/`、`制漫剧/`、`写歌/`、`制MV/`、`拍广告/` 作品根或仓库根，路由到各线自己的进度脚本，输出当前前沿/下一步；不回写 `_进度.md` |
| `update` | 通用**更新/重制计划分发**：当前 n2d 完整支持 skill 快照比对 + 最小重制计划；song/mv/novel/ad 识别后友好提示待接入，不虚标自动化能力 |
