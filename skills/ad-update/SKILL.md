...
```bash
python3 skills/ad-update/scripts/update_plan.py check "<广告项目根>" --write-plan
python3 skills/ad-update/scripts/update_plan.py record "<广告项目根>"
python3 skills/ad-update/scripts/update_plan.py check "<广告项目根>" --json
```
...
- **触及投放与付费产物前先确认**：若计划覆盖出图、出视频、合成或交付，先确认客户 brief、合规项、后端、规格、预算和旧素材保留策略。
