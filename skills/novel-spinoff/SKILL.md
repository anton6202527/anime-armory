...
- **外传写作中不大段复刻原作原文**。原作的对话/动作可以**复述**（"她又问了一遍那个问题"）但不能整段照搬；锚点对齐靠**事件骨架**而非**文本搬运**。这是工艺要求也是法律边界。
...
> **派生同构阶段表**：本 skill 的 `_meta.json` / `_进度.md` 必须遵守 `novel-craft/references/contract.md`。机器阶段 key 固定为 `setup → source_model → direction_spec → title → outline → demo → draft → review → export`；本 skill 中 `source_model` = 锚点/角色/世界观，`direction_spec` = 时间线关系/分叉点确认。
...
| 错误 | 纠正 |
|---|---|
| 拿当代受版权网文做配角外传 | 拒做，引导改用公版原作（《晋书》《史记》《聊斋》《封神》《镜花缘》等）或用户自有/已授权作品 |
...
| 一次性把 30 章塞进主对话写 | 上下文爆 + 风格漂；逐章拆给子任务/子代理写，并把 Demo 第 1 章塞进 prompt 锚定文风 |
| Demo 过审后不跑 `draft_packets.py` | 缺单章上下文包和状态账本，长篇会漂 |
| Demo 过审但没写 `审稿/demo_gate.json` | 后续子任务没有机器锚点，文风/承诺会漂；过审即留痕 |
