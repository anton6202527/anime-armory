...
> **可选市场体检（批量前最便宜的 go/no-go）**：Demo 过审后，可对 Demo 章跑一次 `novel-score`（"配角外传题材/开篇钩子够不够火、值不值得写满 N 章"）——在投入批量续写前用最低成本验证方向。题材冷/开篇弱时改 premise 比写完 80 章再发现划算得多。用户说"先确认能不能火"尤其要跑。
...
python3 novel-review/scripts/mechanical_check.py "<作品根>" --pov "<配角名>" --terms "<规范术语逗号分隔>"
```
...
```bash
python3 novel-craft/scripts/export.py "<作品根>" \
