...
# -*- coding: utf-8 -*-
"""
expand.py — 章节/*.md 合并 → 导出/*.txt（v1）。docx 导出 v2 加。

用法:
    python3 expand.py <作品根> [--title <书名>]
"""
...
def main():
    ...
    title = args.title or meta.get("title") or f"{meta['source_title']}-扩写"
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(f"# expanded_from: {meta['source_title']}\n")
    ...
    print("     docx 导出可借 novel-spinoff/scripts/export.py 的 write_docx 函数；v2 直集成。")
