...
覆盖类别：绝对化用语·医疗保健极限词·化妆品禁用功效·虚假/绝对承诺·迷信封建·促销欺诈·
房地产违规（升值/投资回报承诺）·数据引证待证（无来源好评率/据调查，warn）·时限诱导待证（虚假紧迫，warn）。
每条命中附 `suggestion`（一句改法），让 gate/review 不止报问题、还给改向。

报告 schema（ad-review 消费）：
    {"region":"<地区>","disabled":<bool>,"summary":{"block":N,"warn":N},
     "findings":[{"severity":"block|warn|info","term":"..","category":"..",
                  "file":"..","line":N,"snippet":"..","suggestion":".."},...]}
...
MEDICAL_TERMS = [
    ...
    # —— 医疗器械「级别」混淆：普通护肤/日化/数码冒充医疗资质，按医疗极限词从严 ——
    "医疗级", "医用级", "临床级", "医院专用", "医疗认证", "药妆",
]
...
FALSE_TERMS = [
    ...
    # —— 金融：保证/承诺收益、刚兑属《广告法》《资管新规》明禁的不可证承诺 ——
    "保证收益", "承诺收益", "刚性兑付", "保本高息", "高收益低风险", "稳赚",
    # —— 教育：升学率/保送/提分保证均为《广告法》第二十四条禁止的效果保证 ——
    "升学率", "保送名额", "提分保证", "包提分", "保升学",
]
...
# 房地产广告：《房地产广告发布规定》第十八条明禁「升值/投资回报」承诺，第十九条禁绝对化区位描述。
# 仅收高确定性的升值/投资承诺词（学区/对口等语境相关项不收，避免误杀事实陈述）。
REALESTATE_TERMS = ["升值空间", "增值潜力", "投资回报", "买到就赚到", "买到就是赚到",
                    "价值洼地", "稳涨不跌", "只涨不跌", "一线江景永不被遮挡"]

# 数据 / 引证宣称：《广告法》第十一条要求引证内容真实、有出处。无来源的「好评率/复购率/据调查」
# 等属高危——合法与否取决于是否能出具来源与样本，故 warn（人判 + 要求补来源），非一律 block。
SUBSTANTIATION_TERMS = ["据调查", "据统计", "实验证明", "临床证实", "权威认证", "官方认证",
                        "好评率", "复购率", "用户好评率", "销量翻番", "好评如潮", "数据来源不明"]

# 时限诱导：制造虚假紧迫感。限时活动本身合法，关键看截止日是否真实——故 warn（人判活动真实性）。
URGENCY_TERMS = ["今天最后", "限今天", "涨价在即", "错过不再", "错过再无", "仅剩最后", "倒计时涨价"]
...
_TRAD_SIMP = {
    ...
    # —— 新增类别高频字（房地产/引证/医疗级/金融/促销）的繁体变体 ——
    "馳": "驰", "證": "证", "醫": "医", "價": "价", "漲": "涨",
    "購": "购", "評": "评", "據": "据", "險": "险", "鉤": "钩",
}
...
CATEGORIES = [
    ...
    ("房地产违规", REALESTATE_TERMS, "block"),
    # 引证/时限：合法与否取决于来源真实性或活动真实性，机检只能初筛 → warn，由人补依据/复核。
    ("数据引证待证", SUBSTANTIATION_TERMS, "warn"),
    ("时限诱导待证", URGENCY_TERMS, "warn"),
]
...
CATEGORY_FIX = {
    ...
    "迷信封建": "删除开光/转运等封建迷信内容。",
    "促销欺诈": "确有其事再宣称：标明真实原价与依据，删除虚构低价/最后机会等诱导表述。",
    "房地产违规": "删除升值/投资回报/区位绝对化承诺（《房地产广告发布规定》明禁）。",
    "数据引证待证": "补出处：标明来源、调查机构、样本与时间；无法溯源则删除该数据/好评率宣称。",
...
def _finding(category, term, sev, lineno, idx, line, norm=None, nterm=None):
    ...
    base_cat = category.split("(")[0]  # "绝对化用语(疑似)" → "绝对化用语"，共用同一条改法
    return {
...
def main():
    ...
    for fnd in report["findings"]:
        ...
        if fnd.get("suggestion"):
            print(f"    ↳ 改法：{fnd['suggestion']}")
