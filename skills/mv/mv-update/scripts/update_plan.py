#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Plan MV project rework after mv skill updates.

Pure standard-library helper. It records and compares content fingerprints for
the mv skill family, then writes a project-local rework plan.
"""
...
CRAFT_DIR = os.path.join(REPO_SKILLS, "mv-craft", "scripts")
...
LINE = "mv"
LINE_LABEL = "制MV"
KIND_SNAPSHOT = "mv_skill_update_snapshot"
KIND_PLAN = "mv_skill_update_plan"
SCHEMA_VERSION = 1
SNAPSHOT_FILE = "mv_skill_update_snapshot.json"
PLAN_JSON = "mv_skill_update_plan.json"
PLAN_MD = "mv_skill_update_plan.md"
...
RETIRED_EXTERNAL_OWNER_MARKERS = ("video-faceswap", "watermark", "公共")
...
OBSERVE_ONLY_SKILLS = {"mv-progress", "mv-update"}
ALWAYS_RELEVANT_SKILLS = {"mv", "mv-craft"} | OBSERVE_ONLY_SKILLS
SKILL_DEFAULT_STAGE = {
    "mv": "setup",
    "mv-craft": "setup",
    "mv-beat": "beat",
...
FILE_STAGE_HINTS: dict[str, tuple[tuple[str, tuple[str, ...]], ...]] = {
    "mv-craft": (
        ...
        ("setup", ("contract.py", "init_project.py", "gate.py", "选择点与偏好")),
    ),
    "mv-script": (
        ("script_review", ("review", "复核")),
...
def state_of(status: str) -> str:
    ...
    if match:
        ...
        if current > 0:
            return "partial"
    if "[~]" in raw or "⏳" in raw or "rough" in low or "占位" in raw or "block" in low or "🔴" in raw:
        return "partial"
...
def is_retired_external(row: dict[str, str]) -> bool:
    label = row.get("label", "")
    owner = row.get("owner", "")
    haystack = f"{label} {owner}".lower()
    return is_optional(label) and any(marker.lower() in haystack for marker in RETIRED_EXTERNAL_OWNER_MARKERS)
...
def stage_from_label(label: str) -> str | None:
    clean = re.sub(r"[（(].*?[）)]", "", label or "").strip()
    for stage in contract.stage_table():
        if str(stage.get("label")) == clean:
            return str(stage["key"])
    for token, stage in LABEL_STAGE_HINTS:
        if token in clean:
            return stage
    return None
...
def parse_stage_rows(root: str) -> list[dict[str, str]]:
    ...
    rows = progress_md.parse_stage_rows(
        text,
        section_keywords=("制MV", "阶段"),
        require_all=True,
        min_cols=3,
        owner_col=1,
        status_col=2,
    )
    return [row for row in rows if not is_retired_external(row)]
...
def progress_context(root: str) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        stage = stage_from_label(label) or SKILL_DEFAULT_STAGE.get(skill or "")
    ...
    return {
        ...
        "song_timing": load_meta(root).get("song_timing") or "",
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="制MV skill 更新影响扫描")
