...
本 skill 涉及的选择点：`MV用途`、`MV视觉风格`、`MV规划粒度`、`卡点策略`、`生视频AI`、`生图AI`、`MV一致性增强`、`出视频规格`、`视频分辨率`、`合成画幅`、`AI视觉使用披露`、`发行目标平台`。
...
| 错误 | 纠正 |
|---|---|
| 将半成品或尚未完成创作的音频送入制MV管线 | 务必确保前置的 `song` 管线输出的音频是最终成品，MV管线只做视觉表现，不承接混音或作曲任务 |
