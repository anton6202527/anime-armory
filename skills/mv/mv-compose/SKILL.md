---
name: mv-compose
description: MV 专业合成与交付 — 严格按 timeline/整数帧 OTIO 拼接选中 clips，以正式歌为主音轴，逐输入执行显式 BT.709 变换，烧录字幕，先生成 ProRes/PCM mezzanine master，再派生 BT.709/H.264/48kHz MP4，并运行视频与 PCM 音频交付 QC。Use when asked to 合成MV / 出MV成片 / 母版 / 交付QC / 烧卡拉OK字幕 / MV导出. Triggers 合成MV, MV成片, MV母版, 交付QC, 卡拉OK烧录, mv-compose.
---
...
本 skill 的可选项**不写死在源码里**。按 `skills/mv/mv-craft/references/选择点与偏好.md` 读项目值、全局默认；仍缺失的普通、可逆项采用本线推荐值写回并继续。当前 picture lock、最终母版验收、不可逆覆盖/发布仍是硬边界。

本 skill 涉及的选择点：`合成画幅`。

## 核心原则
- **歌是主音轴**：MV 用整首 `歌/song.*` 作主音轨，支持 `.wav/.mp3/.m4a/.flac`；clip 自带音效一律静音/极低。不做配音 ducking。
- **timeline + OTIO 驱动剪辑**：默认读 `timeline_manifest.json`，开跑先刷新 V1 画面 + A1 正式歌曲的 `timeline.otio`/receipt，再过 compose gate。OTIO 时间值必须是整数帧并通过官方 adapter round-trip；缺 timeline/已选视频/新鲜 OTIO/签收会阻断。`--allow-fallback` 只能按目录写预览，不能成为正式交付。
...
- **色彩边界**：正式合成先生成 `生产数据/color/color_input_manifest.json`，逐个、按时间线同序绑定当前视频 SHA。BT.709 limited 输入显式 setparams；BT.709 full 输入必须记录并执行 full→limited 变换；HDR/不支持色域硬拦；无标签输入只在具名、理由非空且绑定当前 hash 的接受记录后按声明处理。只补输出标签不算转换。
...
- **交付可追溯**：合成阶段只生成母版、交付版、色彩清单、整数帧 OTIO 收据与 `delivery_qc.json`。QC 会解码最终 PCM，在歌曲首/中/尾窗口做互相关，检查偏移与漂移；AI disclosure 和 provenance 必须在成片稳定后依次独立执行，避免 compose 后继续改成片却保留陈旧来源链。
...
2. 对时间线当前视频生成逐输入色彩清单并过闸；按清单执行 full→limited / setparams，再统一目标画幅。帧率继承锁版素材或由 `MV_OUTPUT_FPS` 指定 → 拼接。
...
7. 强制 delivery QC：除时长、integrated loudness/true peak 外，解码输出 PCM，并以首/中/尾窗口对输入歌做互相关，检测固定偏移、局部错位和漂移。
8. compose 健康度通过后回写 compose；随后依次执行 `ai_usage.py`、`provenance.py`、`mv-review` 具名签收与发布决策。C2PA 副本由 provenance 阶段生成，不由 compose 冒充完成。

## 依赖（仅通用工具，无 skill 依赖）
- **ffmpeg**（必需）。`.ass` 卡拉OK逐字需 libass 编译版（`ffmpeg -filters | grep subtitles`）。
- **Pillow**（无 libass 时的字幕降级，`render_lyrics.py` 用）。
- **OpenTimelineIO Python 包**（正式版必需）：`export_otio.py` 必须用官方 adapter round-trip 后，compose 完成态才通过；缺包时只会留下明确 blocked 收据。
...
| 把 BT.709 full 当 limited 只改标签 | 重新生成 schema v2 色彩清单并执行显式 full→limited；无标签必须具名、理由非空且绑定当前文件 hash |
| QC 只看容器时长/响度 | 以最终解码 PCM 对原曲首/中/尾互相关；固定偏移或漂移超限必须回合成修复 |
| compose 后直接发布 | 成片稳定后依次完成 disclosure → provenance → 具名 review → release decision → handoff receipt |
