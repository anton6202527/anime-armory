...
_COMMON_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "..", "_lib"))
...
def is_retired_external(row):
    owner = str(row.get("owner", "")).lower()
    label = str(row.get("label", "")).lower()
    haystack = f"{owner} {label}"
    return is_optional(row.get("label", "")) and any(m.lower() in haystack for m in RETIRED_EXTERNAL_OWNER_MARKERS)
...
def report(root, limit):
    ...
    rows = [row for row in rows if not is_retired_external(row)]
