#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""mv-progress/scan.py — 制MV进度扫描器（只读）。

默认扫描 `创作区/制MV/<曲名>/_进度.md`，并兼容旧 `制MV/<曲名>/_进度.md`。
单项目口径复用 mv-craft/scripts/progress.py；本脚本只负责发现作品并聚合输出。
"""
...
def load_single_project_progress(repo_root: str):
    path = os.path.join(repo_root, "skills", "mv", "mv-craft", "scripts", "progress.py")
    spec = importlib.util.spec_from_file_location("mv_craft_progress", path)
...
def run_report(progress_mod, root: str, rel: str, limit: int) -> str:
    ...
    try:
        ...
        out.append(f"（扫描失败，跳过本 MV：{type(exc).__name__}: {exc}）")
        return "\n".join(out)
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(description="制MV进度仪表盘（只读）")
    ap.add_argument("project_roots", nargs="*", help="可选：一个或多个 MV 项目根，或 _进度.md 文件")
    ...
    if not works:
        print(f"未找到任何含 _进度.md 的 MV 项目。线根目录：{CREATION_ROOT_DIR}/{LINE_DIR}/")
        return 0
