...
```bash
python3 skills/mv/mv-progress/scan.py                    # 扫描 创作区/制MV/ 下所有 MV 项目
python3 skills/mv/mv-progress/scan.py <MV项目根>           # 只看指定 MV
python3 skills/mv/mv-progress/scan.py --root <仓库根>      # 从其它目录调用时指定仓库根
```
...
按脚本输出给一个明确前沿。免费确定性下一步可由外层编排直接 chain。若下一步是 `mv-image`、`mv-video`、`mv-compose`，报告当前后端/规格/画幅和预算授权状态：已有精确绑定且有效的阶段预算包可继续；缺失、扩大、过期或合同变化才结构化停止。后配歌曲路线若还没有最终音频，不得推进正式卡点之后的产物。
