...
本 skill 的可选项**不写死在源码里**。按 `skills/mv/mv-craft/references/选择点与偏好.md` 读项目值、全局默认；仍缺失的普通、可逆项采用本线推荐值写回并继续。确定性预检与可逆回流可由外层连续派发。
...
```bash
python3 skills/mv/mv-score/scripts/score_pacing.py 创作区/制MV/<曲名>/ [--json]
```
...
python3 skills/mv/mv-score/scripts/score_pacing.py 创作区/制MV/<曲名>/ --threshold 80 \
...
```bash
python3 skills/mv/mv-score/scripts/score_pacing.py 创作区/制MV/<曲名>/        # 机器卡点预评分 → 评分/pacing_prescore.json
