...
The user-facing convention lives in `skills/mv-craft/references/选择点与偏好.md`. This module only
...
class SettingSpec:
    ...
    The prose source of truth remains `skills/mv-craft/references/选择点与偏好.md`; this compact schema
...
def write_settings(work_root: str, fields: Dict[str, str], *, note: Optional[str] = None, bold_keys: bool = False):
    ...
    lines = ["# 设置 — 本作私有选择点（skills/mv-craft/references/选择点与偏好.md）", ""]
    if note:
        lines += [f"> {note}", ""]
...
def append_record(work_root: str, message: str, *, date: Optional[str] = None) -> None:
    ...
    if not content:
        content = "# 设置 — 本作私有选择点（skills/mv-craft/references/选择点与偏好.md）\n"
