...
def http_get_text(url, cache=None):
    ...
    if url in cache:
        return cache[url]
    req = Request(url, headers={"User-Agent": "novel-author-import/1.0"})
...
def build_manifest(title, payload, rights_status, *, rights_declared):
    ...
    return {
        ...
        "requires_user_rights": rights_status not in {"public-domain", "original"},
...
def write_project(root, title, payload, outputs, rights_status, *, replace=False, rights_declared=False):
    target = root
    tmp = None
    ...
    manifest = build_manifest(title, payload, rights_status, rights_declared=rights_declared)
