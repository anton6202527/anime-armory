...
- **后期 pass**：clip 出好后用本地口型工具把正面演唱镜的嘴型对到人声轨。**工具优先序 LatentSync（身份保持最好，主角脸是 MV 命门）> MuseTalk（近实时但不保面部特征）> Wav2Lip**（与 n2d-video 同源口径，能力会变以模型矩阵/各家官方为准）。重型权重在 conda env、不入本仓。
