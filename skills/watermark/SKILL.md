...
# watermark — 通用水印（公共能力 · 合规 AI 标识 + 品牌水印）

给**图片或视频**烧水印。**这是一个公共能力**，不绑定任何生产线——制漫剧、制MV、出图/出视频、faceswap、或用户单独使用都能调它（与 `video-faceswap`/`image-faceswap` 同级）。只用通用工具：**Pillow**（渲染水印层）+ **ffmpeg**（视频 overlay）。图还是视频按输入扩展名自动判定。
