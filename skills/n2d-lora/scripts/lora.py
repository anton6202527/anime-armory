...
def cmd_register(args: argparse.Namespace) -> int:
    ...
    if not lora_verdict_ok(verdict):  # verdict 判定与 gate 同源
        ready_blocks.append(f"validation_verdict_not_pass:{verdict or 'missing'}")
    for key in LORA_REPORT_REQUIRED_FIELDS:  # 必填报告字段与 gate registry 校验同源
        if not str(report.get(key, "")).strip():
            ready_blocks.append(f"missing_report_field:{key}")
    manual_review = report.get("manual_review") if isinstance(report.get("manual_review"), Mapping) else {}
    if "dataset_has_warnings" in (report.get("warnings") or []) and not manual_review.get("allow_dataset_warnings"):
        ready_blocks.append("dataset_warnings_without_override")
    if "dataset_has_warnings" in (report.get("warnings") or []) and manual_review.get("allow_dataset_warnings") and not str(manual_review.get("notes") or "").strip():
        ready_blocks.append("dataset_warnings_override_notes_missing")
