...
6. 记录生产数据：
   ...
   ```
7. **执行合规包预声明的水印策略** —— 见下「加水印」节，调本线 `n2d-watermark` skill，产物落 `合成/第N集/`。水印后再补一条 `stage=compose --event manual --meta watermark=done`，并把最终资产路径写回 `合规/compliance_manifest.json` 的 `watermark.final_assets[]`，否则 `review` gate 会阻断。

## 加水印（策略前置 · 执行在成片文件上）
水印不是成片后才想起来的补救项。是否需要、用哪种标识、是否依赖平台隐式水印、最终资产路径，都必须先写入 `合规/compliance_manifest.json`；compose 只负责按合规包执行。具体烧录仍调与 faceswap 同级的本线 [[n2d-watermark]] skill（图/视频同一工具）：
...
```bash
# 合规 AI 标识
python3 <n2d-watermark-skill>/watermark.py 合成/第N集/成片_第N集_zh.mp4 合成/第N集/成片_第N集_zh_水印.mp4 --mode ai
# 品牌账号水印
python3 <n2d-watermark-skill>/watermark.py 合成/第N集/成片_第N集_zh.mp4 合成/第N集/成片_第N集_zh_水印.mp4 --mode brand --text "@账号" --pos br --opacity 0.8
```
