...
本 skill 涉及的选择点：`BGM来源`、`画幅`、`制作模式`（决定配音轨是否需先拟合到已成片镜头长·见「先出视频后配音」节）、`视频原生音轨`（丢弃 / 低音量混入环境声 / 保留原片音轨）、`水印`（见「加水印」节：AI 合规标识=每次确认的合规闸门；品牌/账号水印=可沿用的偏好，含文字/logo/位置/透明度）。
...
- **按转场类型接 clip，别盲拼**（接力链末端兜底）：读 `故事板.md`/`storyboard.json` 每个接缝的 `转场类型` 决定接法，而不是一律裸切——
  ...
  - **实现现状（别被上面误导）**：`compose.sh` 目前对所有接缝一律 `concat -c copy` 裸拼，**未自动**做 per-seam `xfade`/空镜缺料报警。上述接法是给你（助手/剪辑者）的判断指引——需要微溶解/缺空镜停报时，当前要么按注记手动跑一遍 ffmpeg `xfade` 局部重编码再并回，要么在合成前人工补齐空镜 clip。自动化接缝引擎是 TODO。
...
## 加水印（成片后 · 调公共 `watermark` skill）
成片后打水印——**不在 compose.sh 里写死**，调与 faceswap 同级的公共 [[watermark]] skill（图/视频同一工具）：
- **AI 合规标识**（`--mode ai`）：⛔ **合规闸门（每次确认·non-negotiable）**。本 skill 成片必含 AI 配音 + AI 生图（可能还有换脸），按 CLAUDE.md 属投放前法律必做项。**这不是普通可复用偏好**——每次投放前都要确认已打 AI 标识；只加不去，禁止剥除。仅当成片纯属内部测试、明确不投放时才可跳过（须显式确认）。
...
```
AI 合规标识是投放前的**合规闸门**（每次确认，不沉默沿用）；品牌/账号水印是可选层，按用户在 `_设置.md` 的 `水印` 选择点沿用。
