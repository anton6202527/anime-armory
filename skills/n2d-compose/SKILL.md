...
本 skill 涉及的选择点：`BGM来源`、`画幅`、`制作模式`（决定配音轨是否需先拟合到已成片镜头长·见「先出视频后配音」节）、`视频原生音轨`（丢弃 / 低音量混入环境声 / 保留原片音轨）、`水印`（见「加水印」节：是否打 + AI合规/品牌 + 文字/logo/位置/透明度）。
...
6. **（可选）加水印** —— 见下「加水印」节，调公共 `watermark` skill，产物落 `合成/第N集/`。

## 加水印（可选 · 调公共 `watermark` skill）
成片后可选打水印——**不在 compose.sh 里写死**，调与 faceswap 同级的公共 [[watermark]] skill（图/视频同一工具）：
- **AI 合规标识**（`--mode ai`）：AI 合成内容投放前的法律必做项；含 AI 配音/生图/换脸的成片投放前**应**打。只加不去。
- **品牌/账号水印**（`--mode brand`）：片头/全程 logo 或账号文字，防盗/打品牌用；文字/logo/位置/透明度按 `_设置.md` 的 `水印` 选择点。
```bash
# 合规 AI 标识
...
```
水印是「甚至可以加」的可选层；是否打、打哪种，按用户在 `_设置.md` 的 `水印` 选择点（合规 AI 标识属投放合规项，建议默认打）。
