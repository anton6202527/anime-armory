...
本 skill涉及的选择点：`BGM来源`、`画幅`、`制作模式`（决定配音轨是否需先拟合到已成片镜头长·见「先出视频后配音」节）、`视频原生音轨`（丢弃 / 低音量混入环境声 / 保留原片音轨）、`水印`、`目标平台`、`发行地区`、`合规用途`。其中 `水印/目标平台/发行地区/合规用途` 只是偏好入口，**实际放行以 `合规/compliance_manifest.json` 为准**，不得只看 `_设置.md`。
...
- **按转场类型接 clip，别盲拼**（接力链末端兜底）：读 `故事板.md`/`storyboard.json` 每个接缝的 `转场类型` 决定接法，而不是一律裸切——
  ...
  - **实现现状（已落地·不再是 TODO）**：`compose.sh` 拼接步已改调 `seam_concat.py`——自动读 `storyboard.json` 每接缝 `continuity.transition` 分类：**硬切→裸拼、微溶解→局部 `xfade`、缺空镜→报警**（写 `合成/<ep>/_work/接缝报告.md` + stderr）。实现策略：硬切/报警相连的 clip 归为一个 run 先 `concat -c copy`（零重编码），只在**溶解接缝**间做 xfade，把重编码压到最小；爽点/反转有意冲击点即使标跳变也不加溶解。**无溶解接缝时等价今天的 `concat -c copy`**；clip 数与 storyboard 对不上、或 ffmpeg 失败 → 自动回退裸拼，绝不中断合成。兜底/溶解秒可用环境变量 `SEAM_FALLBACK`（默认硬切）/`SEAM_DISSOLVE_SEC`（默认 0.25）覆盖。缺空镜仍只报警**不自造素材**——要消除生硬跳切需人工补一个空镜 clip 再合成。`seam_concat.py --plan-only` 可干跑看接法计划。
...
- **`合成/第N集/`** = 本阶段的工作区：`配音/`（n2d-voice 产物，前置阶段已落这里）、`_voicecache/`、中间件 `_work/`、成片 `成片_第N集_{mode}.mp4`、以及（可选）水印产物。
...
| 合成前未检查 `合规/compliance_manifest.json` | 水印、AI 标识、平台审核是合规闸门，必须先在合规包声明策略，compose 仅负责执行 |
...
6. 记录生产数据：
   ...
   python3 skills/n2d-dashboard/scripts/dashboard.py record <作品根> \
     ...
     --meta watermark=<ai|brand|none|pending>
   ```
7. **执行合规包预声明的水印策略** —— 见下「加水印」节，调公共 `shared-watermark` skill，产物落 `合成/第N集/`。水印后再补一条 `stage=compose --event manual --meta watermark=done`，并把最终资产路径写回 `合规/compliance_manifest.json` 的 `watermark.final_assets[]`，否则 `review` gate 会阻断。

## 加水印（策略前置 · 执行在成片文件上）
水印不是成片后才想起来的补救项。是否需要、用哪种标识、是否依赖平台隐式水印、最终资产路径，都必须先写入 `合规/compliance_manifest.json`；compose 只负责按合规包执行。具体烧录仍调与 faceswap 同级的公共 [[shared-watermark]] skill（图/视频同一工具）：
- **AI 合规标识**（`--mode ai`）：⛔ **合规闸门（每次确认·non-negotiable）**。本 skill 成片必含 AI 配音 + AI 生图（可能还有换脸），按项目约定属投放前法律必做项。**这不是普通可复用偏好**——每次投放前都要确认已打 AI 标识；只加不去，禁止剥除。仅当合规包声明 `distribution_intent=internal_only` 且成片纯属内部测试时才可跳过投放水印。
- **品牌/账号水印**（`--mode brand`）：片头/全程 logo 或账号文字，防盗/打品牌用；文字/logo/位置/透明度按 `_设置.md` 的 `水印` 选择点（这一项可沉默沿用）。
```bash
# 合规 AI 标识
python3 <shared-watermark-skill>/watermark.py 合成/第N集/成片_第N集_zh.mp4 合成/第N集/成片_第N集_zh_水印.mp4 --mode ai
# 品牌账号水印
python3 <shared-watermark-skill>/watermark.py 合成/第N集/成片_第N集_zh.mp4 合成/第N集/成片_第N集_zh_水印.mp4 --mode brand --text "@账号" --pos br --opacity 0.8
```
执行后必须回写 `合规/compliance_manifest.json`：`watermark.ai_visible.status=done`（或对应策略项）、`watermark.final_assets[].episode=第N集`、`path=<水印成片路径>`。AI 合规标识是投放前的**合规闸门**（每次确认，不沉默沿用）；品牌/账号水印是可选层，但一旦合规包声明为投放策略，也必须完成。
