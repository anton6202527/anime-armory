...
按 `../_偏好约定.md` 读 `<作品根>/_设置.md`。涉及：`品牌包装模板`、`字幕语言`、`音乐来源`、`cutdown版本`、`交付比例`、`交付规格`、`水印-AI合规标识`（每次确认）、`水印-品牌账号`。合成是**花钱/不可逆**阶段，正式跑前确认。
...
3. **AI 标识水印（合规·必）**：调公共 `shared-watermark` 给成片打 AI 标识（投放前必打；只加不去）。品牌水印按 `水印-品牌账号`。
...
6. **交付规格归一**：按 `交付规格`（平台默认 -16 LUFS / 广电 TVC -23 LUFS）用 ffmpeg `loudnorm` 归一响度、确认安全框，更新 `_进度.md` 交付版本矩阵每行成片路径。
7. 回写 `_进度.md` 剪辑包装 ✅ + 交付矩阵，提示 AI 披露（`ad-craft/ai_usage.py`）。
...
| 投放前不打 AI 标识 | 必过 `shared-watermark` AI 标识（只加不去）|
