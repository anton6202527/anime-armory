...
LIB_DIR = os.path.join(REPO_SKILLS, "mv", "_lib")
...
def build_execution_steps(root: str, plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not plan.get("rebuild_needed"):
        steps.append({"type": "command", "purpose": "查看当前前沿", "command": f'python3 skills/mv-progress/scan.py "{root}"'})
        if plan.get("changed_skills") or plan.get("baseline_bootstrapped"):
            steps.append({
                "type": "command",
                "purpose": "接受当前产物后记录新基线",
                "command": f'python3 skills/mv-update/scripts/update_plan.py record "{root}"',
                "run_when": "用户确认无需返工",
            })
        return steps
    steps.append({
    ...
    steps.append({"type": "command", "purpose": "查看生产前沿", "command": f'python3 skills/mv-progress/scan.py "{root}"', "run_when": "返工前后"})
    steps.append({
        "type": "command",
        "purpose": "验收后记录新基线",
        "command": f'python3 skills/mv-update/scripts/update_plan.py record "{root}"',
