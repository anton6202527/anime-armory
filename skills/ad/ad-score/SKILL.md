...
可选项不写死在源码。按 `../skills/ad/ad-craft/references/选择点与偏好.md` 读用户私有选择：先读 `<作品根>/_设置.md`，缺则全局默认预填并告知一句，再缺则首次问一次→写回→之后沉默沿用（合规/不可逆/花钱多的点每次仍确认）。评分阈值默认 `--threshold 80`（不传则建议性不阻断）。
