...
按 `../skills/ad/ad-craft/references/选择点与偏好.md` 读 `<作品根>/_设置.md`。涉及：`生图模型`、`生图渠道`、`一致性增强`、`基础视觉风格`、`交付比例`、`生成粒度`、`重抽预算策略`。普通可逆项缺失时采用推荐值写回；具体模型+渠道、exact manifest scope、保守成本上界和 input SHA 在阶段预算包创建时一次确认，包内不逐图重复问付款。旧 `生图AI` 必须迁移。brief 的 claim 分型依据/rights/legal_lines 及分镜 claim 披露此时必须闭合。
