...
def build_execution_steps(root: str, plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not plan.get("rebuild_needed"):
        steps.append({"type": "command", "purpose": "查看当前前沿", "command": f'python3 skills/ad/ad-progress/scan.py "{root}"'})
    ...
    steps.append({"type": "command", "purpose": "查看生产前沿", "command": f'python3 skills/ad/ad-progress/scan.py "{root}"', "run_when": "返工前后"})
    steps.append({
        "type": "command",
        "purpose": "验收后记录新基线",
        "command": f'python3 skills/ad/ad-update/scripts/update_plan.py record "{root}"',
