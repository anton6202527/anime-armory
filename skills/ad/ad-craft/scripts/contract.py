...
CHOICE_POINTS = {
    "普通选择策略": ("推荐方案自动继续", "逐项询问"),
    "审阅策略": ("用户授权制作代理", "逐阶段用户确认", "自定义"),
    "付费授权策略": ("按阶段预算包一次确认", "每次调用确认", "自定义"),
    "广告类型": AD_TYPES,
...
# 只有权利/法规口径需要在使用时重新确认。付费调用不再把模型、渠道、规格
# 分拆成多次确认；它们绑定进一次性的阶段预算包（scope/model/channel/max_calls/
# max_attempts/cost_ceiling/hash）。在预算包内连续执行，任何字段或上限变化再确认。
RECONFIRM_CHOICE_POINTS = (
    "广告法地区", "音乐来源",
)

SPEND_ENVELOPE_CHOICE_POINTS = (
    "生图模型", "生图渠道", "生视频模型", "生视频渠道", "出视频规格", "重抽预算策略",
)
