...
1. **不拆集**。广告不切「第N集」。一条主片是一个整体（可以很长）。多时长（30/15/6s）、多比例（16:9/9:16/4:5/1:1）、A·B 是**交付件 deliverable**，不是集——登记在 `_进度.md` 的「交付版本矩阵」，由 `ad-compose` 按 placement adaptation 原生重剪/重做，机械裁切仅走已签核路径。
...
9. **渲染规格单一真值**。比例、分辨率和 FPS 只由 `生产数据/render_profile.json` 解释；`source_generation` 与 `master_render` 分列，容器尺寸不能冒充原生细节。
10. **跨比例不是裁切许可**。适配模式、具名批准、焦点/shot plan、安全区与风险证据只认 `生产数据/placement_adaptation.json`；中心网格或 `reframe.py` 能运行不等于可交付。
