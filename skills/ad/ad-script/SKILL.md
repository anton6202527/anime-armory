...
按 `../skills/ad/ad-craft/references/选择点与偏好.md` 读 `<作品根>/_设置.md`。涉及：`主片时长`、`基础视觉风格`、`字幕语言`、`广告法地区`、`生成粒度`。`广告法地区`（合规点）每次确认；`关闭` 仅非中国大陆投放且用户明确时。
...
4. **跑广告法机检（分层闸门）**：
   ```bash
   python3 skills/ad/ad-script/ad_law_check.py "<作品根>" --region 中国大陆 --json "<作品根>/脚本/广告法机检报告.json"
   ```
...
```bash
python3 skills/ad/ad-script/scripts/idea_payoff_ledger.py "<作品根>" --write   # 创意承诺→分镜兑现
python3 skills/ad/ad-script/scripts/copy_quality_audit.py "<作品根>" --write   # VO 文案质量
python3 skills/ad/ad-script/scripts/shot_variety_audit.py "<作品根>" --write   # 分镜视觉多样性（出图前）
python3 skills/ad/ad-script/scripts/product_craft_audit.py "<作品根>" --write  # 产品镜工艺三轴（光位/质感/角度）
python3 skills/ad/ad-script/scripts/performance_cue_audit.py "<作品根>" --write # 人物镜表演三轴（情绪/视线/可演动作）
python3 skills/ad/ad-script/scripts/beat_structure_audit.py "<作品根>" --write # 叙事结构/节拍工艺（传统广告结构纪律）
python3 skills/ad/ad-script/scripts/see_say_audit.py "<作品根>" --write        # 声画对位（DRTV see-say）
python3 skills/ad/ad-script/scripts/claim_substantiation_audit.py "<作品根>" --write  # 承诺-证据配对预检
```
...
- **see_say 第七轮增**：`info_beat_no_insert`（信息态非人物特写覆盖：信息态 VO 句
...
```bash
cd skills/ad/ad-script && python -m pytest test_ad_law_check.py test_finalize_storyboard.py
```
