...
按 `../skills/ad/ad-craft/references/选择点与偏好.md` 读用户私有选择：先读 `<作品根>/_设置.md`；缺则用全局默认；再缺则普通、可逆项采用推荐值写回，同项目沉默沿用。`广告法地区`、`音乐来源` 等合规/权利口径在使用时确认；当前真实 v2 消费已接入 ad-image/ad-video 的 provider runner，模型、渠道、规格和调用统一进一次阶段预算包。ad-compose 的不可逆覆盖/公开交付仍按自身硬闸，不因选择点常量自动获得付费授权。
