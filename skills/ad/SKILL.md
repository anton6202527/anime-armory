...
---
> 规模统计：Skill 数 14 | SKILL.md 总行数 1258 | 目录文本总行数 59078
...
> **一键成片默认**：`普通选择策略=推荐方案自动继续`、`审阅策略=用户授权制作代理`、`付费授权策略=按阶段预算包一次确认`。普通创意路线、构图、节奏、镜头/版位原生适配和内部技术审查由制作代理采用有证据优势的推荐方案并留痕；不存在证据优势时仍优先走可回滚的默认路线。付费前把 scope、模型、渠道、最大调用/重抽次数、费用上限和输入哈希合成一个 spend envelope，确认一次后包内连续执行；只有超预算、改模型/渠道、扩大范围或输入变化才重确认。claim/音乐/肖像/声音/字体权利、法规辖区、逐图当前像素验收、机械裁切风险接受、公开投放和最终逐项签收仍停下。

## 偏好（私有 · 用户选择，不写死在本 skill）

按 `../skills/ad/ad-craft/references/选择点与偏好.md` 读用户私有选择：先读 `<作品根>/_设置.md`；缺则用全局默认；再缺则普通、可逆项采用推荐值写回，同项目沉默沿用。`广告法地区`、`音乐来源` 等合规/权利口径在使用时确认；出图/出视频/合成绑定到一次阶段预算包，不再按每个模型、渠道、规格和调用分别打断。

涉及选择点：`普通选择策略`、`审阅策略`、`付费授权策略`、`广告类型`、`创意路线`、`基础视觉风格`、`主片时长`、`交付比例`、`cutdown版本`、`生图模型`、`生图渠道`、`一致性增强`、`生视频模型`、`生视频渠道`、`视频模型路由`、`出视频规格`、`视频分辨率`、`配音后端`、`音乐来源`、`品牌包装模板`、`字幕语言`、`AI视觉使用披露`、`广告法地区`、`交付规格`、`生成粒度`、`目标平台`、`发行地区`。模型是具体版本，渠道只是 CLI/API/网页入口；旧 `生图AI` 不再作为正式选择点。

> 作为生产线入口：开新项目（`创作区/拍广告/<项目名>/`）时，先从一句话 brief 和目标平台推导首跑推荐值并运行 `python3 skills/ad/scripts/init_project.py "创作区/拍广告/<项目名>" --brand <品牌>`；不为 `创意路线`、`基础视觉风格`、交付比例/时长分别开菜单。只有 brief 缺品牌/产品/USP/受众/目标等无法推导的事实，或账号约束会改变可执行路径时，合并成一次问题。视频阶段默认 `视频模型路由=自动按镜头路由`，不在立项时强问具体模型/渠道；只有客户/投放/账号固定、用户明确指定、或 router/probe 找不到可执行后端时才覆盖落档。旧 `--video-backend` / `生视频AI` 兼容。
