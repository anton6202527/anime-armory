...
def clip_has_named_characters(clip: Mapping[str, Any]) -> bool:
    ...
    if _has_any(text, ("无人物", "空镜", "empty shot", "no character")):
        return False
    return _has_any(text, ("角色", "人物", "主角", "脸", "发型", "服装", "王敦", "婷婷", "沈念", "character", "face"))
...
def choose_route(
    ...
    *,
    ...
    if shot_type == "fight_exchange":
        ...
        primary = "kling"
        fallback = ["veo", "seedance"] if lip_sync_setting != "关闭" or "保留" in native_audio_setting else ["seedance", default_backend]
        route = {
            "primary_backend": primary,
