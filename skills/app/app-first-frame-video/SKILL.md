...
1. 普通可逆项采用宿主推荐的具体视频模型、渠道、比例、分辨率、时长和数量并保持可编辑。
2. `prepare` 生成绑定首帧 SHA 的 job；真实付费提交由宿主消费绑定同一输入、模型、渠道、数量和费用上限的已有阶段授权，包内连续执行。创建/扩大授权、授权过期或绑定变化时才确认。
...
- 创建/扩大/失效的付费授权、覆盖已验收视频或切换已绑定模型时停下确认；有效授权包内不重复打断。

## 命令

```bash
python3 skills/app/app-first-frame-video/scripts/first_frame_video.py init \
  --source first-frame.png --output shot.first-frame-video.json

python3 skills/app/app-first-frame-video/scripts/first_frame_video.py prepare \
