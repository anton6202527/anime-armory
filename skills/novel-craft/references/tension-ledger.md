...
4. **悬念真空**：如果连续 ≥2 章账面上活跃钩子 + 未兑现承诺数为 0，哨兵报 `🟡 建议级：suspense_vacuum`。与规则 3 正交——那是"张力分数低"，这是"读者手里没有任何未决问题"：哪怕文字张力尚可，没有 dramatic question 吊着就是追读断点。修法：开新钩子/新承诺；若是已解钩子忘了记 `resolved_in_chapter` 导致的误判，补记即消（已关闭但缺关闭章的条目按"一直活跃"处理，宁漏勿误）。
