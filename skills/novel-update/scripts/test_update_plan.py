...
def fake_repo(root):
    ...
    for skill in skills:
        write(os.path.join(root, "skills", skill, "SKILL.md"), f"---\nname: {skill}\n---\n# {skill}\n")
...
def test_check_bootstraps_missing_baseline():
...
def test_changed_review_skill_triggers_rework_up_to_current_stage():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write(os.path.join(repo, "skills", "novel-review", "SKILL.md"), "---\nname: novel-review\n---\n# changed\n")
...
def test_observe_only_skill_change_does_not_rebuild_text():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        update_plan.command_record(type("Args", (), {"project_root": project})())
        write(os.path.join(repo, "skills", "novel-dashboard", "SKILL.md"), "---\nname: novel-dashboard\n---\n# changed\n")
