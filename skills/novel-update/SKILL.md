...
- **输出**：
  - `生产数据/novel_skill_update_snapshot.json`
  - `生产数据/novel_skill_update_plan.json`
  - `生产数据/novel_skill_update_plan.md`
- **读写边界**：只写快照和计划；不改正文、不删产物、不改 `_进度.md`。
...
- 无基线时，`check` 默认建立一份临时基线并输出 `baseline_bootstrapped=true`；这只能从当前时刻开始追踪变化。确认现有产物可接受后，再跑 `record` 固化。
...
| 无基线时认为没有风险 | bootstrap 只能从当前时刻开始追踪；确认现状后应立即 `record` |
