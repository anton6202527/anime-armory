...
NOVEL_SCRIPTS = os.path.join(SKILLS, "novel", "scripts")
...
def command_for_stage(root: str, stage: dict[str, Any]) -> list[str]:
    ...
    if key == "score":
        return [
            f'python3 skills/novel-score/scripts/collect_market_baseline.py "{root}/评分" --target-platform "<目标平台>" --allow-fetch-errors',
            f'python3 skills/novel-score/scripts/score.py "{root}" --scope opening',
        ]
    if key == "review":
        return [
            f'python3 skills/novel-review/scripts/consistency_audit.py "{root}"',
            f'python3 skills/novel-review/scripts/build_review_report.py "{root}"',
        ]
    if key == "export":
        return [
            f'python3 skills/novel-craft/scripts/export.py "{root}"',
        ]
    if key == "release_manifest":
        return [f'python3 skills/novel-craft/scripts/release_manifest.py "{root}"']
    if key == "screen_ready":
        return [f'python3 skills/novel-craft/scripts/screen_adaptation_ready.py "{root}"']
    if key == "draft":
        return [
            f'python3 skills/novel-craft/scripts/draft_queue.py "{root}" init',
            f'python3 skills/novel-craft/scripts/draft_packets.py "{root}" --next',
        ]
    if key == "post_write":
        return [f'python3 skills/novel/scripts/post_write.py "{root}" --chapter 第NN章 --conclusion "{root}/审稿/state_verify_第NN章.json"']
    if key == "outline":
        return [f'python3 skills/novel-craft/scripts/scene_cards.py "{root}"']
    return []
...
def decide_next_action(root: str, *, write_pipeline_plan: bool = False) -> dict[str, Any]:
    ...
    if batch.get("dead_letter_count"):
        action.update({
            ...
            "recommended_commands": [f'python3 skills/novel-batch/scripts/queue.py dead-letter "{root}" --json'],
        })
        return action
    ...
    if blockers and not revision.get("exists"):
        action.update({
            ...
            "recommended_commands": [f'python3 skills/novel-craft/scripts/revision_planner.py "{root}"'],
        })
        return action
    ...
    if semantic_jobs:
        action.update({
            ...
            "recommended_commands": [
                f'python3 skills/novel-craft/scripts/semantic_job.py show "{root}/{semantic_jobs[0]}"'
            ],
        })
        return action
    ...
    if not next_stage:
        action.update({
            ...
            "recommended_commands": [f'python3 skills/novel-dashboard/scripts/dashboard.py "{root}" --write --html'],
        })
        return action
