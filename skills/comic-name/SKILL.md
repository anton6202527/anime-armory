...
3. 给每格写 `thumbnail_rect`、`panel_shape`、`border_style`、`gutter_intent`、`bubble_first`、`effects_hint`。
4. 记录原稿口径：`trim_box`、`safe_area`、`bleed`、`inner_frame`。
5. 输出 `name_board.json` 和 SVG 草图。
6. 若页流或文字密度不顺，回 `comic-script` 改分格；确认后再跑 `comic-layout`。
