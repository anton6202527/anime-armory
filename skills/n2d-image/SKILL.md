...
- **关键 PROP 禁形逐图复核（image_qc hard block）**：`image_qc` 对带 `must_not_have` 的 `PROP_xx` 不只查 prompt 是否传禁项，还会扫描已落档 `Clip_xx*.png`，生成 `生产数据/image_qc/<ep>/prop_shape_review/` 并排复核图；每张 PNG 都必须重出到无禁形，或在 `prop_shape_confirmations.json` 逐条写 `{"asset":"PROP_01","png":"图片/Clip_01_x.png","verdict":"ok"}`。未确认项会进入 `summary.hard_blocks`、`to_findings()` 和重生成清单，`dashboard gate --stage image` 返回非零。尤其瓶/酒/药类：无嘴白瓷小瓷瓶只允许唯一短颈圆口；侧嘴、斜嘴、喷口、奶嘴状瓶嘴、双口、额外开口、管状嘴都算硬伤。
...
> "本集出图 prompt 已生成完毕（共享层定妆 N 项 + 本集分镜 **X 张**）。接下来进入**出图环节**：先补齐共享定妆库（硬闸门），再按 `生成优先序`（默认**关键镜🔑优先**）排队生成。**生图前先请你选这次的切分颗粒度**（每次都问）：①逐个（一张/一Clip，最细，每个停审）②小批（默认每批 5，批大小可改）③按场景/段落分批 ④整集一次过。生图后端按 `_设置.md` 的 `生图AI`（默认 Codex，可选 Dreamina/即梦官方 CLI、Seedream/可灵主体库/Nano Banana 等官方/已登录后端；Sora Cameo 仅旧项目/人工路径；全集统一一个、不混用）；所选后端无法自动落 PNG 会停下报告，不偷偷换后端、不走第三方逆向/web 自动化兜底。"
