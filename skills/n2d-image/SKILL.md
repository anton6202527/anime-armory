...
本 skill 涉及的选择点：`基础视觉风格`（只继承，不在本阶段重选）、`生图AI`、`一致性增强(LoRA)`、`重抽预算策略`、`生成粒度`、`生成优先序`、`合规用途`。涉及版权、改编权、角色肖像/形象授权的判断不走普通偏好，必须写入 `合规/compliance_manifest.json`；出图阶段只需保证合规包已声明、不漏未授权形象。`distribution_intent=internal_only` 只免**平台审核/出海本地化**，**角色授权照常 BLOCK**，不因内部 demo 豁免（详见 `n2d-compliance`）。AI 生图内容的 **AI 标识/AI 披露/水印** 义务**不再由本流水线强制**，移到工具之外处理。
...
- **出图前角色 DNA 定档铁律（P0 · 每个角色做到哪一档，先说清再出图）**：付费出任何角色定妆前，必须先产出/刷新一张**「角色 DNA 一致性定档表」**——逐角色/形态判定**该升到哪一档**，**输出给用户** + 落进 `出图/共享/prompt/00_索引.md`（人读策略真值；机器侧当前档仍以 `identity_registry.json` 的 `scope`/`lora.status`/各后端 `status` 为真值）。一致性阶梯三档 + 一附加：**档① 参考图派生+锚点句**（默认·所有入镜角色）→ **档①.5 face_embedding 脸嵌入锁**（P2a·IP-Adapter FaceID 等免训练锚·backend 无关·比纯参考图组稳，无持久主体后端也能用，参考图压不住但还不值得上原生/LoRA 时的中间档）→ **档② 后端原生主体ID/主体库**（Seedream Universal Reference / 可灵主体库 / Sora Cameo / Kling Character ID / Seedance Face Lock / Veo；参考图压不住时·无 GPU）→ **档③ LoRA**（贯穿几十集核心角 + 前几档压不住，走 `n2d-lora`）；**附加·完整表情库/动作参考**（情绪戏、动作戏多的角色按风险 opt-in）。定档表列：`角色/形态 ｜ 体量 ｜ 当前档 ｜ 目标档 ｜ 表情库 ｜ LoRA ｜ 升档触发条件`。**基础定妆包不按主配角放松，也不按 ROI 省 45°**：所有人物、所有会复用形态都必须有正面 / 45° / 侧面 / 背面、半身或全身服装参考、脸部特写或同源表情参考、三视图人审拼版；只有完整表情库、动作参考、主体库/LoRA 仍按触发条件升档。逐角色分档方法见 `references/lora_consistency.md「出图前角色 DNA 定档框架」`。
- **后端身份能力矩阵铁律（执行真值）**：定档不要凭后端名字猜；读取 `skills/n2d/_lib/n2d_contract.py` 转出的 `IMAGE_IDENTITY_PROFILES`。Codex/OpenAI/Dreamina/即梦/Nano Banana/Gemini 属于**多参考/图生图但无持久主体 ID**，每镜仍要喂定妆组、基础脸锚/表情库和锚点句；Seedream/可灵/Sora 才属于**可注册持久主体/角色 ID**，核心长线角和多人同框高危镜优先注册；LoRA 只在前两档压不住时启动。**中间档 `face_embedding`（P2a·IP-Adapter FaceID 等免训练脸嵌入锁·backend 无关）**：填 reference-group↔LoRA 的断档——给无持久主体 ID 后端一个比纯参考图组强、又不必训 LoRA 的身份锚，登记在 `identity_adapters.image.face_embedding`（`{status:ready, type:ip_adapter_faceid}`）。`scripts/face_drift_risk.py` 按矩阵输出 `multi_reference / face_embedding / native_unregistered / native_subject / lora` 风险档（强度递增），不再把 Dreamina 或 Nano/Gemini 误判成 Seedream。
...
- **专项镜头模板继承铁律（复杂镜头 · 像素层锁变量）**：若 `storyboard.json clips[]` 有 `template/template_contract`，本阶段必须把模板契约誊抄到 `00_总览.md` 的「本集专项镜头模板速查」和对应逐镜 prompt。打斗/追逐/反打/法术爆发/飞行/亲密互动/多人站位的轴线、站位、接触点、关键帧、光效资产、负向约束，必须在首帧/尾帧里锁住；不能只靠 n2d-video 运动 prompt 事后补。`gate.py --stage image_preflight` 会先阻断 storyboard 缺模板或模板字段不全。
...
- **服饰/形态定妆铁律（P0 · 换装不是状态修饰）**：同一角色只要出现会复用、会改变轮廓/主色/身份识别的可见形态，就必须独立登记 `characters[].forms[]` 并单独出一套 `reference_group`：红衣觉醒态、白衣常态、战甲态、伪装态、幼年态、妖化态都各自有 `asset_key` 和定妆组，逐镜 prompt 必须绑定匹配的 `CHAR_xx/形态`。禁止用白衣/常态/旧形态参考去生红衣/换装/觉醒镜头，也禁止把不同服饰的正侧背/表情图混进同一 form。短期可逆的伤、泪、血迹、脏污、破损、能量光效进 `visual_state_ledger` / 分镜状态演进，不进 registry；一旦换装会跨镜复用或改变服装主色/剪影，就升级成独立 form。`gate.py` 会拦 reference_group 文件名不含本 form `asset_key` 的注册，`image_qc` 会拦“正向 prompt 写红衣/白衣/战甲却绑定非对应形态”的单人角色镜。
...
- **角色 DNA 全链对照**：出图前对照 `references/角色一致性checklist.md`（建卡→定妆→复用→锚点→出视频 一条铁律链 + 出图前 30 秒速查）——把跨集“脸 + 发型 + 服装 + 配饰”做成可勾选流程。
- **角色定妆基础包铁律（生产拆图 + 人审拼版）**：凡是人物定妆（主角、长线角色、配角、路人但会入镜的人物）都必须按**标准正 / 45° / 侧 / 背**建参考，不再按题材、重要程度或近景占比省 45° / 背面 / 脸部特写。基础包至少六类：① 正面主参考（=`出图/共享/图片/定妆_<角色>.png`，中性光、锁脸）② 45°/三分之二侧脸参考（`图片/定妆_<角色>_45度.png`，锁转头、过肩、反打最高频角度）③ 侧面参考（`图片/定妆_<角色>_侧.png`，锁鼻梁/下颌/发型侧轮廓）④ 背面参考（`图片/定妆_<角色>_背.png`，锁后脑勺发型/背部服饰/披风/腰带绑法）⑤ 半身或全身服装参考（`图片/定妆_<角色>_半身.png` 或 `图片/定妆_<角色>_全身.png`，锁衣领/袖型/腰带/配色/体态）⑥ 基础脸锚（`图片/定妆_<角色>_脸部特写.png` 或至少一张 `图片/表情_<角色>_<情绪>.png`，同源裁切/派生，不重新抽新脸）。另存**人审拼版** `图片/定妆_<角色>_三视图.png`，由已通过自检的正/侧/背三张拼成，不作为分镜主参考。完整 `_表情.png`、动作参考、主体库/LoRA 按角色长期性和镜头风险升档，不全员前置。形态变体（觉醒态…）按同一基础包各出；先用常态主参考派生，避免变成另一个人。**命名约定：多张可喂图 AI 的拆分 PNG 叫“定妆组”；单张合成总览才叫“三视图/设定表”。**详见 `references/角色一致性checklist.md §二` 与 `references/prompt_format.md §1.2`。
...
- **多参考喂养按后端分层铁律（④·`image_qc` C4·info）**：`persistent_subject=True`（Seedream/可灵/Sora）逐镜应按**已注册主体 ID + 单张干净强锚**引用——2026 实践「单强锚 > 弱参考拼盘」，不必每镜堆全角度组（多样集只在**注册主体**环节喂）。所以对持久主体后端把「喂全组」的 `native_multiref_underfed` nudge 换成 `native_subject_anchor_ok`（ID+单强锚口径），不再误导其堆图；只有多参考后端（Codex/Dreamina/Nano 等 `persistent_subject=False`）才提示喂满全角度组。
...
- **跨 AI 锚定句铁律**：若**图 AI ≠ 生视频模型**，每个 image prompt 末尾**必须**拼接目标生视频模型的"图像风格锚定句"，否则生视频模型的 image2video 运动估计会崩。
...
- **生图后端规则（安全硬闸门 + 后端选择点）**：
  ...
  - **② 后端选择（官方/已登录后端放行 → `n2d/_lib/n2d_contract.py` `APPROVED_IMAGE_BACKENDS`）**：`生图AI` 是**真选择点**，默认 Codex；**放行官方/已登录多参考一致性后端**：Dreamina/即梦官方 CLI、Seedream Universal Reference / 可灵 Kling 主体库 / Nano Banana(Gemini) / Sora Character Cameo。`gate.py --stage image_preflight` **不再因"非 Codex"阻断**，只阻断 ① 项目内**后端混用** ② **第三方逆向/未授权出图路径**。旧项目 `_设置.md` 写 `同视频AI` 时必须改成显式后端名。**默认起点按剧长分档（知情权衡）**：**长篇/多集（≥3 集）/ 核心常驻角色 / 多人同框吃重 → 推荐默认选支持原生主体的后端**（Seedream / 可灵 / Sora，注册一次按 ID 跨镜跨集引用，2026 工业默认）；**单集/短 demo/配角 → Codex 即可**。`image_preflight` 对「长线×无持久主体后端」出 `生图AI一致性` WARN 建议升档（`check_long_running_weak_backend`·advisory 不阻断）。梯子与理由见 `n2d/references/模型矩阵.md`「一、图」。
...
- **多角色同框是硬触发点（不靠"想起来"）**：**别试图让弱后端在单次生成里画好多张脸——这是结构性不可能**（无持久主体 ID + 注意力串扰，实测基线一致性≈85%、≥3 具名脸开始崩）。处理顺序按 ROI 从高到低，前两步是默认、后三步是兜底：
  ...
  - **② 同框路由到原生主体 ID 后端（有则用·无则回退 Codex）**：含 ≥2 具名角色的镜**优先**路由到可注册持久主体的官方后端（Nano Banana Pro 免训练最多 5 人 / Seedream 多图编辑 / 可灵 Kling Character ID / Sora Cameo）并写 `native_subject_slots`。**当前默认 `生图AI=Codex`；路由不到强后端时不强制切换，继续用 Codex 并做好 ③④ 强化**（gate 不因"非强后端"阻断，只在缺 split/槽位/区分锚点时阻断或提示）。注意：多参考别超喂，**2–5 张最佳，过阈反降结构精度**。
...
- **按后端能力路由**（`IMAGE_IDENTITY_PROFILES`）：multi_reference 后端（Codex/OpenAI/Dreamina/Nano）组**逐镜多样参考包**（受 `max_reference_images` 约束）；persistent_subject 后端（Seedream/可灵/Sora）未注册→**提示注册时喂多样集（多角度+多表情+多光）而非单 sheet**（Kling 优先 Custom Model 吃多帧/视频），已注册→按 ID 引用 + 参考双保险；
...
- **后端支持就强制注册核心长线角色（不靠"想起来"）**：当 `生图AI` 解析到的后端**支持原生主体**（Seedream/可灵/Sora 等，`identity_registry` 该后端 `default_status=unregistered`）而**核心长线角色仍未注册**时，付费出图前必须先注册主体——这是省钱前置，不是事后补救。`gate.py --stage image_preflight` 已机检兜底：选了支持原生主体的后端 + 核心长线角色（registry `scope` 含 全篇/全程/长线/主角/主反派…）在该后端 `status` 非 `registered/ready` → 出 `原生主体注册` **BLOCK**。**Codex/OpenAI/Dreamina 等无持久主体的后端不触发本硬闸**——它们本就没有可注册的主体层，自动回退第①档参考图派生。短线配角/单元妖按「ROI 默认最小化」不前置注册，机检也不 nudge。**但长线剧例外**：弱后端跑**长篇/多集（≥3 集）**时，`image_preflight` 会另出 `生图AI一致性` **WARN**（`check_long_running_weak_backend`），建议核心常驻角色**整项目升到支持原生主体的后端**（②）或 LoRA（③）。
...
- **出图前脸漂风险预测（事前预案，把 LoRA/加强参考从「事后升档」前移）**：**`dashboard gate --stage image_preflight` 现在会自动跑这条 + 下条物料预案，把 high/医 medium 项内联进同一份预检报告（high→WARN·medium→INFO，advisory 不阻断）——一个入口同时拿齐阻断项+漂移预案，agent 不必再记得单独跑。** 也可单独跑 `python3 skills/n2d-image/scripts/face_drift_risk.py <作品根> 第N集`。它读 `storyboard.json` + `identity_registry.json`，按本集分镜的高危信号（近景占比/大表情数/多人同框/极端角度命中 `angle_policy.risky` + 锁脸档位）给每个角色算**本集脸漂风险分**（high/medium/low），对 high/medium 角色给可执行建议（建表情库 expressions、补侧/背/角度参考、多人同框换多参考后端或拆正反打、high 且未上 LoRA 提示 `n2d-lora init`）。预测档（high/medium）**只提示不阻断**——指导「角色 DNA 一致性定档表」该给谁升档/建表情库。**②实测漂移回灌**：本脚本还读 `生产数据/identity_drift_report.json`（`n2d-identity` 对**已出图集**真量出的跨集漂移：embedding 质心 high / block 级脸漂镜）——命中角色升 **⛔block**（既成事实非预测），退出码 2，让 SOP/gate 卡住"带病续出下一集图"，把"等跨集漂了再事后升档"压成"上一集一漂、下一集出图前就停"。无该报告 / 无 insightface → 仅预测档生效，不假报。落 `生产数据/face_drift_risk_第N集.{json,md}`。
- **出图前物料漂移风险预测（E·场景/道具/武器/特效版的脸漂风险分）**：同样扫完 registry 后、付费出图前，跑 `python3 skills/n2d-image/scripts/asset_drift_risk.py <作品根> 第N集`。它读 `asset_registry.json` + `storyboard.json`，按本集分镜高危信号（跨集复用度/本集出镜次数/drift_forbidden 项数/结构·颜色强锁/多形态）给每个**场景/道具/特效**算漂移风险分，high/medium 给可执行建议（高复用进共享库一次出别每集重画、补场景四视图/不同机位、锁结构件数、锁颜色拖尾防窜色、多形态上结构化状态机）。只提示不阻断，落 `生产数据/asset_drift_risk_第N集.{json,md}`。背景漂移和脸漂一样穿帮，这条把"哪些物料本集容易漂"也前移到出图前。
...
**分支 1：所选官方后端能落 PNG**
- 按 `_设置.md` 的 `生图AI`（默认 Codex；可选 Dreamina/即梦官方 CLI、Seedream/可灵主体库/Nano Banana/Sora Cameo 等官方/已登录后端）选定**单一后端**。扫描到 `同视频AI` / `同视频模型`、第三方逆向或 web 自动化出图口径必须忽略并改回显式官方后端；**全集统一一个后端，不混用**。
...
4. **跨 AI 锚定句**：组装 prompt 时若 `global_style.md` 标的 图AI ≠ 生视频模型，自动在 prompt 末尾追加生视频模型的锚定句（详见 `references/platforms.md`）
...
| 图 AI ≠ 生视频模型时漏拼锚定句 | image prompt 末尾必须拼生视频模型锚定句 |
