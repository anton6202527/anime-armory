...
- **角色定妆组铁律（最低三件套 · 厚锚点）**：贯穿多集的核心 / 长线角色，定妆**不止一张正脸**。但不要求一上来做严格工业级三视图，先做**最低有效定妆组**：① 正脸主参考（=`出图/common/定妆_<角色>.png`，中性光、锁脸）② 侧脸角度参考（`定妆_<角色>_侧.png`，锁鼻梁/下颌/发型侧轮廓）③ 半身或全身服装参考（`定妆_<角色>_半身.png` 或 `_全身.png`，锁衣领/袖型/腰带/配色/体态）。背面（`_背.png`）和表情条（`_表情.png`，喜 / 怒 / 哀 / 惊）是增强项：角色会大量背身/转身/情绪特写时再补。一次性配角仍可只出单张主参考。形态变体（觉醒态…）按同一规则至少出最低三件套；先用常态主参考派生，避免变成另一个人。详见 `references/角色一致性checklist.md §二` 与 `references/prompt_format.md §1.2`。
