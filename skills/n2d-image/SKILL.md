...
本 skill 涉及的选择点：`基础视觉风格`（只继承，不在本阶段重选）、`生图AI`、`一致性增强(LoRA)`、`重抽预算策略`、`生成粒度`、`生成优先序`、`合规用途`。涉及版权、改编权、角色肖像/形象授权的判断不走普通偏好，必须写入 `合规/compliance_manifest.json`。AI 生图内容的 **AI 标识/水印** 义务同样由该合规包的 `ai_disclosure`/`watermark` 段驱动（落地在成片阶段由 `n2d-compose` 调 `n2d-watermark` 执行，出图阶段只需保证合规包已声明、不漏未授权形象）；`distribution_intent=internal_only` 只免**平台审核/出海本地化**，**角色授权·AI 标识·水印照常 BLOCK**，不因内部 demo 豁免（详见 `n2d-compliance`）。
...
- **中段锚帧（opt-in·治长镜中段漂移，绝大多数 Clip 不出）**：尾帧焊的是 **Clip 之间**的接缝；长 Clip（≥8s、表演节拍 ≥3 拍）、打斗等高运动模板镜、或有中段动作漂移重抽记录的镜头，**Clip 内部**中段也可能漂。仅当 `storyboard.json` 该 Clip 的 `continuity.midframe`（单锚帧，命名=首帧名+`_mid`）或 `continuity.anchors`（N 锚帧链，命名=首帧名+`_a1.._aN`；可由 `n2d-script/scripts/anchor_planner.py` 自动识别规划+人确认 `--write` 注回）已声明（gate 缺项/不递增/越界阻断）时，按声明逐张出**锚帧 PNG**，内容 = 该 Clip `表演节拍` 在各 `at_sec`/`split_at_sec` 时刻的中间拍姿态，**同一套定妆组 + 同光位锚 + image2image 派生锁人**（生成方式同正反打尾帧：以首帧/同镜成图为母图只改姿态，不重画脸/发髻/服装；打斗模板镜的锚帧对齐 `template_contract.beats` 的拆招拍：起手/发力/命中/受击/收势）；供 n2d-video 拆 K+1 段双帧接力。出完回填 `midframe.midframe_png`/`anchors[].anchor_png`，并跟尾帧一样过 `image_qc`（锚帧崩 = 相邻两段视频都崩，必须先修）。没声明就不出——这是 K+1× 视频成本的救镜工艺，不是新默认。
...
- **出图落档机检铁律（生图后闸门 · 一致性把控）**：本集分镜全部落档后必须跑 `python3 skills/n2d-image/scripts/image_qc.py <作品根> 第N集`。它把 `n2d-review` 的一致性机检（崩脸 G1 / 服装 N1 / 场景 O2 / 接缝接力 / 锚点门 N3）**前移到出图落档**，复用同一套已校准纯函数与阈值（单一真值源），并 lint 逐镜 prompt（角色镜是否有参考图块/视线方向/锚点句/身份锁定句，**`CHAR_xx/形态` 是否在 identity_registry 合法存在**，尾帧身份交接是否有专用目标定妆锁定，**近景大表情镜是否引用表情库 expressions/脸部特写**（`no_expression_lib_ref` warn，治表情镜脸漂）——gate 结构检查不替你验这些）。与生图前的 gate 互补：gate 查契约结构，image_qc 查真出的像素 + 漏拼。`verdict=block`（崩脸 / **接缝接力断**（尾帧没接上下镜首帧，出视频必跳切；seam_analyze 已对设计切镜降 info，故 block=真断）/ 纯文生图 / 非法 CHAR_id / **尾帧身份交接未锁目标身份** / **降级精度下的近景脸**（无 insightface 时 Pillow 降级无法验同人，CU/MCU/反打镜不放行））必须修复后重跑；`verdict=review`（服装/场景调色板初筛、漏字段）交人二次判、确认误报可放行；不被噪声淹没。**降级精度近景脸会自动拼一张「定妆主参考 ↔ 本镜脸」并排对比图**（`生产数据/image_qc/第N集/face_review/`，hard-block finding 附图路径 + Haar 几何粗筛优先级），让人眼一屏秒判同人。详见 `scripts/image_qc.py` + `n2d-review/scripts/face_compare_stitch.py`。
...
- **生图后端规则（安全硬闸门 + 后端选择点）**：
  ...
  - **② 后端选择（官方/已登录后端放行 → `common/n2d_contract.py` `APPROVED_IMAGE_BACKENDS`）**：`生图AI` 是**真选择点**，默认 Codex；**放行官方/已登录多参考一致性后端**：Dreamina/即梦官方 CLI、Seedream Universal Reference / 可灵 Kling 主体库 / Nano Banana(Gemini) / Sora Character Cameo。`gate.py --stage image_preflight` **不再因"非 Codex"阻断**，只阻断 ① 项目内**后端混用** ② **第三方逆向/未授权出图路径**。旧项目 `_设置.md` 写 `同视频AI` 时必须改成显式后端名。
...
- **判定分两级**：`verdict=block`（硬阻断——崩脸、纯文生图、引用了 registry 不存在的 `CHAR_xx`）→ 必须修复/重抽后重跑，不得推进；`verdict=review`（初筛人判——outfit/scene/seam/锚点门/漏视线锚点句等像素直方图机检初筛）→ 交人二次判，确认误报可放行（同 video_qc 的 `--allow-qc-block` 哲学，不被噪声淹没）；`verdict=ok` → 放行。
- **先告知环境再判阶段**：报告落 `生产数据/image_qc/第N集/`，其中 `qc_environment.precision_level` 必须读给用户或摘要给用户。`full` 且 `verdict=ok` 才能建议进入 `video`；`full` 但 `block` → 回退/停在 `image`，列出受影响 PNG/镜头/prompt，最小范围重出；`full` 但 `review` → 停在 `image_human_review`，人审确认后再进 `video`；`degraded` → 停在 `image`，先安装 full stack 并重跑，或逐项人审并记录放行理由；`none` → 停在 `image_qc_setup`，安装依赖后重跑，不能把缺依赖当作图片通过。
