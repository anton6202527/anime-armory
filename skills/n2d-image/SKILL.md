...
- **后端支持就强制注册核心长线角色（不靠"想起来"）**：当 `生图AI/生图渠道` 解析到的后端**支持原生主体**（Seedream/可灵等，`identity_registry` 该后端 `default_status=unregistered`）而**核心长线角色仍未注册**时，付费出图前必须先注册主体——这是省钱前置，不是事后补救。`gate.py --stage image_preflight` 已机检兜底：选了支持原生主体的后端 + 核心长线角色（registry `scope` 含 全篇/全程/长线/主角/主反派…）在该后端 `status` 非 `registered/ready` → 出 `原生主体注册` **BLOCK**。**Codex/OpenAI/Dreamina 等无持久主体的后端不触发"注册主体"硬闸**——它们本就没有可注册的主体层，自动回退第①档参考图派生。短线配角/单元妖按「ROI 默认最小化」不前置注册，机检也不 nudge。**但长线剧例外**：无持久主体后端跑**长篇/多集（≥3 集）**时，`image_preflight` 会另出 `生图AI一致性`（兼容维度名）**BLOCK**（`check_long_running_weak_backend`）：核心/常驻角色必须有 native subject / Face Lock / face_embedding / LoRA 之一；若 registry 未标核心/常驻角色也 BLOCK，先补 scope/tier 再判断谁必须升档。
