...
- **出图前角色 DNA 定档铁律（P0 · 每个角色做到哪一档，先说清再出图）**：付费出任何角色定妆前，必须先产出/刷新一张**「角色 DNA 一致性定档表」**——逐角色/形态判定**该升到哪一档**，**输出给用户** + 落进 `出图/共享/prompt/00_索引.md`（人读策略真值；机器侧当前档仍以 `identity_registry.json` 的 `scope`/`lora.status`/各后端 `status` 为真值）。一致性阶梯三档 + 一附加：**档① 参考图派生+锚点句**（默认·所有入镜角色）→ **档② 后端原生主体ID/主体库**（Seedream Universal Reference / 可灵主体库 / Sora Cameo / Kling Character ID / Seedance Face Lock / Veo；参考图压不住时·无 GPU）→ **档③ LoRA**（贯穿几十集核心角 + 前两档压不住，走 `n2d-lora`）；**附加·完整表情库/动作参考**（情绪戏、动作戏多的角色按风险 opt-in）。定档表列：`角色/形态 ｜ 体量 ｜ 当前档 ｜ 目标档 ｜ 表情库 ｜ LoRA ｜ 升档触发条件`。**基础定妆包不按主配角放松，也不按 ROI 省 45°**：所有人物、所有会复用形态都必须有正面 / 45° / 侧面 / 背面、半身或全身服装参考、脸部特写或同源表情参考、三视图人审拼版；只有完整表情库、动作参考、主体库/LoRA 仍按触发条件升档。逐角色分档方法见 `references/lora_consistency.md「出图前角色 DNA 定档框架」`。
- **后端身份能力矩阵铁律（执行真值）**：定档不要凭后端名字猜；读取 `skills/n2d/_lib/n2d_contract.py` 转出的 `IMAGE_IDENTITY_PROFILES`。Codex/OpenAI/Dreamina/即梦/Nano Banana/Gemini 属于**多参考/图生图但无持久主体 ID**，每镜仍要喂定妆组、基础脸锚/表情库和锚点句；Seedream/可灵/Sora 才属于**可注册持久主体/角色 ID**，核心长线角和多人同框高危镜优先注册；LoRA 只在前两档压不住时启动。`scripts/face_drift_risk.py` 会按这个矩阵输出 `multi_reference / native_unregistered / native_subject / lora` 风险档，不再把 Dreamina 或 Nano/Gemini 误判成 Seedream。
