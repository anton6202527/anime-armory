...
- **出图前角色 DNA 定档铁律（P0 · 每个角色做到哪一档，先说清再出图）**：付费出任何角色定妆前，必须先产出/刷新一张**「角色 DNA 一致性定档表」**——逐角色/形态判定**该升到哪一档**，**输出给用户** + 落进 `出图/共享/prompt/00_索引.md`（人读策略真值；机器侧当前档仍以 `identity_registry.json` 的 `scope`/`lora.status`/各后端 `status` 为真值）。一致性阶梯三档 + 一附加：**档① 参考图派生+锚点句**（默认·所有入镜角色）→ **档② 后端原生主体ID/主体库**（Seedream Universal Reference / 可灵主体库 / Sora Cameo / Kling Character ID / Seedance Face Lock / Veo；参考图压不住时·无 GPU）→ **档③ LoRA**（贯穿几十集核心角 + 前两档压不住，走 `n2d-lora`）；**附加·表情库**（情绪戏多的核心角 opt-in）。定档表列：`角色/形态 ｜ 体量 ｜ 当前档 ｜ 目标档 ｜ 表情库 ｜ LoRA ｜ 升档触发条件`。**ROI 驱动、默认最小化**：体量越短档越低，**绝不在第1集就给全员前置高档/表情库/LoRA**（漫剧角色多在前几集退场，前置=烧在站不住的角色上）；升档只在「触发条件」命中时发生。逐角色分档方法见 `references/lora_consistency.md「出图前角色 DNA 定档框架」`。
- **后端身份能力矩阵铁律（执行真值）**：定档不要凭后端名字猜；读取 `skills/n2d/_lib/n2d_contract.py` 转出的 `IMAGE_IDENTITY_PROFILES`。Codex/OpenAI/Dreamina/即梦/Nano Banana/Gemini 属于**多参考/图生图但无持久主体 ID**，每镜仍要喂定妆组、表情库和锚点句；Seedream/可灵/Sora 才属于**可注册持久主体/角色 ID**，核心长线角和多人同框高危镜优先注册；LoRA 只在前两档压不住时启动。`scripts/face_drift_risk.py` 会按这个矩阵输出 `multi_reference / native_unregistered / native_subject / lora` 风险档，不再把 Dreamina 或 Nano/Gemini 误判成 Seedream。
  - **Codex 弱后端锁脸 gate**：普通多参考不等于持久角色 ID。`image_preflight` 在 `persistent_subject=false`（Codex/OpenAI/Dreamina/Nano/Gemini 等）时会硬卡四类脸漂真凶：① 多人同框只写普通多参考、不登记「分别出图+合成/单人分层出图」；② 多人同框虽写了分层/合成但缺 `多人同框身份槽位`（每个 `CHAR_xx/形态` 对应自己的画面位置、脸部参考、表情库和 primary 星标）；③ 近景/反打/大表情镜缺同源脸部特写或 expressions 表情库参考；④ 暗光/黑烟/VFX 叠脸却缺「眼鼻嘴三角区清晰、特效不遮脸、不重画五官」约束。解决优先级：先拆单人/合成或统一切持久主体后端，其次同源 image2image 接力，最后才靠锚点句和普通多参考兜底。注意：`若多主体仍不稳再分别出图` 这类条件式兜底不算登记降级，必须把「分别出图+合成/单人分层出图」写成本镜硬执行；多人同框也不得用 `参考图①/reference image 1` 做泛化身份锁，必须逐主体写 `CHAR_xx/形态` 对应自己的定妆/脸部特写/表情库。
...
- **资产身份注册层铁律（P0 · 角色 ID / Face Lock / LoRA 适配）**：共享定妆库不再只是一堆 PNG 和 `00_索引.md`；必须同步维护 `出图/共享/identity_registry.json`，把每个角色形态登记成可复用身份：`reference_group`（正/侧/背/服装/三视图 + **`expressions` 同源情绪定妆**）、`reference_atlas`（角色参考图数据库：正脸 / 45° / 侧脸 / 半身 / 全身 / 背影 + 表情参考 + 动作参考 + 逐镜选图策略）、`identity_adapters`（图后端角色 ID/主体库、视频 Face Lock/reference controls、LoRA 状态）、`angle_policy`（允许角度/高危角度/需补参考）和 `drift_forbidden`（零容忍漂移项）。**`expressions` / `reference_atlas.expression_refs` 治近景表情漂移**：近景高频/情绪戏核心角必出一组「一脸多情绪」定妆（`{emotion: 中性/喜/怒/悲/惊, path}`，与正面同一张脸只换面部肌肉、脸型五官比例不变），供下游 `n2d-video` 近景大表情镜（哭/笑/怒/惊的 CU）做**首尾双帧只插值**（首=起表情、尾=止表情），避免模型自由生成跨情绪表情时把脸重画；缺则该角大表情近景须降级 MCU。**`reference_atlas.action_refs` 治动作姿态漂移**：持剑/御剑/施法/奔跑等高频动作只对核心/动作重角色按需补，不给短线角色全员前置烧图。逐镜 prompt 的 `资产身份注册层` 必须写具体 `CHAR_xx/形态`，执行端据此从 registry 自动取定妆组做 image2image / 多图参考派生；禁止只写中文角色名或纯靠文生图重新发明人物。每次补定妆/注册后跑 `python3 skills/n2d-identity/scripts/identity.py <作品根> --write` 生成 `identity_adapter_matrix`，让出图/出视频/审片读取同一份可执行 binding。详见 `references/资产身份注册层.md`。`gate.py --stage image|video|compose|review` 缺 registry、字段不全、逐镜缺 `CHAR_xx/形态`、后端 mode/status 错、LoRA ready 假登记即阻断。
- **角色 DNA 四层铁律（P0 · 不再只说锁脸）**：产线统一把角色身份称为**角色 DNA**，四层固定为 **脸 / 发型 / 服装 / 配饰**。角色卡、`identity_registry.json`、逐镜 prompt 和 QC 报告都必须按这四层说话：① 脸=脸型、五官比例、肤色、疤/鳞/痣；② 发型=发色、发髻/披发轮廓、发饰；③ 服装=款式、领口、袖型、材质、主辅色、剪影；④ 配饰=发簪、腰牌、护甲、钥匙、法宝挂件等标志物。`drift_forbidden` 不得只写 `face_shape`，还要覆盖会改变观众认人判断的 `hairstyle`、`outfit_palette`、`signature_accessory` 或更具体的锚点；逐镜 `近景/反打身份锁定`、尾帧/中段锚帧自检也要写“锁角色 DNA 四层”。只锁脸会出现第1集白发、第20集黑发、第50集红发这类观众认定换人的硬伤。
- **角色圣经铁律（Character Bible · 人读总入口）**：每个作品根必须维护 `设定库/角色圣经.md`，它是角色设定的人读总入口；`identity_registry.json` 是机器执行真值。角色圣经按 `CHAR_xx/形态` 聚合锚点句、四层角色 DNA 和气质/动作习惯；生成或修改定妆 prompt、分镜 prompt、video prompt、QC/返工说明时，先读角色圣经，再绑定 registry 的 `CHAR_xx/形态`，不得从散落角色卡临场拼设定。角色卡、角色圣经、registry 三者冲突时先停下对账；生产 ID 以角色圣经 + registry 为准。
- **角色资产包铁律（Character Assets · 可迁移打包层）**：核心/长线角色应维护项目内 `设定库/character_assets/<CHAR_ID>__<slug>/manifest.json`，并按 `reference/ prompts/ lora/ voice/ adapters/ qc/` 分区归拢可迁移资产。它解决 100+ 集换模型、换工作流、换视频工具时的继承问题；**不替代** `identity_registry.json` / `character_bible.json`，也不重新定义角色 DNA。新增或升档核心/长线角色时，同步写 `characters[].asset_bundle` 引用；短线角色不强制铺空目录。跨项目复用必须走 `n2d-asset-market` 导出模板并 fork，旧项目的后端 Character ID / Face Lock / LoRA ready / voice id 不得直接当新项目可执行资产。
...
- **角色表演层（P0 · 气质/动作进 prompt，不塞 registry DNA）**：四层角色 DNA 锁视觉身份；**气质/性格/动作习惯**锁“像不像这个角色在演”。角色卡和角色圣经可写 `气质/动作习惯`（如清冷克制、低眉含泪、量人目光、叩案施压），逐镜 prompt 在 `③人物` 或 `表演层` 继承；它不写进 `identity_registry.character_dna` 四键，避免把可变表演误当固定外观。
...
- **近景/反打身份锁定铁律（表情镜专项）**：CU/ECU、正反打、过肩反应、表情特写这类镜头不能只靠“深蓝宫装/整齐高髻”这种大类锚点。逐镜 prompt 必须增加 `近景/反打身份锁定` 字段：① 引用 `reference_group.expressions[]` 或 `定妆_<角色>_脸部特写.png` / `定妆_<角色>_表情.png`；② 明确锁 `脸型 / 五官比例 / 发型发髻 / 标志配饰 / 服装配色`；③ 相邻接力帧（如 Clip12_end → Clip13）要写“若发髻/脸型不一致即返工”。没有表情库时，可以从已通过的正面/半身定妆**裁切脸部特写作为参考图**先补，不要重新抽一张“表情定妆”制造第二张脸；这张脸部特写只能喂给官方后端做真实 image2image 派生，不能本地贴回正式镜头。`gate.py --stage image` 会阻断缺这条的正反打/反应/表情近景。
...
- **近景/大表情** → 加表情库 `expressions`（缺真情绪表情库只剩中性特写时**标补拍**，与 image_qc 的 `no_expression_lib_ref` 互补：规划器 pre-gen 选、image_qc post-gen 验）；
...
**落地**：建议侧车，**人审后把处方落进** `出图/第N集/prompt/01_分镜出图.md`（规划器不自动改已定稿 prompt）。`gate.py --stage image_preflight` 读 plan 做**落实对账**：有未落实行动项 → `参考规划落实` WARN（advisory，不阻断兜底）。规划器是 opt-in 前置，没跑就没有 plan、gate 静默。
...
- **后端支持就主动引导注册（不靠"想起来"）**：当 `生图AI` 解析到的后端**支持原生主体**（Seedream/可灵/Sora 等，`identity_registry` 该后端 `default_status=unregistered`）而**核心长线角色仍未注册**时，付费出图前必须**主动提示用户去注册主体**——这是省钱前置，不是事后补救。`gate.py --stage image_preflight` 已机检兜底：选了支持原生主体的后端 + 核心长线角色（registry `scope` 含 全篇/全程/长线/主角/主反派…）在该后端 `status` 非 `registered/ready` → 出 `原生主体注册` **WARN**（advisory，不阻断合法的参考图兜底）。**Codex/OpenAI/Dreamina 等无持久主体的后端不触发本提示**——自动回退第①档参考图派生（这就是默认 Codex 下你不会被打扰的原因）。短线配角/单元妖按「ROI 默认最小化」不前置注册，机检也不 nudge。
...
- 同步盘清：核心/长线角色是否有 `characters[].asset_bundle` 指向 `设定库/character_assets/<CHAR_ID>__<slug>/manifest.json`；若已建资产包，读取其 reference/prompts/lora/voice/adapters/qc 摘要和缺口。资产包缺失不阻断短线角色；核心/长线角色缺失时先建索引包或记录补包任务。
...
- 若新增项是核心/长线角色或把角色升档到主体库/LoRA/高频表情库，同步创建或更新 `设定库/character_assets/<CHAR_ID>__<slug>/manifest.json` 与 `_index.json`，至少含 `reference/ prompts/ lora/ voice/ adapters/ qc/` 分区和 `truth_sources`；并在 `identity_registry.json.characters[].asset_bundle` 写 manifest 引用。该 manifest 只归拢资产和缺口，不复制一套新角色 DNA。
...
- **判定分两级 + 角色脸覆盖硬闸 + 贴脸反作弊硬闸**：`verdict=block`（硬阻断——崩脸、接缝断、纯文生图、非法 `CHAR_xx`、尾帧身份/接力硬伤、降级精度近景、`face_reference_coverage.missing>0`、`prohibited_face_patch.outputs>0`）→ 必须修复/重抽后重跑，不得推进；`verdict=review`（初筛人判——outfit/scene/道具特效/锚点门/漏视线锚点句等）→ 交人二次判，确认误报可放行；`verdict=ok` → 放行。`face_reference_coverage` 是铁律：每张已落档角色图都必须有 full 精度定妆/身份主参考比对证据，warn/noface 不算通过；`prohibited_face_patch` 是更高优先级的事实闸门：最新落档事件若来自本地贴脸/换脸/裁脸贴回画面，即使 embedding 分数过线也一律不合格，必须真实重抽或用官方 image2image 派生替换。
