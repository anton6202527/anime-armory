...
- **判定分两级 + 角色脸覆盖硬闸**：`verdict=block`（硬阻断——崩脸、接缝断、纯文生图、非法 `CHAR_xx`、尾帧身份/接力硬伤、降级精度近景、`face_reference_coverage.missing>0`）→ 必须修复/重抽后重跑，不得推进；`verdict=review`（初筛人判——outfit/scene/道具特效/锚点门/漏视线锚点句等）→ 交人二次判，确认误报可放行；`verdict=ok` → 放行。`face_reference_coverage` 是铁律：每张已落档角色图都必须有 full 精度定妆/身份主参考比对证据，warn/noface 不算通过。
