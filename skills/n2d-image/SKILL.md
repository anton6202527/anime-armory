...
- **标准三视图铁律（定妆＝对齐档案，不是三张好看图）**：人物定妆的「标准」不在出几张，而在**中性同源 + 三视对齐**。闭源平台文生图只会给「三张分别好看的角度图」（身高/比例/视平线/光各漂），拼出来必然不像设定稿。两组硬条件，定妆自检必过：① **中性同源**——同模型同基底 prompt 只替换正/侧/背那一截、纯浅灰底柔和匀光、无表情无戏、双手可见 ≥1024px，并保持 `_设置.md` 所选项目画幅；② **对齐**——正/侧/背**同身高·同比例·水平视平线对齐·同景别距离**（「标准三视图」与「三张好看图」的分界线）。生产优先「**整张 turnaround 总图同框出 → 拆正/侧/背**」一次锁对齐（多参考后端最适合）；要工业级正交转身（同高三机位）闭源给不了，走 ComfyUI turnaround pose 模板 + IP-Adapter FaceID（与 LoRA 数据集同源产物，见 `references/lora_consistency.md` Stage 1/3）。对齐不达标 = 与脸漂同级硬伤、不得落档。详见 `references/prompt_format.md §1.2「标准三视图生产铁律」` 与 `references/角色一致性checklist.md §二`。
