...
- **正反打 prompt 消费铁律**：`dialogue_shot_reverse` 镜头必须从 P-2 `axis_blocking_map.json#shot_reverse_patterns` 和 P-3 `continuity_bible.json#shot_reverse_continuity` 继承正反打合同：180° 行动轴线、A/B 屏幕左右、互补视线、OTS/clean single/insert coverage、镜头高度/距离匹配、越轴策略和缓冲镜。逐镜 prompt 不能只写“反打/对话近景”；必须写清“谁画左/画右、谁看画左/画右、镜头不看读者、何时用建立/插入物重新定向”。缺这些字段的 `dialogue_shot_reverse` 会被 review gate 阻断。
...
- **出图前角色 DNA 定档铁律（P0 · 每个角色做到哪一档，先说清再出图）**：付费出任何角色定妆前，必须先产出/刷新一张**「角色 DNA 一致性定档表」**——逐角色/形态判定**该升到哪一档**，**输出给用户** + 落进 `出图/共享/prompt/00_索引.md`（人读策略真值；机器侧当前档仍以 `identity_registry.json` 的 `scope`/`lora.status`/各后端 `status` 为真值）。一致性阶梯三档 + 一附加：**档① 参考图派生+锚点句**（默认·所有入镜角色）→ **档①.5 face_embedding 脸嵌入锁**（P2a·IP-Adapter FaceID 等免训练锚·backend 无关·比纯参考图组稳，无持久主体后端也能用，参考图压不住但还不值得上原生/LoRA 时的中间档）→ **档② 后端原生主体ID/主体库**（Seedream Universal Reference / 可灵主体库 / Kling Character ID / Seedance Face Lock / Veo；Sora Cameo 旧项目人工路径；参考图压不住时·无 GPU）→ **档③ LoRA**（贯穿几十集核心角 + 前几档压不住，走 `n2d-lora`）。**本机 LoRA 慢速熔断默认回档①强执行版**：若本机训练 probe/正式样本显示 CPU fallback、速度不可接受或用户偏好为放弃本机训练，则不等待本机 LoRA，改登记 `image2image_reference_chain`（真实图片入参 + reference_manifest + full QC），走共享定妆/脸锚/表情库的 image2image/多图参考链；LoRA 可保留为云训或以后补强，不作为本集出图前置。**附加·完整表情库/动作参考**（情绪戏、动作戏多的角色按风险 opt-in）。定档表列：`角色/形态 ｜ 体量 ｜ 当前档 ｜ 目标档 ｜ 表情库 ｜ LoRA ｜ 升档触发条件`。**基础定妆包不按主配角放松，也不按 ROI 省 45°**：所有人物、所有会复用形态都必须有正面 / 45° / 侧面 / 背面、半身或全身服装参考、脸部特写或同源表情参考、三视图人审拼版；只有完整表情库、动作参考、主体库/LoRA 仍按触发条件升档。逐角色分档方法见 `references/lora_consistency.md「出图前角色 DNA 定档框架」`。
...
- **风格归属佐证（风格层 · 选定风格 vs 实际渲染机检）**：六字段是**存在性**+**负面词**约束，不能证明出图真是那个风格——选了「国漫写实」却整集漂成「照片剧照感/插画设定图感/欧美脸」时，色调可自洽、六字段可写全，但风格归属错了。补法：定妆阶段必须先为选定风格出 **1–2 张风格锚图**（style anchor，体现该风格该有的样子，默认存 `出图/共享/图片/风格锚_<风格名>.png`），登记进 `style_contract.style_anchor`（路径数组）并写入 `出图/共享/style_anchor_registry.json`。出图后 `image_qc` 的 `style_attribution`（纯 Pillow·默认环境可跑）以风格锚为基准，量本集帧的**风格指纹（饱和度/明暗对比/线条边缘度）**整集中位数对照——明显偏离锚=**warn·人判**（提示是否踩风格禁忌、回 n2d-image 重出偏离镜或重锚），归入 `style_consistency` 维度。**未登记锚 / 登记锚图缺失 / registry 未 ready 一律 production BLOCK**：runner 付费生成 Clip 前卡住，`image_qc` 和 image gate 也硬拦，不再降级成人审 warn；装了 VLM 后端可在上层升级成语义判定。阈值 env 可调（`N2D_STYLE_SAT_SCALE/CONTRAST_SCALE/EDGE_SCALE/DEVIATION`），应在真实渲染集按风格/后端标定。
...
- **角色定妆基础包铁律（生产拆图 + 人审拼版）**：凡是人物定妆（主角、长线角色、配角、路人但会入镜的人物）都必须按**标准正 / 45° / 侧 / 背**建参考，不再按题材、重要程度或近景占比省 45° / 背面 / 脸部特写。基础包至少七类：① 正面主参考（=`出图/共享/图片/定妆_<角色或角色_年龄档_形态>.png`，中性光、锁脸）② 45°/三分之二侧脸参考（`图片/定妆_<角色或角色_年龄档_形态>_45度.png`，锁转头、过肩、反打最高频角度）③ 侧面参考（`图片/定妆_<角色或角色_年龄档_形态>_侧.png`，锁鼻梁/下颌/发型侧轮廓）④ 背面参考（`图片/定妆_<角色或角色_年龄档_形态>_背.png`，锁后脑勺发型/背部服饰/披风/腰带绑法）⑤ 半身或全身服装参考（`图片/定妆_<角色或角色_年龄档_形态>_半身.png` 或 `_全身.png`，锁衣领/袖型/腰带/配色/体态）⑥ 基础脸锚（`图片/定妆_<角色或角色_年龄档_形态>_脸部特写.png` 或至少一张 `图片/表情_<角色或角色_年龄档_形态>_<情绪>.png`，同源裁切/派生，不重新抽新脸）⑦ **人审拼版** `图片/定妆_<角色或角色_年龄档_形态>_三视图.png`。三视图由已通过自检的正/侧/背三张拼成，**不作为分镜主参考，也不能替代正/45°/侧/背/半身/脸锚任一拆图**。完整 `_表情.png`、动作参考、主体库/LoRA 按角色长期性和镜头风险升档，不全员前置。形态变体（觉醒态、幼年态、18岁少年态、成年态、化神态…）按同一基础包各出；先用上一阶段主参考派生，避免变成另一个人。**命名约定：多张可喂生图模型的拆分 PNG 叫“定妆组”；单张合成总览才叫“三视图/设定表”；年龄敏感 form 的每张定妆图都要带年龄/年龄档。**详见 `references/角色一致性checklist.md §二` 与 `references/prompt_format.md §1.2`。
...
- **后端支持就强制注册核心长线角色（不靠"想起来"）**：当 `生图AI/生图渠道` 解析到的后端**支持原生主体**（Seedream/可灵等，`identity_registry` 该后端 `default_status=unregistered`）而**核心长线角色仍未注册**时，付费出图前必须先注册主体——这是省钱前置，不是事后补救。`gate.py --stage image_preflight` 已机检兜底：选了支持原生主体的后端 + 核心长线角色（registry `scope` 含 全篇/全程/长线/主角/主反派…）在该后端 `status` 非 `registered/ready` → 出 `原生主体注册` **BLOCK**。**Codex/OpenAI/Dreamina 等无持久主体的后端不触发"注册主体"硬闸**——它们本就没有可注册的主体层，自动回退第①档参考图派生。短线配角/单元妖按「ROI 默认最小化」不前置注册，机检也不 nudge。**但长线 production 例外**：无持久主体 ID 后端跑**长篇/多集（≥3 集）**时，`image_preflight` 会另出 `生图AI一致性`（兼容维度名）**BLOCK**（`check_long_running_weak_backend`，函数名兼容旧测试）：核心/常驻角色必须有 native subject / Face Lock / face_embedding / 可用于当前后端的 LoRA / 已登记 `image2image_reference_chain` 之一；demo/探索仅 WARN，进入批量/production 前必须补锁。若 registry 未标核心/常驻角色也按同档处理，先补 scope/tier 再判断谁必须升档。
