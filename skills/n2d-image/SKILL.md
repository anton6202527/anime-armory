...
- **出图前角色 DNA 定档铁律（P0 · 每个角色做到哪一档，先说清再出图）**：付费出任何角色定妆前，必须先产出/刷新一张**「角色 DNA 一致性定档表」**——逐角色/形态判定**该升到哪一档**，**输出给用户** + 落进 `出图/共享/prompt/00_索引.md`（人读策略真值；机器侧当前档仍以 `identity_registry.json` 的 `scope`/`lora.status`/各后端 `status` 为真值）。一致性阶梯三档 + 一附加：**档① 参考图派生+锚点句**（默认·所有入镜角色）→ **档② 后端原生主体ID/主体库**（Seedream Universal Reference / 可灵主体库 / Sora Cameo / Kling Character ID / Seedance Face Lock / Veo；参考图压不住时·无 GPU）→ **档③ LoRA**（贯穿几十集核心角 + 前两档压不住，走 `n2d-lora`）；**附加·完整表情库/动作参考/45°增强**（近景、反打、转头、动作戏多的角色按风险 opt-in）。定档表列：`角色/形态 ｜ 体量 ｜ 当前档 ｜ 目标档 ｜ 表情库 ｜ LoRA ｜ 升档触发条件`。**ROI 驱动、默认最小化**：基础定妆包不按主配角放松，所有人物都要有正/侧/背、半身或全身服装参考、脸部特写或同源表情参考；但**不在第1集给全员前置 45°、完整表情库、动作参考、主体库/LoRA**（漫剧角色多在前几集退场，前置=烧在站不住的角色上）。升档只在「触发条件」命中时发生。逐角色分档方法见 `references/lora_consistency.md「出图前角色 DNA 定档框架」`。
...
- **角色定妆基础包铁律（生产拆图 + 人审拼版）**：凡是人物定妆（主角、长线角色、配角、路人但会入镜的人物）都必须按**标准正 / 侧 / 背三视图**建参考，不再按题材或重要程度省背面。基础包至少五类：① 正面主参考（=`出图/共享/图片/定妆_<角色>.png`，中性光、锁脸）② 侧面参考（`图片/定妆_<角色>_侧.png`，锁鼻梁/下颌/发型侧轮廓）③ 背面参考（`图片/定妆_<角色>_背.png`，锁后脑勺发型/背部服饰/披风/腰带绑法）④ 半身或全身服装参考（`图片/定妆_<角色>_半身.png` 或 `图片/定妆_<角色>_全身.png`，锁衣领/袖型/腰带/配色/体态）⑤ 基础脸锚（`图片/定妆_<角色>_脸部特写.png` 或至少一张 `图片/表情_<角色>_<情绪>.png`，同源裁切/派生，不重新抽新脸）。另存**人审拼版** `图片/定妆_<角色>_三视图.png`，由已通过自检的正/侧/背三张拼成，不作为分镜主参考。**45°/三分之二侧脸是建议增强**：转头、过肩、反打高频角色优先补；完整 `_表情.png`、动作参考、主体库/LoRA 按角色长期性和镜头风险升档，不全员前置。形态变体（觉醒态…）按同一基础包各出；先用常态主参考派生，避免变成另一个人。**命名约定：多张可喂图 AI 的拆分 PNG 叫“定妆组”；单张合成总览才叫“三视图/设定表”。**详见 `references/角色一致性checklist.md §二` 与 `references/prompt_format.md §1.2`。
...
| 人物只出单张正脸 / 半身定妆 | 必须补基础包：正面主参考 / 侧面 / 背面 + 半身或全身服装参考 + 脸部特写或同源表情参考 + 三视图人审拼版；人物定妆不再按题材或重要程度省背面 |
