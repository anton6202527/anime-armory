---
name: n2d-image
description: Stage 4 of n2d pipeline — for a 作品 episode whose mode-aware 分镜设计 is done (`分镜设计` 列 ✅: 原生音画 uses storyboard-driven 镜头时长, 配音先行 requires real 配音, 先出视频后配音 may use explicit rough timing), generate the two-layer 出图 prompt pack (shared 定妆库 + 本集分镜), then generate images. The generation axis is a CONCRETE MODEL (design-constitution C5): choice point 生图模型 defaults to GPT Image 2 / OpenAI GPT Image family (exact model id verified per run); 生图AI is the access channel (default Codex CLI). Other models (Seedream, Kling 主体库, Nano Banana Pro, Dreamina/即梦官方 CLI) via their official channels, Sora Cameo legacy/manual only — the gate only blocks MODEL MIXING within a project and unauthorized/third-party reverse paths. Writes progress to `_进度.md` (出图prompt + 出图 columns). Use when asked to 出图, 出图prompt, 生成定妆, 生成分镜图, GPT Image/GPT Image 2 生图, Codex 生图, Dreamina 生图, 即梦生图, or anything image-generation-related for a n2d project. Triggers 出图, 出图prompt, 定妆, 分镜出图, 生图模型, GPT Image, GPT Image 2, Codex, codex, Dreamina, 即梦, image2image, 生图.
---

# n2d-image — Stage 4：出图 prompt + 生图

你是 **AI 漫剧出图制作**。本 skill 关心一件事：把**模式感知分镜设计已定稿**的一集（`分镜设计` ✅；`原生音画` 由 storyboard 推出 `镜头时长.json`、不要求先跑配音；`配音先行` 才要求真实配音；`先出视频后配音` 只在用户显式选择 rough timing 后放行），先生成"开箱即用"的两层出图 prompt 文件夹，然后**用所选生图模型**生成 PNG，落档 + 更新进度。**生成轴=具体模型（设计宪法 C5）**：选择点 `生图模型` 默认 **GPT Image 2 / OpenAI GPT Image 系列**（n2d 归一名；执行前以 OpenAI 官方 GPT Image/ChatGPT Images 2.0 口径和本次 model id 证据为准）——「由什么生成这张图」必须指认到**具体模型**，不写 `Codex`/`某后端`/`渠道商` 这类壳（`codex` 只是访问入口）。GPT Image 系列是 2026 第一梯队的高保真多参考/编辑模型，支持 4K 级尺寸和强上下文一致性，**但当前官方文档未证实可注册的服务端持久主体 ID/handle**；它不能按原生主体库使用。访问入口是 `生图AI`(=`生图渠道`)，默认 **Codex CLI**；其它可选模型（Seedream 5.0/4.5 / 可灵主体库模型 / Nano Banana 2/Pro 等）经各自官方/已登录渠道，Sora Character Cameo 仅旧项目/人工路径。出图阶段唯二硬闸门：① **同一项目/同一集不得混用多个生图模型**（混用才是跨镜漂移真凶）；② **禁止第三方逆向/未授权出图路径**（含糊的 `同视频AI` / `某后端`、非官方 CLI、web 自动化出图仍禁）。这条统一规则只约束**生图作用域**：它不禁止 `n2d-model-router` 在出视频阶段按镜头能力逐 Clip 选择视频 primary；视频侧必须用 `backend_consistency_scope=per_clip_allowed_with_baseline`、基线、身份交接、执行配方和成片 QC 来管住一致性。若所选模型/渠道无法落 PNG，就停下报告，不要偷偷换兜底。
...
- **用户人物参考图脸锚铁律（只取身份，不继承整图）**：用户给的主角图/人物参考图默认只作为**脸部身份锚**，只提取脸型、五官比例、眼神气质、肤质、体态/年龄感等身份信息；**不得继承**参考图里的服装、裸露程度、发型/发饰、表情、姿态、配饰、场景、光色、IP/水印或原图剧情状态。服饰、表情、配饰、武器、法宝、觉醒/妖化/战场/常服等各种形态，必须以**小说原文、角色圣经、identity_registry 的 form/wardrobe_profile、asset_registry 和本镜分镜状态**为准；除非用户明确说“这张图的衣服/发型/配饰也要沿用”，否则外部图只进 `face_anchor_refs`/身份母本，不进服装/形态真值。生成 prompt 和 runner 必须把这条写进正向约束，负向补“不要继承参考图衣装/裸露/姿态/表情/配饰/场景”。
...
- **人物全身照鞋靴铁律（全身必须头到脚）**：凡人物全身、标准立绘、正面/45°/侧面/背面、三视图/turnaround、全身动作参考，必须从头到脚完整入画，鞋靴/脚部清楚可见；不得裁掉脚、不得用衣摆/烟雾/画幅把鞋完全遮住、不得用半身构图冒充全身。只有明确命名为半身、脸部特写、手部/局部参考的目标可豁免。
...
- **风格归属佐证（风格层 · 选定风格 vs 实际渲染机检）**：六字段是**存在性**+**负面词**约束，不能证明出图真是那个风格——选了「国漫写实」却整集漂成「插画设定图感/欧美脸」时，色调可自洽、六字段可写全，但风格归属错了。补法：定妆阶段为选定风格出 **1–2 张风格锚图**（style anchor，体现该风格该有的样子，建议存 `出图/共享/图片/定妆_风格锚_*.png`），登记进 `style_contract.style_anchor`（路径数组）。出图后 `image_qc` 的 `style_attribution`（纯 Pillow·默认环境可跑）以风格锚为基准，量本集帧的**风格指纹（饱和度/明暗对比/线条边缘度）**整集中位数对照——明显偏离锚=**warn·人判**（提示是否踩风格禁忌、回 n2d-image 重出偏离镜或重锚），归入 `style_consistency` 维度。**未登记锚则优雅回退**为「人判清单」warn（列风格名+禁忌逐图核对），不臆造不硬拦；装了 VLM 后端可在上层升级成语义判定。阈值 env 可调（`N2D_STYLE_SAT_SCALE/CONTRAST_SCALE/EDGE_SCALE/DEVIATION`），应在真实渲染集按风格/后端标定。
...
- **首帧=起幅 + 运镜留余量铁律（出图为视频铺路）**：喂 image2video 的 clip **首帧抓起幅、不抓动作顶点**——顶点由尾帧接力承接（`图片/镜头N_end.png`），首帧若就是顶点，视频没有"动到哪去"的余量。且首帧构图要**为故事板里计划的运镜预留框**：推近→框略宽、环绕→主体周围留空、跟摇→运动方向留 lead room。**出图只读故事板时长不够，要读其节奏/张力反推运镜再定构图。**阶段2 若已产 `生产数据/director_camera_plan_第N集.json/md`，逐镜优先读取其中 `image_prompt_injection`，把 `镜头/机位`、`起幅·运动余量`、`构图防呆` 抄进 `01_分镜出图.md`；若缺 sidecar，先回 n2d-script 跑 `director_camera_plan.py --write`，不要临场凭感觉给首帧留框。封面/定格图例外，仍抓顶点。
- **三帧契约（默认产图片资产·缺帧 severity 按后端能力分级·出图前规划中段锚帧）**：**默认每个 Clip 规划 3 帧（首帧 + 中段锚帧(`_mid`) + 尾帧）**，`anchor_planner.py --write` 定稿时默认补齐——中帧始终是默认应产的图片资产。但 `gate.py --stage image_preflight|image` 缺中段锚帧的 **severity 纯按"路由后端能否消费中帧"分级**（charter `ENFORCEMENT_DECISIONS: three_frame_graduated_severity`，2026-06-27 取代旧的"对所有后端无条件 BLOCK"）：后端能消费中帧（原生多帧/首尾拆段接力·`backend_supports_three_plus_frames=True`，如即梦智能多帧/可灵首尾档）→ **BLOCK**（缺中帧=成片退化）；否则（真 first-frame-only 或后端未选/未知，中帧消费不了、仅作 QC/参考）→ **WARN**（中帧仍建议补，但"是否为不可消费的中帧花这笔出图钱"由作者按 cost 决定；WARN≠豁免）。不看 `policy.midframe_default`（那是 anchor_planner 的"已规划"标记·正常流程恒 true）。`_mid` 须以同 Clip 首帧 image2image 派生（锁脸/造型），取表演节拍中间拍，不纯文生图。只有极短镜(<3s)或末镜无尾帧这类镜头语义可写 `midframe_exempt_reason`。**SOTA(2026)**：原生 3 关键帧极罕见（仅即梦智能多帧/Pika），多数后端 first+last，故对真 first-frame-only 后端不为只能当 QC 参考的中帧硬拦出图。
- **检查清单双段硬闸门**：每个待生成的 prompt 块（共享定妆、本集分镜）在提交生图前必须同时具备两段检查；缺任一段禁止生图，先补 prompt。
  ...
  - 执行阶段必须先核对标题存在，再提交 `生图模型 + 生图AI/生图渠道` 所选官方/已登录入口的生图任务；不要边生图边补检查清单。Dreamina/即梦官方 CLI 可用，第三方逆向或 web 自动化仍禁。
...
- **Clip 图前完整资产基础包铁律（总闸门）**：任何 `出图/第N集/图片/Clip_*` / `镜头*` 分镜 PNG 生成前，本集引用到的所有共享资产必须先完成“可传给模型的基础包”，不能只写文字名或只出一张好看图就进 Clip。人物/形态：正面主参考、45°、侧面、背面、半身或全身服装参考、脸部特写或同源表情参考、三视图人审拼版全部 ready，并完成配饰/标志物核对；场景：主图，反复场景再补多机位/多视图；道具/配饰/武器/法宝/VFX：`PROP/WEAPON/OUTFIT/VFX_xx` 均须有参考图、结构/数量/尺度/材质/色卡/禁漂项，核心武器还要 `weapon_profile` 与握持/比例参考。缺任一项时，当前阶段唯一允许的生图任务是“补共享定妆/资产图并过自检”，**不得生成 Clip 分镜图**。**共享库先行顺序不可被 `--skip-preflight`、P0 垂直切片、抽样验证或 `--shots Clip_XX` 局部跑数豁免**：P0 可以干跑、打包 prompt、补共享定妆/资产图，但只要目标包含任一 Clip 分镜 PNG，就必须先过 runner 级 `enforce_shared_first_interlock`；`--skip-preflight` 只能作为普通 dashboard preflight 的带 waiver 逃生口，不是共享库顺序锁的逃生口。
- **资产身份注册层铁律（P0 · 角色 ID / Face Lock / LoRA 适配）**：共享定妆库不再只是一堆 PNG 和 `00_索引.md`；必须同步维护 `出图/共享/identity_registry.json`，把每个角色形态登记成可复用身份：`reference_group`（正 / 45° / 侧 / 背 / 半身或全身 / 脸锚 / 三视图七类基础包；旧项目 `expressions` 可兼容作脸锚）、`reference_atlas`（角色参考图数据库：正脸 / **45° ready** / 侧脸 / 半身或全身 / 背影 + 基础脸锚 + 表情参考 + 动作参考 + 逐镜选图策略）、`identity_adapters`（生图模型原生角色 ID/主体库、视频 Face Lock/reference controls、LoRA 状态）、`angle_policy`（允许角度/高危角度/需补参考）和 `drift_forbidden`（零容忍漂移项）。**`reference_atlas.base_views.three_quarter` 与 `face_anchor_refs` / `reference_atlas.face_anchor_refs` 都是基础包硬闸**：所有人物至少要有一张 45°/三分之二侧脸 ready 图，以及一张同源脸部特写或表情参考（`{label/emotion, path}`，与正面同一张脸，脸型五官比例不变）；**`expressions` / `reference_atlas.expression_refs` 是完整情绪库增强**，近景高频/情绪戏多的角色再补一脸多情绪（建议用 **GPT Image/OpenAI 系列表情表 expression-sheet** 以正面脸锚为母图一次性派生同源多情绪，比逐张重抽更同源——见「近景微表情深化铁律」）。下游 `n2d-video` 近景大表情镜（哭/笑/怒/惊的 CU）必须引用这些参考并做**首尾双帧只插值**（首=起表情、尾=止表情），避免模型自由生成跨情绪表情时把脸重画；缺则该镜阻断或用 MCU/侧脸保真实现后重写 prompt。**`reference_atlas.action_refs` 治动作姿态漂移**：持剑/御剑/施法/奔跑等高频动作只对核心/动作重角色按需补，不给短线角色全员前置烧图。逐镜 prompt 的 `资产身份注册层` 必须写具体 `CHAR_xx/形态`，执行端据此从 registry 自动取定妆组做 image2image / 多图参考派生；禁止只写中文角色名或纯靠文生图重新发明人物。每次补定妆/注册后跑 `python3 skills/n2d-identity/scripts/identity.py <作品根> --write` 生成 `identity_adapter_matrix`，让出图/出视频/审片读取同一份可执行 binding。详见 `references/资产身份注册层.md`。`gate.py --stage image|video|compose|review` 缺 registry、字段不全、逐镜缺 `CHAR_xx/形态`、后端 mode/status 错、LoRA ready 假登记即阻断。
...
- **角色定妆基础包铁律（生产拆图 + 人审拼版）**：凡是人物定妆（主角、长线角色、配角、路人但会入镜的人物）都必须按**标准正 / 45° / 侧 / 背**建参考，不再按题材、重要程度或近景占比省 45° / 背面 / 脸部特写。基础包至少七类：① 正面主参考（=`出图/共享/图片/定妆_<角色>.png`，中性光、锁脸）② 45°/三分之二侧脸参考（`图片/定妆_<角色>_45度.png`，锁转头、过肩、反打最高频角度）③ 侧面参考（`图片/定妆_<角色>_侧.png`，锁鼻梁/下颌/发型侧轮廓）④ 背面参考（`图片/定妆_<角色>_背.png`，锁后脑勺发型/背部服饰/披风/腰带绑法）⑤ 半身或全身服装参考（`图片/定妆_<角色>_半身.png` 或 `图片/定妆_<角色>_全身.png`，锁衣领/袖型/腰带/配色/体态）⑥ 基础脸锚（`图片/定妆_<角色>_脸部特写.png` 或至少一张 `图片/表情_<角色>_<情绪>.png`，同源裁切/派生，不重新抽新脸）⑦ **人审拼版** `图片/定妆_<角色>_三视图.png`。三视图由已通过自检的正/侧/背三张拼成，**不作为分镜主参考，也不能替代正/45°/侧/背/半身/脸锚任一拆图**。完整 `_表情.png`、动作参考、主体库/LoRA 按角色长期性和镜头风险升档，不全员前置。形态变体（觉醒态…）按同一基础包各出；先用常态主参考派生，避免变成另一个人。**命名约定：多张可喂生图模型的拆分 PNG 叫“定妆组”；单张合成总览才叫“三视图/设定表”。**详见 `references/角色一致性checklist.md §二` 与 `references/prompt_format.md §1.2`。
...
- **帧生成顺序铁律（定妆库 → 首帧 → 中帧 → 尾帧 · 一条 image2image 派生链，不并行、不各自文生图）**：一个 Clip 的多帧是**严格有序、有依赖**的派生链，顺序与母图焊死——① **定妆库先于一切**：先建共享定妆/资产库（锁脸/发型/服装/配饰/质感、场景、风格、道具），才出任何分镜帧（两层出图）。② **首帧 firstframe 最先出**，是唯一直接以定妆库 `image2image` 派生的帧，抓**起幅/起表情**、留运镜余量（见「首帧=起幅铁律」）。③ **中帧 `_mid`/`_aK` 次出**，以**同 Clip 首帧为母图** `image2image`，只改姿态取表演节拍中间拍，定位必须「位于首帧与尾帧之间」（落档记账写 `--meta self_check=pass` + `midframe_role=between_first_and_end`，`scripts/codex_image_runner.py:1401-1407`；缺则 `video_preflight` BLOCK，避免"只锁人锁景、动作未成立"的 `_mid` 混进闸门）。④ **尾帧 `_end` 最后出**，以**首帧（可再叠加已出的中帧/锚帧）为母图** `image2image`，构图+光位+人物状态三者对齐下一镜首帧（见「尾帧接力铁律」）。**机器约束**：`scripts/codex_image_runner.py:936-948` 对任何非 firstframe target 强制把同 Clip 首帧注入 `source_frame`（`same_clip_firstframe`）、尾帧再注入已有中帧/锚帧（`same_clip_anchor`）——**首帧不存在则中/尾帧无源图可派生**；`image_qc`/`gate.py --stage image` 对声明了尾帧却缺 image2image 接力声明的角色镜记 hard block（`tail_relay_not_image2image`）。**违反后果**：跳过首帧直接文生中/尾帧 = 模型重抽一张"同类型新演员"的脸，是最常见的脸漂源（与「尾帧图生图接力铁律」同根）。视频后端若只能 first-frame-only，中/尾帧仍先作为图片资产与 QC/参考/改路由依据生成；不得用后端短板跳过中帧。
...
- **生图模型/渠道规则（安全硬闸门 + 后端选择点）**：
  ...
  - **② 模型 + 渠道选择（官方/已登录后端放行 → `n2d/_lib/n2d_contract.py` `APPROVED_IMAGE_BACKENDS`）**：`生图模型` 是生成轴，默认 GPT Image 2 / OpenAI GPT Image 系列；`生图AI` 是旧字段名，实际表示 `生图渠道`/访问入口，默认 Codex CLI。**放行官方/已登录多参考一致性模型/渠道**：GPT Image/OpenAI 系列 via Codex/OpenAI、Dreamina/即梦官方 CLI 对应官方图像模型、Seedream Universal Reference / 可灵 Kling 主体库 / Nano Banana(Gemini)。Sora Cameo 只作为旧项目/人工路径兼容，不作新项目默认推荐。`gate.py --stage image_preflight` **不再因"非 Codex"阻断**，只阻断 ① 项目内**模型/渠道混用** ② **第三方逆向/未授权出图路径**。旧项目 `_设置.md` 写 `同视频AI` 时必须改成显式生图渠道，并补齐对应 `生图模型`。**默认起点按剧长分档（知情权衡）**：**长篇/多集（≥3 集）/ 核心常驻角色 / 多人同框吃重 → 推荐默认选支持原生主体的模型/渠道**（Seedream / 可灵，注册一次按 ID 跨镜跨集引用，2026 工业默认）；**单集/短 demo/配角 → GPT Image/OpenAI 系列 via Codex/OpenAI 即可**。`image_preflight` 对「长线×无持久主体后端」严重级别按落地真值执行：第 3 集起若核心/常驻角色缺 native subject / Face Lock / face_embedding / LoRA，或 registry 未标核心/常驻角色，`生图AI一致性`（兼容维度名）**BLOCK**；短 demo/配角/未到长线阈值才保持 WARN/建议。梯子与理由见 `n2d/references/模型矩阵.md`「一、图」。
...
- **后端支持就强制注册核心长线角色（不靠"想起来"）**：当 `生图AI/生图渠道` 解析到的后端**支持原生主体**（Seedream/可灵等，`identity_registry` 该后端 `default_status=unregistered`）而**核心长线角色仍未注册**时，付费出图前必须先注册主体——这是省钱前置，不是事后补救。`gate.py --stage image_preflight` 已机检兜底：选了支持原生主体的后端 + 核心长线角色（registry `scope` 含 全篇/全程/长线/主角/主反派…）在该后端 `status` 非 `registered/ready` → 出 `原生主体注册` **BLOCK**。**Codex/OpenAI/Dreamina 等无持久主体的后端不触发"注册主体"硬闸**——它们本就没有可注册的主体层，自动回退第①档参考图派生。短线配角/单元妖按「ROI 默认最小化」不前置注册，机检也不 nudge。**但长线 production 例外**：无持久主体 ID 后端跑**长篇/多集（≥3 集）**时，`image_preflight` 会另出 `生图AI一致性`（兼容维度名）**BLOCK**（`check_long_running_weak_backend`，函数名兼容旧测试）：核心/常驻角色必须有 native subject / Face Lock / face_embedding / LoRA 之一；demo/探索仅 WARN，进入批量/production 前必须补锁。若 registry 未标核心/常驻角色也按同档处理，先补 scope/tier 再判断谁必须升档。
...
按 `references/cli_registry.md` 的优先级只在**用户选定的 `生图AI/生图渠道`** 内部选可自动产 PNG 并能落到作品目录的入口。Codex 会话内置 `image_gen` 可用时优先；`codex` CLI 仅作为能力探测/插件管理入口，当前 help 没有独立 `images generate` 子命令时，不要把它当成直接生图 CLI。若 `生图AI=Dreamina/即梦`，扫描并调用官方 `dreamina` CLI 的图片能力，同时在记录里写清实际 `生图模型`。
...
**分支 1：所选官方后端能落 PNG**
- 按 `_设置.md` 的 `生图模型 + 生图AI/生图渠道`（默认 OpenAI GPT Image 系列 via Codex；可选 Dreamina/即梦官方 CLI、Seedream/可灵主体库/Nano Banana 等官方/已登录组合；Sora Cameo 仅旧项目/人工路径）选定**单一模型+渠道**。扫描到 `同视频AI` / `同视频模型`、第三方逆向或 web 自动化出图口径必须忽略并改回显式官方后端；**全集统一一组模型+渠道，不混用**。
