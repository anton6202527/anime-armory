...
- **关键帧按用途出图，不设普通镜默认三帧**：出图前先读 `continuity.frame_strategy/anchors[].use`。普通单拍只出首帧，`need_endframe=true` 再出尾帧；R1/R2/R3 高风险连续动作或用户显式 opt-in 才出 `_mid/_aK`；E1 多镜位在每个 `lens/camera/shot_size` 切点出 `use=edit_cut` 边界图。`use=qc/reference` 只作验收，不承诺被视频后端消费。所有执行锚仍须从同 Clip/同 reference_group 母本 image2image 派生并登记时间、用途、来源；声明缺字段/缺 PNG 继续 BLOCK。这样避免为低风险短镜无条件多烧一张图，也避免把“后端能拆段”误报成原生三帧能力。
