...
- **正反打 prompt 消费铁律**：`dialogue_shot_reverse` 镜头必须优先消费 `脚本/第N集/shot_reverse_contract.json`，并兼容继承 P-2 `axis_blocking_map.json#shot_reverse_patterns` 和 P-3 `continuity_bible.json#shot_reverse_continuity`：180° 行动轴线、A/B 屏幕左右或 9:16 纵深高低位、互补视线、OTS/clean single/insert coverage、镜头高度/距离匹配、越轴策略和缓冲镜。逐镜 prompt 不能只写“反打/对话近景”；必须写清“谁在屏幕左/右或前/后、看向哪边、谁的肩/侧背在前景、谁是焦点、镜头距离和情绪功能”。`consumed_contracts_image_prompt_第N集.json` 会记录 `shot_reverse_contract` 的 path/sha256。
