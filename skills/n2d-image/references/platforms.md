...
- **图 AI**：出图工具,可以是视频 AI 本身、也可独立（Codex 内置生图 / OpenAI / Gemini / Flux / ComfyUI 等）。**默认 `生图AI: Codex only`：只用 Codex/官方 OpenAI 生图入口，不可用就停下报告，绝不自动落 Dreamina/即梦**；仅当 `_设置.md` 显式 `生图AI: 即梦` 才用即梦。每次出图前按 `cli_registry.md` 重新扫描本机能力。
...
| 场景 | 组合 | 备注 |
|---|---|---|
| 国风短剧（默认 Codex only） | Codex/官方 OpenAI 生图 + 即梦锚定句 → 即梦视频 | 默认：只用 Codex 生图，不可用就停下报告；**不自动落 Dreamina** |
| 国风短剧（同家闭环·需显式选） | 即梦图 → 即梦视频 | **仅当 `_设置.md` 显式 `生图AI: 即梦`**；无需锚定句 |
...
## 档案：即梦 AI（图阶段=仅 `_设置.md` 显式 `生图AI: 即梦` 时用；视频阶段=默认）
