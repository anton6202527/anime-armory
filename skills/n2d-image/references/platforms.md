...
- **平台皮 = 可换（后期换不动内容）**：提示词写法·语言·画幅·clip时长·负面词,以及结尾那句"图像风格锚定句/视频兼容锚定句"——由图 AI / 生视频模型决定；具体生视频后端未固定时先用通用兼容锚定。
- **生视频模型只主导"皮"**：它定最终成片风格基线 + image2video 运动估计分布;但**只约束图的风格、绝不决定画什么**。`生视频渠道` 只决定实际调用哪个产品/API。
- **图 AI**：出图工具。`生图AI` 是**选择点**，默认 Codex；**当前放行官方/已登录多参考后端**（Dreamina/即梦官方 CLI、Seedream / 可灵主体库 / Nano Banana / Sora Cameo）。两条硬规则：① **全集统一一个官方后端，不混用**（混用=跨镜漂移）；② **禁止第三方逆向 CLI、`同视频AI` / `同视频模型` 含糊口径和 web 自动化出图**。每次出图前按 `cli_registry.md` 重新扫描所选后端能力。
...
**生视频后端选择后移**：图片由所选官方/已登录图后端（默认 Codex/OpenAI，或 Dreamina/Seedream/可灵主体库/Nano Banana/Sora Cameo）生成。若用户已固定生视频模型，按下方规则拼该模型锚定句；若未固定，不询问生视频后端，先用通用视频兼容锚定并记录 `video_backend_decision=deferred`，到 n2d-video 再由 router/probe 选择能消化现有首帧的后端。
...
5. 未固定生视频模型时，使用通用视频兼容锚定；n2d-video 路由必须优先选择与现有首帧风格兼容的后端。用户临时固定不兼容后端时，停下提示重出图/重拼锚定或改路由，不得静默硬切。
