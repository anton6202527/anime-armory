...
- **图 AI**：出图工具。`生图AI` 是**选择点**，默认 Codex；**当前放行官方/已登录多参考后端**（Dreamina/即梦官方 CLI、Seedream / 可灵主体库 / Nano Banana / Sora Cameo）。两条硬规则：① **全集统一一个官方后端，不混用**（混用=跨镜漂移）；② **禁止第三方逆向 CLI、`同视频AI` 含糊口径和 web 自动化出图**。每次出图前按 `cli_registry.md` 重新扫描所选后端能力。
