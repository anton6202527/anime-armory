...
- **尾帧接力（仅同段落连续硬切·按需）**：MV 默认卡点硬切，接点靠"视觉身份一致 + 卡点准"，**不强求 n2d 那套首尾帧接力**。但凡 mv-video clip 表标 `需要尾帧?=是` 的接缝（同段落·非卡点切·人物姿态连续，如副歌内一段连续动作分两 clip），**除首帧外再出一张尾帧 PNG `出图/段落/<段落>_<描述>_end.png`，其构图/姿态 = 下一 clip 首帧**——用上一镜 end_state 派生、喂同一套定妆组锁人，供 mv-video 首尾双帧引导焊接点。换段/卡点切不出尾帧（省 credit）。尾帧也按导演视角八维出，只是构图对齐下一首帧。
...
3. 按段落出分镜 → `出图/段落/<段落>_<描述>.png`，每张拼锚点句。**接力补尾帧**：clip 表标 `需要尾帧?=是` 的接缝，额外出 `<段落>_<描述>_end.png`（=下一 clip 首帧构图）。
...
| clip 表标了 `需要尾帧` 却只出首帧 | 同段落连续硬切接缝补尾帧 PNG `<段落>_<描述>_end.png`（=下一首帧构图），供 mv-video 首尾双帧锁接点 |
