...
NOVEL_LIB = os.path.join(REPO_SKILLS, "_lib")
...
def build_execution_steps(root: str, plan: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not plan.get("rebuild_needed"):
        steps.append({
            "type": "command",
            "purpose": "查看当前前沿",
            "command": f'python3 skills/novel/novel-progress/scan.py "{root}"',
        })
        if plan.get("changed_skills"):
            steps.append({
                "type": "command",
                "purpose": "接受当前产物后记录新基线",
                "command": f'python3 skills/novel/novel-update/scripts/update_plan.py record "{root}"',
                "run_when": "用户确认无需返工",
            })
        return steps
    ...
    steps.append({
        "type": "command",
        "purpose": "刷新生产控制台",
        "command": f'python3 skills/novel/novel-dashboard/scripts/dashboard.py "{root}" --write',
        "run_when": "返工后",
    })
    steps.append({
        "type": "command",
...
def build_plan(root: str, *, bootstrap: bool = False) -> dict[str, Any]:
    ...
    if needs_record:
        plan["notes"].append("当前项目没有内容基线；先 record，或显式用 check --bootstrap 建立临时基线后，才能检测后续 skill 变化。")
    if bootstrapped:
...
def command_check(args: argparse.Namespace) -> int:
    ...
    if not os.path.isdir(root):
        print(f"[err] 找不到作品根：{root}", file=sys.stderr)
        return 2
    plan = build_plan(root, bootstrap=args.bootstrap)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    check.add_argument("--bootstrap", action="store_true", help="无基线时显式建立临时基线（会写入生产数据）")
