...
def build_evidence_tasks(result):
    ...
    for idx, warning in enumerate(result.get("coverage_warnings") or [], 1):
        tasks.append({
            ...
            "suggested_commands": [
                "python3 skills/novel/novel-score/scripts/collect_market_baseline.py \"<作品根>/评分\" --target-platform \"<目标平台>\" --allow-fetch-errors --manual-evidence \"红果短剧|YYYY-MM-DD|第三方榜单|结论|URL\"",
                "python3 skills/novel/novel-research/scripts/research_pack.py scaffold \"<作品根>\" --topic \"平台市场\" --domain platform --risk high --keyword \"红果\" --keyword \"抖音\"",
            ],
        })
    return tasks
