...
按 `../skills/novel/novel-craft/references/选择点与偏好.md` 读：先 `<作品根>/_设置.md`，再全局默认；仍缺失的普通、可逆项采用本线推荐值写回并继续，权利/发布等高风险边界才询问。
...
### 0. 定位 + 定档
- 找作品根/文本。读 `_设置.md` 的 `目标平台` → 选权重档；缺失时按本线证据优势采用推荐值并写回，只有显式 `逐阶段用户确认` 或无法安全推断的核心方向才询问。
