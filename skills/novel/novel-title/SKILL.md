---
name: novel-title
description: Brainstorm 5–8 book title candidates for a novel project, ranked by 5-dim scoring (hook / platform-fit / character identity / anti-collision / memorability). Supports major Chinese web/comic-drama platforms (起点 / 番茄 / 七猫 / 晋江 / 抖音漫剧 / 红果 / 历史向 / 跨平台). Use when asked to 起书名, 测书名, 取书名, 想几个书名, 给个好名字, brainstorm book titles. Can be invoked standalone or by other novel-* skills (novel-spinoff Step 3, novel-expand, novel-condense; novel-score 的「书名体检」判 needs_rename 时也会经 next_actions 路由到这里重出候选). Triggers 起书名, 测书名, 取书名, 改书名, 书名候选, 取个名字.
---

# novel-title — 书名候选 + 平台对位评分

输入一个小说项目的**核心设定**（角色 / 类型 / 视角 / 目标平台），输出 5–8 个候选书名 + 5 维评分排名 + 推荐 + 备选 + 一句话定位。

可独立调用，也可被其他 skill 在内部某一步调起。

## 偏好（私有 · 用户选择，不写死在本 skill）

本 skill 的可选项**不写死在源码里**。按 `../skills/novel/novel-craft/references/选择点与偏好.md` 读用户私有选择：项目值优先，其次全局默认；仍缺失的普通、可逆项采用本线推荐值写回并继续。权利/合规、不可逆发布和最终验收仍确认；模型费用绑定阶段预算包，包内不逐章重复问。

本 skill 涉及的选择点：`目标平台`。

## 输入

至少需要：
- **目标平台**：起点 / 番茄 / 七猫 / 晋江 / 抖音漫剧 / 红果 / 历史向 / 跨平台
- **主角 / 核心人物名**（如有）
- **类型 / 钩子**（修真 / 都市 / 历史 / 同人外传 / 等）
- **可选**：用户已有的**暂定名**（作为候选 #0 一并打分）

## 工作流

1. **核对输入**：先从项目和设置提取；普通缺项采用推荐值。只有主角/核心钩子等无法推断且会实质改变命名方向时才问一个最小问题。
2. **生成 5–8 个候选**：按 `references/title-patterns.md` 的配比（主推平台 2-3 + 跨平台稳健 1-2 + 高钩子尝试 1-2 + 保守雅名 0-1 + 用户暂定 0-1）。若 `<repo>/生产战绩/差异化候选.json` 存在，本书题材命中其中白空间组合时，让"高钩子尝试"名向该差异化卖点靠（如倒叙/马甲/反套路角度），把选题闭环的差异化优势带进书名。
3. **5 维评分**：钩子 / 平台契合 / 角色识别 / 抗撞名 / 可记忆性，各 1-5 分，满分 25。
4. **撞名检查（硬要求）**：前 4 名候选必须联网/榜单查重；把 web 搜索结果或榜单页面交给 `scripts/check_collision.py`，落 `设定/书名撞名检查_<YYYY-MM-DD>.json|md`。硬撞名直接淘汰，软撞名降分并说明。若浏览器/搜索引擎已查但无明确命中，用 `--checked "候选|查询说明|URL|平台"` 留下无命中搜索记录；若搜到疑似撞名，用 `--hit "候选|命中标题|URL|平台"`。**`status=unchecked`（没给任何 `--source`/`--hit`/`--checked`、或来源全抓取失败）= 没查过，≠ 不撞名**——绝不能当 `clear` 推荐，必须补来源/命中重查。
5. **排表 + 推荐**：按总分降序；给一句话定位；标"我推" + "备选 A / B" + 弃用说明。
6. **采用推荐**：自动采用通过撞名检查且总分最高的候选并持久化；保留前 4 名和评分供用户随时覆盖。同分仍无法按平台契合/钩子破平、或改名会覆盖已发布身份时才停。
7. **回写**：
   - 如果项目目录已存在（被其他 skill 调起时），把书名写进项目 `_meta.json.title` 并在 `设定/书名候选.md` 留底。
   - 否则只在主对话报告。

## 输出

- `设定/书名候选.md`（如果有项目目录）—— 评分表 + 推荐 + 弃用说明
- `设定/书名撞名检查_<YYYY-MM-DD>.json|md`（如果有项目目录）—— 候选撞名证据
- 主对话报告：表格 + 已采用推荐 + 前 4 名备选（用户可随时覆盖）

## 评分规则、平台命名习惯、配比要求

详见 `references/title-patterns.md`。

## 何时不用本 skill

- 只想"顺带看看现有书名行不行"（不要新候选）→ `novel-score` 评分时的「书名体检」附加项已覆盖；它判 needs_rename 后才路由回本 skill
- 用户已经定死了书名 → 不需要候选
- 项目还在第 0 / 1 步（连人物设定都没建）→ 太早；先把人物 / 设定卡建出来再回来取名

## 常见错误

| 错误 | 纠正 |
|---|---|
| 忽略目标平台就拍 5 个候选 | 先读项目/全局设置；缺失时用通用推荐档并落档，只有明确平台交付冲突才问 |
| 5 个候选都同一类钩子 | 要覆盖 5 种钩子方向，对照评分才有意义 |
| 给的候选撞名严重 | 前 4 名必须跑 `scripts/check_collision.py`；硬撞名直接淘汰，不进推荐 |
| 被 spinoff 调起却没回写 _meta.json | 一定要更新 _meta.json.title，否则 export 时会回退到默认名 |
