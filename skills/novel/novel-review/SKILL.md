...
- **机检（确定性，先跑）**：一键串跑用 `scripts/consistency_audit.py <作品根> [--pov 角色名] [--anchor 设定/风格指纹.json]`，它把家族里全部确定性检测器（基础三件 + 2026 新增子检测器，共 14+ 个，见下）一次跑完并汇总到 `审稿/consistency_audit.json`：
  - `scripts/mechanical_check.py` —— 格式/字数带宽（默认读 `_meta.target_wordcount_min_max` / scale / target_words_per_chapter，可用 `--min/--max` 覆盖）/章号与章纲对齐/视角"我"密度提示/称谓·术语漂移/**原文照搬（n-gram vs 原作.txt）**/**模板化行文启发式**（兼容维度名 `AI腔`；议论文连接词、万能套话、句长单调等只到 advisory；不得据此判断作者身份、平台审核结论或真实读者反应，`--no-ai-tell` 关闭）/**跨章重复率·机械文风**（相邻章字级 shingle Jaccard、机械开篇、跨章整句复用、句首词/短句式模板、压缩比与 distinct-n 都是内部风格代理，advisory·绝不 🔴；系统面板等刻意模板会命中，交人判）。术语**权威源优先**：人确认的 `设定/角色别名.json`（status=confirmed，由 `novel-wiki/alias_scaffold.py` 生成候选+人确认）里的规范名/别名直接采信（绕过正则启发式，含中点/单字/带「的之」的合法专名也不漏），其余从 `设定/设定圣经.md`、`角色卡.md`、`世界观.md`、`锚点表.json` 正则抽取补充，也可用 `--terms` 追加（`--no-auto-terms` 关闭确认表+正则两路自动抽取）。
  ...
  - **微短剧/漫剧源书平台合规预检**（小说侧）：当 `_设置.md` / `_meta.json` 命中 `微短剧/短剧/漫剧/红果/抖音`，先跑
    `python3 skills/novel/novel-review/scripts/platform_compliance.py <作品根>`。
    它做标题/正文关键词候选定位与上线前许可证/备案待办提示。所有关键词命中均标 `confidence=heuristic`、只到 warning，必须人工看叙事上下文；不能因出现血腥、性侵、未成年人或政治词语就自动判违规。**经典 IP 复核**：`rights_status=public-domain` 或 `_meta.classic_ip_adaptation=true` 的源书在漫剧/微短剧目标下会 warn `classic_ip_alteration_review`；相关 2026-04 条目当前只有二手行业来源，发布前必须回查主管部门原文，不得冒充硬规则。产物 `审稿/platform_compliance.json` 不替代平台审核、法律意见或成片报审。
...
1. **跑机检脚本** → 确定性问题清单；同时落盘机器结果。一次跑全套：
   `python3 skills/novel/novel-review/scripts/consistency_audit.py <作品根> [--pov 角色名] [--anchor 设定/风格指纹.json]`（内部串跑 mechanical_check + reader_contract_sentry + logic_sentry + style-drift + hook/voice/tone/thread/antagonist/timeline/minor/arc_gate/foreshadow/power_system 等全部子检测器，汇总 `审稿/consistency_audit.json`）。
   只想跑基础机检也可单独：`python3 skills/novel/novel-review/scripts/mechanical_check.py <作品根> ... --json-out 审稿/mechanical_findings.json`。
   **AI 腔账单闭环（2026-07）**：`审稿/mechanical_findings.json` 落盘后，`draft_packets` 会自动把其中 AI 腔/机械文风惯犯（排比三连/破折号密度/句首词高频/套话/句式模板…）按问题类型归组注入**下一章写章包**的「本书 AI 腔账单」段——跨项目实证这类问题降 polish 后基本没人修（金睛缉妖录 33 条 polish 原样发布），根因是检测在审稿轮而习惯在写作轮，回灌初稿期才能止损。所以**每轮 review 都要 `--json-out` 落盘机检文件**，否则账单断供。
   ...
   `python3 skills/novel/novel-review/scripts/reader_contract_sentry.py <作品根> --chapter 第NN章`。
   每 3-5 章或自然 arc 写完后跑：
...
3. **汇总报告** → 先用汇总器把机检 + 一致性审计 + 人判 JSON 转成调度器可消费的报告：
   `python3 skills/novel/novel-review/scripts/build_review_report.py <作品根> [--human-assessment 审稿/human_findings.json]`。
...
开放写作与开放审稿尽量使用不同的模型家族或独立人工编辑，并让判官只读匿名正文、量规、读者契约和可引用证据。成对比稿采用顺序交换；关键结论记录模型家族、prompt hash、source snapshot 与分歧，不把同一模型换 persona 当成真正独立判官。

只有一个模型时，仍可用独立会话、隐藏生成过程、固定量规和反证检查降低自评偏差，但报告必须标 `low_confidence`。temperature 差异本身不证明判官独立。任何模型结论都先汇总为候选 finding，由人类或可复核证据决定是否接受；机检继续由确定性脚本完成。
...
1. **先跑本地静态治理检查**：`python3 skills/novel/novel-review/scripts/self_audit.py [--project-root "<作品根>"]`。它不联网、不改文件，检查 registry/README/author 路由同步、`_进度.md` 写入口、`state_ledger` 原子写、批量写章队列、市场基准新鲜度，并**摄入生产线埋点**——`<作品根>/生产数据/优化信号.jsonl`（`novel/_lib/friction_log.py`，由 `novel-score` 等阶段在遇到豁免/覆盖缺口/低判定等摩擦时写入）里所有 open 信号翻成 `FRICTION-*` finding，让"生产时遇到的摩擦"自动进自审差距清单，无需用户重述就能持续触发自我优化。处理掉一条后用 `friction_log.resolve_signals` 标 resolved 即不再复现。
