...
GENRE_KEYWORDS: Tuple[Tuple[str, Tuple[str, ...]], ...] = (
    ...
    # 2026-07 扩容：以下题材同样有等级/数值成长，此前不在 POWER_GENRES 白名单里 →
    # starter registry 不被 seed → 整个战力/等级一致性机检静默跳过（覆盖盲区）。
    ("游戏", ("副本", "公会", "NPC", "玩家", "氪金", "装备栏", "血条", "蓝条", "技能冷却",
            "游戏世界", "全服", "工会战", "爆率")),
    ("无限流", ("轮回空间", "主神", "主神空间", "轮回者", "副本世界", "恐怖片世界", "生存点",
             "强化肉身", "兑换强化", "轮回小队")),
    ("末世", ("丧尸", "末世", "废土", "变异", "进化者", "异能等级", "基地", "囤积物资",
            "尸潮", "辐射")),
    ("西幻", ("骑士", "魔法师", "法师塔", "巨龙", "精灵", "兽人", "魔法等级", "圣阶",
            "法术位", "冒险者公会")),
)
GENRE_MIN_HITS = 2

# 触发力量体系自检的题材（有明确等级/数值成长的题材才需要逐章数值一致性机检）。
# 注意：这是 **seed 侧**白名单（决定立项要不要给 starter registry）；检测器侧
# power_system.py 只看 registry 是否存在——手建 registry 的任何题材都会被机检。
...
# 通用单调数值字段（progression 快照里出现即逐角色查倒退；年龄倒流是长篇高发数值穿帮，
# 此前完全无机检——快照带 年龄/修为 字段即可被同一套退档机制覆盖）。
...
REGRESS_EXEMPT_KEYWORDS = ("跌境", "废修", "废了", "自废", "封印", "重伤", "夺舍失败", "境界跌落", "散功",
                          # 年龄/时间类合法回退：重生/转世/回溯题材快照标注即可豁免
                          "重生", "转世", "时光回溯", "返老还童", "夺舍", "回到过去")
