...
def test_sentence_start_and_short_template_signals():
    ...
    assert out["points"] == -2  # 🟢句首词不扣分；本例由两个 🟡 句式信号扣分
...
def test_retention_prior_levels_and_cap():
    ...
    assert rep.retention_prior(mild)["level"] == "none"
    assert rep.retention_prior(mild)["points"] == 0
    green_only = {
        "adjacent_max_jaccard": 0.02,
        "mechanical_opener_groups": 0,
        "repeated_sentences": 0,
        "sentence_start_token_groups": 8,
        "compression_ratio": 0.32,
        "low_chapter_compression_count": 3,
    }
    assert rep.retention_prior(green_only)["points"] == 0
