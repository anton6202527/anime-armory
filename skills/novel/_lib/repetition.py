...
def retention_prior(summary):
    ...
    if starts >= 2:
        score += 1
        reasons.append(f"{starts} 组高频句首词：起手词过度集中")
    elif starts:
        reasons.append(f"{starts} 组高频句首词（轻）")
    ...
    if cratio is not None and float(cratio) <= REP_COMPRESS_LOW_GREEN:
        score += 1
        reasons.append(f"全书压缩比偏低（多样性不足）：{float(cratio):.0%} ≤ {REP_COMPRESS_LOW_GREEN:.0%}")
    low_ch = int(summary.get("low_chapter_compression_count") or 0)
    if low_ch >= 2:
        score += 1
        reasons.append(f"{low_ch} 章压缩比偏低：局部文本多样性不足")
    elif low_ch:
