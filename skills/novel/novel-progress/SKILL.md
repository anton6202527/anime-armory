...
你是小说线的**只读进度向导**。扫描 `创作区/写小说/` 下各剧作的 `_进度.md`，报告完成度并给出下一步建议。

## 输入 / 输出 / 读写边界

- **输入**：`创作区/写小说/<剧名>/_进度.md`。
...
```bash
python3 skills/novel/novel-progress/scan.py                 # 扫描 创作区/写小说/ 下所有项目
python3 skills/novel/novel-progress/scan.py <作品根>         # 只看指定项目
```
