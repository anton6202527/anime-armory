...
**A**：把暂定名作为**候选 #0** 一并打分，明确指出它相对其他候选的弱势（例如：历史向命名 → 漫剧平台契合度低），在表格里保留“沿用 vs 换名”的对照。默认采用总分最高且通过撞名检查的候选；暂定名若最高就沿用，用户随时可覆盖。

候选 #0 也按 5 维评分；如果它总分最高就自动保留——这是合法路径，不是强行换名。
...
## Q14：一键成书时哪些步骤自动继续，哪些必须停？<a id="q14"></a>

**A**：默认 `审阅策略=用户授权制作代理`。普通、可逆选择按“项目设置 → 全局默认 → 本线推荐”解析，采用后写回 `_设置.md`；免费确定性 helper、gate、回扫和可逆修复尽量在同一任务 chain。蓝图、设定圣经和 Demo 仍要由与 writer 分离的 specialist 实际复核，并留下与当前输入/产物 hash 绑定的代理证据；代理审阅必须明示不是独立人审。

只有这些边界停：核心 author intent 或权利来源确实缺失；显式 `human_required`/`逐阶段用户确认`；跨来源冲突；连续三次同因失败；不可逆发布/覆盖与最终署名/成品验收；阶段预算包创建、扩大、过期或 input/model/channel/scope/cost 合同变化。已有精确绑定且有效的阶段预算包在余量内不逐章、逐调用重复问。

`pipeline_runner.py` / `author_workflow.py` / `novel-update` 分别是状态/导航/计划层，不直接调用文本 provider，也不把初始化骨架冒充完成；真正的语义写作与审阅由 supervisor 派发到既有 skill/specialist。
