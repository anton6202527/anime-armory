...
ALLOWED_COMMAND_SCRIPTS = (
    "skills/novel/novel-review/scripts/reader_contract_sentry.py",
    "skills/novel/novel-review/scripts/consistency_audit.py",
    "skills/novel/novel-review/scripts/build_review_report.py",
    "skills/novel/novel-score/scripts/score.py",
    "skills/novel/novel-score/scripts/collect_market_baseline.py",
    "skills/novel/novel-craft/scripts/revision_planner.py",
    "skills/novel/novel-dashboard/scripts/dashboard.py",
)
...
def default_command_template(kind: str) -> str:
    if kind == "review":
        return (
            "python3 skills/novel/novel-review/scripts/reader_contract_sentry.py "
            "\"{root}\" --chapter {chapter_label}"
        )
    if kind == "score":
        return "python3 skills/novel/novel-score/scripts/score.py \"{root}\" --scope chapter --chapter {chapter}"
    if kind == "revision":
        return "python3 skills/novel/novel-craft/scripts/revision_planner.py \"{root}\""
    if kind == "dashboard":
        return "python3 skills/novel/novel-dashboard/scripts/dashboard.py \"{root}\" --write"
    if kind == "market_evidence":
        return (
            "python3 skills/novel/novel-score/scripts/collect_market_baseline.py "
...
class NovelBatchQueue:
    ...
    def _governance_actions(self, dead: list[dict[str, Any]], retry: list[dict[str, Any]], expired: list[dict[str, Any]]) -> list[str]:
        actions: list[str] = []
        if expired:
            actions.append("run `python3 skills/novel/novel-batch/scripts/queue.py reclaim <作品根>` before claiming new work")
        if dead:
