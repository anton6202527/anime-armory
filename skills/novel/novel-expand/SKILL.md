...
先读 `novel-craft/references/draft-pipeline.md`，跑 `python3 skills/novel/novel-craft/scripts/draft_packets.py "<作品根>" --next|--range A-B` 生成逐章任务包。
...
```bash
python3 skills/novel/novel-craft/scripts/export.py "<作品根>" --formats txt,docx[,outline]   # 家族通用导出器
```
