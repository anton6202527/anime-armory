...
1. **判定是否需要资料包**
   ...
   - 可先跑需求清单，给作者一个“缺什么资料才能继续写”的落地列表：
     ```bash
     python3 skills/novel/novel-research/scripts/research_pack.py needs "<作品根>"
     python3 skills/novel/novel-research/scripts/research_pack.py needs "<作品根>" --chapter 4 --json
     python3 skills/novel/novel-research/scripts/research_pack.py jobs "<作品根>" --chapter 4
     ```
   - `needs` 是诊断，`jobs` 是执行清单；后续 agent/作者按 `research_jobs` 的 P0/P1 顺序深搜、补来源、回写资料包。
   - 任务领取/关闭用：
     ```bash
     python3 skills/novel/novel-research/scripts/research_pack.py job-update "<作品根>" RJ-001-MEDICAL \
   ...
   - 资料包补好后，把事实落到具体章节/场景，防止“查了资料但写章时乱用或忘用”：
     ...
     python3 skills/novel/novel-research/scripts/research_pack.py scene-usage "<作品根>" --chapter 4 --json
     ```
...
3. **生成/更新资料包**
   ```bash
   python3 skills/novel/novel-research/scripts/research_pack.py scaffold "<作品根>" \
...
4. **校验资料包**
   ```bash
   python3 skills/novel/novel-research/scripts/research_pack.py check "<作品根>"
   python3 skills/novel/novel-research/scripts/research_pack.py check "<作品根>" --chapter 4 --json
   ```
...
5. **刷新审计**
   ```bash
   python3 skills/novel/novel-research/scripts/research_pack.py refresh-audit "创作区/写小说"
   python3 skills/novel/novel-research/scripts/research_pack.py refresh-audit "创作区/写小说" --json
   ```
