...
机器单一真值源：`skills/novel/novel-craft/scripts/contract.py`。
...
2. **同构阶段清单型** — 形如下面两段、用 `novel-create-stage-table` / `novel-derived-stage-table` marker + `<!-- stage:<key> -->` 逐项的阶段级清单，由 `skills/novel/novel-craft/scripts/progress.py` 读取。**注意**：该 reader 遇到章节矩阵型会打印 `[redirect]` 指向 `novel/progress.py`（它只读阶段清单型）。
