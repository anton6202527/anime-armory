...
- `设定/章纲.md` 已按当前 `审阅策略` 通过：默认独立 specialist 签收，显式逐阶段人审项目由具名用户/真人签收。
...
- `_设置.md` 已落 `小说生成模式`、`章节生成粒度` 与 `创作工艺档`；缺则按 `skills/novel/novel-craft/references/选择点与偏好.md` 读取全局默认，再采用本线推荐值写回并继续。旧项目缺 `创作工艺档` 时运行期安全回退 `genre_novel`。
