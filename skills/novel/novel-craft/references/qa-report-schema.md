...
- `python3 skills/novel/novel-review/scripts/build_review_report.py "<作品根>" [--human-assessment 审稿/human_findings.json]`：把机检/人判汇总成 `审稿/review_report.json`，供 gate 消费。
- `python3 skills/novel/novel-craft/scripts/report_gate.py "<作品根>"`：按 export 硬闸检查 rights/review/score 阻断，默认阻断时返回非 0；`--progress-mode` 用于续跑提示，缺 review 只显示 warning。商业/漫剧项目确需跳过评分时，用 `--waive-missing-score --reason "<原因>"` 写入带 scope 的 `missing_score_report` 豁免。
- `python3 skills/novel/novel-craft/scripts/progress.py "<作品根>"`：展示下一阶段 owner/gate/on_fail，同时展示 QA gate 阻断。
- `python3 skills/novel/novel-craft/scripts/export.py "<作品根>" ...`：默认执行 QA gate；除非用户明确要求并传 `--ignore-qa-gate`，否则阻断未清不能导出。
