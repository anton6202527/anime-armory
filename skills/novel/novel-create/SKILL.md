...
- **作者意图先行，别替用户决定故事**：先从原始种子提取 logline / 主角 / 核心机制或人物困境 / 主要阅读快感 / 冲突。普通工艺缺项用推荐值补齐；只有这些核心作者事实确实无法推断时才一次问一组，不把金手指、打脸爽点强加给文学/传统小说。详见 `references/interview.md`。
...
- **Demo gate 最重要**：前 1-3 章定文风、人物与阅读承诺、设定自洽；商业连载/改编源书再验证兑现密度与追读钩。默认由与 writer 分离的 specialist reviewer 逐章签收并连续推进；显式 `逐阶段用户确认` 才停给用户审。
...
从用户的只言片语 + 碎片提取下列字段；普通工艺缺项用推荐值，只有核心作者事实无法推断时才合并成一组问题：
...
- **输出**：txt/docx/outline。
> 用户给了碎片（风格样本/笔记/半成品）→ 先冻结原文并记录你的理解，再只补会改变作者意图的真缺口。**别让用户重答他已经给过的。**
...
### 7. 续写余下 + 状态增量 + 回扫 + 导出
- **定模式**：按 `skills/novel/novel-craft/references/选择点与偏好.md` 读/问 `小说生成模式`（极速初稿/稳妥初稿/商业连载）、`小说生成工作流`（默认单步/三步迭代/边写边自检）、`小批回扫间隔`（3章/5章/关闭）和 `章节生成粒度`（逐章/小批/全书草稿）。缺省推荐 `稳妥初稿 + 5章回扫 + 逐章`；长篇/商业连载/漫剧源书在 `draft_packets.py --step auto` 下自动升三步迭代，用户明确写 `默认单步` 才降回单包。用户明确要写完每章立刻自动自检时选 `边写边自检`，用户明确要快时用 `极速初稿 + 小批`。
...
python3 skills/novel/novel-craft/scripts/draft_queue.py "<作品根>" claim --agent "<名字>"
```
再按认领章号出任务包：
```bash
...
```bash
python3 skills/novel/novel-craft/scripts/arc_packets.py "<作品根>" --arc 4-8
```
该窗口写完后跑：
```bash
python3 skills/novel/novel-review/scripts/arc_gate.py "<作品根>" --arc 4-8
```
...
- **队列回写**：该章 review/对账通过后跑 `python3 skills/novel/novel-craft/scripts/draft_queue.py "<作品根>" done NN --agent "<名字>"`；返工则 `fail NN --reason "<原因>"` 或 `todo NN`。
...
python3 skills/novel/novel-craft/scripts/ai_usage.py "<作品根>" \
  ...
  --review-step 人工通读
```
- **导出**：`python3 skills/novel/novel-craft/scripts/export.py "<作品根>" --formats txt,docx,outline`（家族通用导出器）→ `导出/`。
...
| 错误 | 纠正 |
|---|---|
| 上来就替用户编故事 | 先提取 logline/主角/核心机制或困境/阅读承诺/冲突；核心作者事实无法推断时才问，普通工艺项用推荐值 |
...
| 把 8 步命令全抛给用户或每步都停 | gate 仍逐步真实通过，但默认由编排层连续派发确定性检查和独立 specialist；只有硬边界停人 |
