...
用户说"同人传 / 角色传 / 给某角色写一本漫剧源小说"时，默认仍走**并行外传**：用原作锚点锁时间线，用外传章节补这个角色自己的来路、任务、留白和成长弧。若用户没指定篇幅，可先做"第一季源书"目标章数（`--target-chapters` 覆盖），避免把 70-100 章漫剧档误写成一次性交付的无限长篇。
...
家族统一铁律（公版 / 自有 / `--i-have-rights` + provenance 留痕；判公版标准）见 `novel/SKILL.md` 合法性继承。本 skill 是家族里**最严**的一档，特有红线：
- 同人是法律灰色区，**比 novel-fetch 更严**：派生作品是原作版权人专有权利。外部当代受版权网文/出版物，且用户没有声明自有或已授权 → **直接拒做**，即使非商用、不发布也不行；拥有阅读权 ≠ 改编权。
- 用户给出本地工作区源书路径并要求制作派生文本时，按仓库根 `AGENTS.md` 的默认约定视作"自有 / 已授权 / 公版"之一继续，但必须在项目 `_meta.json` / `_设置.md` 留下 `rights_status=user-declared` 或更精确来源；公开发行、商业投放或跨区发布前仍需 QA gate 复核改编权。
- **遇到来源不明的当代网文不要"读了再说"**——读了再写派生 = 绕铁律一圈。直接拒做，引导改用公版（《晋书》《史记》《聊斋》《封神》《镜花缘》等）或用户自有/已授权作品。
- **外传写作中不大段复刻原作原文**：对话/动作可**复述**不可整段照搬；锚点对齐靠**事件骨架**而非文本搬运（工艺 + 法律边界）。

## 工作流（八步）

总览：**0** 核对输入/权利 → **1** 建骨架 → **2** 设定三卡 → **3** 书名推荐 → **4** 章纲 → **5** Demo + 独立审阅 → **6** 写余下章节 → **7** 回扫 → **8** 导出。

每个 gate 都必须真实通过，但默认 `审阅策略=用户授权制作代理` 时，设定、书名推荐、章纲、Demo 与回扫由独立 specialist/确定性检查签收后连续推进；显式 `逐阶段用户确认` 才停给用户审。作者意图、来源权利、跨来源冲突、不可逆发布与最终验收仍由真人决定。
...
先从用户输入、项目设置和原作证据核对；普通缺项用推荐值写回。只有原作、配角、时间线分叉点等核心事实缺失时才向用户提一个合并问题：
...
> **源语言/文体体检**：默认假设原作是现代中文白话文。原作若是**文言文/古文或外文**，先 `python3 skills/novel/_lib/source_language.py <项目根>` 体检——非现代白话需先 `--scaffold` 建 `设定/source_comprehension.md` 源理解层并补全、把 `status` 置 confirmed（否则 qa_gate 阻断 `SOURCE-LANG-COMPREHENSION`）。外传从理解层（现代白话理解）写、锚点对照仍按原作事件。
...
4. 报告锚点数、留白点数和候选支线；默认由独立 specialist 复核并采用证据最强的推荐支线后进入第 3 步。只有候选会改变核心分叉点且无证据优势，或项目显式逐阶段人审时才停。

5. **读者契约** —— 按 `novel-craft/references/reader-contract.md` 填 `设定/读者契约.md`：
   - 外传的核心题旨：这个配角视角要回答什么问题，而不是只复述原作。
   - 读者承诺：会补哪些留白、兑现哪些情感/反转、哪些锚点绝不改。
   - 好看机制与文学质感：靠视角差、误读、隐忍、反差，还是靠人物弧光；文风要贴 Demo。
   - 禁偏清单：不可盖过原作主角、不可推翻硬锚点、不可把外传写成主线改写。
...
- novel-title 走它自己的流程（5–8 候选 / 5 维评分 / 撞名检查 / 自动采用最高分推荐，保留备选供覆盖）。
- novel-title 回写本项目 `_meta.json.title` 与 `设定/书名候选.md`。

详细评分规则、各平台命名习惯、配比要求详见 `skills/novel/novel-title/references/title-patterns.md`。

推荐名落档后：
- 确认 `_meta.json.title` 已被 novel-title 回写。
- 用统一进度入口标记书名阶段，例如 `python3 skills/novel/novel-craft/scripts/progress.py set "<作品根>" title done`；若项目使用章节矩阵进度，则用 `python3 skills/novel/progress.py set "<作品根>" 第N章 <列名> ✅`。
- 按当前审阅策略通过并写回后 → 进第 4 步。
...
- **总章数 / 字数分档先按 `skills/novel/novel-craft/references/split.md` 反推**（按 `_meta.json.target_platform` 选档）——抖音漫剧友好通常 70-100 章，网文长篇 12-20 章，差一个量级。
- 章纲编织（三幕 / 锚点织网 / 钩子）详见 `skills/novel/novel-craft/references/outline.md` + 本 skill 的 `references/workflow.md`。

写完必须过章纲审阅 gate；默认派给独立 specialist 签收并继续，显式人审策略才停。**章纲未通过前不要进第 5 步 Demo**。

### 第 5 步 — Demo（前几章）+ 独立审阅【最重要 gate】

**不在敲定章纲后立刻写全本**——先写**前几章 Demo**，由与 writer 分离的 reviewer 验文风、视角、节奏、锚点对齐。Demo 通过才进第 6 步写完整本。
...
写法同第 6 步逐章生成（见下），但**每章独立写、独立审**：默认在同一任务派给 specialist reviewer，审阅通过才写下一章；显式 `逐阶段用户确认` 时才把当前章贴给用户并停。这样能在第 1 章发现视角/口吻/锚点/文风问题，又不把普通代理审阅变成人工等待点。

Demo 审核 checklist（交给当前审阅策略对应的 reviewer）：
...
reviewer 给出 fix → 在 Demo 章里 Edit 修改 + 把意见沉淀到 `设定/角色卡.md` "口吻习惯"段或 `references/pov-craft.md` 项目本地副本里，作为后续章节的隐式指南。**返工后再次交同一审阅策略复核**。
...
python3 skills/novel/novel-review/scripts/mechanical_check.py "<作品根>" --pov "<配角名>" --terms "<规范术语逗号分隔>"
```
...
```bash
python3 skills/novel/novel-craft/scripts/export.py "<作品根>" \
...
| **跳过 Demo 直接写满本** | 最致命错误。视角/口吻/锚点对不对 1 章就能看出；Demo 须由独立 reviewer 逐章审，默认代理审阅可连续推进 |
| 章纲未过 gate 就开写 Demo | 章纲是 Demo 的前置；默认 specialist 签收，显式人审项目才等用户 |
