...
4. **强制打 AI 标识（法律必做）**：调公共 `watermark` skill（合规水印已合并到那里，本 skill 不再自带打标脚本；图/视频同一个工具，按扩展名自动判定）：
   ```bash
   python3 <watermark-skill>/watermark.py _raw_换脸.png 换脸成片.png --mode ai --text "本图含 AI 换脸合成 / AI-generated"
   ```
...
- **yt-dlp**（取链接，可选）。
- 公共 **`watermark`** skill（强制 AI 标识打标，基于 Pillow；合规水印已统一到那里）。
...
| 自己写打标脚本 | 统一调公共 `watermark` skill（`--mode ai`，图/视频同一工具） |
