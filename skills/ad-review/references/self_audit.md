...
把"人工复盘整条 ad 流水线"固化成一条可复跑流程。默认产出 = 一份**建议报告**（report-only，不自动改 skill）。ad 线自包含，本手册只对照 ad-* 自己的 skill + references，不引用 n2d/mv/novel/song 任何脚本或文档。
...
**0 block / 0 warn 才进入下一步联网市场对标。** 该脚本是 ad 线专属产线治理脚本（其它线模式②目前只有联网对标，无对应本地脚本——四线共享的是本 self_audit.md 工艺，不是脚本本身）。
