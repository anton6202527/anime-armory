...
IMAGE_BACKEND_DOCS = (
    "skills/ad-image/SKILL.md",
    "skills/ad-craft/references/选择点与偏好.md",
)
...
def repo_root_from_here() -> Path:
    # .../skills/ad-review/scripts/self_audit.py → parents[3] == repo root
...
def load_contract(root: Path):
    ...
    path = root / "skills" / "ad-craft" / "scripts" / "contract.py"
    if not path.is_file():
        return None, None
...
def check_ad_law_single_entry(root: Path, findings: List[Finding]) -> None:
    ...
    """
    checker = root / "skills" / "ad-script" / "ad_law_check.py"
    if not checker.is_file():
        add(findings, "block", "广告法机检单入口", rel(root, checker),
            "广告法机检脚本不存在——ad 线核心硬闸门缺失。",
            "恢复 skills/ad-script/ad_law_check.py（绝对化/医疗/化妆品/金融教育/促销/房地产词库）。")
        return
    report_token = "广告法机检报告.json"
    consumers = {
        "ad-craft/scripts/gate.py": root / "skills" / "ad-craft" / "scripts" / "gate.py",
        "ad-review/scripts/review.py": root / "skills" / "ad-review" / "scripts" / "review.py",
    }
    ...
    if bypassed:
        ...
        add(findings, "info", "广告法机检单入口", "skills/ad-script/ad_law_check.py",
...
def check_delivery_specs_external(root: Path, findings: List[Finding]) -> None:
    ...
    if offenders:
        ...
        add(findings, "info", "交付规格外置", "skills/ad-craft/scripts/contract.py",
...
def check_image_backend_docs(root: Path, findings: List[Finding]) -> None:
    ...
    if missing_docs or forbidden_promoted:
        ...
        add(findings, "info", "生图后端白名单", "skills/ad-craft/scripts/contract.py",
...
def check_ai_usage_disclosure(root: Path, findings: List[Finding]) -> None:
    ...
    if error:
        add(findings, "block", "自审引擎错误", rel(root, cpath),
            f"无法导入 ad-craft contract.py（自审工具故障，非作品问题）：{error}")
        return
    usage_py = root / "skills" / "ad-craft" / "scripts" / "ai_usage.py"
    if not usage_py.is_file():
        add(findings, "block", "AI披露一致性", rel(root, usage_py),
            "AI 使用/授权披露脚本不存在——投放前 review 的留痕项会永远缺失。",
            "恢复 skills/ad-craft/scripts/ai_usage.py（写 合规/ai_usage.json）。")
        return
...
def check_choice_point_catalog(root: Path, findings: List[Finding]) -> None:
    ...
    if not isinstance(choice_points, dict) or not choice_points:
        ...
        return
    catalog = root / "skills" / "ad-craft" / "references" / "选择点与偏好.md"
    raw = text(catalog)
    if not raw:
        add(findings, "warn", "选择点对齐", rel(root, catalog),
            "选择点目录文档不存在，无法核对 CHOICE_POINTS 是否都暴露给用户。",
            "补 skills/ad-craft/references/选择点与偏好.md，列全 CHOICE_POINTS 的键 + 缺省。")
        return
