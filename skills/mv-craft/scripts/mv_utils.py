...
def content_hash(path):
    """Return the full SHA-256 digest used by MV receipts and stale checks.

    Historical MV payloads named these fields ``*_sha256`` while this helper
    emitted a truncated SHA-1.  Keeping one honest digest format is more
    important than preserving that accidental representation: old receipts
    naturally become stale and are regenerated by their owning stage.
    """
...
def json_hash(payload):
    """Hash a JSON-compatible value using a stable UTF-8 serialization."""
    encoded = json.dumps(
...
def timeline_edit_contract(timeline):
    """Return the picture-lock portion of a timeline manifest.

    Take selection may update ``video_path`` and ``selected_at`` after picture
    lock.  Those are media substitutions, not editorial changes.  Cut order,
    timing, retime policy and seam intent *are* editorial and therefore remain
    hash-bound.
    """
    rows = []
    for row in (timeline or {}).get("clips") or []:
        ...
        rows.append({
            ...
            "seam_contract": row.get("seam_contract") or {},
        })
    return {
        "song_path": (timeline or {}).get("song_path"),
        "rate": (timeline or {}).get("rate"),
        "clips": rows,
    }


def timeline_edit_hash(timeline):
    """SHA-256 for the edit decisions that a picture lock actually approves."""
    return json_hash(timeline_edit_contract(timeline or {}))
