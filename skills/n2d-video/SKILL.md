...
它会更新 `生产数据/script_contract_applied_第N集.json` 的 `出视频` scope，并绑定当前 `script_quality_contract` SHA 与 `出视频/第N集/prompt/01_clips.md` SHA。`video_preflight/video` gate 会检查这份收据；缺失、过期或字段不全，都先回视频 prompt 或 n2d-script 修。
