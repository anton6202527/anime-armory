...
- **正反打视频消费铁律**：`dialogue_shot_reverse` 视频 prompt 必须继承 P-2 `axis_blocking_map.json#shot_reverse_patterns` 与 P-3 `continuity_bible.json#shot_reverse_continuity`：A/B 屏幕左右不换、互补视线不看镜头、OTS/clean single/insert/reaction 成对 coverage、镜头高度/距离/焦段连续、主光与背景深度延续。若剧情需要越轴，prompt 的 `接缝执行包` 和 `执行配方` 必须写 `buffer_or_reestablishing`（建立镜/插入物/中线移动/明确转场），且最后 0.5 秒保持落幅给剪点；不得让视频模型临场改站位、改视线或把画外角色拉回清晰正脸。
...
视频 prompt 写完后必须给剧本合同签收消费收据：

```bash
python3 skills/n2d-video/scripts/script_contract_receipt.py <作品根> 第N集 --scope 出视频
```

它会更新 `生产数据/script_contract_applied_第N集.json` 的 `出视频` scope，并绑定当前 `script_quality_contract` 的稳定 `content_hash`、合同文件 SHA（审计用）与 `出视频/第N集/prompt/01_clips.md` SHA。`video_preflight/video` gate 会先检查这份收据，再调用 `n2d-review/scripts/script_contract_verify.py` 逐 Clip 机检 prompt 是否真的写入 dramatic_function / audience_effect / retention promise / audience question；缺失、过期、字段不全或 verifier block，都先回视频 prompt 或 n2d-script 修。
