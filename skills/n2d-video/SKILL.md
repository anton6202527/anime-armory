...
- **物理段验收真值源铁律（`_mid` 不折叠）**：出视频验收/QC 的最小单位是 `video_runner` manifest item（`items[].clip` + `items[].target`），不是 MP4 文件名里复用的源图编号。`Clip01_x.mp4` 和 `Clip01_x_mid.mp4` 是两个物理视频段时，必须分别按 manifest 的 `Clip_01` / `Clip_02`（或对应真实 item clip）验收；`_mid` 不是非 `_mid` 的替代品，也不能被报告折叠成同一条 `Clip_01`。QC 报告必须同时列 `Clip` 与 `Source MP4`，抽帧文件名必须带 manifest clip 或唯一物理段前缀，禁止只用 `Clip01` 数字导致抽帧覆盖、warn 成对重复或 accept 错对象。只有要交给 compose 的最终 MP4 才算该物理段通过；逻辑镜头分组只供人理解，不作为验收主键。
