...
本 skill 涉及的选择点：`基础视觉风格`（只继承，不在本阶段重选）、`生视频模型`（本阶段普通镜/兜底或固定模型）、`生视频渠道`（本阶段实际调用渠道/产品/API）、`视频模型路由`（默认自动按镜头路由）、`视频备用后端`（固定模式下可设 `无`，避免把本机不可用 CLI 写成 fallback）、`出视频规格`（三档预算·每次调模型/渠道前告知·见下「出视频规格」节）、`英雄镜多版`（默认关闭·costly·开启则英雄镜跨后端多版·见下「跨后端英雄镜多版」）、`视频分辨率`、`画幅`、`对口型`、`生成粒度`、`生成优先序`、`制作模式`（决定占位闸门是否放行·见输入前置条件）、`视频原生音轨`（只由 n2d-compose 执行处理，n2d-video 阶段不剥音轨）、`目标平台`、`发行地区`、`合规用途`。后 3 项必须同步落 `合规/compliance_manifest.json`，不得只写 `_设置.md`。旧 `生视频AI` 兼容读取。
...
- **原生音画 / 原生音轨策略铁律（P1）**：按 `制作模式` 分流。默认 `配音先行` 时，视频层只产**无声 Image2Video**，`audio_intent=none`、`speech_policy=no_native_speech`、`compose_policy=丢弃`，对白/音效/BGM 交给后期层；只允许低风险镜头 opt-in 环境声/音效：纯空镜、转场、远景氛围、背身/侧脸/剪影、无口型、无台词。`配音先行` 下正面说话特写、角色台词、旁白、系统音、克隆音色镜头禁止启用原生人声。仅当用户显式选择 `原生音画` 时，说话镜才由路由表标 `native_speech`，让原生同步音画后端一次出台词+口型+环境声。每个 Clip prompt 必填 `原生音画策略`：`audio_intent`、`risk`、`mouth_visible`、`speech_policy`、`compose_policy`、`review`；配音先行下开启环境声/音效时必须 `risk=low + mouth_visible=no + speech_policy=no_native_speech`。**`mouth_visible` 别凭印象填**：先跑 `python3 skills/n2d-model-router/scripts/mouth_detect.py <作品根> 第N集 --write` 按首帧 PNG（装 insightface 时）+ 分镜文本预填/复核，落 `生产数据/mouth_visible_audit_第N集.json`；图↔文本/图↔prompt 冲突会在 video gate 阻断（以图为准）。`mouth_visible` 真值就是每 Clip prompt 的 `原生音画策略` 字段，照 sidecar 建议改 prompt，避免给说话镜误开环境声。详见 `references/原生音画opt-in.md`。
...
声音约束：{native_speech 镜写“按脚本文本生成本镜台词并口型同步，后期保留原片音轨”；非 native_speech 镜写“无对白、无旁白、不要生成原生人声”；若故事板标声音先行，仅给 n2d-compose 使用};
...
5. ✅ 物理守卫：运动精修写明重心、锁定部位、遮挡层级和不穿模/不拉脸约束
6. ✅ ②镜头运动：推/拉/跟/环绕/固定等词明确，速度词明确，不只写"运镜"
7. ✅ 动态细节 & 环境交互：烛火/雨丝/衣袂/雾气/灵光等 ≥1 条，且包含动作对光影/粒子/道具/背景的反馈，不改首帧设定
...
10. ✅ 在场链：已读取 `entity_schedule.required_presence/offscreen_presence/forbidden_presence` 与 `continuity.entry_exit`，正向只保留登记实体，负向禁止未登记人物/道具/背景路人
11. ✅ 模型路由：已读取 `video_model_routes.json`，本镜有 primary/fallback/mode/policy_resolution/rationale/degrade_plan，且平台参数只写目标后端支持的能力
12. ✅ 原生音画策略：已填 audio_intent/risk/mouth_visible/speech_policy/compose_policy；route=native_speech 的说话镜写 native_speech + 保留原片音轨，非 native_speech 镜默认丢弃，只有低风险无口型无台词镜头才 opt-in 环境声/音效；凡保留原片音轨、低音量混入环境声、native_sfx 或 native_speech，都必须同步写 `生产数据/native_av_physics_第N集.json`，逐 Clip 结构化记录声源、口型、动作声可见证据、空间声学和后期保留策略
...
15. ✅ 近景身份锁定：CU/MCU/反打/说话镜已写脸型、五官比例、发型发髻、标志配饰、服装配色、脸部特写/表情参考或实现分解方案；已写 表情锚/表情幅度/锁脸不锁情，大表情近景已走首尾双帧或 MCU 保真实现
16. ✅ 复杂动作实现：复杂动作已有控制资产或实现分解方案，不靠删戏/降目标规避
