...
本 skill 涉及的选择点：`基础视觉风格`（只继承，不在本阶段重选）、`生视频模型`（本阶段普通镜/兜底或固定模型）、`生视频渠道`（本阶段实际调用渠道/产品/API）、`视频模型路由`（默认自动按镜头路由）、`视频备用后端`（固定模式下可设 `无`，避免把本机不可用 CLI 写成 fallback）、`出视频规格`（三档预算·每次调模型/渠道前告知·见下「出视频规格」节）、`英雄镜多版`（默认关闭·costly·开启则英雄镜跨后端多版·见下「跨后端英雄镜多版」）、`视频分辨率`、`画幅`、`视频生成音频策略`（默认 `无声视频流`，决定调用 video-only 还是音频条件/原生音画生成流）、`对口型`、`生成粒度`、`生成优先序`、`制作模式`（决定占位闸门是否放行·见输入前置条件）、`视频原生音轨`（只由 n2d-compose 执行处理，n2d-video 阶段不剥音轨）、`目标平台`、`发行地区`、`合规用途`。后 3 项必须同步落 `合规/compliance_manifest.json`，不得只写 `_设置.md`。旧 `生视频AI` 兼容读取。
...
- **原生音频保留到 compose 处理（2026 新坑）**：Veo 3.1 / Seedance 2.0 等会**原生生成同步音频**（环境音甚至台词）。n2d-video 阶段的职责是拿回平台**原片**、抽帧验画、落档；**不要提前 `-an` 去音轨，不要转成无声版覆盖原片，也不要把原片挪进 `_raw_with_audio/` 再用无声版占正式 Clip 名**。调用时按 `原生音画策略` 写声音约束：非原生音画默认"无对白、无旁白、不要生成原生人声"，并优先走 video-only 流；`视频生成音频策略=配音对齐口型` 才允许 `voice_conditioned_lipsync` 把配音作为口型条件；低风险 opt-in 镜头可写"允许环境声/动作音效，禁止人声/台词/旁白"。`原生音画` 下 route 标 `native_speech` 的说话镜必须反过来写清"台词+口型由原生音画后端生成，compose_policy=保留原片音轨"。只要本集准备保留原片音轨、低音量混入环境声、native_sfx 或 native_speech，就同步写 `生产数据/native_av_physics_第N集.json`，逐 Clip 记录声源、可见动作证据、空间混响和后期处理策略；没有 sidecar，不进入正式合成。若平台生成了音轨，在 `00_总览.md` 标记"含原生音轨"，交 n2d-compose 按 `视频原生音轨` / 制作模式统一处理。`gate.py --stage video|compose` 会阻断 `出视频/第N集/视频/` 下的 `.noaudio.mp4`、`*_noaudio.mp4`、`_raw_with_audio/` 等提前剥音轨痕迹。
