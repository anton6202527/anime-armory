...
- **视觉锚点闭环**：脚本从 P-3 `continuity_chain`/storyboard 读取 `seam_mode`。只有 `continuous_take_relay` 必须把首帧与尾帧同时送入可消费双关键帧的后端，并核验边界帧 SHA；其他接缝不因相邻画面不同而报错，而是分别消费动作相位、视线、反应、插入、J/L 声桥、溶解或有意不连续证据。首帧缺失仍是 image2video BLOCK；relay 缺尾帧/双帧执行配方则 BLOCK，非 relay 不强造尾帧。
