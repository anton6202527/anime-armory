...
CHAR_RE = re.compile(r"\b(CHAR(?:[_-][A-Z0-9_]+|\d+))\b", re.IGNORECASE)
CLIP_RE = re.compile(r"\bClip[_\s-]?(\d+)\b", re.IGNORECASE)
LOC_RE = re.compile(r"\b(LOC(?:[_-][A-Z0-9_]+|\d+))\b", re.IGNORECASE)
...
def infer_dialogue_ref(text: str) -> str:
    m = re.search(r"allowed_character_dialogue_indices\s*=\s*(\[[^\]]*\])", text or "")
    if m:
        return f"dialogue_fact_contract.allowed_character_dialogue_indices={m.group(1)}"
    m = re.search(r"allowed_voiceover_indices\s*=\s*(\[[^\]]*\])", text or "")
    if m:
        return f"dialogue_fact_contract.allowed_voiceover_indices={m.group(1)}"
    m = re.search(r"(?:dialogue_ref|台词索引)\s*[=:：]\s*([^;\n]+)", text or "", re.IGNORECASE)
    if m:
    ...
    *,
    ...
    if audio_intent == "native_speech":
        ...
        clip["speaker_source"] = {
            ...
            "mouth_visible": mouth,
            "dialogue_ref": infer_dialogue_ref(prompt_text),
        }
        clip["lip_sync"] = {
            "policy": "native_dialogue_match",
            "status": "needs_review",
            "mouth_visible": mouth,
