...
`line_NN.wav` 作口型条件）。但**后期pass**这一回退档此前落不了地——选了也没有脚本可跑。本脚本补这条：
  ...
  --apply：检测到本地 MuseTalk/LatentSync/Wav2Lip 可用时逐 clip 调用（CLI 路径由环境变量给，
           本仓不内置重型权重）；工具/输入缺失或输出为空时返回非零，绝不把“只落清单”伪装成执行成功。
...
def needs_post_pass(route: Mapping[str, Any], setting: Any) -> bool:
    ...
    # Mixed auto-routing may deliberately create a neutral base plate before a
    # voice has been cast.  That per-Clip contract outranks the project-wide
    # convenience toggle: the plate is not a final talking shot until this pass.
    if route.get("post_lipsync_required") is True or _norm(route.get("audio_strategy")) == "base_video_then_post_lipsync":
        return True
...
def select_post_pass_clips(routes: Sequence[Mapping[str, Any]], setting: Any) -> List[Dict[str, Any]]:
    ...
    for r in routes:
        ...
        out.append({
            ...
            "reason": (
                "hybrid_base_video_contract" if r.get("post_lipsync_required") is True
                else "forced_post_pass" if lipsync_mode(setting) == "post_pass"
                else "degrade_from_voice_conditioned"
            ),
            "performance_audio_paths": list(r.get("performance_audio_paths") or []),
            "line_indices": list((r.get("sound_route") or {}).get("line_indices") or []),
            "base_video_only": bool(r.get("base_video_only")),
        })
...
def _clip_line_indices(root: str, ep: str) -> Dict[str, List[int]]:
    ...
    for clip in sb.get("clips", []) or []:
        ...
        if not cid:
            continue
        lines = (
            clip.get("lines") or clip.get("line_indices") or clip.get("voiceover_lines")
            or clip.get("voiceover_indices") or clip.get("dialogue_indices") or []
        )
...
def _audio_for_clip(root: str, ep: str, line_idx: Sequence[int]) -> List[str]:
    ...
    paths = []
    for i in line_idx:
        # voiceover indices are 1-based; render_voice filenames are 0-based.
        # Keep the direct index as a legacy fallback for older storyboards.
        for slot in (max(0, int(i) - 1), int(i)):
            cand = os.path.join(voice_dir, f"line_{slot:02d}.wav")
...
def plan(root: str, ep: str) -> Dict[str, Any]:
    ...
    for c in selected:
        ...
        route_audio = []
        for raw in c.get("performance_audio_paths") or []:
            path = str(raw or "")
            path = path if os.path.isabs(path) else os.path.join(root, path)
            if os.path.isfile(path):
                route_audio.append(path)
        indices = c.get("line_indices") or line_map.get(cid, [])
        audio = route_audio or _audio_for_clip(root, ep, indices)
    ...
    if mode == "off":
        if selected:
            notes.append("项目开关虽为 `对口型=关闭`，但混合路由含 base_video_only 说话镜；逐镜后期表演合同优先，必须完成这些 jobs 才能进 compose。")
        else:
...
def main(argv: List[str]) -> int:
    ...
    if ns.apply:
        ...
        for j in res["jobs"]:
            if j["status"] != "ready":
                j["status"] = "blocked_missing_input"
                fails += 1
                continue
