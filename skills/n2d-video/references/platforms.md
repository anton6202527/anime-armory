...
用户问“每个 Clip 分几张帧”时，先回答能力边界：**首尾两帧是较稳的 fallback，但不是所有 API 都无条件支持；首/中/尾三帧不是主流统一能力，只有 Dreamina 原生多帧已在本仓库打通。** 未知/新后端不默认认定支持多帧，必须先刷新官方/API/CLI 能力证据并落 smoke 或人工确认；证据不足时中锚只能当 QC/参考，或改首尾帧、拆段接力、换原生多帧后端。
