...
```
image prompt = [图AI 的 prompt 写法] + [固定生视频模型锚定句 或 通用视频兼容锚定句]
```

**默认结构 = 图 AI(`生图AI` 所选官方/已登录后端，默认 Codex，可选 Dreamina/即梦官方 CLI) + 视频模型路由(`自动按镜头路由`) + 生视频模型/渠道（延后到 n2d-video 出视频前由 router/probe 决定；`_设置.md` 里的值只作固定模式/普通镜兜底/调用入口偏好）**。新作品首跑不问具体视频模型/渠道；固定模式、账号/交付约束、用户明说，或 n2d-video 阶段 router/probe 找不到可执行后端时才问。第三方逆向 CLI、`同视频AI` / `同视频模型` 含糊口径和 web 自动化出图仍禁。
...
4. 图 AI 按 `生图AI` 选择点统一到一个官方后端；视频模型读路由表，普通镜/兜底读 `_设置.md` 的 `生视频模型`，调用渠道读路由表与 `生视频渠道`，不要求新作品首跑先选具体后端
5. 锚定句语言：按目标生视频模型/渠道的消化风格选；即梦/可灵渠道通常用中文版，Veo 等海外渠道用英文版

### 推荐组合速查

> 与 `n2d-image/references/platforms.md` 同一张表，保持一致：**图阶段按 `生图AI` 统一官方/已登录后端；Dreamina/即梦官方 CLI 可用于出图；视频阶段默认按 `video_model_routes.json` 的逐 Clip primary/fallback 执行，`生视频模型`/`生视频渠道` 只作兜底与固定模式输入**。

| 场景 | 组合 | 备注 |
|---|---|---|
| 国风短剧（自动路由） | `生图AI` 所选官方/已登录图后端 + 通用国风视频兼容锚定 → n2d-video 阶段路由到 Seedance/Kling/Veo 等兼容后端 | 图默认 Codex；生视频后端不在首跑选择；全项目统一图后端 |
...
### 禁止：第三方逆向 / web 自动化出图
图片阶段按 `生图AI` 统一到一个官方/已登录后端；Dreamina/即梦官方 CLI 可出图。生视频模型/渠道在 n2d-video 出视频前由 router/probe + 适配层决定；图阶段用所选后端生成首帧，固定后端时拼对应模型锚定句，未固定时拼通用视频兼容锚定。
...
3. 在项目 `global_style.md` 的"视频模型路由 / 生视频后端决策 / 目标图AI"三行记上即可。**核心分镜/角色卡/字幕无需改动**——只换 prompt 的平台适配层。
