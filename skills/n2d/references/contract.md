...
| 模式 | 语义 |
|---|---|
| `原生音画` | **默认**（2026-06-16 起）。native AV 模式：说话镜由支持原生同步音画的后端一次生成台词+口型+环境声，主流程不把 `配音` 列作为硬依赖；配音层只用于旁白/系统音或单镜回退。最快看到出图/出视频、无重同步返工。仿真人音色授权仍由 compliance gate 管。 |
| `配音先行` | 强配音控制时推荐。真实配音先出，实测时长驱动分镜、出图、出视频；逐句音色/语速可控。 |
