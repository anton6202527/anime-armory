...
调度（n2d）→ 识别"情境 A 首跑"：
  先给「制作模式」菜单选一次（影响全程出片顺序，不静默默认）：
    A 配音先行（默认·长期量产推荐：TTS 固定音色 + 无声视频 + BGM/SFX 分层合成）/ B 原生音画（快速预览 / native AV，一次出同步音画）/ C 先出视频后配音（快速 demo 或配音还没就绪）
...
n2d-script（阶段1·剧本改编）→
  ...
  6. 报告：第1集剧本齐，下一步按制作模式走——默认配音先行先 n2d-voice（真音时长驱动镜头）；原生音画才直接回 n2d-script 做阶段2分镜
