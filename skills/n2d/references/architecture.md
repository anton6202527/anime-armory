...
- **当前默认**：图 AI = `生图AI` 所选官方/已登录后端（默认 Codex，可选 Dreamina/即梦官方 CLI）；若正式生图前扫到多个可自动落 PNG 的后端，先给用户菜单和建议并写回选择；若一个都没有，停止并提示准备生图后端。视频阶段默认 `视频模型路由=自动按镜头路由`。未固定后端时，具体生视频模型/渠道延后到 n2d-video；`_设置.md` 的 `生视频模型/生视频渠道` 只作固定模式、普通镜兜底或调用入口偏好。即使渠道=即梦，也必须在 `生图AI` 中显式写 Dreamina/即梦，不能写含糊的 `同视频AI` 或 `同视频模型`。
- **跨 AI 桥接**：图 AI 与最终生视频模型不一定同厂。已固定生视频模型时，image prompt 末尾拼该模型的"图像风格锚定句"；未固定时用通用视频兼容锚定，n2d-video 路由只能选能消化现有首帧的后端。若用户临时固定不兼容后端，必须提示重出图/重拼锚定或改路由。
