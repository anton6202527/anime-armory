...
| 配音 | n2d-voice | 配音先行：逐句配音 + 时长清单.json。真实配音写 `✅`；任一句占位/估算写 `⏳rough`，不能冒充定稿。原生音画：主流程可选，不作为说话镜硬依赖 |
...
- **当前默认**：生图组合 = `生图模型 + 生图AI/生图渠道` 所选官方/已登录组合（默认 OpenAI GPT Image 系列 via Codex，可选 Dreamina/即梦官方 CLI）；若正式生图前扫到多个可自动落 PNG 的渠道，先给用户菜单和建议并写回选择；若一个都没有，停止并提示准备生图后端。视频阶段默认 `视频模型路由=自动按镜头路由`。未固定后端时，具体生视频模型/渠道延后到 n2d-video；`_设置.md` 的 `生视频模型/生视频渠道` 只作固定模式、普通镜兜底或调用入口偏好。即使渠道=即梦，也必须在 `生图AI` 中显式写 Dreamina/即梦，不能写含糊的 `同视频AI` 或 `同视频模型`。
...
生图渠道：Codex   ← 默认；可换官方/已登录图后端；Dreamina/即梦官方 CLI 可用于图片
```
...
调度（n2d）→ 识别"情境 A 首跑"：
  先给「制作模式」菜单选一次（影响全程出片顺序，不静默默认）：
    A 配音先行（低返工：真实 TTS 先锁时长，再做无声视频）/ B 原生音画（快速预览 / native AV，一次出同步音画）/ C 先出视频后配音（默认·后配音：先用估算/占位时长出无声视频，真实配音后补拟合）
    → 用户选后落 _设置.md（见 SKILL「制作模式 · 首跑选择」）。下面按默认 C 走。
...
n2d-script（阶段1·剧本改编）→
  ...
  6. 报告：第1集剧本齐，下一步按制作模式走——默认先出视频后配音先 n2d-voice 产占位/估算时长（配音=⏳rough），再回 n2d-script 做阶段2分镜；配音先行才用真音时长驱动镜头，原生音画才直接回 n2d-script 做阶段2分镜
