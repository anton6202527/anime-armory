...
## BGM 机器合同
先运行 `python3 skills/n2d/n2d-compose/bgm_contract.py <作品根> 第N集 --write-missing`，再填写并签收 `合成/第N集/bgm_contract.json`。可选 `licensed_file`、`generated`、`none`；`placeholder` 仅内部粗剪且 review 必阻断。真实文件必须由合同 `source.file` 指认，不能只临时 export `BGMFILE` 绕过来源/版权记录。
...
```bash
python3 skills/n2d/n2d-compose/scripts/final_timeline_probe.py <作品根> 第N集 --write --json
