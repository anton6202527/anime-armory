...
def test_audit_placeholders_downgrades_legacy_voice_key_marker(tmp_path):
    root = tmp_path / "demo"
    root.mkdir()
    progress = PROG.replace("| 第1集 | 100 | ✅ | ✅ | ✅ | ✅ | ⬜ |", "| 第1集 | 100 | ✅ | ✅ | ✅ | ✅ | ✅ |")
    (root / "_进度.md").write_text(progress, encoding="utf-8")
    ...
    (voice_dir / "时长清单.json").write_text(
        json.dumps([{"idx": 0, "voice_key": "say:Tingting#placeholder"}], ensure_ascii=False),
        encoding="utf-8",
    )
    script = os.path.join(os.path.dirname(__file__), "progress.py")
...
def test_audit_acceptance_downgrades_fake_review_done(tmp_path):
    root = tmp_path / "demo"
    root.mkdir()
    progress = """# demo
