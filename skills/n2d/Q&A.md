...
- [Q34：能不能“先出视频、后期再配音”？（默认逐镜混合路由）](#q34)
...
### 18.2 为什么 image prompt 要有视频兼容锚定？
...
```
image prompt = 主体 + 外貌锚定 + 动作 + 环境 + 景别 + 画风词 + [固定生视频模型锚定句 / 通用视频兼容锚定句]
                                                          ↑
                                                       接在画风词之后
```
...
**不用。** 同厂/同分布链路确实更稳：训练数据闭环、翻译损耗少、参考图机制更容易互通。但它是 n2d-video 出视频前的路由/固定后端决策，不是开局默认菜单。未固定时先用通用视频兼容锚定，后续由 router/probe 选择能消化首帧的后端。

跨家方案的价值是**成本和可用性**（例如某个官方图后端更便宜或更稳，只在视频阶段付费），代价是需要锚定句和兼容性检查。
...
### 27.1 先锁时间基准，再按镜头决定音画顺序
不是所有镜头都该先配音，也不是所有镜头都该先视频：
- 对白近景/正反打/口型可见：有获批表演或 guide 轨就先音频驱动；没有就先出 neutral-mouth base plate，后置独立口型。
- 旁白/内心/口外音：文本估时推进，最终配音后置。
- 动作/空镜/蒙太奇：画面先行。
- 适合的单镜：可走经能力核验的 native AV。

### 27.2 前期估时不生产占位 WAV
运行 `voice_preflight.py prepare` 生成 `timing_estimate.json`，其中 `audio_generated=false`。它能服务 animatic/字幕初定时和画面先行镜头，但不能冒充可见口型表演或最终混音。音色定妆后才批量生成 final WAV。
...
“一键”减少的是普通选择和重复付款询问，不是审计。`run.py next --json` 只认一个 current frontier，worker 必须按动作卡 `task_id + plan_digest + episode + stage` exact claim；正式生产统一绑定 `n2d_content_fingerprint`。异步视频首次 submit 后只凭原 `submit_id` 恢复，未知付费状态不重提。母版由 RenderTransaction CAS 晋级并产 current MediaArtifactReceipt；整集仍只有 canonical release verdict 与绑定同一母版的 fresh acceptance receipt 才算 `master_delivery_complete`。付费 image/video 由真实人一次签 phase envelope，包内连续执行；成本未知/超额、过期、模型/渠道/scope/input hash 改变才重授权。runner 不能自批，当前像素、版权/合规、不可逆覆盖、公开发布和最终母版签收仍是硬边界。
...
## Q34：能不能“先出视频、后期再配音”？（默认逐镜混合路由）<a id="q34"></a>

**结论**：可以，而且旁白、口外音、动作、空镜和蒙太奇经常更适合后配。默认不再整项目一刀切，而是“声音选角先行，最终配音后置；时间基准先行，逐镜混合路由”。

- 可见口型对白：先有表演轨，或先出中性口型基础视频再做独立 lipsync。
- 旁白/口外音：无 WAV 文本估时，最终声音看画面发挥。
- 动作/空镜/蒙太奇：画面先行完全合理。
- 用户明确要求整集全部先出画面时，才使用项目级 `先出视频后配音`；真实配音偏离估时仍可能引发局部重切。
...
## Q47：Clip 图片生成顺序应该从 Clip_01 开始吗？<a id="q47"></a>

**结论**：**不一定。Clip 图不是铁律要求从 Clip_01 开始；只有选择 `生成优先序=分镜顺序` 时，才按 Clip_01 → Clip_N 叙事顺序生成。**

硬规则只有一条：**共享定妆 / 共享资产永远先于任何 Clip 分镜图**。角色、形态、场景、道具、武器、VFX 等本集会引用的共享资产没有 ready、没有过自检或基础包不完整时，当前阶段只能补共享资产，不能生成 `Clip_*` 分镜图。这条顺序锁不可被 `--skip-preflight`、P0 垂直切片、抽样验证或局部 `--shots Clip_XX` 豁免；runner 会在进入生成循环前执行 `enforce_shared_first_interlock`，所以“先跑一个 Clip 验证”只能是干跑 / prompt 打包，不能实际落 Clip PNG。

共享资产齐后，分镜图的排队由 `n2d-image` 的 `生成优先序` 选择点决定：

- **关键镜优先（默认）**：先出爽点、反转、钩子、封面候选等标 🔑 的镜头，先定基调、先锁主角脸，适合首次打样和快速暴露高风险问题。
- **分镜顺序**：Clip_01 → Clip_N，适合稳定批量、需要顺看叙事连续与逐类接缝是否成立的阶段。
