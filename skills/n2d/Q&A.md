...
- [Q37：n2d-watermark 和 n2d-video-faceswap / n2d-image-faceswap 要做成公共能力还是每条线各自一份？](#q37)
- [Q38：小说怎么拆？「集 → 镜头(分镜) → Clip」三层粒度各自的依据是什么？](#q38)
- [Q39：故事板是怎么来的？就是把几个镜头放一起吗？](#q39)
- [Q40：即梦和 Seedance 是一回事吗？「生视频模型」和「生视频渠道」为什么要分开？固定即梦亏不亏？](#q40)
- [Q41：即梦 multiframe2video（智能多帧）怎么用？跟中段锚帧什么关系？为什么删了「分段故事版参考(拼贴板)」模式？](#q41)
...
**新建留存圣经** `n2d/references/导演节奏.md`（`分镜语法.md` 的姊妹篇）：
...
| **n2d 调度** | 实战参考区挂出 `导演节奏.md`，标为全阶段共用 |
...
- **跨 AI 桥接拼锚定句**：图 AI ≠ 生视频模型（如 GPT Image2 出图 + Seedance 2.0 出视频）时，每条 image prompt 末尾必须拼生视频模型的图像风格锚定句，否则 image2video 运动估计崩（Q18）。
...
> **注：`制作模式` 是唯一"首跑必给菜单"的选择点**——即便全局默认是 `配音先行`，新作品第一次拆集也要把两模式 + 一句原因念给用户选一次（不走"全局默认静默预填"那条），用户选后落 `_设置.md`、之后沉默沿用、随时可改。菜单原话见 SKILL「制作模式 · 首跑选择」段，例外机制见 `skills/n2d/references/选择点与偏好.md`「制作模式」条。本仓库前几集正因早期配音没就绪而走了 B，这就是 B 的合理用途。

**为什么默认配音先行**：本产线的设计是**真实配音时长驱动镜头/Clip 时长**——先量出每句台词念出来多长，再据此切镜头、定 Clip 长。这样音画天然对得准、节奏（黄金3秒/钩子憋放/集尾硬断）可控、返工最少。

**`先出视频后配音` 不推荐的三条理由**（写进 `n2d/SKILL.md`「制作模式」节，各阶段复述）：
...
**实现（怎么"真能跑通"而不只是文档喊话）**：选 `先出视频后配音` 后——① `n2d-voice` 先出占位/估算 `时长清单.json` 当时间脚手架（不追音质）；② 阶段2 finalize 用 `FINALIZE_ALLOW_PLACEHOLDER=1` 放行占位闸门；③ `n2d-video` 占位闸门放行（复述警告后继续）；④ 视频出齐后、合成前**插一步真实配音** `n2d-voice`（CosyVoice/克隆）；⑤ `n2d-compose` 把真音拟合到已成片镜头长。调度器 `n2d` 按 `制作模式` 在 progress.py 线性路由之上叠加这套顺序。
...
| 合规 | 平台披露、水印、授权、审核运行前就有证据 | n2d-compliance + n2d-watermark + gate + dashboard 留痕 |
...
8. **合规与平台披露自动化**：[YouTube](https://support.google.com/youtube/answer/14328491) 对未披露合成内容可能人工加标、处罚或移除；[TikTok](https://support.tiktok.com/en/using-tiktok/creating-videos/ai-generated-content) 会识别 C2PA/AI 内容凭证并要求真实 AIGC 标注；中国《[人工智能生成合成内容标识办法](https://regulations.ai/regulations/RAI-CN-NA-MIASCXX-2025)》已于 2025-09-01 生效，要求显式标识和隐式元数据/水印。n2d 的 n2d-watermark + compliance 方向正确，但要继续补平台 profile。
...
**模型适配层 `n2d-model-router`**：出视频不再固定一个视频模型包打天下。默认 `视频模型路由=自动按镜头路由`，`生视频模型` 只作为普通镜和兜底模型，`生视频渠道` 只决定实际调用入口；每个 Clip 按镜头类型、专项模板、身份注册层、原生音画策略、时长上限和失败风险选择 `primary_backend` / `fallback_backends`。
...
## Q37：`watermark` / `video-faceswap` / `image-faceswap` 等功能要做成公共能力，还是每条线各自一份？<a id="q37"></a>

**结论（2026-06 修订）**：改为**每条线各自一份**——`n2d-watermark` / `n2d-video-faceswap` / `n2d-image-faceswap`，mv/ad 同理（`mv-watermark`…）。早期曾按「公共物理工具（Shared Skills）」设计，本次明确推翻。

### 37.1 为什么推翻：目标从「省代码」变成「可单独打包分发」
原方案把换脸/水印当成像 `ffmpeg` 一样的公共物理工具，理由是省代码、合规改一处。这在「单仓库一起用」时成立。
但实际目标变了：**每条创作线要能被单独 copy/push 出去独立运行**（toa/tov 只推某几条线）。一旦存在公共依赖，单独拿走一条线就缺工具、跑不起来。**“自带全部依赖、零外部引用”压倒了 DRY**——所以换脸/水印随每条线各带一份，`<line>/_lib/` 里的工具模块也各带一份。

### 37.2 代价与缓解：合规逻辑 N 份的漂移风险
这么做的代价是真实的：AI 标识水印是**法律强制项**，复制成多份后改一处易漏其他。缓解措施：
- 保留一个**告警级合规平价校验脚本**（不阻断、可删），断言各线 `*-watermark` 的 AI 标识核心逻辑与基准一致；
- 改合规逻辑时全线 `grep` 同步。

### 37.3 独立性如何保证
每条线的视觉/剧情**业务逻辑**本就互不 import；现在连工具层也各自一份后，`grep` 独立性闸门可断言 `skills/<line>/` 内不出现 `from common`、不出现别线前缀、不出现 `shared-` 引用——任何回流即失败。

> 一句话原则（修订后）：**每条线自包含、可单独分发**优先于省代码；公共层（`skills/common/`、`shared-*`）拆除，工具随线复制，用独立性闸门 + 合规平价校验兜住重复带来的漂移风险。

---

## Q38：小说怎么拆？「集 → 镜头(分镜) → Clip」三层粒度各自的依据是什么？<a id="q38"></a>

**结论**：粒度分**三层**，依据各不同，一条总纲贯穿：**节拍盖过字数，配音盖过估算**。

```
小说 ──①──→ 集 ──②──→ 镜头(分镜) ──③──→ Clip(故事板) → 视频
        戏剧节拍       配音时长+节奏        后端单次上限
```

**① 小说 → 集（episode）**：一集 = 一个完整戏剧节拍（冲突→爽点→反转→钩子），不是一章。三层拆：时长→字数 band 出粗胚 → 按 P0–P6 把边界 snap 到"结尾断点 + 完整憋放闭环 + 下集能冷开场"处 → 配音实测时长校准。详见 `n2d-script/references/拆集法.md`。

**② 集 → 镜头（分镜，问得最多的一层）**——四条依据，主次分明：
1. **配音实测时长 = 量化硬锚（核心）**：分镜在配音**之后**做。`镜头占屏 = 该镜台词时长 + 气口留拍`，且 `∑镜头 == voice.wav`（`finalize_storyboard.py`）。单镜台词过长 → 拆多镜。这是 n2d 区别于"按字数估/每镜固定秒数"的招牌。
2. **节奏/张力曲线（定长短不均）**：等长=催眠。铺垫/留白用长镜 3–5s，爽点/打斗/吵架用碎切 1–2s，临近爽点逐个变短，爽点后 1–2s 留白（`导演节奏.md §四`）。
3. **内容类型（定拆法）**：多人/复杂动作拆单人正反打；打斗按五帧拆招；仙侠奇观按 `仙侠场面分镜.md`；复杂镜先选 `专项镜头模板库.md` 模板。
4. **镜头数 band（兜底量纲）**：每集 6~16 镜，随 `单集时长` 浮动，是验收范围不是目标。

**③ 镜头 → Clip（故事板分组）**：相邻分镜合成 Clip（每 Clip 1~2 镜），`Clip 时长 = 所含镜头时长之和`，**上限按所选后端**（即梦≤8s / Seedance≤15s…）。能一镜到底就别为凑时长切碎（更少接缝、跨镜更稳）。

> 一句话：**不是"一句一镜"或"N 秒一镜"的机械切法——配音逐句时长当量化锚 + 节奏曲线塑长短 + 内容类型定拆法，三者叠加。** 注意占位配音的镜头时长是估算值，补真音后阶段2 要回跑重定时（占位闸门拦的就是这个）。

---

## Q39：故事板是怎么来的？就是把几个镜头放一起吗？<a id="q39"></a>

**结论**：表层是"把相邻分镜合成 Clip"，但**分组只是表皮——故事板真正的身份是契约层**。把它只看成分组就低估了。

**分组依据（几个镜头能放一起）**：每 Clip = 1~2 个分镜（也可能 1 镜单独成 Clip）。合并规则——① 只合**相邻**；② Clip 时长不超后端单次上限；③ **节奏曲线优先**（铺垫长镜可合并、爽点碎切保持碎不合）；④ 能一镜到底就别切碎；⑤ 换场/时间大跳处必断。所以不是"每 N 镜一组"，是**在时长上限内、按节奏和连续性把相邻镜头按需合并**。

**但故事板每个 Clip 还必带一整套契约**（这才是它存在的理由，缺则 gate 阻断）：

| 契约块 | 管什么 | 下游谁吃 |
|---|---|---|
| **continuity / 衔接设计** | 上一 Clip 出点=下一 Clip 入点（同一句）、转场、需要尾帧? | n2d-image 出尾帧、n2d-video 焊接点、compose 接法 |
| **visual_contract**（keystone） | 景别阶梯 / 轴线·视线 / 光位 / 状态演进 | n2d-image 烤进首帧像素 |
| **style_contract** | 风格名/基调/光色/运动边界 | 出图/出视频继承 |
| **template_contract** | 复杂镜 blocking/camera_rule/关键帧 | 出图/出视频锁动作 |
| **节奏注记** | 铺垫·长镜/加速·碎切/爽点·CU硬切/留白·定格 | 决定镜头时长曲线 |

视频改不动的导演决策（轴线/光位/状态/景别）都在这层定死——这是"前期价值"的落点。**关键帧全从这层派生**：首帧=Clip 起幅、尾帧=`continuity.end_state`、中段锚帧=`continuity.anchors`。

> 一句话：**分组定"一次生成多长一段画面"，契约定"这段画面怎么接、什么景别光位、怎么连贯"。故事板就是"敲定每个 Clip 要哪些关键帧"的地方。**

---

## Q40：即梦和 Seedance 是一回事吗？「生视频模型」和「生视频渠道」为什么要分开？固定即梦亏不亏？<a id="q40"></a>

**结论**：**即梦底层就是字节 Seedance**。所以把选择点拆成 `生视频模型`（用哪个模型）+ `生视频渠道`（经哪个产品/API/CLI 调）——同一个 Seedance 模型，可经**即梦官方 CLI** 调，也可经**火山方舟 API** 调。早期文档把"即梦"和"Seedance"当并列后端，是错的。

**证据**（`n2d-video/references/platforms.md`）：即梦 CLI 的 `text2video`/`frames2video`/`image2video` 用的就是 `seedance2.0` 系列模型（单镜 4–15s），`multimodal2video` 是 Seedance 2.0 旗舰「全能参考」，`multiframe2video` 是「智能多帧」。锚定句也"同即梦（字节自家，训练分布相同）"。

**固定即梦亏不亏 —— 大多不亏**：
- 旧顾虑"即梦 8s 装不下多镜、要换 Seedance/可灵"是**错的**：即梦 `frames2video` 用 seedance2.0 就 4–15s，`multiframe2video` 更是 2–20 关键帧出长镜。
- "换 Seedance 要新账号/积分"也**不成立**：都在字节生态里，同模型同训练分布，连"多后端混用导致跨镜质感跳变"的顾虑都几乎没有。
- 固定单渠道的**真实好处**：账号/积分集中、跨镜质感统一、工具链可预测、流程简单、合规水印好统一。
- 真正吃亏的是**打斗/长镜/奇观**多的集——那时把 `视频模型路由` 改回 `自动按镜头路由`、给复杂段配更擅长的模型即可。

> 一句话：**问"用什么模型"和"经什么渠道调"是两件事；即梦=Seedance 的一个渠道，长镜/多镜在即梦内原生可做，不必为此换厂。**

---

## Q41：即梦 multiframe2video（智能多帧）怎么用？跟中段锚帧/三帧契约什么关系？为什么删了「分段故事版参考(拼贴板)」模式？<a id="q41"></a>

**结论**：`multiframe2video` 是即梦官方 CLI 的原生多关键帧能力（**2–20 张关键帧 → 一条连续视频**，每段 0.5–8s、总≥2s、自动运镜、原生消除拼接"刹车感"）。它是**中段锚帧链的原生执行路径**，并让旧的「分段故事版参考(拼贴板)」模式彻底过时——后者已于 2026-06 退役删除。

**和中段锚帧/三帧契约的关系（执行分两条路，video_runner 按后端能力自动选）**：
- **① 原生多帧（首选·即梦）**：把 `[首帧, 中锚1..K, 尾帧]` 一次喂给 `multiframe2video` → 模型自做帧间插值/运镜，**无内部焊缝、无 concat、不翻倍视频成本**。`continuity.anchors` 的 `at_sec` → 每段时长（校验 [0.5,8]）。规划层（anchor_planner / 三帧契约）产物完全不变，只是执行器选了这条。
- **② 拆段接力 + concat（兜底）**：只收两帧的后端才退回"拆 K+1 段 frames2video 再 ffmpeg 焊回"。
- 注意：`multiframe2video` **不支持 model_version/分辨率覆盖**（比例随首帧、分辨率走后端默认）——要控分辨率走 image2video/frames2video。

**为什么删拼贴板模式（"本来就不合理"）**：
1. `分段故事版参考` 把多帧**压成一张网格大图**喂后端。但主流后端（即梦多帧/可灵多图/Veo reference）**全都收分离的关键帧数组**，没谁要网格板——拼成板反而降采样、还有"整板被动画化"的穿帮风险。它从一开始就是对"多参考后端怎么工作"的误解。
2. 同选择点的 `后端原生多镜` 值**根本没代码实现**，只是个声明。
3. 真正的"多关键帧→连续镜"现在由 `continuity.anchors`(故事板) + `multiframe2video` 自动驱动，**不该是一个要用户手选的"模式"**。

所以整个 `视频故事版模式` 选择点连同 `storyboard_board.py`/`segment_slicer.py` 一并退役。**注意别混淆两个"故事板"**：删的是"把帧拼成大图"的**拼贴板**；`storyboard.json` 那个**导演故事板**（定景别/构图/顺序/情绪、派生关键帧）**保留不动**，它才是前期价值所在。

> 一句话：**故事板(导演设计) → n2d-image 出关键帧(首/中锚/尾) → multiframe2video 原生收多帧出连续镜**——这条干净链路取代了"拼贴板敲关键帧"的绕路。各渠道 CLI 真实参数以 `references/cli_snapshots/` 的 `--help` 快照为准（`probe_cli.py` 抓取/定期校验）。

---

## Q42：开了三帧契约后，是不是每个 Clip 都至少有 3 帧（首+中锚+尾）了？<a id="q42"></a>

**结论**：**不是，也不该是**。三帧契约（`policy.midframe_default=true`）让**大多数** Clip 有"首帧+中段锚帧+尾帧"，但**两类镜头天然豁免**，只有 2 帧——这是契约设计里就留好的口子，不是漏洞。

**两类 2 帧豁免**：
1. **极短镜（<3s）**：中帧和首/尾几乎重合、无信息量，硬塞中锚是浪费 → 写 `midframe_exempt_reason` 豁免，保留"首+尾"。
2. **最终镜**：没有"下一镜"可接，本就无尾帧 → 是"首+中锚"。

> 实测某集 20 个 Clip：16 个 3 帧；4 个 2 帧（3 个 <3s 极短镜豁免中锚=首+尾；1 个末镜无尾帧=首+中锚）。`gate` 接受这 4 个，因为它们满足"声明了 `midframe_exempt_reason` 或本就是末镜"。所以"每镜≥3帧"不是一刀切目标。

**一个必须分清的关键区别 —— 契约帧数 ≠ 已生成的图**：
- storyboard 里**声明**了几帧（`continuity.anchors`/`need_endframe`）是**契约层**；
- 这些帧对应的 PNG（尤其 `_mid` 中锚）**是否已经出图**是**像素层**，两者常不同步。
- 例：某集契约上 16 镜声明了 3 帧，但 17 张 `_mid` 中锚 PNG 还没生成（出图 68/85），真要走 `multiframe2video` 得先 `/n2d-image` 补出这些 `_mid` 图。
- 查的时候：契约看 `storyboard.json`，像素看 `出图/第N集/图片/` 实际文件数 + `_进度.md` 的 `出图 X/Y` 分母（分母已含中锚目标）。

> 一句话：**三帧是默认不是铁律（极短镜/末镜豁免）；而且"声明了3帧"≠"3张图都出了"，付费出视频（multiframe2video）前要先确认 `_mid` 图真的生成了。**

---

## Q43：长镜头、打斗镜头会出现超过 3 帧（首+多中锚+尾）吗？<a id="q43"></a>

**结论**：**会**。三帧契约（首+1中锚+尾）只是普通镜的默认底；**打斗/长镜由 anchor_planner 的 R1/R2/R3 规则出多个中锚**，帧数随时长涨：10s 打斗→4 帧、15s 打斗→5 帧。multiframe2video 最多收 20 帧、每段 0.5–8s，完全装得下。

**多帧从哪来**（`anchor_planner`）：
- **R1 高运动模板镜**（fight_exchange/chase/magic_burst/flight/hug_or_pull/intimate_interaction）：段目标更短 `fight_target=3.5s`，一拍一段贴换招 → 锚帧最密。
- **R2 ≥8s 多拍长镜**：段目标 `target_seg=5s`，密度适中（长对话/establish 不必像打斗那么碎）。
- **R3 漂移实证镜**：dashboard 有中段漂移 redraw 记录的镜头。
- 普通镜（未命中 R1/R2/R3）：开三帧契约时只得 1 个 `_mid`（3 帧），极短镜/末镜豁免（见 Q42）。

**踩过的坑（2026-06-13 修）—— 两个"最短段地板"别混**：早期 `anchor_planner` 只有一个 `min_seg=4.0s` 地板，是**旧 frames2video 拆段时代**留的（每段当独立 clip 要 ≥4s）。接 multiframe2video（每段只要 ≥0.5s）后忘了改，导致 `fight_target=3.5s < 4s` **被架空**——10s 打斗只出 1 锚（3 帧），多帧能力没真正用上。修法是**解耦两个地板**：

| 地板 | 值 | 管什么 |
|---|---|---|
| `min_seg`（relay 拆段地板） | 4.0s | R1/R3 触发门槛（≥2×=8s 才值得加锚）+ D0 的 use=split/qc 判定 |
| `multiframe_seg_min`（多帧密度地板） | 1.5s | R1/R2/R3 给后端排**锚帧密度**——让打斗/长镜真出多锚 |

修后：打斗 10s→2 锚(4帧)、15s→3 锚(5帧);长镜密度由 `--target-segment` 调。**触发门槛没放宽**（短镜不会被过度加锚），只是"命中后出几锚"对齐了 multiframe 的真实能力。

> 一句话：**普通镜 3 帧封顶（默认），打斗/长镜按 R1/R2/R3 出 4、5… 帧（时长越长越密）；密度地板要跟执行后端走——multiframe 用 1.5s，别拿旧的 4s relay 地板卡死。**

---

## Q44：n2d-score 和 n2d-review 的关系？各司其职，还是 n2d-review 会调起 n2d-score？（为什么和 novel 那对不一样）<a id="q44"></a>

**结论**：**不是对称"各司其职"，是分层管道——`n2d-review` 触发/编排 `n2d-score` 作下游一步**。数据流单向：`n2d-review`（检测）→ `n2d-score`（聚合打分）→ `n2d-review-ui`（人审画布）。n2d-score 站在 n2d-review 的输出之上，不反向调 review。

**文档依据**：
- n2d-review 输出"…**按需触发 score/review-ui/batch 的下游产物**"；其作品质检流程第 3 步就是跑 `n2d-score/scripts/score.py --run-checks`。
- n2d-score："把 **n2d-review 的确定性机检**、dashboard 数据、gate/review findings…汇总成每集机器分"；"**n2d-review 负责产生确定性机检和 gate finding**"。

**分层里的分工**：

| | n2d-review | n2d-score |
|---|---|---|
| 角色 | **检测引擎 + 质检流程编排者** | **打分/聚合/自动回流层** |
| 干什么 | 跑 `mechanical_check`/`consistency_audit`（崩脸/服装/场景/接缝/字幕/语义/状态/多模态…）出**逐条 findings**，人判，定位到镜头 | 把 findings + dashboard + visual checks **汇总成七维机器分**，按阈值判"该自动回流哪个 stage"，`--enqueue-low` 推 `n2d-batch` 返工 |
| 找新问题吗 | **找**（它是检测器） | **不找**——只聚合已有 findings 成分数 + 阈值决策 |
| 取代人判吗 | 机检+人判 | 明确**不取代人判**，机器分喂 review-ui 画布 |

一句话：**n2d-review 是"查"，n2d-score 是"把查的结果打成分 + 决定自动返工"。**

**为什么和 novel 那对不一样（命名平行，关系不同·最易踩坑）**：
- **novel**：`novel-score`（能不能火/值不值得改·市场+品质，读热榜+战绩库）vs `novel-review`（硬伤：视角穿帮/OOC/plot hole）。问的是**两个正交问题**（值不值得 vs 对不对），互相独立调用 → **真·各司其职**。
- **n2d**：`n2d-score` **不是"能不能火"**，它和 n2d-review 查的是**同一类东西**（一致性/QA：语义继承/状态/角色/字幕/音画同步…）。review 是检测引擎，score 是它之上的**打分+自动回流层** → **分层管道，不正交**。
- n2d 里对应"能不能火"的是 `n2d-feedback`（投放回灌）+ 脚本阶段的 `novel-score`，不在 n2d-score 身上。

> 一句话对比：**novel 的 score/review = 两种问题各管一摊；n2d 的 score/review = 同一摊事的两层（review 出原始 findings，score 聚合成分数并决定返工）。**
