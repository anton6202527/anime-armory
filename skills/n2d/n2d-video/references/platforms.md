...
## 四项架构：内容轴(剧情) × 生图模型/渠道 × 生视频模型/渠道（2026-06 起）

skill 改为**生图模型 + 生图渠道 + 生视频模型 + 生视频渠道**：
- **生图模型**：决定首帧/定妆的生成能力、参考图预算、编辑能力和主体一致性能力（如 GPT Image 2 / Seedream / Nano Banana Pro）
- **生图渠道**：决定实际通过哪个产品/API/CLI 调用生图模型；旧字段 `生图AI` 只作渠道/访问入口兼容名（如 Codex CLI / OpenAI API / Dreamina 官方 CLI）
...
1. 若项目已固定生视频模型，image prompt **必须**在末尾追加目标生视频模型的"图像风格锚定句"（见各档案）
2. 锚定句作用：约束生图模型输出**生视频模型训练分布里的视觉特征**（面孔/风格/光感）→ 让视频模型的 image2video 运动估计稳定
3. 若未固定生视频模型，出图阶段使用通用视频兼容锚定并记录 `video_backend_decision=deferred`；n2d-video 路由必须优先选择能消化现有首帧风格的后端
4. 生图按 `生图模型 + 生图渠道/生图AI` 统一到一组官方或已登录入口；视频模型读路由表，普通镜/兜底读 `_设置.md` 的 `生视频模型`，调用渠道读路由表与 `生视频渠道`，不要求新作品首跑先选具体后端
5. 锚定句语言：按目标生视频模型/渠道的消化风格选；即梦/可灵渠道通常用中文版，Veo 等海外渠道用英文版
...
```
主体 + 外貌/妆造锚定 + 动作表情 + 环境/光线 + 景别构图 + 画风词 [+ 固定模型锚定句 / 通用视频兼容锚定句]
                                                                  ↑
                                                              固定后端拼对应模型；未固定拼通用兼容
```
...
> **分辨率铁律**：所有平台**默认 720p**（省积分/出片快），普通缺省时直接写入 `_设置.md`；用户已明确要求 1080p 时按用户值。实际分辨率、模型、渠道、调用数与成本上界在阶段预算包创建时一次绑定，包内不为分辨率重复暂停。

> **时长分层铁律（2026-07）**：后端上限只约束 `backend_request_sec`，不能决定剪辑语法。先按 storyboard 的景别/机位变化把父 `story_clip` 拆成物理 `video_shot`，再把每条 `edit_target_sec` 量化到后端允许的请求档位。一个 5s 父 Clip 若含 ECU→CU 两个明确镜位，就是两条 take；一个 2.1s take 遇到后端最短 4s，就请求 4s、2.1s 后保持落幅并裁尾，不得整段变速或把情节拉长。只有单一镜位、单一连续动作且风险可控时才考虑长 take；连续 take 超后端上限仍须拆段。机器真值见 `n2d_platform_profiles.py::duration_control/quantize_video_duration`。
...
用户问“每个 Clip 分几张帧”时，先回答能力边界：**不设固定张数。普通单拍通常首帧或首尾两帧；首/中/尾三帧不是主流统一能力，只有 Dreamina 原生多帧已在本仓库打通。** 多镜位先拆成独立 take，每个 take 再选首帧/首尾帧；只有高风险连续动作才规划中段锚。未知/新后端不默认认定支持多帧，必须先刷新官方/API/CLI 能力证据并落 smoke 或人工确认；`use=qc/reference` 的中锚不得作为时间轴输入，需中段锁定时改拆段接力或换原生多帧后端。
...
|---|---|---|---|---|
| 打斗 / 命中 / 双人接触 | 可灵 Kling | Seedance / 即梦 | 首尾帧、运动笔刷、多主体互动、Character ID；后续可接 ComfyUI/LTX pose/depth/instance 控制 | `mode=frames2video`；`motion_control=required`；一 Clip 只做一个命中动作；必须有 ready manifest，或有 `degrade_only` 实现分解 manifest |
...
- **原生音画策略**：`配音先行` 不接管 n2d 配音；仅低风险环境声/动作音效 opt-in，正面说话镜走配音或 voice-conditioned lip-sync。`制作模式=原生音画` 且 route 写 `native_audio_policy=native_speech` 时，才允许说话镜保留原片音轨，并走 native AV 字幕/声纹/合规 gate
...
- **多镜单次生成（multishot_native·2026-06）**：两类后端登记此能力——① **Seedance 2.0** 一次出多镜头叙事，跨镜人物一致、转场无缝（单次最多吃 9 图 + 3 视频 + 3 音频参考）；② **可灵 Kling 3.0** 单次最多 **6 个连续镜头 + 共享音轨时间线**、字符一致性增强。对连续接力镜组，比「更长单镜」再进一步——一次 co-generate 整段彻底消缝。`n2d-model-router` 对 primary 后端支持多镜（能力字段 `multishot_native`，由 `MULTISHOT_NATIVE_BACKENDS` 自动收录，不 hardcode 厂商）的连续接力镜组标 `multishot_groups` 候选（**advisory**：提示可一次出，不自动合并、不改逐 Clip mode，保逐 Clip 可重跑粒度——这也化解了与「配音先行」的张力：advisory 不强制合并，配音先行项目可照常逐 clip 出 + 缝合）。注意：**即梦渠道执行后端是 Dreamina `multiframe2video`（原生多关键帧，非多镜叙事），多镜单次生成在即梦渠道未核验**——直连 Seedance / 可灵 API 才走这条；付费批量前按各后端 `frame_control.verified` 复核当前 API 是否暴露多镜接口。
...
- **原生音画策略**：原生同步音频强；`配音先行` 默认不让 Veo 生成角色台词，海外/预览可对空镜环境声 opt-in。`制作模式=原生音画` 且 route 写 `native_audio_policy=native_speech` 时，可保留原片音轨；若项目已有 n2d-voice 配音轨，必须明确选择配音轨或原生音轨，避免双人声
...
3. 在项目 `global_style.md` 的"视频模型路由 / 生视频后端决策 / 目标生图模型与渠道"三行记上即可。**核心分镜/角色卡/字幕无需改动**——只换 prompt 的平台适配层。
