    ...
    if not suffix:
        if _requires_dialogue_fact_contract(prompt):
            raise RuntimeError(
                ...
                f"{episode} {clip}. Run `python3 skills/n2d/n2d-review/scripts/dialogue_fact_guard.py "
...
def verify_cli_contract(cli: str, command: str) -> None:
    ...
    if not ok:
        raise RuntimeError(
            ...
            f"Re-run `python3 skills/n2d/n2d-video/scripts/probe_cli.py probe` to refresh snapshots "
...
def update_progress(root: Path, episode: str) -> None:
    ...
    progress_py = SKILLS_DIR / "progress.py"
