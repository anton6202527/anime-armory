#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""源同步守望 —— 收尾 hook 可用。扫所有有源指纹的漫剧，源文件一改就提示。

每次 agent 回合结束可由 hook 调用：遍历 `创作区/制漫剧/*/小说/_源指纹.json`
（兼容旧 `制漫剧/*/小说/_源指纹.json`），
对比当前本剧 `小说/*.txt` 与基线指纹，**只在发现漂移时**打一行提醒
...
def main():
    ...
    drifts = []
    patterns = [
        os.path.join(repo, "创作区", "制漫剧", "*", "小说", "_源指纹.json"),
        os.path.join(repo, "制漫剧", "*", "小说", "_源指纹.json"),
    ]
    ...
    if drifts:
        print("⚠️ 源同步提醒：本剧源文件已变动，对应漫剧源过期——")
        for name, ch, ad, rm, risky in drifts:
            ...
            print(line)
        print("  → 看落集+处置：`python3 skills/n2d/source_check.py <漫剧作品根>`；"
              "处理完用 `--record` 更新基线。")
    sys.exit(0)
