...
SKILL_FILE_STAGE_HINTS: Dict[str, Tuple[Tuple[str, Tuple[str, ...]], ...]] = {
    "n2d-script": (
        ("script_stage2", ("finalize_storyboard", "validate_timings", "delete_shot",
                           "validate_storyboard_contract", "story_quality_pack", "script_quality_gate",
                           "director_blocking_pack", "production_breakdown", "setup_payoff_ledger",
                           "beat_audit", "spectacle_contract_audit",
...
N2D_LIB_FILE_STAGE_HINTS: Tuple[Tuple[str, Tuple[str, ...]], ...] = (
    (
        ...
        (
            ...
            # 作品卡简介最早在立项/开发包阶段生成；封面回填是同一字段合同的后续消费者。
            "_lib/work_card_meta.py",
        ),
    ),
    (
        "voice",
    ...
    (
        ...
        (
            ...
            "_lib/n2d_intent.py",
            # 非人物 insert 覆盖直接改变分镜构成，最早从 P-2 分镜细化回放。
...
N2D_IMAGE_SHARED_LOCK_RULE_FILES: Dict[str, str] = {
    "skills/n2d/n2d-image/SKILL.md": "skill_rule_unknown",
    "skills/n2d/n2d-image/references/prompt_format.md": "prompt_format",
    "skills/n2d/n2d-image/references/角色一致性checklist.md": "identity_checklist",
    "skills/n2d/n2d-image/references/资产身份注册层.md": "asset_identity_registry",
    "skills/n2d/n2d-image/references/资产引用注册层.md": "asset_reference_registry",
    "skills/n2d/n2d-image/references/lora_consistency.md": "lora_consistency",
    "skills/n2d/n2d-image/references/platforms.md": "backend_capability_rules",
}
...
def n2d_image_shared_lock_impact(rel_path: str) -> Optional[str]:
    ...
    if rel_path.startswith("skills/n2d/n2d-image/references/"):
        return "unknown_reference_rule"
    if rel_path == "skills/n2d/n2d-image/SKILL.md":
...
def scoped_changed_files(
    ...
    def scoped(files: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            if skill_name_for_path(os.path.join(REPO_ROOT, k)) in allowed and not is_test_path(k)
        }
...
def steps_for_strict_image_refresh(root: str, ep: str, start_key: Optional[str], until_key: str) -> List[Dict[str, str]]:
    ...
    qc = f'python3 skills/n2d/n2d-image/scripts/image_qc.py "{root}" {ep}'
    return [
        # 1) 按最新 skill 刷新分镜/出图 prompt，生成新的审查标准。
        command_step(
            f'python3 skills/n2d/n2d-batch/scripts/queue.py plan "{root}" --episodes {epn} '
...
def image_qc_verify_command(root: str, ep: str) -> str:
    ...
    return (f"python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate \"{root}\" {ep} --stage image"
    ...
    for stage_key in stage_keys:
        ...
        if gate_stage:
            steps.append(command_step(
                f"python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate \"{root}\" {ep} --stage {gate_stage}",
                role="refresh_gate",
            ))
    return steps
...
def review_refresh_steps(root: str, ep: str, until_key: str) -> List[Dict[str, str]]:
    ...
    if stage_index(until_key) >= stage_index("image"):
        steps.append(command_step(
            f"python3 skills/n2d/n2d-review/scripts/consistency_audit.py \"{root}\" {ep}"
            "  # 刷新 review findings；不重制产物",
            role="refresh_review_findings",
        ))
    if not steps:
        steps.append(command_step("python3 skills/n2d/n2d-review/scripts/self_audit.py --json",
                                  role="self_audit"))
    return steps
...
def steps_for_rerun(root: str, ep: str, start_key: Optional[str], until_key: str,
    ...
    if not start_key:
        gate_stage = (stage_for_key(until_key) or {}).get("gate_stage")
        if not gate_stage:
            return [command_step(f"python3 skills/n2d/n2d-update/scripts/update_plan.py record \"{root}\" {ep}",
                                 role="record_snapshot")]
        return [
            command_step(f"python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate \"{root}\" {ep} --stage {gate_stage}",
                         role="refresh_gate"),
        ]
    ...
    steps: List[Dict[str, str]] = [
        command_step(
            "python3 skills/n2d/n2d-batch/scripts/queue.py plan "
...
def build_plan(root: str, ep: str, *, regen_mode: Optional[str] = None) -> Dict[str, Any]:
    ...
    if image_in_scope and shared_lock_reuse:
        ...
        notes.append(
            ...
            "`python3 skills/n2d/n2d-image/scripts/asset_impact.py <作品根> <改动的定妆资产>` "
            "级联出引用它、需跟着重出的本集分镜。"
        )
    ...
    if image_consistency and image_consistency.get("freshness") == "stale":
        notes.append(
            ...
            "当前结论不可信，先重跑 `python3 skills/n2d/n2d-image/scripts/image_qc.py <作品根> "
            f"{ep}` 再据此判断。"
        )
    ...
    if contract_inheritance and not contract_inheritance.get("inherited"):
        ...
        if st == "missing":
            notes.append(
                ...
                f"`python3 skills/n2d/n2d-video/scripts/inherit_contract.py <作品根> {ep}`，"
    ...
    if contract_inheritance and contract_inheritance.get("status") == "ok":
        if contract_inheritance.get("freshness") == "stale":
            notes.append(
                ...
                f"`inherited` 结论不可信，先重跑 `python3 skills/n2d/n2d-video/scripts/inherit_contract.py <作品根> {ep}` 再判。"
            )
