...
def n2d_plan(root: str, episode: Optional[str], image_targets: List[str], video_targets: List[str]) -> Dict[str, Any]:
    ...
    steps: List[Dict[str, str]] = [
        command_step(f"python3 skills/n2d/n2d-update/scripts/update_plan.py check {quote(root)} {episode} --write-plan --regen-mode 严审刷新"),
    ]
    if image_targets:
        target_flags = repeat_flag("--affected-shot", image_targets)
        queue_cmd = (
            f"python3 skills/n2d/n2d-batch/scripts/queue.py plan {quote(root)} --episodes {epn} "
        ...
        steps.extend([
            command_step(f"python3 skills/n2d/n2d-image/scripts/image_qc.py {quote(root)} {episode} --regen-list"),
            agent_step(
            ...
            agent_step(
                ...
                f"再执行验收：`python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate {quote(root)} {episode} --stage image`；"
    ...
    if video_targets:
        ...
        steps.extend([
            command_step(
                f"python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate {quote(root)} {episode} --stage video"
            ...
            agent_step(
                ...
                f"再执行验收：`python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate {quote(root)} {episode} --stage video`；"
                "若只是生成队列计划，不能把这一步当成已验收。"),
        ])
    flow = [
