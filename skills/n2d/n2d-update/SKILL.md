...
- **帧策略合同遵循**：读 `storyboard.json`，只把 E1 明确镜位切点、R1/R2/R3 高风险连续动作、显式 `midframe_default_mode=explicit_opt_in` 或已声明 `frame_strategy=native_multiframe|split_relay|edit_cut` 的 Clip 计为必须有执行锚；普通 risk-only 镜不因后端支持多帧就强制三帧。同时核验所有已声明中锚/尾帧 PNG 是否存在，缺口指回 `anchor_planner.py --write` 与 n2d-image。
...
```bash
python3 skills/n2d/n2d-update/scripts/update_plan.py check <作品根> 第1集 --write-plan
python3 skills/n2d/n2d-update/scripts/update_plan.py record <作品根> 第1集
python3 skills/n2d/n2d-update/scripts/update_plan.py check <作品根> --all --write-plan
# 少量图片/视频选择性刷新（证据驱动，不审片）：
python3 skills/n2d/n2d-update/scripts/update_plan.py media <作品根> 第1集 --image Clip_001 --video Clip_002 --write-plan
```
...
```bash
python3 skills/n2d/n2d-update/scripts/update_plan.py media <作品根> 第3集 --image Clip_001,Clip_002 --video Clip_004 --write-plan
```
...
```bash
python3 skills/n2d/n2d-update/scripts/update_plan.py check <作品根> <集号> --write-plan
```
...
- `three_frame_compliance`（字段名为旧兼容）：帧策略合同（`required_anchor_clips` 为 E1/R1-R3/显式 opt-in，`violating_clips` 为其中缺执行锚的 Clip，另含尾帧与缺文件清单）。storyboard 未定稿为 `null`。
...
- 确认重制范围后：python3 skills/n2d/n2d-batch/scripts/queue.py plan <作品根> --episodes K --rerun-from <stage> [--affected-shot …]
- 或直接回跑对应 stage skill（n2d-image / n2d-video / …）从 <rerun_from> 起
- 重制结束后记录新基线：python3 skills/n2d/n2d-update/scripts/update_plan.py record <作品根> 第K集
