...
- **通过率两层定义固定（2026-07 审计对齐代码）**：`generation_pass_rate = generation_passes / (generation_passes + generation_fails)`（生产尝试效率）；`deliverable_pass_rate` = 存在任一 QA 阻断即 **0**，否则等于 generation_pass_rate（可交付口径·fail-closed）；**`final_pass_rate` 现为 `deliverable_pass_rate` 的向后兼容别名**，不再是旧公式——对外验收/告警一律看可交付口径。没有记录 pass/fail 时显示 `—`，不得凭感觉补。
...
- **首集速度显式量化**：dashboard 另算 `flow_speed.time_to_first_episode_hours`（首条生产事件→第1集 compose/review 首次无 block）和 `gates_passed_per_day`。它用于识别 gate 堆叠造成的窗口期损耗，不降低任何发布标准；内部快跑只能复用缓存/少跑 advisory 报告，不能跳过 load-bearing gate。
...
- **行业基准只作参照线，不作闸门**：`dashboard.md` 的「行业基准对照」段把一次通过率/重抽率/每分钟成本/跨集一致性并排到行业宣传基准（默认读 `references/industry_benchmark.json`，采集 2026-06），给一条「你 X% vs 行业 Y%」的达标/差距参照。**这是只读参照、不参与告警/阻断**（厂商口径不一、会过期）。覆盖：`_设置.md` 写 `基准一次通过率 / 基准重抽率`，或 `生产数据/industry_benchmark.json`；基准本身以一次 `n2d-review` 流程自审复核为准，刷新基准只改 JSON，不改 Python。留存基准必须通过 `python3 skills/n2d/n2d-dashboard/scripts/validate_benchmark_schema.py`：每条首屏/留存/App D1-D14 信号都要有 `source_ids / definition / checked_at / expires_at / confidence`，否则不能合入。项目级覆盖若包含无效 `retention_benchmarks`，运行时会保留仓库参考留存线，并在 dashboard 输出 schema 错误，防止旧/脏基准静默污染节奏与反馈分析。
...
```bash
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate <作品根> 第1集 --stage image_preflight   # 正式生图前
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate <作品根> 第1集 --stage image             # 生图后落档回验
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate <作品根> 第1集 --stage video_preflight   # 正式出视频前
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate <作品根> 第1集 --stage video             # 出视频后落档回验
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate <作品根> 第1集 --stage compose
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate <作品根> 第1集 --stage review
```
...
```bash
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py build <作品根>
...
python3 skills/n2d/n2d-dashboard/scripts/event_ledger.py audit <作品根> --write
python3 skills/n2d/n2d-dashboard/scripts/event_ledger.py audit <作品根> --strict-trace --write
...
python3 skills/n2d/n2d-dashboard/scripts/event_ledger.py replay <作品根> --write
```
...
```bash
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py forecast <作品根> 第2集 --budget 100 --unit CNY
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py forecast <作品根> 第2集 --json
```
...
```bash
python3 skills/n2d/n2d-identity/scripts/identity.py <作品根> --episodes 1-N --write   # 先落档漂移报表
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py identity <作品根>               # 再看 KPI（--json 喂回 LLM）
```
...
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py record <作品根> ... --notify
# 轮询常亮看板：events 变化即重建，本机起 http.server 看自动刷新 HTML
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py watch <作品根> --interval 15 --serve 8787 --notify
# 单次（适合 cron / CI）：cron 行 = */5 * * * * python3 .../dashboard.py watch <作品根> --once --webhook $URL
python3 skills/n2d/n2d-dashboard/scripts/dashboard.py build <作品根> --html --fail-on-critical
```
