...
**Review warn 回灌口径**：`demo/standard` profile 可以在无 block、总分达线时给 `warn` 放行内部预览，但必须把缺证据维度写进 `data_collection_tasks`；`production/overseas/release` profile 下，字幕 OCR、口型/音画同步、实际锚帧消费、运动/物理/多人对话 sidecar、交付包装证据、生产成本配方任一缺失，不得被当作通过，只能是缺证据 warn 或 gate 升级后的 block。评分报告要保留“需要补哪个 skill 的哪个 sidecar”，供 `n2d-review` 回灌矩阵和 `n2d-batch` 最小返工使用。
...
```bash
python3 skills/n2d/n2d-score/scripts/score.py <作品根> 第1集 --run-checks
```
