...
def is_stale_self_ledger_blocker(blocker: Dict[str, Any], ep: str) -> bool:
    dim = str(blocker.get("dim") or "")
    loc = str(blocker.get("loc") or "")
    msg = str(blocker.get("msg") or "")
    return (
        "验收总账" in dim
        and f"consistency_ledger_{ep}.json" in (loc + " " + msg)
    )
...
def build_data_collection_tasks(dims: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    return [{
        ...
        "command": "python3 skills/n2d/n2d-score/scripts/score.py <作品根> <集> --run-checks --profile <demo|standard|production|overseas>",
    }]
