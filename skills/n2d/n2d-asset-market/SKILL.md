...
description: 制漫剧系列内跨作品资产库/模板市场：把角色原型、identity_registry 片段、定妆组、场景LOC、道具PROP、武器WEAPON、独立服装OUTFIT、特效VFX、视频模型路由经验、题材母题和打斗套路导出成自包含 asset pack；导入新剧时 fork 新身份并重置账号/项目绑定的后端状态。跨系列或跨机器也只交付单个 pack，不建立目录级依赖。Use when asked about 跨项目角色库, 模板市场, 资产库, 服装库, 武器库, 特效库, 复用定妆, 复用角色, 导入角色模板, 导出角色模板, identity_registry 复用, 路由模板, 母题复用, 系统面板模板, 招式复用, 打斗复用, 武打模板, combat 复用, 沉淀打斗套路, 成本摊薄.
---

# n2d-asset-market — 制漫剧系列资产库 / 模板市场

你是 **n2d 资产管理员**。系列库固定在 `创作区/制漫剧/_资产库/`；目标不是让新剧引用旧剧目录，而是把可复用内容做成自包含包，降低同系列作品从零建库的成本。跨系列、跨仓库或跨机器仍只交付所需的单个包，由目标侧复制/分叉后适配。
...
- **输出**：`创作区/制漫剧/_资产库/.../asset_pack.json`、包内 `files/`、`portability` 自包含声明、导入后的目标项目 registry fragment、`fork_history`、路由模板包；项目内内容账本 `生产数据/asset_registry.jsonl`、`asset_registry_summary.json/md`。
...
- `角色库/` 是**单作品角色资产包层**：所有入镜具名人物至少有基础包；主角/核心长线/预计出场 10 集及以上为 `core_full`，复现配角为 `recurring_standard`，具名短线角色为 `named_minimal`，局部群像为 `restricted_partial`。它归拢 reference / prompts / lora / voice / adapters / qc，并指回本项目 `identity_registry.json` / 角色圣经，可作为显式导出的来源。
- `创作区/制漫剧/_资产库/characters/<slug>/asset_pack.json` 是**跨项目模板市场层**：服务新剧 fork 复用，导入后必须生成新项目本地身份，重置后端 Character ID / Face Lock / LoRA ready / voice id，并重新跑 QC。
- `创作区/制漫剧/_资产库/{scenes,props,weapons,outfits,vfx}/<slug>/asset_pack.json` 是**非角色资产模板层**：服务场景、关键道具、主角装备、独立服装、特效表现复用，导入后合并进新剧 `asset_registry.json`，仍需按新剧剧情校准约束并绑定逐镜 prompt。
...
- `角色库/`：所有具名入镜人物的分档 reference / prompts / lora / voice / adapters / qc 汇总；角色体量决定深度，不决定有没有包。
...
这些**不是自动进入跨项目 `创作区/制漫剧/_资产库/`**。跨项目资产库需要主动触发：用户或 agent 判断某个资产“稳定、审过、授权清楚、值得跨剧复用”后，再执行对应 export。原因是跨项目复用会涉及 IP 边界、授权、命名、质量筛选和是否 fork，不能把生产过程里的所有临时定妆/废稿/低质量图自动外溢成模板。
...
- `创作区/制漫剧/_资产库/.../asset_pack.json`：跨项目可复用模板，必须人工判断授权和质量后主动 export。

常用命令：

```bash
python3 skills/n2d/n2d-asset-market/scripts/asset_registry.py scan <作品根> --write
python3 skills/n2d/n2d-asset-market/scripts/asset_registry.py verify <作品根>
```
...
- **角色库内容随单包走**：`export-character` 除 registry reference_group 外，还会把源 `角色库/<CHAR_ID>__<slug>/` 中的 prompts / reference / voice / adapters / qc 与非模型说明文件复制进 `files/character_bundle/`；`.safetensors/.ckpt/.pt/.pth/.onnx` 明确排除并记入 manifest。`import-character` 在目标作品创建新的 `角色库/<新ID>__<新名>/`、重写 ID/姓名/manifest，再重置后端身份和模型状态。
- **资产包带授权字段**：默认 `template_only`。没有明确授权时，只复用模板结构，不复用“同一个可识别角色”。
- **单包自包含**：`asset_pack.json.files[]` 的每个路径都必须落在包目录内并通过 SHA256；`portability.requires_source_library=false`。跨系列/机器前运行 `verify-pack`，不得把来源作品、来源系列 `_资产库/` 或仓库根路径当运行时依赖。
- **系列各自拥有库**：本 skill 只管理 `制漫剧/_资产库/`；其它生产线拥有自己的 `_资产库/` 和导入适配。跨系列是显式文件交接，不允许 n2d import 其它系列实现，也不允许其它系列运行时回读 n2d 目录。
...
- **先轻量后市场**：本 skill 先做本地 `创作区/制漫剧/_资产库/` + CLI。等多部剧跑出真实复用频次，再做 UI、评分、排行。
...
```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py hint
```
...
```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py verify-pack 创作区/制漫剧/_资产库/characters/冷宫废妃
```

导出角色模板：

```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py export-character <作品根> --character-id CHAR_XXX --slug 冷宫废妃
```

导入角色模板到新剧：

```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py import-character <作品根> 创作区/制漫剧/_资产库/characters/冷宫废妃 --as-id CHAR_SHENNIAN --as-name 沈念
...
```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py export-scene <作品根> --asset-id LOC_XXX --slug 冷宫寝殿
python3 skills/n2d/n2d-asset-market/scripts/market.py import-scene <作品根> 创作区/制漫剧/_资产库/scenes/冷宫寝殿 --as-id LOC_01 --as-name 冷宫寝殿
```

导出 / 导入道具模板：

```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py export-prop <作品根> --asset-id PROP_XXX --slug 赐死托盘
python3 skills/n2d/n2d-asset-market/scripts/market.py import-prop <作品根> 创作区/制漫剧/_资产库/props/赐死托盘 --as-id PROP_01 --as-name 赐死托盘 --owner CHAR_LIU
```

导出 / 导入武器模板：

```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py export-weapon <作品根> --asset-id WEAPON_XXX --slug 霜纹长剑
python3 skills/n2d/n2d-asset-market/scripts/market.py import-weapon <作品根> 创作区/制漫剧/_资产库/weapons/霜纹长剑 --as-id WEAPON_01 --as-name 霜纹长剑 --owner CHAR_SHEN
```

导出 / 导入独立服装模板：

```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py export-outfit <作品根> --asset-id OUTFIT_XXX --slug 玄青窄袖官袍
python3 skills/n2d/n2d-asset-market/scripts/market.py import-outfit <作品根> 创作区/制漫剧/_资产库/outfits/玄青窄袖官袍 --as-id OUTFIT_01 --as-name 玄青窄袖官袍
```

导出 / 导入特效模板：

```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py export-vfx <作品根> --asset-id VFX_XXX --slug 青白剑气拖尾
python3 skills/n2d/n2d-asset-market/scripts/market.py import-vfx <作品根> 创作区/制漫剧/_资产库/vfx/青白剑气拖尾 --as-id VFX_01 --as-name 青白剑气拖尾
```

导出视频模型路由经验：

```bash
python3 skills/n2d/n2d-asset-market/scripts/market.py export-routes <作品根> 第1集 --slug 宫斗对峙路由
```
...
python3 skills/n2d/n2d-asset-market/scripts/market.py export-motif <作品根> --slug 系统面板
# 导入：合并进目标 motif_registry + asset_registry，progression 成长档重置（新剧从 Lv.1 起）
python3 skills/n2d/n2d-asset-market/scripts/market.py import-motif <作品根> 创作区/制漫剧/_资产库/motifs/系统面板
```
...
python3 skills/n2d/n2d-asset-market/scripts/market.py export-combat <作品根> --combat-id COMBAT_万妖妖力近战 --slug 万妖妖力近战
# 导入：合并进目标 combat_registry + asset_registry；reskin 重置（清关键帧 PNG，须新剧重出）
python3 skills/n2d/n2d-asset-market/scripts/market.py import-combat <作品根> 创作区/制漫剧/_资产库/combat/万妖妖力近战 --as-id COMBAT_雷霆近战
```
...
3. 检查 `创作区/制漫剧/_资产库/<type>/<slug>/asset_pack.json` 的 `license`、`style_tags`、`tags` 与 `portability`。
4. 运行 `market.py verify-pack <包目录>`；只有 self-contained、文件存在且 hash 一致的包才能跨系列/机器交付。
5. 如果只是原型模板，保持 `license.reuse=template_only`；没有授权证据时不得把“同一个可识别角色/武器/服装”当成跨项目可执行资产复用。
...
若源项目已有 `角色库/<CHAR_ID>__<slug>/manifest.json`，导出角色模板时优先读取其中的 reference/prompts/lora/voice/adapters/qc 缺口说明；导出的 `asset_pack.json` 仍要按跨项目 schema 写 fork_required、adapter 重置策略和 portability。导出完成后，目标作品只消费 pack 副本，不再依赖源 `角色库/`。
...
- `n2d-image`：新增共享定妆前先查本系列资产库，命中则导入定妆组和 `identity_registry` / `asset_registry` fragment；所有入镜具名角色在项目内维护分档 `角色库/` manifest。
