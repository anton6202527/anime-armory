    ...
    for script_name, human in (("face_drift_risk.py", "脸漂"), ("asset_drift_risk.py", "物料漂移")):
        ...
        if report is None:
            add(INFO, "漂移预案", ep,
                ...
                f"出图前可手动跑 skills/n2d/n2d-image/scripts/{script_name} 看预案。")
            continue
...
def check_skill_freshness(root: str, ep: str, stage: str) -> None:
    ...
    record_cmd = f'python3 skills/n2d/n2d-update/scripts/update_plan.py record "{root}" {ep}'
    check_cmd = f'python3 skills/n2d/n2d-update/scripts/update_plan.py check "{root}" {ep}'
        ...
        if isinstance(reg, dict):
            ...
            if pending:
                ...
                add(
                    ...
                    "`python3 skills/n2d/n2d-identity/scripts/identity.py <作品根> --write` 刷新 adapter matrix。"
...
def check_keyshot_candidate_plan(root: str, ep: str, stage: str = "image_preflight") -> None:
    ...
    if not keyshots:
        return
    sev = BLOCK if profile == "production" else WARN
    write_cmd = f'python3 skills/n2d/n2d-image/scripts/keyshot_candidates.py "{root}" {ep} --write'
    if not persisted:
        add(
            sev,
            "关键镜候选",
    ...
    if not isinstance(selection, dict) or selection.get("kind") != "n2d_candidate_selection":
        add(
            ...
            f"生成候选后跑 `python3 skills/n2d/n2d-image/scripts/candidate_select.py \"{root}\" {ep} --apply`。",
            return_to_stage="image",
        )
        return
