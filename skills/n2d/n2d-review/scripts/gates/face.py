...
def check_identity_adapter_matrix(root: str) -> None:
    ...
    data = load_json(p)
    if not isinstance(data, dict):
        add(BLOCK, "资产身份闭环", p, "缺少或无法解析 identity_adapter_matrix.json；先运行 `python3 skills/n2d/n2d-identity/scripts/identity.py <作品根> --write`")
        return
    ...
    try:
        ...
        return
    if not matrix_fp:
        add(BLOCK, "资产身份闭环", p, "identity_adapter_matrix.summary 缺 anchor_fingerprint；请重跑 `python3 skills/n2d/n2d-identity/scripts/identity.py <作品根> --write`")
        ...
        add(
            ...
            "共享定妆/锚点被改后必须重跑 `python3 skills/n2d/n2d-identity/scripts/identity.py <作品根> --write`。",
        )
...
def check_core_expression_anchor_coverage(root: str, ep: str) -> None:
    ...
    for cid, clip_idxs in sorted(missing.items()):
        ...
        add(BLOCK, "表情一致性", reg_path,
            ...
            "`python3 skills/n2d/n2d-image/scripts/image_qc.py <作品根> --finalize-expr CHAR_xx/形态/情绪` "
