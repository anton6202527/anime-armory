...
IMAGE_BACKEND_DOCS = (
    ...
    "skills/n2d/n2d-review/references/checklist.md",
)
...
def check_gate_entry(root: Path, findings: List[Finding]) -> None:
    ...
    if hits:
        add(
            ...
            "改为 `python3 skills/n2d/n2d-dashboard/scripts/dashboard.py gate ...`；`gate.py --json` 只保留为调试/机器消费入口。",
        )
    else:
...
def check_cross_cutting_coverage(root: Path, findings: List[Finding]) -> None:
    path = root / "skills" / "n2d" / "n2d-progress" / "scan.py"
    raw = text(path)
...
def check_benchmark_external(root: Path, findings: List[Finding]) -> None:
    path = root / "skills" / "n2d" / "n2d-dashboard" / "references" / "industry_benchmark.json"
    if not path.is_file():
        add(
            findings,
            "warn",
...
def load_charter_module(root: Path):
    ...
    path = root / "skills" / "n2d" / "n2d-review" / "scripts" / "consistency_charter.py"
    if not path.is_file():
...
def load_meta_audit_module(root: Path):
    ...
    """
    path = root / "skills" / "n2d" / "n2d-review" / "scripts" / "meta_audit.py"
    if not path.is_file():
...
def check_consistency_charter(root: Path, findings: List[Finding]) -> None:
    ...
    gate_py = root / "skills" / "n2d" / "n2d-review" / "scripts" / "gate.py"
    if error:
        add(findings, "info", "一致性charter", "skills/n2d/n2d-review/scripts/consistency_charter.py",
...
def check_detector_inventory(root: Path, findings: List[Finding]) -> None:
    ...
    scripts_dir = root / "skills" / "n2d" / "n2d-review" / "scripts"
    loc = "skills/n2d/n2d-review/scripts/detector_inventory.py"
...
def check_meta_audit(
    ...
    *,
    ...
    loc = "skills/n2d/n2d-review/scripts/meta_audit.py"
    if error:
        add(
            findings,
            "warn",
