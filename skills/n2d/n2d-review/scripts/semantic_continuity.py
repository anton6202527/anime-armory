...
def is_narration_role(role: str) -> bool:
    ...
    return "旁白" in text or text.lower() in NARRATION_ROLES


def role_present_in_storyboard(role: str, storyboard_text: str) -> bool:
    if role in storyboard_text:
        return True
    if is_narration_role(role) and "narration_indices" in storyboard_text:
        return True
    return False
...
def analyze(root: str, ep: str) -> Dict[str, Any]:
    ...
    if voice:
        ...
        for role in sig["roles"]:
            if not role_present_in_storyboard(role, storyboard_text):
