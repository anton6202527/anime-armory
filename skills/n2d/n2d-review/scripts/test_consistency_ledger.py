...
cd skills/n2d/n2d-review/scripts && python -m pytest test_consistency_ledger.py
"""
...
def test_worse_and_band_to_sev():
    ...
    assert cl.band_to_sev("block") == "block"
    assert cl.band_to_sev("high") == "medium"
...
def test_build_ledger_overall_is_worst_of_three():
    ...
    # 角色：事前 high 是预测预案 → final ledger 只作 medium 复核
    assert by_id["CHAR_01"]["prevent"] == "medium" and by_id["CHAR_01"]["overall"] == "medium"
    # 资产：契约 warn → overall warn(=medium 阶)
    assert by_id["PROP_01"]["contract"] == "warn" and by_id["PROP_01"]["overall"] == "warn"
    # 综合排序：同为 medium 阶时保持实体在输出中
    assert led["rows"][0]["id"] == "CHAR_01"
    assert led["counts"]["high"] == 0
    assert led["counts"]["medium"] == 3
    assert "domains" in led
    assert led["delivery_surface"]["status"] == "pass"
...
def test_self_ledger_signal_is_filtered():
    assert cl._is_self_ledger_signal({
        "severity": "block",
        "dimension": "验收总账",
        "loc": "生产数据/consistency_ledger_第1集.json",
        "message": "一致性验收总账未清零",
    }, "第1集")
    assert not cl._is_self_ledger_signal({
        "severity": "block",
        "dimension": "字幕正确性",
        "loc": "生产数据/native_av_subtitle_alignment_第1集.json",
        "message": "缺词级对齐",
    }, "第1集")
...
def test_score_warn_status_is_medium_not_hard_high():
    ...
    assert cl._score_status_severity("insufficient_data") == "medium"
    assert cl._score_status_severity("fail") == "block"
