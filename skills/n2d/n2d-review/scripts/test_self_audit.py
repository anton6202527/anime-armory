...
def _minimal_repo(root: Path) -> None:
    ...
    (root / "skills" / "n2d" / "n2d-progress").mkdir(parents=True)
    (root / "skills" / "n2d" / "n2d-dashboard" / "references").mkdir(parents=True)
    (root / "skills" / "n2d" / "n2d-image").mkdir(parents=True)
    ...
    (root / "skills" / "n2d" / "n2d-progress" / "scan.py").write_text(
    ...
    (root / "skills" / "n2d" / "n2d-dashboard" / "references" / "industry_benchmark.json").write_text(
        '{"collected":"2026-06","sources":[],"one_pass_rate":0.9,"redraw_rate":0.1}\n',
        encoding="utf-8",
    )
    (root / "skills" / "n2d" / "n2d-image" / "SKILL.md").write_text(
...
def test_self_audit_loads_facade_contract_without_engine_error(tmp_path: Path) -> None:
    ...
    (tmp_path / "skills" / "n2d" / "n2d-image" / "SKILL.md").write_text(
...
def test_self_audit_flags_bare_production_gate(tmp_path: Path) -> None:
    _minimal_repo(tmp_path)
    (tmp_path / "skills" / "n2d" / "n2d-image" / "SKILL.md").write_text(
        "python3 skills/n2d/n2d-review/scripts/gate.py <作品根> 第N集 --stage image\n",
        encoding="utf-8",
    )
    report = self_audit.audit(tmp_path)
