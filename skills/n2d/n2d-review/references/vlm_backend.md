...
消费方：`skills/n2d/n2d-image/scripts/vlm_verify.py`、`image_qc.py --prop-shape-vlm-confirm` 等。
...
```bash
export N2D_VLM_CMD='conda run -n n2dvlm python skills/n2d/n2d-review/scripts/backends/vlm_cmd_mlxvlm.py --image {image} --prompt {prompt}'
```
...
消费方：`skills/n2d/n2d-review/scripts/appearance_judge_runner.py`。
...
```bash
export N2D_APPEARANCE_BATCH_CMD='conda run -n n2dvlm python skills/n2d/n2d-review/scripts/backends/appearance_mlxvlm.py'
```
...
```bash
conda run -n n2dvlm python skills/n2d/n2d-review/scripts/backends/vlm_cmd_mlxvlm.py \
  --image <image.png> \
