...
- 已接的真实埋点：`skills/n2d/n2d-voice/render_voice.py` 占位降级分支。新埋点照抄上面一行即可，**无需登记**（不是 detector，不进 detector_inventory；加脚本不触发 README F1）。
...
```bash
python3 skills/n2d/n2d-review/scripts/self_audit.py --work <作品根>          # markdown 差距清单
python3 skills/n2d/n2d-review/scripts/self_audit.py --work <作品根> --json   # 机器消费
```
