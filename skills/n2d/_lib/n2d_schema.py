...
CONSISTENCY_DIMENSIONS: Dict[str, Dict[str, Any]] = {
    ...
    "scene_consistency": {
        ...
        "audit_labels": ("场景(O2)", "接缝接力", "轴线视线(X1)", "天气时辰(W1)", "光位方向(W2)", "字幕安全区(L2)", "空间站位(B1)", "物件常驻(O3)", "视线状态回读(X2)", "场景平面(FP1)"),
        "keywords": ("场景", "接缝", "尾帧", "场景资产", "轴线", "视线", "站位", "遮挡", "前后景", "天气", "时辰", "字幕安全区", "字幕带", "构图", "物件常驻", "平面图"),
    },
    ...
    "semantic_continuity": {
        ...
        "audit_labels": ("语义谱系(P0)", "称谓口头禅(A1)", "台词语域(D1)"),
        "keywords": ("语义", "谱系", "继承", "称谓", "口头禅", "人设", "语域", "semantic", "voiceover", "storyboard"),
    },
    "state_continuity": {
        "label": "状态百科",
        "weight": 8,
    ...
    "interaction_continuity": {
        ...
        "weight": 8,
        "return_to_stage": "script_stage2",
        "scope": "回 n2d-script 阶段2补 interaction_graph/contact_graph、左右手/持有状态、持有账本和递交/释放因果；必要时重跑 n2d-model-router 补 motion_control。",
        "audit_labels": ("交互接触(I1)", "持有账本(POS)", "结构化交互图谱(I2)"),
        "keywords": ("交互", "接触", "持有", "持有账本", "递给", "抓握", "motion_control", "interaction", "contact", "possession"),
    },
