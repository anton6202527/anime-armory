...
def _baseline_cmp_field(key: str, value: Any) -> str:
    if key == "access":
        canonical, status = canonical_backend(str(value or ""))
        if status == "approved" and canonical:
            return canonical
    return _baseline_cmp_value(value)
...
def image_backend_baseline_status(root: str) -> Dict[str, Any]:
    ...
    changed = [
        ...
        if _baseline_cmp_field(key, (baseline or {}).get(key)) != _baseline_cmp_field(key, current.get(key))
    ]
    return {
