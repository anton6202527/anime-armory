...
`skills/n2d/references/选择点与偏好.md` 要求所有易变候选清单（模型/平台/价格/规格/法规…）都带采集日期，
...
# ── 候选源登记表（跨 5 条线的易变候选清单）──────────────────────────────────
# 每条：相对仓库根的路径、它覆盖的选择点、刷新最长容忍天数、备注。
# 新增易变候选清单（新模型矩阵/新后端 catalog）务必在此登记，否则机检/刷新都看不到它。
CANDIDATE_SOURCES: List[Dict[str, object]] = [
    ...
    {
        "id": "n2d-video-backends",
        "path": "skills/n2d/_lib/n2d_platform_profiles.py",
    ...
    {
        "id": "voice-backends",
        "path": "skills/n2d/_lib/voice_backends.py",
