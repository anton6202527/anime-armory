...
- **多锚帧逐级接力铁律（参考上限下也不得退回首帧）**：同一 Clip 有 `first → a1 → a2 → … → end` 时，`aK` 的第一接力附件必须是当前像素已验收的 `a(K-1)`，尾帧必须优先接最后一张已存在锚帧；不能因为后端参考图上限而总是保留 firstframe、截掉最近锚帧，也不能在补满附件时把已淘汰的旧首帧重新塞回。runner 的 dry-run / generation receipt 必须显示实际 `reference_input_paths`，生成前核对第一项接力源；若与最近锚帧不符，禁止付费生成。该规则只改变接力源优先级，不减少脸锚、角色半身锚和必要场景/道具参考的最低覆盖。
- **定时子镜道具可见性铁律**：一个 Clip 含多个物理子镜时，first/_aK 只能消费该 `at_sec` 子镜 `desc` 与 first 的 `continuity.start_state` 中已经入画的道具。`object_ids`、整 Clip 场景摘要、状态演进或后续 `end_state` 里“稍后才发现/拿起/发光”的道具只能留在连续性合同，不得进入当前帧场景词、preserve、附件或正向实体；compiler 必须把它写成“本时点尚未入画”并过滤未来状态，防止伏笔提前泄漏。
- **观众独享信息铁律**：storyboard `entity_schedule.knowledge_state` 若写明 `AUDIENCE` 已看见某异常、同时 `CHAR_xx` 仍“不知/未察觉”，该物理锚帧必须把异常放在角色视线不可达的道具外侧、背面、下表面或画外局部；角色不得看见、触发或做知情反应。像“盆底幽光”这类歧义词应编译为外侧下表面，不得误画进角色正托着可见的内腔/正面。
- **表情拼表网格 QC 铁律**：声明为六联表/2×3 expression sheet 的定妆资产，机器验脸必须先按脸框中心落入 3 列×2 行网格，每格只保留面积最大的主脸框，再检查六格覆盖、单格脸占比、单格短边（≥384px）与主脸框短边（≥96px）。不得把同格领口/下半脸的 Haar 小框重复计成人脸，也不得拿整张拼表短边套单张核心脸锚的 1024px 门槛；独立脸部特写仍按核心脸锚教头线单独审计。
- **Full QC 分环境铁律**：InsightFace 与 DINO/torch 若因 macOS conda 依赖冲突分装在不同环境，不得任选一边后把另一项冒充已通过，也不要求在同一环境重复安装整套重依赖。以含 InsightFace 的解释器运行 `image_qc.py`，并设置 `N2D_SEMANTIC_PYTHON=/path/to/含torch与transformers的python`；主 QC 只把只读 `semantic_drift.py --json` 派到该解释器，随后将 DINO 比对结果、执行解释器与人脸 full 精度证据合并进同一当前哈希报告。
...
- **每张图片前后双重一致性硬闸（全项目/全剧集默认，不是可选验收模式）**：遵守设计宪法 B14。正式生图一次只能解析并提交一张目标图；生成前必须通过 `image_preflight`、共享库先行、当前 target 的 reference bundle/compiled request 对账，并把实际附件逐项落为规范项目内路径 + owner/用途 + 当前 SHA-256。承载身份却喂 0 张脸锚、`参考附件=[]`、派生帧未引用当前已签收母帧、实际输入只写“会话参考包/精确路径不可得”，或上一张没有绑定当前像素 SHA 的 `qa/accepted`，一律在付费前 BLOCK。生成后立即跑 full `image_qc`；face/hair/outfit 任一 `block`，以及需判身份的 `warn/unverifiable`，都不得自动进入下一张，必须把输出与脸锚/服装锚/场景道具锚/相邻帧并排查看原像素，再按当前 PNG SHA 记录 accepted 或 rejected。`--skip-preflight`、`--skip-image-qc`、`--skip-final-gate`、demo/profile、批量或局部 `--shots` 不得绕过；这三项仅保留为旧命令可识别参数，正式执行会直接拒绝。**已被整集 gate 判坏的当前 PNG 不走 waiver**：用 `codex_image_runner.py ... --shots <精确单图> --force --repair-redraw`，它只接受一个目标，要求现有 full `image_qc` 对该图有 face/hair/outfit/覆盖失败，绑定坏图 SHA 与 QC 报告 SHA 写 `n2d_single_image_consistency_preflight` 收据后才付费重抽；共享顺序锁、参考附件核验、compiled request 和生成后 full QC 仍照常执行。
...
- **预算前硬挂钩铁律（P0 · 执行层不许松动）**：`image_preflight` 闸门不再只是「默认跑」的纪律提示——`codex_image_runner.py` / `dreamina_image_runner.py` 在**每张图片生成前**先跑 runner 级 shared-first 顺序锁和 `gate --stage image_preflight`，**block 即拒绝生成、不花钱**（镜像 `video_runner` 的 `run_preflight_gate`）；并且每张生成后立即跑 full `image_qc`，最终 `--stage image` 继续接 `check_input_frame_qc`，**崩脸/接缝断/降级精度近景/脸覆盖缺口在最近的出图闸门即 BLOCK**，不再拖到最贵的「出视频」工位才发现。按 B14，`--skip-preflight` / `--skip-final-gate` / `--skip-image-qc` 不再是可用逃生口：参数仅为兼容旧自动化而保留，正式执行一律拒绝；共享库先行、逐图前检、逐图后检和当前像素 SHA 目视验收均不可 waiver。整集 gate 因目标坏图而阻断返修时，只能走上条 `--repair-redraw` 的 full-QC 证据绑定单图返修闸，不能使用 skip 参数。gate 侧若存在其它非一致性降级，只能处理其明确领域，不能把缺失/降级的身份与连续性证据改写成通过。
...
- **重抽预算铁律（两档全局统一 · n2d/mv/ad 同义）**：`重抽预算策略` 只保留两档，按 `../skills/n2d/references/选择点与偏好.md` 读 `_设置.md`→全局默认；仍缺时自动采用 **预算充足**，不为普通预算档另停一次，用户可在付费确认前覆盖。旧值 `预算不足` / `预算不够` 一律归并为 `预算一般`。这里的“满意”以本张图的落档自检 + 用户/制作判断为准，每次重抽都必须记 dashboard 事件、保留候选或废料，不设固定次数上限：
...
- **关键镜候选后端止损铁律**：`keyshot_candidate_runner.py --backend auto` 必须跟随本项目已锁定/已签核的生图后端，支持 `codex` 与 `dreamina`；项目已单向切到 Dreamina 或 Codex 配额耗尽后，best-of-N 不得绕回 Codex 死磕。可显式 `--backend dreamina`，仍复用同一 compiler、真实多参考、事件与候选目录，且每生成一张候选都要先做机器检查与原像素实际查看，再继续下一张。
- **关键镜候选提示隔离铁律**：候选 runner 的 `variant_note` 只能写模型能理解的视觉微差与“同一分镜合同”约束，不得把源目标路径、候选目录、registry 名、任务 ID 或选片流程说明送进模型 prompt；内部映射只留 sidecar/compiled receipt，compiler 检出内部路径泄漏必须在付费提交前阻断。
...
> 入口在哪：阶段 B 末尾会自动扫描主要人物的 LoRA / 指定参考图；发现可用资产时进入本节，未发现则走共享定妆默认方案。本节不是另一道独立提问。
...
4. 执行者必须实际查看当前像素，并按 prompt 自检脸、手、人体、构图、身份/资产、光位和禁忌。默认一键策略下写 hash-bound executor visual receipt 后继续，不弹普通确认；只有硬伤、QC 降级或必须真人裁决的 finding 才停。
...
**出图 prompt 就绪后的默认动作**：
> 报告共享定妆与分镜图总量，扫描官方/已登录生图渠道，按当前能力证据、项目规模、身份一致性、成本和编辑能力选出一个推荐的 `生图模型 + 生图渠道`。把模型/渠道与 `生成粒度=逐个` 一并放进付款动作卡；用户一次放行即采用，之后逐张自动 QC。无可执行渠道、能力证据不足或用户固定约束冲突时才停。
...
- **1 个可用**：采用这组可执行模型/渠道并写入付款动作卡；项目已有可用固定值时保持不变，已有值不可用时在同一付款卡说明切换原因并允许覆盖。
- **≥2 个可用**：按当天能力证据、项目规模、核心角色/多人同框、编辑需求和成本给出一个推荐并写入付款动作卡，不另停一次。长线身份一致性优先原生主体/角色 ID 或强多参考；单集快速迭代可优先 GPT Image via Codex。用户显式固定值始终优先，禁止跨目标混用。
...
**主要人物一致性素材（自动发现 + 可覆盖）**：付费前扫描 identity registry、项目内 LoRA、角色参考和共享定妆。发现已绑定且新鲜的 LoRA/参考图就自动消费；都没有时采用“先生成共享定妆，再多参考派生 + 锚点句”的推荐路线，不另停一次。该推荐及预计额外生成量进入同一付款动作卡，用户可在放行前上传或指定自己的素材。

- **发现 LoRA** → 按 `references/lora_consistency.md` 校验并接产线。
- **发现参考图** → 登记为定妆主参考，后续相关镜头统一做图生图/多参考派生。
- **未发现** → 走阶段 C 共享定妆 + 阶段 D 多参考派生，不强推 LoRA。
- 任何外部素材都必须满足授权/合规合同；未知授权不能因自动发现而放行。
...
需要校准 compiler/profile 时，先用 `image_prompt_metrics.py register` 注册固定时域 A/B，再通过 `N2D_IMAGE_PROMPT_EXPERIMENT_ID` + `N2D_IMAGE_PROMPT_VARIANT` 给预分配资产打完整标签；不得把不同时间、不同剧情难度的未标记版本 cohort 当随机实验。阶段/批次结束后运行 `python3 skills/n2d/n2d-image/scripts/image_prompt_metrics.py report <作品根> --write`，联合 production events 与真实 image_qc 统计首抽通过、身份漂移、手部失败、重抽、成本和 input token；只有样本达标、显著提升且安全指标不退化才升级 profile。完整命令见 `references/image_prompt_compiler.md`。

**每张/每单位落档后先跑出图落档机检；本集分镜全部落档后再跑一次收尾总闸（生图后闸门，与 gate 互补）**：
```bash
# 首选 full QC 环境（有 Pillow/cv2/insightface/onnxruntime/buffalo_l）
/opt/homebrew/Caskroom/miniforge/base/envs/facefusion/bin/python skills/n2d/n2d-image/scripts/image_qc.py <作品根> 第N集 --json
/opt/homebrew/Caskroom/miniforge/base/envs/facefusion/bin/python skills/n2d/n2d-dashboard/scripts/dashboard.py gate <作品根> 第N集 --stage image

# 只有确认该 python 有 full stack 时，才可换成其它解释器
python3 skills/n2d/n2d-image/scripts/image_qc.py <作品根> 第N集 --json
```
...
- **判定分两级 + 角色脸覆盖硬闸 + 贴脸反作弊硬闸**：`verdict=block`（硬阻断——崩脸、接缝断、纯文生图、非法 `CHAR_xx`、尾帧身份/接力硬伤、降级精度近景、`face_reference_coverage.missing>0`、`prohibited_face_patch.outputs>0`）→ 必须修复/重抽后重跑，不得推进；`verdict=review`（初筛人判——outfit/scene/道具特效/锚点门/漏视线锚点句等）→ 交人二次判，确认误报可放行；`verdict=ok` → 放行。加 `--strict-pixel` 可把像素机检 block（服装换装/场景换景/道具特效漂移）从 review 升为硬阻断 block（默认 off，保留宽松判定）。脸、发型或服装指纹若因俯身、侧转、透视、湿发、血污或裁切等产生确定性假阳性，只能在实际并排查看当前原像素后，分别用 `--face-confirm-ok` / `--hair-confirm-ok` / `--outfit-confirm-ok` 写入当前 PNG SHA、reviewer、review_kind 和具体理由；确认只改变最终 verdict，必须保留机器原分数与 `manual_original_verdict`，PNG 一旦重出旧收据自动失效。不得为明显换脸/高髻/发色、服装结构/配色或轮廓漂移签收据，也不得降低全局阈值。`face_reference_coverage` 是铁律：每张已落档角色图都必须有 full 精度定妆/身份主参考比对证据，warn/noface 不算通过；`prohibited_face_patch` 是更高优先级的事实闸门：最新落档事件若来自本地贴脸/换脸/裁脸贴回画面，即使 embedding 分数过线也一律不合格，必须真实重抽或用官方 image2image 派生替换。
...
python3 skills/n2d/n2d-batch/scripts/queue.py plan <作品根> --from-asset-impact 计划.json
```
仍可手动单跑：
```bash
