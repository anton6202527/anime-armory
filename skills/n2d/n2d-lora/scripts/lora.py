...
agent 内部会按阶段运行：
- python3 skills/n2d/n2d-lora/scripts/lora.py suggest <作品根>  # 读漂移报表打印升档建议
...
- python3 skills/n2d/n2d-lora/scripts/lora.py train-job <作品根> --character-id CHAR_XXX --form 常态 --provider fal
- python3 skills/n2d/n2d-lora/scripts/lora.py validate <作品根> --character-id CHAR_XXX --form 常态 --model-path ... --approved  # 数据集无 warning 时
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    print(f"[next] python3 skills/n2d/n2d-lora/scripts/lora.py dataset '{root}' --character-id {args.character_id} --form '{card['form']}' --copy-references")
    return 0
...
def cmd_suggest(args: argparse.Namespace) -> int:
    ...
    if not report_path.is_file():
        ...
        print(f"[next] 先跑 python3 skills/n2d/n2d-identity/scripts/identity.py '{root}' --write 生成 identity_drift_report.json")
        return 1
    report = read_json(report_path)
    if not isinstance(report, Mapping):
    ...
    if not report.get("available"):
        ...
        print(f"[next] 在装好 insightface/cv2 的环境重跑 python3 skills/n2d/n2d-identity/scripts/identity.py '{root}' --write")
        return 0
