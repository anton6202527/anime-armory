...
def _batch_seed_from_schedule(root: Path, ep: str, schedule: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "batch_queue_command": f"python3 skills/n2d/n2d-batch/scripts/queue.py plan {root} --from-shooting-schedule 生产数据/{BATCH_SEED_JSON.format(episode=ep)}",
        "policy": {
...
def _write_batch_seed(root: Path, ep: str, schedule: Mapping[str, Any], *, force: bool = False) -> Tuple[str, str]:
    ...
    if force or not md_path.exists():
        lines = [
            ...
            "## Import",
            "",
            f"```bash\npython3 skills/n2d/n2d-batch/scripts/queue.py plan {root} --from-shooting-schedule 生产数据/{BATCH_SEED_JSON.format(episode=ep)}\n```",
            "",
...
def check(root: Path, ep: str, *, write_missing: bool = False) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "files": rows,
        "scaffold_command": f"python3 skills/n2d/n2d-script/scripts/production_breakdown.py {root} {ep} scaffold --write",
        "next_when_blocked": (
