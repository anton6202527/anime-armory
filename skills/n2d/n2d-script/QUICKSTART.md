...
Command:
```bash
python3 skills/n2d/n2d-script/scripts/split_novel.py "<小说路径>" --by-chapter
# 默认首批 10 集；续切用 --limit N，全篇粗切用 --all
python3 skills/n2d/n2d-script/scripts/split_novel.py "<小说路径>" --by-chapter --limit 30
python3 skills/n2d/n2d-script/scripts/split_novel.py "<小说路径>" --by-chapter --all
# 中段开工：先补 `设定库/中段开工前情资产包.md`；用户只给模糊剧情点时，先读章节/关键词定位章号再传参
python3 skills/n2d/n2d-script/scripts/split_novel.py "<小说路径>" --by-chapter --start-chapter 48 --limit 10
# 边界词典按 _设置.md `题材` 自动切换（古装爽文/女频情感/悬疑/都市）
# 拆集骨架体检（逐集边界 + 剧级追更骨架：弱钩集群/无闭环/卡点定位，按 `变现模式`）：
python3 skills/n2d/n2d-script/scripts/boundary_audit.py <作品根>
# 建卡后：跨集角色形象生命周期预扫（年龄/换装/形态里程碑）→ 人确认
python3 skills/n2d/n2d-script/scripts/lifecycle_scan.py <作品根> --write
```
...
python3 skills/n2d/n2d-script/finalize_storyboard.py <作品根> 第N集
...
python3 skills/n2d/n2d-script/scripts/beat_audit.py <作品根> 第N集
python3 skills/n2d/n2d-script/scripts/beat_audit.py <作品根> --series   # 量产抽检套路同质化
```
