...
def check_anchor_contract(rows: List[Dict[str, Any]], clip: Dict[str, Any], loc: str, enforce_midframe: bool) -> None:
    ...
    if not isinstance(cont, dict):
        ...
        return
    for field in ("start_state", "end_state", "transition"):
    ...
    if req and not has_anchor_list:
        ...
        add(
            ...
            "重动作多中帧",
            loc,
            f"{req['reason']}，必须写 continuity.anchors[] 多中帧链；{suffix}请跑 skills/n2d/n2d-script/scripts/anchor_planner.py <作品根> <集> --write 后重跑 n2d-image。",
        )
    duration = duration_of(clip)
...
def validate(root: str, ep: str) -> Dict[str, Any]:
    ...
    for index, clip in enumerate(clips, 1):
        ...
        if index < len(clips):
            check_seam_contract(rows, clip, loc)
