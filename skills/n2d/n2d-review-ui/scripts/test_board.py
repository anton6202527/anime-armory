#!/usr/bin/env python3
"""board.py tests. Run: cd skills/n2d/n2d-review-ui/scripts && python -m pytest test_board.py"""
...
def test_done_episodes_uses_acceptance_column(tmp_path):
    root = tmp_path / "制漫剧" / "验收剧"
    root.mkdir(parents=True, exist_ok=True)
    (root / "_进度.md").write_text(
        "\n".join([
            "| 集 | 字数 | raw | 剧本改编 | 配音 | 分镜设计 | 成片 | 验收 |",
            "|---|---|---|---|---|---|---|---|",
            "| 第1集 | 800 | ✅ | ✅ | ⬜ | ✅ | ✅ | ✅ |",
            "| 第2集 | 700 | ✅ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |",
        ]),
    ...
    assert m["episodes"][0]["accepted"] is True
    assert m["summary"]["done_episodes"] == 1
...
def test_board_carries_review_ui_deeplink(tmp_path):
    # 每集带 review_ui 深链和 episode_app 深链；exists 准确反映对应 HTML 是否已生成
    root = tmp_path / "制漫剧" / "深链剧"
    make_work(root)
    m = board.build_manifest(root)
    e1 = m["episodes"][0]
    assert e1["review_ui"]["url"] == "review_ui_第1集.html"
    assert e1["episode_app"]["url"] == "episode_app_第1集.html"
    assert e1["review_ui"]["exists"] is False  # 还没生成 → 点会提示命令
    assert e1["episode_app"]["exists"] is False

    (root / "生产数据").mkdir(parents=True, exist_ok=True)
    (root / "生产数据" / "review_ui_第1集.html").write_text("<html></html>", encoding="utf-8")
    (root / "生产数据" / "episode_app_第1集.html").write_text("<html></html>", encoding="utf-8")
    m2 = board.build_manifest(root)
    assert m2["episodes"][0]["review_ui"]["exists"] is True
    assert m2["episodes"][0]["episode_app"]["exists"] is True

    out = board.render_html(m2)
    assert "openEpisode" in out and "openReview" in out and "#clip=" in out  # board 侧深链 JS 在


def test_board_reads_dashboard_episode_metrics(tmp_path):
    root = tmp_path / "制漫剧" / "指标剧"
    make_work(root)
    (root / "生产数据").mkdir(parents=True, exist_ok=True)
    (root / "生产数据" / "dashboard.json").write_text(json.dumps({
        "kind": "n2d_production_dashboard",
        "episodes": [{
            "episode": "第1集",
            "cost_totals": {"credits": 3.5},
            "duration_hms": "1m02s",
            "runtime_hms": "7s",
            "final_pass_rate": 0.8,
            "redraw_rate": 0.2,
    ...
    m = board.build_manifest(root)
    metrics = m["episodes"][0]["metrics"]

    assert metrics["cost_text"] == "3.5 credits"
    assert metrics["qa_blockers"] == 1
    assert metrics["qa_warnings"] == 4
    assert "通过 ${fmtRate" in board.render_html(m)
