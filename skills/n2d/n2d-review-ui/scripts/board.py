...
def _dashboard_episode_map(root: Path) -> Dict[str, Dict[str, Any]]:
    data = load_json(production_dir(root) / "dashboard.json")
    out: Dict[str, Dict[str, Any]] = {}
    if isinstance(data, dict):
        for item in data.get("episodes") or []:
            if isinstance(item, dict) and item.get("episode"):
                out[n2d_route.normalize_episode(str(item["episode"]))] = item
    return out
...
def _cost_text(costs: Dict[str, Any]) -> str:
    ...
    for unit, value in sorted(costs.items()):
        try:
            shown = f"{float(value):.2f}".rstrip("0").rstrip(".")
...
def episode_block(
    ...
    app_html = production_dir(root) / f"episode_app_{ep_norm}.html"
    episode_app_link = {"url": f"episode_app_{ep_norm}.html", "json": f"episodes/{ep_norm}.json", "exists": app_html.is_file()}
    dashboard_ep = dashboard_ep or {}

    return {
        "episode": ep_norm,
        "num": row.get("_num"),
        "word_count": row.get("字数") or row.get("字数估计"),
        "accepted": n2d_route.is_done(row.get("验收", "")),
        "review_ui": review_ui_link,
        "episode_app": episode_app_link,
        "stages": stage_states,
        ...
        "metrics": {
            "cost_text": _cost_text(dashboard_ep.get("cost_totals", {})),
...
def build_manifest(root: Path) -> Dict[str, Any]:
    ...
    dashboard_eps = _dashboard_episode_map(root)
    episodes = [
        episode_block(
            root,
            html_dir,
            ...
            stages,
            row,
            dashboard_eps.get(n2d_route.normalize_episode(str(row.get("_ep") or row.get("集") or ""))),
        )
    ...
    done_eps = sum(
        1 for e in episodes
        if e.get("accepted") or (e["done_stages"] >= e["total_stages"] and e["total_stages"] > 0)
    )
...
.lane-head{{position:absolute;display:flex;flex-direction:column;gap:2px;padding:8px 10px;width:178px;cursor:pointer;border-radius:8px}}
...
.lane-metrics{{display:grid;grid-template-columns:1fr 1fr;gap:2px 6px;color:var(--muted);font-size:10px;line-height:1.25;margin-top:3px}}
.lane-metrics span{{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
...
<div class="help">拖拽平移 · 滚轮缩放 · 空格重置 · 点集头→本集工作台 · 点 Clip→人审画布</div>
...
// layout constants
const LANE_X=24, HEAD_W=190, STAGE_X0=HEAD_W+24, STAGE_W=104, STAGE_H=48;
...
function fmtRate(v){{return v===null||v===undefined||v===''?'—':(Number(v)*100).toFixed(1)+'%';}}
function build(){{
  ...
  (data.episodes||[]).forEach((ep,ei)=>{{
    ...
    parts.push(`<div class="lane-head" data-ep="${{ei}}" title="点开该集工作台" style="left:${{LANE_X+8}}px;top:${{y+8}}px"><b>${{esc(ep.episode)}}</b>`+
      ...
      `<div class="lane-metrics"><span title="成本">${{esc(mt.cost_text||'—')}}</span><span>QA ${{mt.qa_blockers||0}}/${{mt.qa_warnings||0}}</span><span>通过 ${{fmtRate(mt.final_pass_rate)}}</span><span>重抽 ${{fmtRate(mt.redraw_rate)}}</span></div>`+
...
// 跨集深链：点集头 → 本集工作台；点 Clip → 该集 review_ui 深画布并定位本 Clip。拖动不触发。
function openEpisode(ei){{
  const ep=data.episodes[ei]; if(!ep) return;
  const app=ep.episode_app||{{}};
  if(!app.url||!app.exists){{
    toast(`${{ep.episode}} 工作台未生成 — 先跑：python3 skills/n2d/n2d-review-ui/scripts/episode_app.py "${{data.root}}" --episode ${{ep.episode}} --write --index`);
    return;
  }}
...
function openReview(ei, clipId){{
  ...
  if(!ru.url||!ru.exists){{
    toast(`${{ep.episode}} 人审深画布未生成 — 先跑：python3 skills/n2d/n2d-review-ui/scripts/review_ui.py "${{data.root}}" ${{ep.episode}} --write`);
    return;
  }}
...
canvas.addEventListener('click',e=>{{
  ...
  if(headEl) openEpisode(+headEl.dataset.ep);
}});
let toastTimer;
function toast(msg){{
