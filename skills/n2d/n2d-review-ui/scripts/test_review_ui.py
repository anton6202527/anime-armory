...
def test_build_manifest_collects_visual_review_assets(tmp_path: Path) -> None:
        ...
        "clips": [
            {
                ...
                "continuity": {
                    ...
                    "need_endframe": True,
                    "anchors": [{
                        "anchor_png": "出图/第1集/图片/Clip01_mid.png",
                        "at_sec": 2,
                        "use": "split",
                        "reason": "动作中点",
                    }],
    ...
    batch = {
        "kind": "n2d_video_batch_manifest",
        "items": [{
            "clip": "Clip_01",
            "anchor_consumption_mode": "native_multiframe",
            "frame_control_mode": "multi_keyframe",
            "anchor_consumption": {"backend": "dreamina", "consumes_endframe": True, "consumes_mid_anchors_natively": True},
    ...
    write_text(root / "生产数据" / f"video_batch_{ep}_01_02.json", json.dumps(batch, ensure_ascii=False))
    ledger = {
    ...
    assert [f["label"] for f in manifest["clips"][0]["anchor_frames"]] == ["首帧", "锚帧1", "尾帧"]
    assert len(manifest["clips"][0]["consumed_frames"]) == 3
    assert manifest["clips"][0]["anchor_consumption"]["mode"] == "native_multiframe"
...
def test_build_manifest_falls_back_when_registered_video_path_is_stale(tmp_path: Path) -> None:
    root = tmp_path / "制漫剧" / "测试剧"
    ep = "第1集"
    storyboard = {
        "episode": 1,
        "clips": [{
            "id": "EP01_CLIP03",
            "label": "黑殿审讯",
            "firstframe_png": "出图/第1集/图片/Clip03_first.png",
            "video_out": "出视频/第1集/视频/EP01_CLIP03.mp4",
        }],
    }
    write_text(root / "脚本" / ep / "storyboard.json", json.dumps(storyboard, ensure_ascii=False))
    write_text(root / "出图/第1集/图片/Clip03_first.png")
    write_text(root / "出视频/第1集/视频/Clip_03_黑殿审讯.mp4")

    manifest = review_ui.build_manifest(root, ep)

    video = manifest["clips"][0]["video"]
    assert video["exists"] is True
    assert video["path"] == "出视频/第1集/视频/Clip_03_黑殿审讯.mp4"
