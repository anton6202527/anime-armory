...
## 模型矩阵刷新（默认只给建议）
`novel2drama/references/模型矩阵.md` 是各轴 **SOTA vs n2d 默认 vs 升级触发** 的防过期快照。每次跑模式②都要检查它是否过期，并在报告里给出**刷新建议 / diff 草案 / 来源链接**；只有用户明确要求“刷新矩阵/落地本轮自审”时，才实际修改该文件：改三表的"SOTA 快照(年-月)"列 + 顶部采集日期 + 升级触发。这避免默认流程自审一边说 report-only、一边偷偷改产线。
