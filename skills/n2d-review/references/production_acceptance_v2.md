...
```bash
python3 skills/n2d-script/scripts/split_novel.py <源小说> --out <作品根>
```
...
```bash
python3 skills/n2d-script/scripts/boundary_audit.py <作品根> 1-10 --strict --json
python3 skills/n2d-script/scripts/boundary_review.py draft <作品根> 1-10 --write

# 保留边界：必须给语义证据
python3 skills/n2d-script/scripts/boundary_review.py sign <作品根> '<blocker_id>' \
...
python3 skills/n2d-script/scripts/boundary_review.py check <作品根> 1-10 --json
```
...
```bash
python3 skills/n2d-identity/scripts/memory_anchor.py <作品根> 第N集 --json
...
```bash
python3 skills/n2d-review/scripts/meta_audit.py \
  --root . \
  --evidence skills/n2d-review/references/external_grounding_2026-07-14.json \
  --run-tests \
  --out <作品根>/生产数据/n2d_review_meta_audit.json \
  --json

python3 skills/n2d-review/scripts/self_audit.py \
  --root . \
  --evidence skills/n2d-review/references/external_grounding_2026-07-14.json \
  --run-meta-tests \
...
```bash
python3 -m pytest -q skills/n2d-review/scripts
python3 -m pytest -q skills/n2d-image/scripts
python3 -m pytest -q skills/n2d-identity/scripts
python3 -m pytest -q skills/n2d-script/scripts
