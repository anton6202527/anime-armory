...
| 标准多视图定妆缺失（Q28） | 机+判 | `core_full` 每个形态是否具备正面 / 前3/4 / 侧面 / 后3/4 / 背面五角独立可喂图 + turnaround 人审拼版 + 同源脸部特写/表情锚；`定妆_<角色>_三视图.png` 仅是旧文件名兼容，不表示只需三个角度；其它档位只有在实际转身/过肩/背身镜或升档后才要求相应角度 | `core_full` 缺或本镜需用却缺 🔴 |
| 角色参考图数据库缺口 | 机+判 | 读 `identity_registry.json.reference_atlas.build_tier`：`core_full` 要正面/前3/4/侧面/后3/4/背面五角 + turnaround + 脸部/表情锚；`recurring_standard` 要正面/前3/4/服装/脸锚；`named_minimal` 要正面/服装/脸锚；局部角色只查允许区域。逐镜只引用 ready 路径，实际镜头需要但未升档的角度必须补齐 | 本档或本镜必需项缺 🔴 / 假引用 planned 🔴 |
