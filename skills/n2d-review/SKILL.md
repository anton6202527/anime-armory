...
- **阶段 gate（确定性，生产前跑）**：正式生产入口统一用 `python3 skills/n2d-dashboard/scripts/dashboard.py gate <作品根> 第N集 --stage image_preflight|video_preflight|image|video|compose|review`。默认在正式调用后端前跑 `image_preflight` / `video_preflight`，生成落档后跑 `image` / `video` 回验。它会调用底层 `scripts/gate.py --json`，把高风险流程规则脚本化检查并把 `block/warn/info`、回流 stage、重跑范围写进 `生产数据/`，同时外发 `gate_findings_<stage>_第N集.json`（kind=`n2d_consistency_findings`，可直接交给 `n2d-batch --from-consistency-findings`）；`block` 退出码 1，先修再进入该阶段。底层 `scripts/gate.py --json` 只作调试/机器消费入口，不作为生产推荐入口。检查范围包括：合规包、`storyboard.json` continuity、尾帧、prompt 检查段、共享定妆、资产身份注册层、占位配音、clip 原生音轨/原生音画 opt-in/时长、基础视觉风格契约、复杂镜头模板、Motion Control、生图 AI 一致性、**P0 语义谱系 / P1 状态百科前置**；`review` gate 另跑 **P2 多模态漂移**。若 `_设置.md`/env 声明 `一致性严格度=production`，或处在 `compose/review` 交付边界、已有多集 storyboard/成片交付产物且未显式写 `demo/宽松`，轴线/视线、音画同步、节奏密度等 advisory 在重复、关键场景或交付边界会升级为 BLOCK。确认为可接受时，需写结构化 `生产数据/consistency_advisory_signoff_第N集.json` 后复跑；字符串或只写维度不再放行。
  ...
  - **原生音画物理一致性硬闸**：`制作模式=原生音画` 时，`video_preflight` 要求出视频总览包含「原生音画物理一致性契约」，字段齐全：声源归属、口型策略、材质/动作声、空间声学、字幕/后期策略。缺任一项 BLOCK 回 `video_prompt`；这是防止原生台词/口型/动作声/混响各自漂移的前置契约。
...
- **人判（LLM 判断题）**：机检覆盖不了的语义维度——崩脸/场景漂移、**clip 接缝跳切/闪烁**、构图景别（对 `n2d-script/references/分镜语法.md`）、节奏体感（对 `n2d/references/导演节奏.md`）、口型、原生音频双人声。逐维见 `references/checklist.md`。
  - **`音画同步` advisory 机检（AV1·2026-06 补齐）**：`scripts/lipsync_consistency.py` 用 SyncNet / 字节 LatentSync / AVSyncNet 量**口型↔配音 AV 偏移**——补上「无 SyncNet」这个早已过时的旧缺口。诚实边界：风格化嘴型 + SyncNet 弱置信不稳，故基础审计 `verdict` 仍封顶到 `warn`，低置信镜判 `inconclusive`（不罚分）；但 `一致性严格度=production` 下，重复同维度、关键场景或 compose/review 交付边界的 AV1 WARN 会由 gate 升 BLOCK，除非写入 `consistency_advisory_signoff_第N集.json` 签收。**实测严重档**另存喂 `n2d-score`（非 gate 退出码）。重活（脸轨抽取 + 推理）需 out-of-repo conda 环境——生产端是 **`scripts/lipsync_measure.py`**（SyncNet 桥接），在 facefusion 环境跑成片→逐 Clip 偏移→写 `生产数据/syncnet_raw_第N集.json`：`conda run -n facefusion python skills/n2d-review/scripts/lipsync_measure.py <作品根> 第N集`（SyncNet 仓库默认 `~/syncnet_python`，env `SYNCNET_HOME` 可改；本机已 clone + 下权重 + facefusion 装好依赖）。消费端缺后端/ffmpeg 且无外部报告时优雅跳过交人判，绝不臆造分。main() 还写 `生产数据/lipsync_第N集.json` 供 `n2d-score` 的 `check_lip_sync` 自动消费（把仅数关键词的启发式升级为实测）。
  ...
  - **视频侧一致性补强（VLM1/VSEM/DAV/CG1/CAM1/MOT1/S2V）**：`consistency_audit.py` 已接入 7 个纯标准库 sidecar 消费器：`video_vlm_consistency.py` 读 VLM/LMM 成片判断题报告并校验 judge schema/抽帧/问题链/投票元数据，`video_semantic_consistency.py` 读 DINO/CLIP/DreamSim 主体/背景相似度，`dialogue_av_consistency.py` 查原生多人对话的说话人、台词轮次、turn-taking、幻觉说话人、口型和镜头对人，`causal_event_consistency.py` 按规则库查碰撞/重力/破碎/燃烧/液体/接触连续/状态守恒等物理因果链证据，`camera_trajectory_consistency.py` 查相机运动、越轴、深度和抖动轨迹，`motion_quality_consistency.py` 查运动平滑、动态程度、冻结帧、jerk 和动作完成度，`subject_video_consistency.py` 查图生视频主体保真、自然度、文本相关、背景解耦和多主体串换。`video_eval_runner.py` 先生成抽帧/问题/sidecar manifest；重模型 runner 不绑进主审计，只要把报告写到 `生产数据/*_第N集.json` 或对应 `出视频/合成` sidecar，审查链路会统一产出 findings。若项目已有成片/视频且文本明显触发原生多人对话、物理因果、相机运动、主体视频或运动质量风险但缺报告，审计给 WARN，让缺证据本身可见。
  - **角色 DNA 用图判**：先读 `设定库/角色圣经.md`（人读总入口）和 `出图/共享/identity_registry.json`（机器真值），再把该集 `出图/第N集/图片/镜头*.png` 与 `出图/共享/图片/定妆_<角色>.png` / 半身 / 侧背 / 表情参考 **并排读图比对**，按五层判：脸（脸型/五官/疤鳞痣）、发型（发色/发髻/披发轮廓/发饰）、服装（款式/主色/剪影）、配饰（发簪/腰牌/钥匙/护甲等）；气质/动作习惯按角色圣经做人审表演层。任一层漂到观众认人断层都标问题。装了 `insightface` 时跑 `scripts/face_consistency.py <作品根> 第N集` 给脸部余弦相似度分——**不写死阈值**，而是**自标定 flag-band**：用本作定妆组内部互相余弦（同角色 正脸↔侧脸/半身 本就该高度相似）当"同一人下限"地板，每镜低于 地板−margin 标 🔴、地板带标 🟡（写死 0.45 对风格化脸要么误杀要么放过；业界 ArcFace 同人≈0.5–0.68 且因画风而异）。**脸 embedder 是选择点 `脸一致性机检后端`（arcface 默认 / styleid·见 `n2d/references/选择点与偏好.md`）**：ArcFace 按真人脸标定，**风格化/漫剧脸 TPR 系统性掉到 ~0.76**（StyleID 论文 arXiv:2604.21689·2026-04），风格化项目切 `styleid`（CLIP-L+StyleID LoRA·需 `N2D_STYLEID_MODEL` 权重，缺权重自动回退 arcface 标 `arcface_fallback`，绝不用裸 CLIP——裸 CLIP 更差 0.17–0.25）；CLI `--encoder arcface|styleid`，解析顺序 CLI>`_设置.md`>env `N2D_FACE_EMBEDDER`>默认 arcface。**fidelity-gate**：跑 `vlm_verify --write` 落 canonical 通过表后，崩 canonical 的镜不进一致性均值（防"稳定地错"刷高分·EntityBench 2026），单列 `render_errors` 回 `n2d-image`。**报告型一致性 KPI**：`n2d-score` 的 `character_consistency_kpi`（intra/cross 对照可投放线 `N2D_CONSISTENCY_RELEASE_LINE`·须随 encoder 重标定）。注意：进入视频前不只靠审片人“看过”，必须先由 `image_qc.face_reference_coverage` 证明每张已落档角色 PNG 都有 full 精度定妆/身份主参考比对记录；缺库、降级、warn/noface、缺比对行都回 `n2d-image`。
  ...
  - **2026 新增一致性机检（6 类·全部由 `scripts/consistency_audit.py` 一键编排自动跑，各自缺库优雅跳过）**。**出视频前硬挡**：这套总审已前移接进 `gate.py --stage image`（出图后闸门），手部多指/天气硬跳/极端身高/禁用称谓/动作硬断等**确定性 🔴 维度**在最近的出图闸门即 BLOCK，挡在最贵的「出视频」之前（不再只在 compose/review 才拦）；降级精度（缺 insightface）在 image 闸门只降 WARN（不硬拦无依赖产线），交付边界 compose/review 才对降级精度 BLOCK，逃生口统一 `N2D_ALLOW_DEGRADED_QC=1`（留痕自负）。各维度：
    ...
    - **天气/时辰推进连续性（W1）**：`scripts/world_continuity.py`——`scene_dna` 能锁 `光色天气` 却无检查器。装 Pillow 测每镜亮度/暖度归 day/dusk/night，同场景相邻镜 day↔night 硬跳=🔴、隔一档=🟡，违 scene_dna 锁=🔴。
  ...
  - **原生音画物理一致性用人判**：审原生音画成片时，先对照总览的「原生音画物理一致性契约」抽查说话镜和动作声：谁发声是否来自画内主体、口型是否随台词变、脚步/衣料/器物声是否只随可见动作触发、远近/遮挡/混响是否随景别和空间变化、whisper 字幕是否能对上原生台词。修法优先回 `video_prompt` 明确音画策略并重出受影响 Clip；后期只补字幕，不用成片硬剪掩盖双人声或错口型。
...
2. **跑机检 + 一致性编排 + 身份闭环 + 合规包 + QA 入账** → 确定性问题清单（按集）：先 `scripts/mechanical_check.py`（字幕/对账/占位），可先用 `scripts/video_eval_runner.py <作品根> 第N集 --write` 生成视频抽帧/问题/sidecar manifest，再 **`scripts/consistency_audit.py <作品根> 第N集` 一键串跑视觉/语义一致性检测器**（**语义谱系 P0 · 状态百科 P1 · 多模态 P2 · 视觉契约继承** · 锚点门 N3 · 脸 G1 · 服装配色 N1 · 片内时序 N2 · 场景 O2 · 糊 N4 · **风格 S1** · **接缝接力** · **字幕对齐 L1**[双语短语边界/阅读速度/译文完整性，补 mechanical_check 条数对账盲区；`scripts/subtitle_align.py` 可单跑] · **音画同步 AV1**[口型↔配音偏移·advisory·`scripts/lipsync_consistency.py`] · **空间站位 B1**[跨镜站位/遮挡序四维坐标连续性·`scripts/scene_blocking_continuity.py`·违注册站位锁=block] · **视频侧一致性补强**[视频 VLM 判题 VLM1、视频语义一致 VSEM、多人对话音画 DAV、物理因果链 CG1、相机空间轨迹 CAM1、运动质量 MOT1、主体视频一致 S2V；读 sidecar，不绑重模型依赖] · **生产一致性补强**[物件常驻 O3、持有账本 POS、视线/状态回读 X2、状态转场视频证据 ST1、交互接触 I1、结构化交互图谱 I2、成片统一 C1、成片时间线探针 FT1、生成配方 RCP、强配方 Schema RCP2、系列包装 PKG、台词语域 D1、场景平面 FP1、成本路由 K1、人审校准集 CAL（含误报/漏报→`scripts/calibrate_thresholds.py --write` 阈值建议闭环）、一致性探针包 PROBE（含后端 benchmark→`scripts/probe_route_recommend.py --write` 路由建议校验）；`scripts/production_consistency.py`，纯标准库]）——默认同时**结构化外发** `生产数据/consistency_findings_第N集.json`（kind=`n2d_consistency_findings`，逐条带维度/严重度/镜头定位/`return_to_stage`，`n2d-feedback` 读它做一致性回灌；`--no-export` 关闭），并生成 `生产数据/generation_recipe_第N集.json` 的配方 hash 账本、登记一条 dashboard `consistency_findings` 事件；脸 G1 在无 insightface 时自动降级 **Pillow 基础机检**（图存在/可解码/分辨率/清晰度，标 `pillow_fallback`+`insufficient_precision`，绝不臆造相似度，n2d-score 按降权分消费），再跑 `python3 skills/n2d-identity/scripts/identity.py <作品根> --write` 生成 adapter matrix、跨集漂移报表和 `consistency_findings_voice_print_第N集.json` 声纹一致性 findings；缺 `合规/compliance_manifest.json` 时先跑 `python3 skills/n2d-compliance/scripts/compliance.py <作品根> 第N集 --init` 并人工补齐。阶段 gate 结果必须同步跑 `python3 skills/n2d-dashboard/scripts/dashboard.py gate <作品根> 第N集 --stage review` 或对应生产 stage 入账。别只跑单个检测器漏掉其余。
