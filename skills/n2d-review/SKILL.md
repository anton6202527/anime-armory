...
- **机检（确定性，先跑）**：`scripts/mechanical_check.py <作品根> 第N集` —— 秒级出确定性问题：字幕↔配音文本/时间码对账、中英字幕错位、占位未精修、单行溢出、配音占位音色未替换、产物完整性对账、钩子/集尾留存信号缺失。
  ...
  ```
- **人判（LLM 判断题）**：机检覆盖不了的语义维度——崩脸/场景漂移、构图景别（对 `n2d-script/references/分镜语法.md`）、节奏体感（对 `novel2drama/references/导演节奏.md`）、口型、原生音频双人声、合规水印。逐维见 `references/checklist.md`。
...
| 🟡 建议级 | 场景轻漂、构图/景别违 `分镜语法`、节奏塌/钩子弱/集尾不够、字幕溢出、卡点没对上爽点时间戳 | 建议改 |
