...
BENIGN_SURPRISE_PHRASES = ("受宠若惊",)


def _emotions_in(text: str) -> set:
    """文本命中的粗情绪桶集合（纯函数·可测）。"""
    normalized = str(text or "")
    for phrase in BENIGN_SURPRISE_PHRASES:
        normalized = normalized.replace(phrase, "受宠若喜")
    return {bucket for bucket, kws in EMOTION_BUCKETS.items() if any(k in normalized for k in kws)}


def _expression_scope_char_ids(clip: Mapping[str, Any], text: str) -> List[str]:
    """EXP1 should track characters intended to be in-frame, not forbidden/offscreen ids.

    Prompt sections contain forbidden_presence/offscreen_presence and reference paths; using
    every CHAR_* token from the merged text turns those bookkeeping ids into false emotion
    jump findings. Prefer structured storyboard presence, then fall back to text for legacy
    projects that only have prose prompts.
    """
    refs: List[str] = []
    ...
    def add(value: object) -> None:
        ...
        if isinstance(value, str):
            for match in re.finditer(r"\bCHAR_[A-Za-z0-9_:-]+\b", value):
                refs.append(match.group(0).rstrip("_"))
            return
        if isinstance(value, Mapping):
            for key in ("id", "character_id", "primary_subject"):
                add(value.get(key))
            for key in ("character_ids", "characters", "required_presence"):
                add(value.get(key))
            return
        if isinstance(value, list):
            for item in value:
    ...
    add(entity.get("characters"))
    add(entity.get("required_presence"))
    intent = clip.get("shot_intent") if isinstance(clip.get("shot_intent"), Mapping) else {}
    add(intent.get("primary_subject"))
    for shot in clip.get("shots") if isinstance(clip.get("shots"), list) else []:
        if not isinstance(shot, Mapping):
            continue
        shot_entity = shot.get("entity_schedule") if isinstance(shot.get("entity_schedule"), Mapping) else {}
        add(shot_entity.get("characters"))
        add(shot_entity.get("required_presence"))
    scoped = [x for x in pc._unique(refs) if x.startswith("CHAR_")]
    return scoped or pc._char_ids(text)
...
def check_expression_continuity(root: str, ep: str) -> dict:
    ...
    for _clip, label, text in _clip_iter(root, ep):
        ...
        chars = _expression_scope_char_ids(_clip, text)
