...
truth.  It promotes only structured human-review receipts that are bound to the
current character/form/tier/view/path/PNG hash.  ``production_consistency.py``
then independently rechecks the emitted artifact and registry fingerprint.
"""
...
class ReceiptError(ValueError):
    """The requested human receipt is unsafe or does not bind exactly."""
...
def record_current_view_receipt(
    ...
    *,
    ...
    """Atomically record one explicit, current-pixel human review receipt.

    This deliberately never creates a missing view, revives ``planned``, or
    finalizes a character form.  A caller must explicitly attest that a human
    inspected the current pixels; automation identities cannot be reviewers.
    """
    ...
    if _reviewer_is_automated(reviewer):
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="从逐视图人审收据构建核心人物 identity_eval_pack")
    ...
    parser.add_argument(
        "--record-current-view",
        action="store_true",
        help="原子记录一个核心人物当前 PNG 的显式逐视图人审收据，不会 finalize 整个形态",
    )
