...
1. **先定字幕语言**（`字幕语言` 选择点，不写死）：按 `../_偏好约定.md` 读 `<作品根>/_设置.md`→全局默认→首次问一次→写回。默认 **中文**（国内投放，只产 `字幕_中文.srt`）；**中英双语/仅英文**（海外投放）时，**先按 `references/formats.md §9` 把英文翻译写进 `脚本/第N集/字幕_英文.srt`**（任意时间码即可，finalize 会重定时），再跑桥接脚本。
   ...
   ```
   产出 `脚本/第N集/字幕_中文.srt`（必出）+（中英双语/仅英文时）`字幕_英文.srt`（均按配音逐句实测时长重定时）+ `脚本/第N集/镜头时长.json`（每镜聚合时长）。`finalize` 默认 = 已存在英文字幕源就重定时、没有就只产中文（`SUB_LANG` 可显式覆盖）。
...
**完成后**：`_进度.md` 勾选 `分镜设计` / `素材清单` / `字幕中` ✅（`字幕语言`=中英双语/仅英文 时再勾 `字幕英`，中文档不产英文不勾）。下一步 /n2d-image。
