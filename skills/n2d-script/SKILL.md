...
- **导演运镜审查 + Prompt 落地（`director_camera_plan.py`）**：阶段2 定稿后读取 `storyboard.json`，逐 Clip 审查故事节奏、景别、既有 `camera_motion`/`template_contract.camera_rule` 是否结构化，输出 `生产数据/director_camera_plan_第N集.json/md`。这份 sidecar 把导演意图拆成可复制到出图的 `起幅·运动余量` 和可复制到出视频的 `导演意图/起幅/落幅/镜头运动/运动精修`，不自动覆写分镜，避免把审美建议伪装成硬闸。**下游 gate 强制消费**：`n2d-review` 的 `check_director_camera_plan_consumption` 在出图(`image_preflight`)/出视频(`video_preflight/video`)付费前核对 prompt 包是否出现导演运镜词汇——sidecar 在但 prompt 零消费时，含高潮/关键镜(KEY_SCENE_MARKERS)→BLOCK、普通镜→WARN（文档级收据，堵"规划好却没落进 prompt"）。
