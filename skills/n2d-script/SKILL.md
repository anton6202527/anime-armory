...
> **首跑先定 `制作模式`（必给菜单，别静默默认）**：拆集前把出片顺序菜单 + 一句原因念给用户选一次——**A 配音先行（默认·长期量产推荐；先 TTS 固定角色音色和时长，再生成无声 Image2Video，BGM/SFX 分层，FFmpeg 合成）/ B 原生音画（快速预览或特殊后端；说话镜一次出台词+口型+环境声，少逐句音色控制）/ C 先出视频后配音（快速 demo 或配音还没就绪；代价是后期补音对不准、可能重切重出）**，选后写 `_设置.md`。菜单原话与完整理由见 `n2d` SKILL「制作模式 · 首跑选择」段。用户已明确表态时按 `skills/n2d/references/选择点与偏好.md` 解析顺序第 0 条直接落档、不再问。
...
> 流程铁律（默认 **配音先行**）：`剧本改编 → 角色配音 → 统计每句台词时长 → 分镜设计 → 出图 …`。配音先行下**分镜在配音之后**——镜头怎么切、每镜多长由真实配音时长决定（节奏可控、后期省成本），视频层只产无声 Image2Video。**原生音画模式例外**：说话镜由视频后端一次出同步音画、无逐句配音，分镜不等 n2d-voice，直接按 `storyboard.json clips[].duration` 估时定稿。**先出视频后配音**只用于快速 demo：先估算时长，真实配音后补，需复述返工风险。本步只做"可配音的剧本"。
...
完成后在 `_进度.md` 勾选阶段1 列：`剧本改编`(voiceover) / `bgm` / `封面` ✅。**下一步按 `制作模式` 走**：默认 `配音先行` 先 `n2d-voice` 配真实音再回跑阶段2；`原生音画` 才直接回跑本 skill 做阶段2分镜；`先出视频后配音` 先用 `n2d-voice` 产占位时长并复述不推荐警告。不要直接出图。同时记录生产数据：
...
> **`制作模式`=`先出视频后配音`（快速 demo·不推荐）时的差异**（见 n2d SKILL「制作模式」节，并向用户复述其不推荐理由）：`时长清单.json` 是 **估算/占位**（先跑一次 `n2d-voice` 出占位轨当时间脚手架，真实配音留到出视频后）。本步 `finalize_storyboard.py` 的占位硬闸门用 `FINALIZE_ALLOW_PLACEHOLDER=1` 放行——产出的字幕/镜头时长是**估算值**，真实配音补上后**很可能要回跑本步重定时、甚至重切重出部分镜头**。`配音先行`（默认）和 `原生音画` 都不要加这个环境变量。
