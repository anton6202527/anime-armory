...
> **首跑先定 `制作模式`（必给菜单，别静默默认）**：拆集前把出片顺序的 A/B/C 菜单 + 一句原因念给用户选一次——**A 配音先行（推荐，配音已就绪选这条）/ B 先出视频后配音（快速 demo 或配音还没就绪；代价是后期补音对不准、可能重切重出）/ C 原生音画（native AV；说话镜一次出台词+口型+环境声，少逐句音色控制）**，选后写 `_设置.md`。菜单原话与完整理由见 `n2d` SKILL「制作模式 · 首跑选择」段。用户已明确表态时按 `skills/n2d/references/选择点与偏好.md` 解析顺序第 0 条直接落档、不再问。
...
> **`制作模式`=`先出视频后配音`（快速 demo·不推荐）时的差异**（见 n2d SKILL「制作模式」节，并向用户复述其不推荐理由）：`时长清单.json` 是 **估算/占位**（先跑一次 `n2d-voice` 出占位轨当时间脚手架，真实配音留到出视频后）。本步 `finalize_storyboard.py` 的占位硬闸门用 `FINALIZE_ALLOW_PLACEHOLDER=1` 放行——产出的字幕/镜头时长是**估算值**，真实配音补上后**很可能要回跑本步重定时、甚至重切重出部分镜头**。`配音先行`（默认）不要加这个环境变量。
