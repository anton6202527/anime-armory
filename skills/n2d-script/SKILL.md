...
- **平台无关核心 + 平台档案**：分镜/卡片/节拍/字幕都平台无关；各平台档案见 `references/platforms.md`。目标视频模型读 `_设置.md` 的 `生视频模型`，调用产品/API 读 `生视频渠道`；新作品首跑必须给菜单选一次。旧 `生视频AI` 只作兼容 fallback。
...
> **首跑先定 `制作模式`（必给菜单，别静默默认）**：拆集前把出片顺序菜单 + 一句原因念给用户选一次——**C 原生音画（默认·推荐；native AV，说话镜一次出台词+口型+环境声，最快看到出图/出视频、无重同步返工，需原生音画后端，少逐句音色控制）/ A 配音先行（追求逐句配音表演、或后端 image2video-only 时选；真实配音时长驱动镜头，音画最准）/ B 先出视频后配音（快速 demo 或配音还没就绪；代价是后期补音对不准、可能重切重出）**，选后写 `_设置.md`。菜单原话与完整理由见 `n2d` SKILL「制作模式 · 首跑选择」段。用户已明确表态时按 `skills/n2d/references/选择点与偏好.md` 解析顺序第 0 条直接落档、不再问。
> **首跑还要定 `生视频模型` + `生视频渠道`（必给菜单）**：拆集前把默认模型和调用渠道菜单念给用户选一次——模型示例 **Seedance 2.0 / Veo 3.1 / Kling 3.0 / Hailuo / Runway / Luma / Pika / 开源 / manual**；渠道示例 **即梦/Dreamina / 豆包 / 海螺AI / 可灵Kling / Google Gemini API / Runway API / 本地开源 / manual**。选后写 `_设置.md`；已有 `_设置.md` 或用户本轮已明确指定时直接沿用/覆盖。注意模型只作为普通镜/兜底模型，默认 `视频模型路由=自动按镜头路由` 时复杂 Clip 仍由 `n2d-model-router` 逐镜选择 primary/fallback。菜单原话见 `n2d` SKILL「生视频模型 + 渠道 · 首跑选择」段。
> **首跑再定 `基础视觉风格`（必给菜单）**：写 `global_style.md` 前按 `n2d/references/visual_styles.md` 展示菜单（写实电影感 / 国漫写实 / 二次元赛璐璐 / 水墨国风 / 厚涂幻想 / 赛博霓虹 / Q版轻喜 / 自定义）。若用户本轮已指定或 `_设置.md` 已有则直接用；若只有全局默认，把它标为预选但仍展示菜单。选后写 `_设置.md`，同项目沉默沿用。
...
跟用户确认（缺省即用默认）：
- 小说路径
- **目标视频模型 + 渠道**（模型决定普通镜/兜底能力与 image2video 运动估计分布；渠道决定实际调用哪个产品/API）—— 读 `_设置.md` 的 `生视频模型` / `生视频渠道`；新作品首跑必须给菜单选一次。旧 `生视频AI` 兼容读取
...
详见 `references/platforms.md` 的"两轴架构"章节。**输出位置 = 作品根（与 `小说/` 同级）**：脚本素材进 `脚本/第N集/`；全局 `_进度.md` 放作品根；`global_style.md` / `characters/` / `locations/` 进作品根的 `设定库/`。把视频模型、视频渠道、图 AI 都记到 `设定库/global_style.md` 顶部。

> **关键铁律**：若**图 AI ≠ 生视频模型**，所有 image prompt 末尾**必须**拼接对应视频模型的"图像风格锚定句"（详细在 `n2d-image` skill 里）。Stage 1 这里只负责把决策写进 `global_style.md` 顶部。
...
见 `references/platforms.md`（各视频模型/渠道/图 AI 档案：提示词语言/画幅/Clip时长/角色一致性机制/运镜词/负面词，及"如何新增平台"）。目标视频模型以 `_设置.md` 的 `生视频模型` 为准，调用渠道以 `生视频渠道` 为准；新作品首跑问一次，不静默固定即梦。旧 `生视频AI` 只作兼容 fallback。
