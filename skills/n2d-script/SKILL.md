...
- **传统正反打落地铁律（对话/对峙）**：`dialogue_shot_reverse` 不是“换个角度拍两个人”，而是 180° 行动轴线 + 屏幕左右关系 + 互补视线 + A/B 成对 coverage。凡使用该模板，`template_contract` 必填 `axis / screen_sides / eyeline / shot_pairing / coverage_order / camera_coverage / lens_height_distance_match / crossing_axis_policy / buffer_or_reestablishing`；P-2 `axis_blocking_map.json` 必须物化 `shot_reverse_patterns[]`，P-3 `continuity_bible.json` 必须继承 `shot_reverse_continuity`。无理由越轴、A/B 左右交换、看镜头替代看戏内对象、近景多人同框不拆反打，全部按连续性硬伤处理。
