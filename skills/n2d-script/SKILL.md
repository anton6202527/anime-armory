...
本 skill 涉及的选择点：`制作模式`（决定阶段2 是否等真实配音·见阶段2 触发）、`生视频AI`、`生图AI`、`画幅`、`单集时长`（拆集/台词篇幅·见第 1 步预设）、`首切范围`（首次拆集是部分先切还是全篇粗切·见第 1 步）、`字幕语言`（中文/中英双语/仅英文·见阶段2）、`脚本批次`（量产时几集一停·见「脚本批次」段）。
...
- **衔接是一条接力链，本 skill 是唯一真值源**：clip 之间能不能顺滑，取决于上游一次把"接缝契约"定死、下游逐级继承而不重新发明。本 skill 在 `故事板.md` 的「衔接设计」里把每个接缝写成**单一契约**——`上一 Clip 的出点 = 下一 Clip 的入点`（同一句话，不是各写各的），并给每个接缝标 `转场类型` + `需要尾帧?`。下游据此协作：n2d-image 在标了 `需要尾帧` 的接缝出**尾帧 PNG**（=下一 Clip 首帧构图），n2d-video **读取**契约而不重写 start_state、有尾帧就用首尾双帧引导，n2d-compose 按 `转场类型` 决定接法（硬切/微溶解/空镜缓冲），n2d-review 逐接缝验收。详见 `references/formats.md §4` 接力字段。
...
3. `故事板.md` + **`storyboard.json`** — Clip 表（相邻分镜合片段；人物运动 + 镜头运动 + 动态细节 + **衔接设计**）。`storyboard.json` 是必需的机器可读接力契约；缺它时下游 gate 会阻断。**Clip 时长 = 所含镜头时长之和（来自 `镜头时长.json`，配音驱动）**；**超所选后端单 Clip 上限才拆 Clip——上限按后端读 `n2d-video/references/platforms.md`「单 Clip 上限铁律」（即梦 image2video≤8s / Seedance 2.0≤15s / 可灵多镜较长 / Veo≈8s），别一刀切 8s**；能一镜到底就别切碎（更长单镜=更少拼接缝、跨镜更稳）。**相邻 Clip 首尾帧构图对齐**（尾帧=下一首帧，用 frames2video 双帧引导更连贯）。
   - **每 Clip 必写衔接设计（接力契约）**：入点 / 出点 / 转场方式 / 轴线与视线方向 / 是否需要空镜缓冲或声音先行(J-cut) / `需要尾帧?`。**铁律：本 Clip 的「出点」必须原样成为下一 Clip 的「入点」**（同一句描述，避免相邻镜各写各的、语义漂移）；并给接缝标 `转场类型`（match cut / eyeline / 动作切 / 空镜缓冲 / J-cut / 硬切）与 `需要尾帧?`（同场景·人物姿态连续的硬切=是，换场/空镜/大跳切=否）。这不是备注：n2d-image 据 `需要尾帧` 出尾帧 PNG，n2d-video 把契约转成 prompt 约束、读取上一镜出点作 start_state，n2d-compose 据 `转场类型` 决定接法与卡点。缺衔接设计的故事板只能 rough preview，不能正式出视频。
...
**完成后**：`_进度.md` 勾选 `分镜设计` / `素材清单` / `字幕中` ✅（默认中文-only 不勾 `字幕英`；项目显式选中英双语/仅英文且已产 `字幕_英文.srt` 时才同步勾 `字幕英`）。下一步 /n2d-image。
