...
- **story_clip / video_shot 双层铁律（防 30 秒啰嗦长镜）**：阶段2 的 `storyboard.json clips[]` 可以作为 `story_clip` 表达一个完整小情节，但付费视频生成和剪辑必须落到 `video_shot` 物理镜头。`story_clip` 允许 15-35s 承载剧情段落；`video_shot` 目标 4-8s，常规软上限 12s，硬上限 15s。任何 `clip.duration > 12s` 必须在 `shot_split_decision.py --write` 里生成 `video_shot_segments[]` 拆镜计划；`clip.duration > 15s` 不允许作为单个付费视频段直提，必须拆成 `Clip_XX_partN`/`video_shot` 后再交 n2d-video。长段不是硬剪成短片，而是拆成建制镜/动作细节/人物反应/落幅钩子的可剪辑镜头。
