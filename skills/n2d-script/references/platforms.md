...
> **机器真值源**：视频后端别名、`max_clip_seconds`、原生音画后端集合等可执行字段集中在 `skills/common/n2d_platform_profiles.py`；生图后端白名单在 `skills/common/n2d_contract.py` 的 `APPROVED_IMAGE_BACKENDS`。本文件用于分镜阶段解释平台皮，若与 common 模块不一致，以 common 模块为准。
...
- **图 AI**：出图工具。按 `生图AI` 选择点执行，默认 Codex，可选官方/已登录多参考后端（Dreamina/即梦官方 CLI / Seedream / 可灵主体库 / Nano Banana / Sora Cameo 等）。不得用“同视频AI”含糊口径变相出图或兜底；第三方逆向 CLI/web 自动化仍禁。
...
**默认 = 图 AI(Codex) + 视频 AI(可为即梦/Seedance/可灵/Veo)**。图阶段不再等于即梦。
...
| 场景 | 组合 | 备注 |
|---|---|---|
| 国风短剧（默认） | `生图AI` 所选官方/已登录图后端 + 即梦/Seedance 锚定句 → 即梦/Seedance 视频 | 默认 Codex；可选 Dreamina/Seedream 等后端；全项目统一 |
| 国风短剧（多参考增强） | Seedream/可灵主体库/Nano Banana/Sora Cameo 等官方图后端 + 对应视频锚定句 → 视频后端 | 只在一致性吃紧或用户选择时切换；不可混用 |
| 国风短剧（即梦闭环） | Dreamina/即梦官方 CLI 出图 + 即梦视频 | 已登录会员可直调；整集统一后端 |
| 国风短剧（同家后端） | 官方图后端 + 同厂视频后端 | 可选增强，不作为兜底；仍需合规和落档能力 |
| 海外英文短剧 | Imagen/DALL-E + Veo 锚定句 → Veo | 全英文 prompt 优先 |
| ❌ 禁止 | 第三方逆向 CLI / `同视频AI` 含糊口径 / web 自动化 | 未授权路径禁用 |
...
### 禁止：第三方逆向 / web 自动化出图
Dreamina/即梦官方 CLI 可作为图 AI 使用；不要使用第三方逆向版 CLI，也不要进入即梦 web 自动化出图。视频 AI 仍可选即梦/Seedance，图阶段用 `生图AI` 显式选择的后端生成首帧并拼视频 AI 锚定句。
