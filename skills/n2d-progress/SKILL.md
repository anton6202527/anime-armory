...
- **`原生音画`（默认 · native AV）**：`配音` 列对主流程视作可选旁白层；脚本不会因为 `配音=⬜` 把前沿推去 `n2d-voice`，会直接推进分镜/出图/出视频。说话镜由原生音画后端一次出台词+口型+环境声，合规（角色/声音克隆授权）仍照常 gate。
- **`配音先行`（强配音控制时选）**：按上表线性路由；`配音=⏳rough` 只算已开始，不算完成，前沿仍会指向 `n2d-voice` 补真实配音。
