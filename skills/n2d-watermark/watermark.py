#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# n2d-watermark — n2d 线专属水印（自包含，随 n2d 线打包）。
...
import sys, os, subprocess, shutil, argparse, tempfile
...
def _load_project_setting(root, key):
    if not root:
        return ""
    common_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "n2d", "_lib"))
    ...
    try:
        ...
        return ""
    return common_settings.get_setting(root, key, "")


def _apply_settings_defaults(a):
    root = getattr(a, "settings_root", None)
    if not root:
        return a
    pref = _load_project_setting(root, "水印")
    if a.mode is None:
        if "AI" in pref:
            a.mode = "ai"
        elif "品牌" in pref:
    ...
    if a.mode == "brand":
        if not a.text:
            a.text = _load_project_setting(root, "水印文字") or None
        if not a.logo:
            a.logo = _load_project_setting(root, "水印logo文件") or None
        if not a.pos:
            a.pos = _load_project_setting(root, "水印位置") or None
        if a.opacity is None:
            a.opacity = _parse_float(_load_project_setting(root, "水印透明度"))
        if a.scale is None:
            a.scale = _parse_float(_load_project_setting(root, "水印大小"))
    return a


def _finalize_defaults(a):
    if a.mode is None:
        a.mode = "ai"
    if a.opacity is None:
        a.opacity = 1.0
    if a.scale is None:
        a.scale = 0.12
    if a.fontscale is None:
        a.fontscale = 0.030
    if a.descscale is None:
        a.descscale = 0.62
    if a.margin is None:
        a.margin = 0.02
    return a
...
def do_video(a):
    ...
    fd, badge = tempfile.mkstemp(prefix="_wm_overlay_", suffix=".png", dir=out_dir)
    os.close(fd)
...
def main():
    ...
    ap.add_argument("--mode", choices=["ai", "brand"], default=None)
    ap.add_argument("--settings-root", default=None, help="作品根；读取 _设置.md 中的品牌水印默认参数")
    ...
    ap.add_argument("--opacity", type=float, default=None, help="brand 文字/logo 透明度 0~1")
    ap.add_argument("--scale", type=float, default=None, help="brand logo 宽 / 画面宽")
    ap.add_argument("--fontscale", type=float, default=None, help="主文字字号 / 画面高")
    ap.add_argument("--descscale", type=float, default=None, help="描述行字号 / 主文字字号")
    ap.add_argument("--margin", type=float, default=None, help="边距 / min(W,H)")
    ap.add_argument("--meta", default=None, help="写入元数据 comment（ai 模式有默认值）")
    a = _finalize_defaults(_apply_settings_defaults(ap.parse_args()))
