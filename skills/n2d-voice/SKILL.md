---
name: n2d-voice
description: Voice/timing stage of n2d — 后配音默认时先产占位/估算时长，配音先行时产真实角色配音：turn a 作品 episode's voiceover.txt into per-line audio + stitched voice track + 时长清单.json (默认先出视频后配音用 rough timing 推进分镜/无声视频，真实配音出视频后再补；配音先行时每句实测时长驱动下游镜头时长；逐句记 voice_key 实际应用音色键，一角一色跨集对账数据源，n2d-identity 消费). Multi-backend pluggable (CosyVoice / GPT-SoVITS 本地克隆 / MiniMax / 火山 / macOS say 占位), with voice-cloning + demucs 人声分离. Writes _进度.md 配音 column. Use when asked to 配音, 生成配音, 角色配音, 声音克隆, CosyVoice, GPT-SoVITS, 时长清单. Triggers 配音, 角色配音, 声音克隆, 克隆音色, CosyVoice, GPT-SoVITS, MiniMax配音, 时长清单, voice_key, voiceover.
---

# n2d-voice — 配音与时长（后配音默认 / 配音先行 / 可选旁白层）
...
- **占位分阶段 = 两遍制 rough→refine（关键）**：占位/估算时长是**第一遍 rough timeline**（只为跑通骨架 / 字幕初定时 / 节奏预览），真实配音是**第二遍 refine**（重定时 + 增量重出受影响镜头）。`render_voice.py` 只要发现任一句占位，就回写 `_进度.md` `配音=⏳rough`；只有全句真实音频才回写 `配音=✅`。`配音先行` 模式下 `⏳rough` 不能越过 image/video 付费闸门；默认 `先出视频后配音` 模式下它是时间脚手架，可推进到出视频，但合成前必须补真音并拟合。macOS `say` 占位**只服务出图/出视频前的 rough timing**，不要把它当成可投放配音。
...
- **`制作模式`=`先出视频后配音`（当前默认后配音）时本步跑两次**（见 n2d SKILL「制作模式」节）：第一次（出图前）只出**占位/估算 `时长清单.json`** 当时间脚手架，**不追求音质**；真实配音第二次跑（**出视频之后、合成之前**）。这条路推进画面更快，但要显式保留音画拟合/局部重切风险。`配音先行` 模式永远在出图前就出真实配音，本步只跑一次。
...
> ⚠️ **占位提醒**：若本集是 macOS `say` 占位（`时长清单.json` 有 `占位:true`），明确告诉用户：占位时长不准。`先出视频后配音` 模式可先推进无声视频，但合成前必须换真实配音并拟合；`配音先行` 模式则必须在正式出图/出视频前换真实配音重跑 + 回跑阶段2 重定时，否则 finalize/n2d-video 会硬闸拦截。
...
| 配音先行模式下出图前仍使用 macOS `say` 占位且不告警 | 占位时长不准，配音先行模式出图前必须换真实配音重定时；后配音模式则要明确标成 rough，并在合成前补真音拟合 |
