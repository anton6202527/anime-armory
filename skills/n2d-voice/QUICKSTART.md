...
```bash
python3 skills/n2d-voice/voice_preflight.py prepare <作品根> 第N集
...
```bash
python3 skills/n2d-voice/voice_preflight.py lock <作品根> <角色> \
  --backend <后端> --voice-id <音色ID> \
  --canonical-sample <试听样本路径> --approved-by <签收人>
python3 skills/n2d-voice/voice_preflight.py check <作品根> 第N集 --purpose final
```
...
```bash
N2D_VOICE_PURPOSE=guide python3 skills/n2d-voice/render_voice.py <作品根> 第N集 zh
```
