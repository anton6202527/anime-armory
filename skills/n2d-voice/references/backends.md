...
| GPT-SoVITS | GPTSOVITS_URL, GSV_REF_AUDIO, GSV_REF_TEXT, GPTSOVITS_SPEED/GSV_SPEED | 本地 inference api；优先尝试官方根路径 API（`refer_wav_path` / `prompt_text` / `text_language` / `speed`），失败后自动回退 CosyVoice 兼容 `/inference_zero_shot` |
