...
| 能力 | 干什么 | 推荐 |
|---|---|---|
| **文字编排类 AI** | 拆集、写剧本/台词/分镜、调度整条流水线 | **Claude/GPT/DeepSeek** |
| **文生图 AI** | 出定妆库 + 分镜画面 | **GImage 2/Banana**|
| **图生视频 AI** | 把分镜图变成会动的 clip | **Seedance 2.0/Kling/Veo** |
...
| 3 出图 | `n2d-image` | 两层出图（**共享定妆库** + 本集分镜）→ 统一 **Codex 生图**；禁止即梦/Dreamina 生成图片 | 分镜 PNG |
...
几个已固化的默认：
- 生图 = **Codex only**，禁止即梦/Dreamina 生成图片；生视频默认可用即梦/Seedance/可灵等；视频分辨率 = 720p（可 1080p）
