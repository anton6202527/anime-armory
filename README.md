...
当前 `r2a` 的发布方式是：先从本地 checkout 生成一份干净打包快照，在本机完成安装包构建，再把产物上传到 `https://github.com/anton6202527/anime-armory` 的 GitHub Release assets；安装包不提交进源码目录，也不写进 git 历史。快照会排除私有 agent 配置、`.git/`、`dist/`、依赖缓存和构建 target；桌面端 demo 只保留各线完成度最高的作品（制漫剧保留固定 demo），但 `创作区/写小说/` 会整条线随包保留并作为非 demo seed works 进入桌面端工作区，只有 `_进度.md` 完成度最高的小说作品显示为 DEMO。
...
- `/r2a --all`：本地构建并上传“下载安装”表里的全部安装包：macOS Apple Silicon `.dmg`、Windows `.exe`、VS Code `.vsix`，更新 README 对应下载链接，并把该 release 标为 latest；桌面端安装包带各线冠军 demo，并额外带 `创作区/写小说/` 全部非 demo seed works；VS Code `.vsix` 不复制这些 demo，只保留扩展目录里自带的轻量种子创作区。`--all` 的 README 链接默认使用 `releases/latest/download/...`。
...
scripts/sync_bundles.sh --demo   # desktop 额外内置各线冠军 demo；写小说全线作为非 demo seed works
```
...
`r2a` now builds from a clean snapshot of the local checkout, produces installers locally, and uploads the finished files to GitHub Release assets under `https://github.com/anton6202527/anime-armory`. Installer files are not committed into the source tree or git history. The snapshot excludes private agent config, `.git/`, `dist/`, dependency caches, and build targets. Desktop demos keep each line's most-complete work, with the comic-drama line pinned to `创作区/制漫剧/那妖魔是姜大人`; the full `创作区/写小说/` line is also bundled as non-demo seed works, and only the novel work with the most complete `_进度.md` is marked as DEMO.
...
- `/r2a --all`: builds and uploads every installer in the download table: macOS Apple Silicon `.dmg`, Windows `.exe`, and VS Code `.vsix`, then updates README download links and marks the release as latest. Desktop packages include the selected demo works plus every bundled novel work as non-demo seed works; the VSIX does not copy those demos and keeps only its own lightweight bundled seed work root. For `--all`, README uses `releases/latest/download/...` by default.
...
scripts/sync_bundles.sh --demo   # include line champion demos; bundle all novel works as non-demo seed works
```
