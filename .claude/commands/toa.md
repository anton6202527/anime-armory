---
description: 同步本仓库(anime-arsenal)与 anime-armory：工具文件双向取最新；可选 demos 单向把每类进度最多的1个作品推到 armory 当示例；clean-demos 清空 armory 全部 demo
argument-hint: "[demos|clean-demos] [force]"
...
- `clean-demos` = 第 3 步推**纯工具树**，**清空 armory 全部 demo**（创作区全删）。与 `demos` 互斥，同传时 `clean-demos` 优先。撤下已公开示例用它。
...
**先按本次调用置参数标志**（含 demos→DEMOS=1；含 clean-demos→CLEAN=1；含 force→FORCE=1。demos 与 clean-demos 互斥，CLEAN 优先）：
```bash
DEMOS=0 ; FORCE=0 ; CLEAN=0   # ← 按本次 /toa 调用的参数手动置位
```
...
if [ "$CLEAN" = 1 ]; then
  : # clean-demos：纯工具树，清空 armory 全部 demo —— 已 rm 创作区，什么都不接回
elif [ "$DEMOS" = 1 ]; then
  ...
  # 不带 demos / clean-demos：保留 armory 已有的 demo（从 armory/main 接回，不从 arsenal 推新）
  for top in 制MV 制漫剧 写小说 写歌; do
...
if [ "$(git rev-parse "$ARM^{tree}")" = "$NEWTREE" ]; then
  ...
  MSG="sync(armory): 工具镜像"; [ "$DEMOS" = 1 ] && MSG="sync(armory): 工具镜像 + 精选 demo（每类进度最多1部）"; [ "$CLEAN" = 1 ] && MSG="sync(armory): 工具镜像（clean-demos：清空全部 demo）"
...
- **CLEAN=1**：armory 四类创作区应**全空**（demo 已清空）。
- 都不带：armory 创作区维持原样（之前 demos 推过的保留；没推过则为空）。
