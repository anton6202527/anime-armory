...
- **原作必须是公版 / 用户自有 / 用户明确声明授权**。这条比 novel-fetch 更严：派生作品是原作版权人的专有权利。
- **当代受版权保护的网文/出版物**（起点/番茄/晋江/出版社新书 等）→ **直接拒做**。同人是法律灰色区，本 skill 不替用户跨这条线，即使是非商用、不发布也不行。即便用户本地已有 .txt（合法购书或别处来源），拥有阅读权 ≠ 拥有改编权。
- **判公版**：作者去世 ≥ 50 年（伯尔尼公约下中国/多数地区）、或来源是 Project Gutenberg / 中文维基文库 / 古登堡等公版库、或用户出示 CC/作者授权证明。临界情况让用户自证，并在产物 provenance 里记一笔。
- **遇到当代网文，不要"读了再说"**——即便只是为了"总结剧情"。一旦读了再写派生作品，等于绕铁律一圈。直接拒做，并引导改用公版原作（《晋书》《史记》《聊斋》《封神》《镜花缘》等）或用户自有/已授权作品。
- **续写中不大段复刻原作原文**。原作的对话/动作可以**复述**（"她又问了一遍那个问题"）但不能整段照搬；锚点对齐靠**事件骨架**而非**文本搬运**。这是工艺要求也是法律边界。
- 用户声明授权走 `--i-have-rights` CLI 开关，并在 provenance 中记录。

## 工作流（八步）

总览：**0** 确认输入 → **1** 建骨架 → **2** 设定三卡 → **3** 书名候选 → **4** 章纲 → **5** Demo + 用户审 → **6** 续写余下章节 → **7** 回扫 → **8** 导出。
...
落点 = `作品集/<原作名>-<配角名>外传/`（可 `--out` 改）。骨架：

```
作品集/<原作名>-<配角名>外传/
├── _meta.json                    # 模式 / 规模 / 配角名 / 原作路径 / 书名 (第 3 步填) / provenance
├── 原作.txt                       # 原作纯文本副本（从 .docx 抽出）
├── 设定/
│   ├── 角色卡.md                  # 配角的独立设定（第 2 步填）
│   ├── 世界观.md                  # 从原作摘的规则（第 2 步填）
│   ├── 锚点表.json                # 该配角在原作中的所有出场点（第 1 步候选 + 第 2 步精筛）
│   ├── 书名候选.md                # 5-8 候选 + 评分（第 3 步填）
│   └── 章纲.md                    # 每章一行 outline（第 4 步填）
├── 章节/
│   ├── 第01章.md                  # Demo 三章（第 5 步写）
│   ├── 第02章.md
│   ├── 第03章.md
│   └── ...                       # 余下章节（第 6 步写）
├── 导出/                          # 第 8 步落终态
...
│   ├── <书名>.docx
│   ├── 大纲.md
│   └── n2d-script/               # 仅当用户要这种输出时存在
└── _进度.md                       # 锚点 ✓ / 书名 ✓ / 章纲 ✓ / Demo ✓ / 各章 ✓ 勾选表
```

脚本只做确定性活：建目录、抽 docx → txt、用正则把"明显提到配角名"的章节段落粗筛进 `锚点表.json` 的 `candidates` 字段（待第 2 步精筛）。LLM 工作由主对话的 Claude 接管。

### 第 2 步 — 建设定（人 / 世 / 锚）三卡

由 Claude 主对话执行（不是脚本）。**这是整部续写质量的天花板，必须做扎实。**
...
1. **锚点表精筛** —— 打开 `锚点表.json`，逐个 candidate 段落判断：
   ...
   - 这一表是**整部续写的硬约束**：新文里该角色在对应时间点的状态/位置/已知情报必须和锚点一致。
   - 详见 `references/timeline-anchoring.md`。

2. **角色卡** —— 填 `设定/角色卡.md`：
   - 外观、出身、能力体系、性格底色、动机 / 心结 / 渴望、和原作主角的关系（爱/恨/敬/惧/敌/友）、说话习惯。
   - **关键**：原作里这个角色被作者**留白**的部分（动机模糊、行为突兀、消失的几章 等），列成一张"留白清单"——这是续写的发力点。

3. **世界观卡** —— 填 `设定/世界观.md`：
   - 从原作摘核心规则（修炼体系/魔法规则/政治格局/地理 等），**只摘已确立的**，不要在续写里发明新规则去推翻原作。
...
由 Claude 主对话执行。打开 `设定/章纲.md`，按规模写出每章一行 outline：
...
**不在敲定章纲后立刻写全本**——先写**前几章 Demo** 让用户验文风、视角、节奏、锚点对齐。Demo 通过才进第 6 步全本续写。
...
由 Claude 主对话调度，每一章派一个 **Agent (subagent_type=general-purpose) 子代理**去写。这样可以保护主对话上下文，长篇也撑得住。
...
子代理输出 = 一份 markdown 文件，写到 `章节/第NN章.md`。**单章目标字数**：长篇约 5000-8000 字/章，中篇约 4000-6000，短篇 6000-10000。
...
由 Claude 主对话执行，分轻量 / 全量两档：

**轻量**（每组 5 章后跑）：扫人物口吻、术语、近章伏笔。问题就地改。

**全量**（全部写完后跑一次）：
- 锚点对齐：每个锚点在新文对应章节里的呈现，是否和原作骨架一致（事件 / 时间 / 在场人 / 角色已知情报）。
- 伏笔回收：章纲里埋的钩子是否回收。
- 留白填补：第 2 步列的"留白清单"是否给了答案。
- 称谓 / 术语 / 时间线漂移：用 grep 抽样检查。
- 文风一致性：随机抽 3-5 章对比口吻，看是否漂出 Demo 锚定的调。

发现问题的章节直接 Edit 修改，**不要重写整章**除非伤筋动骨。

### 第 8 步 — 导出三种形态

```bash
python3 <skill>/scripts/export.py "<作品根>" \
...
- 默认作品根 = `作品集/<原作名>-<配角名>外传/`；用 `--out` 可改。
- 三种最终产物都进 `导出/`；中间产物（章节 md / 设定卡 / 锚点表 / 书名候选）保留在作品根，方便用户复盘和二次修改。
- 详见 `references/formats.md`。

## 何时不用本 skill

- 写**完全原创**小说（无原作 / 无配角锚定）→ 不需要本 skill 的锚点机制，直接让 Claude 写就行。
...
| 错误 | 纠正 |
|---|---|
| 拿当代受版权网文做配角外传 | 拒做，引导改用公版原作（《晋书》《史记》《聊斋》《封神》《镜花缘》《镜花缘》等）或用户自有/已授权作品 |
| 当代网文"先读了再说，只总结剧情" | 也拒；读了再写派生 = 绕铁律一圈 |
| 跳过锚点表直接写 | 并行外传必出 OOC / 时间线漂移；锚点表是续写的硬地基 |
| 跳过第 3 步书名直接用临时名写完整本 | 整本写完才换名，封面/章纲/角色卡都要重弄；书名要在动笔前定 |
| **跳过 Demo 直接写 30 章** | 最致命的错误。视角/口吻/锚点对不对，1 章就能看出来；30 章写完才发现要重来，沉没成本极高 |
| 章纲没和用户确认就开写 Demo | 章纲是 Demo 的前置；第 4 步必须用户点头 |
| Demo 三章一口气写完才贴给用户 | 第 1 章错的口吻会传染到第 2-3 章；每章独立审 |
| 一次性把 30 章塞进主对话写 | 上下文爆 + 风格漂；逐章用 subagent 写，并把 Demo 第 1 章塞进 prompt 锚定文风 |
| 锚点章节大段复刻原作原文 | 用事件骨架对齐，不搬文本；既是工艺也是法律边界 |
| 在续写里"修正"原作设定 | 设定只能扩展不能推翻；想推翻就改用"分叉外传"模式 |
| 导出散落 | 终态一律进 `导出/`，中间产物留作品根 |
