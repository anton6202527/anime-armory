...
| `song-compose`(规划) | 词→带人声的歌：云 Suno / 本地 ACE-Step(Mac可跑) / DiffRhythm | 🔜 |
| `song-cover`(规划) | 翻唱/换声：RVC / so-vits-svc | 🔜 |
...
| 卡点 | `mv-beat`(规划) | librosa 检测 BPM+鼓点 → beatgrid | 🔜 |
| 出图 | `mv-image`(规划) | mv 自建：共享定妆 + 分段分镜 PNG | 🔜 |
| 出视频 | `mv-video`(规划) | mv 自建：图生视频（按段落+卡点） | 🔜 |
| 卡拉OK字幕 | `mv-lyric-sync`(规划) | whisperx 词级对齐 → karaoke.ass | 🔜 |
