...
- **爽剧节奏 = 留存工程**：单集时长按 `单集时长` 选择点定（不写死，预设见第 1 步；缺省「前长后短」=前1-3集~120s/中段~90s/后段~60s），镜头数随时长 6~16 个浮动。**导演的第一任务是"让观众这一秒划不走"**——0-3秒冷开场、前15秒立钩子+矛盾、中段每15-20秒一个钩子/信息增量、≥1次反转、集尾 cliffhanger 硬断。详见 `novel2drama/references/导演节奏.md`，这是治"节奏平/中段划走/不追下集"的核心。
...
| 预设 | 单集时长 | 节奏定位 | 拆集字数 band（原文 target/min/max） |
|---|---|---|---|
| **前长后短**（默认·推荐） | 前1-3集 ~120s / 中段 ~90s / 后段 ~60s | 开篇铺垫足、钩子充分；越往后越碎越快 | 基准 820 / 550 / 1150（前段精修拉满、后段收紧） |
| 均衡90s | 每集 ~90s | 稳定中速 | 820 / 550 / 1150 |
| 快节奏60s | 每集 ~60s | 红果/抖音碎节奏、强留存 | 550 / 380 / 780 |
| 标准120s | 每集 ~120s | 接近主流漫剧单集（~2分钟） | 1100 / 750 / 1500 |
| 自定义 | 用户给秒数 | — | 秒×9 / ×0.6 / ×1.4 |

> ⚠️ **字数↔时长是高方差代理，非 1:1**：原文→台词压缩率因段落剧烈波动——对白/打斗段压缩少（~800字≈100s），环境/心理描写段压缩多（~800字≈40s）。band 只给"容量区间"，**真实时长一律以配音实测为准**（换算仅粗估：念白 ~5字/秒 × 台词占原文 ~0.55 ≈ 秒×9）。

```bash
# 时长选定 → band 三参数一起传（示例：均衡90s）
python3 <skill>/scripts/split_novel.py "<小说路径>" --target 820 --min 550 --max 1150
# 有「第X章」时强烈建议加 --by-chapter，让边界先贴章节（更接近节拍）：
python3 <skill>/scripts/split_novel.py "<小说路径>" --by-chapter --target 820 --min 550 --max 1150
```
...
**前长后短的落地**：拆集用基准 `--target`（中段值，如 820）出粗胚；真正的"前长后短"在**台词改编(本阶段)+分镜(阶段2)**实现——把 per-段时长写进 `_设置.md`（`前段(1-3集)=120s / 中段=90s / 后段=60s`），阶段1 台词按集号取对应秒数控制篇幅、阶段2 镜头数随之浮动。脚本自动剥离开头简介/标签/看点等元数据（`--keep-frontmatter` 可保留）。
