...
详见 `references/platforms.md` 的"两轴架构"章节。**输出位置 = 小说同级目录下的 `分镜剧本/`**（无需用户指定）。把两个 AI 都记到 `global_style.md` 顶部。
...
**约定**：小说应放在 `artifacts/<剧名>/小说/<剧名>.docx`。若用户给的是裸文件，先 mv 进去。**默认输出到 `artifacts/<剧名>/分镜剧本/`**（脚本的 `--out` 可整体改根目录，`--subdir` 改子文件夹名）。
...
1. 通读小说（或抽样若干集）确定 `global_style.md`：
   - 顶部记 **目标视频AI** + **目标图AI**
   - 画风词、世界观、统一负面词
2. 为**主要角色/场景**建卡，存入 `characters/`、`locations/`。格式见 `references/formats.md §1 §2`：
...
> **本 skill 不写 `出图prompt/`**。物料齐后用户调 `/n2d-image`，那个 skill 才负责出图 prompt 两层架构。
...
```
第K集 物料齐：
- 8 类素材：分镜/故事板/素材清单/voiceover/bgm/封面/字幕中/字幕英 ✅
- _进度.md 已勾选
下一步建议：
- 调 /n2d-image <作品根> 第K集  生成出图prompt + PNG
- 或继续精修第K+1集物料
```
