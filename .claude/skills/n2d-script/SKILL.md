...
**先定单集时长**（`单集时长` 选择点）：时长不写死，按当下 AI 漫剧主流（2026：竖屏漫剧单集多 1~2 分钟，精品/剧场向 3~5 分钟）给预设，开拆前**首次问一次**→写 `_设置.md`→之后沉默沿用。读 `<作品根>/_设置.md`，缺则全局默认（默认「前长后短」）。

| 预设 | 单集时长 | 节奏定位 | 拆集 `--target` 字数（原文） |
|---|---|---|---|
| **前长后短**（默认·推荐） | 前1-3集 ~120s / 中段 ~90s / 后段 ~60s | 开篇铺垫足、钩子充分；越往后越碎越快 | 基准 ~820（前段精修拉到 ~1100、后段收到 ~550） |
| 均衡90s | 每集 ~90s | 稳定中速 | ~820 |
| 快节奏60s | 每集 ~60s | 红果/抖音碎节奏、强留存 | ~550 |
| 标准120s | 每集 ~120s | 接近主流漫剧单集（~2分钟） | ~1100 |
| 自定义 | 用户给秒数 | — | 按换算 ≈ 秒×9 字 |

> 换算依据：中文念白 ~5 字/秒，台词占原文 ~0.55 → **原文 target ≈ 秒 × 9**。最终时长由配音(`n2d-voice`)+分镜(阶段2)实测决定，本步只设粗胚篇幅。

```bash
# 按选定时长把 --target 传给拆集脚本（示例：均衡90s → 820）
python3 <skill>/scripts/split_novel.py "<小说路径>" --target 820
# 按章节边界 + 字数双约束切（更贴戏剧节拍）：
python3 <skill>/scripts/split_novel.py "<小说路径>" --by-chapter --target 820
```
