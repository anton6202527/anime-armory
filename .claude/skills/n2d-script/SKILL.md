...
3. `故事板.md` — Clip 表（相邻分镜合片段；人物运动 + 镜头运动 + 动态细节）。**Clip 时长 = 所含镜头时长之和（来自 `镜头时长.json`，配音驱动）**；超即梦 8s 上限则拆 Clip。**相邻 Clip 首尾帧构图对齐**（尾帧=下一首帧，用 frames2video 双帧引导更连贯）。设计完对照 `分镜语法.md` 末尾的 8 条自查清单。
...
**阶段2 分镜设计必读 `references/分镜语法.md`**——景别系统/节奏铁律/连贯三规则(轴线·30°·视线方向)/转场逻辑/运镜克制/构图/8条自查清单。
