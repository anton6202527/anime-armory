...
常用选项:
    ...
    --out 目录          输出根目录（默认=小说同级目录下的『分镜剧本』）
    --subdir 名字       缺省 --out 时的子文件夹名（默认：分镜剧本）
    --target/--min/--max 每集目标/最小(尾段并入阈值)/最大 字数（默认 1000/800/1400）
    --name 标题         素材文件头用的标题（默认取小说文件名）

支持 .txt / .docx 输入。默认输出到 <小说目录>/分镜剧本/，每集一个 第N集/ 文件夹，内含:
    raw.txt 分镜剧本.md 故事板.md 素材清单.md voiceover.txt bgm.txt 封面.md
    字幕_中文.srt 字幕_英文.srt
项目根另生成 characters/ locations/ global_style.md _进度.md。
...
def main():
    ...
    ap.add_argument("--out", default=None,
                    help="输出根目录（直接包含 第N集/）。缺省=小说同级目录下的『分镜剧本』文件夹")
    ap.add_argument("--subdir", default="分镜剧本",
                    help="缺省 --out 时，在小说同级目录下创建的子文件夹名（默认：分镜剧本）")
    ...
    # 默认把素材放进与小说同级的『分镜剧本』文件夹；--out 可整体覆盖
    if args.out:
        root = args.out
    else:
        novel_dir = os.path.dirname(os.path.abspath(args.novel))
        root = os.path.join(novel_dir, args.subdir)
