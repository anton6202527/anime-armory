...
支持 .txt / .docx 输入。默认输出布局：
    <作品根>/common/_进度.md
    <作品根>/common/global_style.md
    <作品根>/common/characters/_角色总表.md
    <作品根>/common/locations/_场景总表.md
...
def main():
    ...
    os.makedirs(os.path.join(common, "characters"), exist_ok=True)
    os.makedirs(os.path.join(common, "locations"), exist_ok=True)
    os.makedirs(os.path.join(root, "脚本"), exist_ok=True)

    write_if_absent(
        os.path.join(common, "global_style.md"),
        f"# {title} — 全局画风与世界观\n\n## 目标平台\n即梦AI（默认）；可选 可灵Kling / Seedance / Veo —— 平台档案见 references/platforms.md\n\n## 画风\n高质量国风AI漫剧风格，电影级光影，统一色调，高细节，动态漫画感。\n\n## 世界观\n（待精修）\n\n## 统一负面词\n（低幼Q版、画风漂移、多余文字水印 等）\n",
    )
    write_if_absent(
        os.path.join(common, "characters", "_角色总表.md"),
        f"# {title} — 角色卡总表\n\n> 全篇首次出现即建卡，后续所有镜头严格复用。格式见 references/formats.md。\n",
    )
    write_if_absent(
        os.path.join(common, "locations", "_场景总表.md"),
