...
> 本仓库包含 **novel / n2d / song / mv / ad** 五条并列创作生产线。每条线都必须自包含、可单独分发：本线脚本只 import 本线 `_lib` 或本线 craft 工具，不依赖 `skills/common/`，也不 import 其他系列实现。跨线只允许**可选文件/数据交接**，例如 novel 导出 n2d 源书、song 交成品歌给 mv；交接缺失时必须优雅降级，不能让本线主流程跑不起来。
